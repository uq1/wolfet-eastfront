...
Section "etpub qagame DLL" qagame
  ...
  SetOutPath "$INSTDIR"
  CopyFiles "$EXEDIR\qagame_mp_x86.dll" "$INSTDIR\etpub\" 1000
...
Section "Example Configuration" cfg
  ...
  Call GetParent
  Pop $R0
  CopyFiles "$R0\docs\Example\Commented\default.cfg" "$INSTDIR\etpub\" 16
  CopyFiles "$R0\docs\Example\Commented\server.cfg" "$INSTDIR\etpub\" 16
  CopyFiles "$R0\docs\Example\Commented\shrubbot.cfg" "$INSTDIR\etpub\" 16
SectionEnd
...
LangString DESC_cfg ${LANG_ENGLISH} "Installs commented server.cfg, default.cfg, and shrubbot.cfg files into your etpub directory with the default settings."
