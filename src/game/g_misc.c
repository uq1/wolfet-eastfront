...
void SP_light( gentity_t *self ) {
	...
	if( !Q_stricmp( mapname.string, "mp_tram" ) 
		|| !Q_stricmp( mapname.string, "mp_dam" )
		|| !Q_stricmp( mapname.string, "mp_trenchtoast" )
		|| !Q_stricmp( mapname.string, "dam" ) )
	{
		G_FreeEntity( self );
		return;
	}
...
/*#ifdef __ETE__
	...
	{
		if (fog_color[0] == 1 && fog_color[0] == 1 && fog_color[0] == 1)
		{// UQ1: True white fog... Make lights a little more yellow to look nicer :)
			if (ent->dl_color[0] == ent->dl_color[1] == ent->dl_color[2] == 1)
			{
				ent->dl_color[0] = 1;
				ent->dl_color[1] = 1;
				ent->dl_color[2] = 0.5;
	...
	if(!(ent->spawnflags & 1))
		trap_LinkEntity(ent);
#else //__NO_MAP_CORONAS__
	G_FreeEntity(ent);
#endif //__NO_MAP_CORONAS__
}
...
void Fire_Lead_Ext( gentity_t *ent, gentity_t *activator, float spread, int damage, vec3_t muzzle, vec3_t forward, vec3_t right, vec3_t up, int mod ) {
	...
	if ( traceEnt->takedamage ) {
		if (
#ifdef __VEHICLES__
			(traceEnt->client && !( traceEnt->client->ps.eFlags & EF_VEHICLE)) || 
#endif //__VEHICLES__
			!(traceEnt->client) )
...
void mg42_spawn_player (gentity_t *ent) {
	gentity_t *base, *gun;
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		VectorCopy(ent->s.origin, base->r.currentOrigin);
		VectorCopy(ent->s.origin, base->s.origin);

		if (!(ent->spawnflags & 2) && ent->s.density != 2) { // no tripod 
			base->clipmask = 0;//CONTENTS_SOLID;
			base->r.contents = 0;//CONTENTS_SOLID;
			base->r.svFlags = 0;
			base->s.eType = ET_GENERAL;	
			base->s.dl_intensity = 1;
			base->takedamage = qtrue;
			base->die = mg42_die;

			base->s.angles2[0] = 0.6;
			base->s.angles2[1] = 0.6;
			base->s.angles2[2] = 0.3;
			...
			{
				if (ent->s.onFireEnd)
				{// This is a airdrop'd gun, still under construction...
					base->s.onFireEnd = 1;
					base->s.weapon = 1;
				}

				base->s.modelindex = G_ModelIndex ("models/mapobjects/multiplayer/flagpole/mg42pole.md3");
			}
			else
			{
				if (ent->s.onFireEnd)
		...
		if (ent->player_mg42)
		{
			...
			VectorMA( offset, 1, forward, offset );
		}
		else
		{
			offset[2]-=24;
		...
		gun->s.eType =			ET_MG42_BARREL;
		gun->health =			base->health; // JPW NERVE
		gun->s.density =		ent->s.density; // UQ1: Added so cgame knows which type it is!

		if (ent->s.density == 2)
			gun->s.modelindex =		G_ModelIndex ("models/multiplayer/ptrs/ptrs.md3");
		...
		if (ent->player_mg42 && ent->s.density == 2)
			VectorSet( offset, ent->s.origin[0], ent->s.origin[1], ent->s.origin[2] + 8 );
		else if (ent->player_mg42)
			VectorSet( offset, ent->s.origin[0], ent->s.origin[1], ent->s.origin[2] + 24 );
		else
			VectorSet( offset, ent->s.origin[0], ent->s.origin[1], ent->s.origin[2] + 16 );
		
		G_SetOrigin (gun, offset);
		VectorCopy(offset, gun->r.currentOrigin);
		VectorCopy(offset, gun->s.origin);
...
//		gun->mg42firetime	 =		0;
		...
		// Link them all up...
		gun->s.onFireEnd = ent->s.onFireEnd;
		gun->bot_goal_entity = base; // The mg42's base is it's goal entity.

		if (ent->bot_goal_entity && ent->bot_goal_entity->bot_goal_entity)
			ent->bot_goal_entity->bot_goal_entity = gun; // The mg42 temp entity's owner is linked to the original sandbags.
		...
		//G_Printf("2. Gun linked at %f %f %f (angles %f %f %f)\n", gun->s.origin[0], gun->s.origin[1], gun->s.origin[2], gun->s.angles[0], gun->s.angles[1], gun->s.angles[2]);
	} 
...
//	char		*type;
	...
	if (!self->player_mg42)
		if ( G_SpawnString( "type", "0", &damage) )
			self->s.density = atoi(damage);
}
...
extern vec3_t	heavyTankMins;
extern vec3_t	heavyTankMaxs;

extern vec3_t	mediumTankMins;
extern vec3_t	mediumTankMaxs;

extern vec3_t	lightTankMins;
extern vec3_t	lightTankMaxs;
...
void UpdateTankTurret ( gentity_t *ent )
{
	...
	VectorSet (head->r.mins , -64, -64, -2);
	VectorSet (head->r.maxs , 64, 64, 48);

	VectorCopy(ent->r.currentOrigin, head->r.currentOrigin);

	if (ent->client)
		type = ent->client->ps.stats[STAT_VEHICLE_CLASS];
	else
		type = ent->s.weapon;

	switch (type)
	{// UQ1: Set size based on tank type...
	case VEHICLE_CLASS_HEAVY_TANK:
		head->r.currentOrigin[2] += heavyTankMaxs[2];
		break;
	case VEHICLE_CLASS_MEDIUM_TANK:
	case VEHICLE_CLASS_FLAMETANK:
		head->r.currentOrigin[2] += mediumTankMaxs[2];
		break;
	case VEHICLE_CLASS_LIGHT_TANK:
		head->r.currentOrigin[2] += lightTankMaxs[2];
		break;
	...
	case VEHICLE_CLASS_PLANE:
		VectorAdd(head->r.currentOrigin, lightTankMaxs, head->r.currentOrigin);
		break;
	case VEHICLE_CLASS_ARTILLERY:
		head->r.currentOrigin[2] += mediumTankMaxs[2];
		break;
	default:
		head->r.currentOrigin[2] += tankMaxs[2];
		break;
	}

	VectorCopy(head->r.currentOrigin, head->s.origin);
	head->clipmask = CONTENTS_CORPSE/*CONTENTS_SOLID*/;
	head->r.contents = CONTENTS_CORPSE/*CONTENTS_SOLID*/;
	head->parent = ent;
	head->s.eType = ET_VEHICLE_TURRET;
...
void RemoveTankTurret ( gentity_t *ent )
{
	...
	trap_UnlinkEntity( ent->turret_ent );
	G_FreeEntity( ent->turret_ent );
	ent->turret_ent = NULL;
}
#endif //__VEHICLES__

int tank_SetWaterLevel( gentity_t *self ) 
{
	vec3_t		point;
	int			cont;
	int			sample1;
	...
	// Ridah, modified this
	point[0] = self->r.currentOrigin[0];
	point[1] = self->r.currentOrigin[1];
	point[2] = self->r.currentOrigin[2] + self->r.mins[2] + 1;	
	cont = trap_PointContents( point, self->s.number );

	if ( cont & MASK_WATER ) {
		sample2 = 48 - self->r.mins[2];
		sample1 = sample2 / 2;

		self->watertype = cont;
		self->waterlevel = 1;
		point[2] = self->r.currentOrigin[2] + self->r.mins[2] + sample1;
		cont = trap_PointContents (point, self->s.number );
		if ( cont & MASK_WATER ) {
			self->waterlevel = 2;
			point[2] = self->r.currentOrigin[2] + self->r.mins[2] + sample2;
			cont = trap_PointContents (point, self->s.number );
			if ( cont & MASK_WATER ){
				self->waterlevel = 3;
			}
...
// UQ1: Check if in water. If it is, slowly drown and blow up...
void tank_check_drown ( gentity_t *self )
{
	if (self->next_fire_time <= level.time)
	{
		int waterlevel = tank_SetWaterLevel(self);

		if ( waterlevel == 3 )
		{// Drowning...
			self->damage = (self->maxhealth*0.1)+1;//250;
			G_Sound(self, G_SoundIndex("sound/misc/hitsound.wav"));
			// don't play a normal pain sound
			self->pain_debounce_time = level.time + 200;
			G_Damage (self, NULL, NULL, NULL, NULL, self->damage, 0, MOD_WATER);
		}

		self->next_fire_time = level.time + 1000; // 1 check per second...
	}
}


void tank_think (gentity_t *self) { // delete itselfs when its used 

	self->nextthink = level.time + 50;
	//self->s.dl_intensity = self->health; // send health to clients
	// UQ1: Now calculate percentage of the new maxhealth values...
	self->s.dl_intensity = (int)((float)((float)self->health/(float)self->maxhealth)*(float)100); // send health to clients

	{
		if (self->s.weapon == VEHICLE_CLASS_APC)
		{// Need to check inrange players for ammo/health giveaways :)
			vec3_t	mins, maxs;
			float	crate_use_distance = 128.0f;
			int		touch[MAX_GENTITIES];
			int		num = 0;
			int		i = 0;

			if (self->vehicle_apc_next_ammo_health_check > level.time + 2000)
				self->vehicle_apc_next_ammo_health_check = level.time + 2000; // Reset timer!

			if (self->vehicle_apc_next_ammo_health_check <= level.time)
			{
				...
				for ( i=0 ; i<num ; i++ ) 
				{
					...
					// Give ammo/health...
					if (!g_realism.integer && hit->client->ps.stats[STAT_HEALTH] < hit->client->ps.stats[STAT_MAX_HEALTH])
					{
						gitem_t		*item;
						gentity_t	*drop = NULL;
				...
				self->vehicle_apc_next_ammo_health_check = level.time + 2000;
			}
		}
	}
	...
	// UQ1: Check for drowning in water...
	tank_check_drown(self);
}


void tank_stopusing( gentity_t *self ) {
}

// this is similar to tankplayer Death
void tank_die (gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod) {
	vec3_t		dir;
	vec3_t		origin;
	...
	RemoveTankTurret(self);

	G_FreeEntity(self);
}

void tank_use (gentity_t *ent, gentity_t *other, gentity_t *activator ) 
{// HUH?!
	trap_LinkEntity (ent);
}
...
qboolean tank_spawn_valid_check (gentity_t *ent)
{// Check if there is a player where the tank should be, and delay spawn...
	int i;
	...
	for (i = 0; i < MAX_GENTITIES; i++)
	{
		...
		if (!test->client && !test->NPC_client && test->s.eType != ET_VEHICLE)
			continue;

		if (test->client && test->client->sess.sessionTeam != TEAM_AXIS && test->client->sess.sessionTeam != TEAM_ALLIES)
			continue;

		if (test->client && test->client->ps.stats[STAT_HEALTH] <= 0)
			continue;
		else if (test->health <= 0)
			continue;

#ifdef __NPC__
		if (test->NPC_client && test->s.teamNum != TEAM_AXIS && test->s.teamNum != TEAM_ALLIES)
			continue;
#endif //__NPC__

		if ((test->client || test->NPC_client) && VectorDistance(ent->s.origin, test->r.currentOrigin) > 128)
			continue;

		if (!(test->client || test->NPC_client) && VectorDistance(ent->s.origin, test->s.origin) > 128)
			continue;

		//G_Printf("Entity %i (type %s) is too close!\n", test->s.number, test->classname);

		// There is a player/bot here... Hold off on spawning!
		return qfalse;
	}

	// Returns qtrue if all is OK!
	return qtrue;
}
...
void tank_spawn (gentity_t *ent) {
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->clipmask	= CONTENTS_SOLID | CONTENTS_PLAYERCLIP;
		tank->r.contents	= CONTENTS_SOLID | CONTENTS_PLAYERCLIP | CONTENTS_TRIGGER;

		tank->r.svFlags =		0;
		tank->s.eType =			ET_VEHICLE; 
		tank->health = tank->maxhealth =			1500;//100;
		tank->s.dl_intensity =	100;
		tank->s.effect1Time = VEHICLE_TYPE_CHURCHILL;

		// UQ1: BAH!!! Seems that brush models *MUST* be registerred by the map makers :(
		/*
		tank->s.solid = SOLID_BMODEL;

		if( level.numBrushModels < 128) 
		{
			int i = 0;
			qboolean registerred = qfalse;

			for (i = 0; i < level.numBrushModels; i++)
			{
				G_Printf("Model %i [ %s ]\n", level.brushModelInfo[i].model, level.brushModelInfo[i].modelname);

				if (level.brushModelInfo[i].model == 999)
				{
					registerred = qtrue;
					break;
				}
			...
			if (!registerred || level.numBrushModels <= 0)
			{
				level.brushModelInfo[level.numBrushModels].model = atoi("*999" + 1);
				Q_strncpyz( level.brushModelInfo[level.numBrushModels].modelname, "tank_churchill", 32 );
				level.numBrushModels++;
			}
		}

		//trap_SetBrushModel( tank, "*999" );
...
*/
		...
		tank->s.nextWeapon = VEHICLE_TYPE_CHURCHILL;

		tank->s.weapon = ent->s.weapon; // UQ1: Set vehicle class!
...
void panzer_spawn (gentity_t *ent) {
	gentity_t *tank;
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->s.nextWeapon = VEHICLE_TYPE_PANZER;
...
void panzeriv_spawn (gentity_t *ent) {
	gentity_t *tank;
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->s.nextWeapon = VEHICLE_TYPE_PANZERIV;
...
void t34_spawn (gentity_t *ent) {
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->s.nextWeapon = VEHICLE_TYPE_T34;
...
void bt7_spawn (gentity_t *ent) {
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->r.svFlags =		0;
		tank->s.eType =			ET_VEHICLE; 
		tank->health = tank->maxhealth =			1100;//100;
		tank->s.dl_intensity =	100;
		...
		tank->s.nextWeapon = VEHICLE_TYPE_BT7;
...
void kv1_spawn (gentity_t *ent) {   //heavy tank more health
	gentity_t *tank;
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->r.svFlags =		0;
		tank->s.eType =			ET_VEHICLE; 
		tank->health = tank->maxhealth =			1800;//135;
		tank->s.dl_intensity =	100;
		...
		tank->s.nextWeapon = VEHICLE_TYPE_KV1;
...
void flametank_spawn (gentity_t *ent) {   //UQ1: Please, new models :)
	gentity_t *tank;                      //Masteries: Wait a bit Unique1 hehe  
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->r.svFlags =		0;
		tank->s.eType =			ET_VEHICLE; 
		tank->health = tank->maxhealth =			1200;//135;
		tank->s.dl_intensity =	100;
		...
		tank->s.nextWeapon = VEHICLE_TYPE_FLAMETANK;
...
void allied_apc_spawn (gentity_t *ent) {   //UQ1: Please, new models :)
	gentity_t *tank;                      //Masteries: Wait a bit Unique1 hehe  
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->s.nextWeapon = VEHICLE_TYPE_ALLIED_APC;
...
void axis_apc_spawn (gentity_t *ent) {
	gentity_t *tank; 
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->s.nextWeapon = VEHICLE_TYPE_AXIS_APC;
...
void stug3_spawn (gentity_t *ent) { //medium tank,mobile gun
	gentity_t *tank;
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->s.nextWeapon = VEHICLE_TYPE_STUG3;
...
void tiger_spawn (gentity_t *ent) { //heavy assault tank
	gentity_t *tank;
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->r.svFlags =		0;
		tank->s.eType =			ET_VEHICLE; 
		tank->health = tank->maxhealth =			1900;//100;
		tank->s.dl_intensity =	100;
		...
		tank->s.nextWeapon = VEHICLE_TYPE_TIGERTANK;
...
void flak88_spawn(gentity_t *ent){ //antitank,interactive gun; remember without ford/backward and mg
	gentity_t *tank;
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->r.svFlags =		0;
		tank->s.eType =			ET_VEHICLE; 
		tank->health = tank->maxhealth =			500;//35;
		tank->s.dl_intensity =	100;
		...
		tank->s.nextWeapon = VEHICLE_TYPE_FLAK88;
...
void pak_spawn(gentity_t *ent){ //antitank,interactive gun; remember without ford/backward and mg
	gentity_t *tank;
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->s.nextWeapon = VEHICLE_TYPE_PAK;
...
void panzer2_spawn(gentity_t *ent){ //Masteries
	gentity_t *tank;
	vec3_t	offset;
	...
	if ( g_knifeonly.integer != 1) {
		...
		tank->r.svFlags =		0;
		tank->s.eType =			ET_VEHICLE; 
		tank->health = tank->maxhealth =			550;//35;
		tank->s.dl_intensity =	100;
		...
		tank->s.nextWeapon = VEHICLE_TYPE_PANZER2;
...
void tank_spawn_think (gentity_t *self) {
	...
	qboolean valid_check = tank_spawn_valid_check(self);
	...
	if ((g_gametype.integer == GT_SUPREMACY || g_gametype.integer == GT_SUPREMACY_CAMPAIGN) && num_flags >= 2)
	{
		...
		if (self->TankMarker)
		{// UQ1: No point looking if we have no marker yet :)
			for (i = 0; i < MAX_GENTITIES; i++ )
			{  
				gentity_t *test = &g_entities[i];
				...
				if ( test->TankMarker == self->TankMarker && i != self->TankMarker && test->health > 0 ) 
		...
		if ( valid_check
			&& (!found /*|| self->s.teamNum != flag_list[best_flag].flagentity->s.modelindex*/) ) 
		{  // init
			if (flag_list[best_flag].flagentity->s.modelindex == TEAM_AXIS)
			{
				...
				case VEHICLE_CLASS_HEAVY_ASSAULT_TANK:
					self->think = tiger_spawn;
					break;
				case VEHICLE_CLASS_HEAVY_TANK:
					self->think = stug3_spawn;
					break;
				case VEHICLE_CLASS_MEDIUM_TANK:
					self->think = panzeriv_spawn;
					break;
				case VEHICLE_CLASS_LIGHT_TANK:
					self->think = panzer2_spawn;//hehe our first light tank
					break;
				case VEHICLE_CLASS_APC:
					self->think = axis_apc_spawn; // UQ1: Add me!
					break;
				case VEHICLE_CLASS_JEEP:
                    break;
				case VEHICLE_CLASS_CAR:
				...
				case VEHICLE_CLASS_PLANE:
                    break;
                case VEHICLE_CLASS_ARTILLERY: //Masteries Artillery
                    self->think = flak88_spawn;
					break;
				case VEHICLE_CLASS_FLAMETANK:
					self->think = flametank_spawn;
					break;
				default:
					//self->think = panzeriv_spawn;
					break;
			...
			else if (flag_list[best_flag].flagentity->s.modelindex == TEAM_ALLIES)
			{
				...
				{
				case VEHICLE_CLASS_HEAVY_ASSAULT_TANK:
					self->think = kv1_spawn; // UQ1: Add me!
					//self->think = is2_spawn; //in a near future
					break;
				case VEHICLE_CLASS_HEAVY_TANK:
					self->think = kv1_spawn;
					break;
				case VEHICLE_CLASS_MEDIUM_TANK:
					self->think = t34_spawn;
					break;
				case VEHICLE_CLASS_LIGHT_TANK:
					self->think = bt7_spawn;
					break;
				case VEHICLE_CLASS_APC:
					self->think = allied_apc_spawn;
                    break;
				case VEHICLE_CLASS_JEEP:
                    break;
				case VEHICLE_CLASS_CAR:
	...
	if (!self->TankMarker && valid_check ) {  // init
	//	G_Printf("Tank spawner %i activated .. \n", self->s.number ); 
		self->TankMarker = self->s.number;
		
		switch (self->s.nextWeapon)
		{
		...
		for (i = 0; i < MAX_GENTITIES; i++ ){  
			if ( &g_entities[i] == self )
				continue;

			if ( g_entities[i].TankMarker == self->TankMarker && i != self->TankMarker && g_entities[i].health > 0 ) { // k , we found our tank, just sleep
				self->think = tank_spawn_think;
				self->nextthink = level.time + g_tankSpawnDelay.integer; // @TODO cvar here , interval between tanks spawn
				found = qtrue;
				break;
...
void SP_tank_heavy_assault (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_HEAVY_ASSAULT_TANK;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
...
void SP_tank_heavy (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_HEAVY_TANK;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
...
void SP_tank_medium (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_MEDIUM_TANK;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
...
void SP_tank_light (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_LIGHT_TANK;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
...
void SP_artillery (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_ARTILLERY;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
...
void SP_tank (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_HEAVY_TANK;
	self->s.nextWeapon = VEHICLE_TYPE_CHURCHILL;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
...
void SP_panzer (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_HEAVY_TANK;
	self->s.nextWeapon = VEHICLE_TYPE_PANZER;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}

void SP_panzeriv (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_MEDIUM_TANK;
	self->s.nextWeapon = VEHICLE_TYPE_PANZERIV;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}

void SP_t34 (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_MEDIUM_TANK;
	self->s.nextWeapon = VEHICLE_TYPE_T34;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}

void SP_bt7 (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_LIGHT_TANK;
	self->s.nextWeapon = VEHICLE_TYPE_BT7;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}

void SP_stug3 (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_MEDIUM_TANK;
	self->s.nextWeapon = VEHICLE_TYPE_STUG3;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}

void SP_kv1 (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_HEAVY_TANK;
	self->s.nextWeapon = VEHICLE_TYPE_KV1;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}

void SP_flak88 (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_ARTILLERY;
	self->s.nextWeapon = VEHICLE_TYPE_FLAK88;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}

void SP_pak (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_ARTILLERY;
	self->s.nextWeapon = VEHICLE_TYPE_PAK;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
...
void SP_flametank (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_FLAMETANK;
	self->s.nextWeapon = VEHICLE_TYPE_FLAMETANK;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
...
void SP_tigertank (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_HEAVY_ASSAULT_TANK;
	self->s.nextWeapon = VEHICLE_TYPE_TIGERTANK;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
...
void SP_axis_apc (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_APC;
	self->s.nextWeapon = VEHICLE_TYPE_AXIS_APC;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
...
void SP_allied_apc (gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_APC;
	self->s.nextWeapon = VEHICLE_TYPE_ALLIED_APC;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
...
void SP_panzer2(gentity_t *self) {
	self->s.weapon = VEHICLE_CLASS_LIGHT_TANK;
	self->s.nextWeapon = VEHICLE_TYPE_PANZER2;
	self->think = tank_spawn_think;
	self->nextthink = level.time + FRAMETIME;
	BotDropToFloor( self );
}
