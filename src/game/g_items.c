...
weapon_t G_GetPrimaryWeaponForClient( gclient_t *client )
{
	...
	if( client->sess.skill[SK_HEAVY_WEAPONS] < 4 ||
		...
		if( COM_BitCheck( client->ps.weapons, WP_PPSH ) )
			return WP_PPSH;
	...
	for( i = 0; i < MAX_WEAPS_PER_CLASS; i++) {
		if( classInfo->classWeapons[i] == WP_MP40 || classInfo->classWeapons[i] == WP_PPSH )
			continue;
...
weapon_t G_GetSecondaryWeaponForClient( gclient_t *client )
{
	...
	} else if( COM_BitCheck( client->ps.weapons, WP_SILENCED_LUGER ) ) {
		return WP_SILENCED_LUGER;
...
void G_DropWeapon( gentity_t *ent, weapon_t weapon )
{
	...
	if (ent->s.eType == ET_NPC)
	{
		...
		client->ps.ammo[BG_FindAmmoForWeapon(weapon)] = GetAmmoTableData(weapon)->defaultStartingAmmo;
		client->ps.ammoclip[BG_FindAmmoForWeapon(weapon)] = GetAmmoTableData(weapon)->defaultStartingClip;
	}
#endif //__NPC__
	...
	// UQ1: Randomize angles...
	angles[YAW] = angles[YAW] + Q_TrueRand(-180, 180);
...
qboolean Pickup_IsHeavyWeapon( weapon_t weap ) {
	int i;
...
qboolean G_CanPickupWeapon( weapon_t weapon, gentity_t* ent ) {
	...
	if (g_gametype.integer <= GT_WOLF || g_gametype.integer == GT_NEWCOOP)
		return qtrue; // UQ1: In COOP and SP, you can pick up anything!

#ifdef __EF__
	// UQ1: Allow all weapons (except heavy ones) to be picked up in EastFront - Requested by Tyrop...
	if (!Pickup_IsHeavyWeapon(weapon) || ent->client->sess.playerType == PC_SOLDIER)
		return qtrue;
#endif //__EF__
...
int Pickup_Weapon( gentity_t *ent, gentity_t *other ) {
	...
	if( ent->item->giTag == WP_AMMO ) {
		...
		if ( ent->parent )
			if ( other->r.svFlags & SVF_BOT && !(ent->parent->r.svFlags & SVF_BOT) ) // make bots thanks for ammo
	...
	if (!ent->count)
		ent->count = GetAmmoTableData(ent->item->giTag)->defaultStartingClip;
	...
	if( alreadyHave ) 
	{
		int current_ammo = other->client->ps.ammo[BG_FindAmmoForWeapon(ent->item->giTag)];
		int current_ammo_clip = other->client->ps.ammoclip[BG_FindClipForWeapon(ent->item->giTag)];
		qboolean is_holding = qfalse;

		if (other->client->ps.weapon == ent->item->giTag)
			is_holding = qtrue;

#ifdef __ETE__
		switch (ent->item->giTag)
		{// UQ1: Check if we should give them an akimbo instead!
		case WP_SILENCED_COLT:
			...
			other->client->ps.ammo[BG_FindAmmoForWeapon(WP_AKIMBO_SILENCEDCOLT)] = current_ammo;
			other->client->ps.ammoclip[BG_FindClipForWeapon(WP_AKIMBO_SILENCEDCOLT)] = current_ammo_clip;
			other->client->ps.ammoclip[BG_FindClipForWeapon(BG_AkimboSidearm(WP_AKIMBO_SILENCEDCOLT))] = current_ammo_clip;
			
			if (is_holding)
			{
				other->client->ps.nextWeapon = WP_AKIMBO_SILENCEDCOLT;
			}
			break;
		case WP_SILENCED_LUGER:
			other->client->ps.ammo[BG_FindAmmoForWeapon(ent->item->giTag)] = 0;
			other->client->ps.ammoclip[BG_FindClipForWeapon(ent->item->giTag)] = 0;
			COM_BitClear( other->client->ps.weapons, WP_SILENCED_LUGER );
			COM_BitSet( other->client->ps.weapons, WP_AKIMBO_SILENCEDLUGER );
			other->client->ps.ammo[BG_FindAmmoForWeapon(WP_AKIMBO_SILENCEDLUGER)] = current_ammo;
			other->client->ps.ammoclip[BG_FindClipForWeapon(BG_AkimboSidearm(WP_AKIMBO_SILENCEDLUGER))] = current_ammo_clip;
			other->client->ps.ammoclip[BG_FindClipForWeapon(WP_AKIMBO_SILENCEDLUGER)] = current_ammo_clip;
			
			if (is_holding)
			{
				other->client->ps.nextWeapon = WP_AKIMBO_SILENCEDLUGER;
			}
			break;
		case WP_COLT:
			other->client->ps.ammo[BG_FindAmmoForWeapon(ent->item->giTag)] = 0;
			...
			other->client->ps.ammo[BG_FindAmmoForWeapon(WP_AKIMBO_COLT)] = current_ammo;
			other->client->ps.ammoclip[BG_FindClipForWeapon(WP_AKIMBO_COLT)] = current_ammo_clip;
			other->client->ps.ammoclip[BG_FindClipForWeapon(BG_AkimboSidearm(WP_AKIMBO_COLT))] = current_ammo_clip;
			
			if (is_holding)
			{
				other->client->ps.nextWeapon = WP_AKIMBO_COLT;
			}
			break;
		case WP_LUGER:
			other->client->ps.ammo[BG_FindAmmoForWeapon(ent->item->giTag)] = 0;
			...
			other->client->ps.ammo[BG_FindAmmoForWeapon(WP_AKIMBO_LUGER)] = current_ammo;
			other->client->ps.ammoclip[BG_FindClipForWeapon(WP_AKIMBO_LUGER)] = current_ammo_clip;
			other->client->ps.ammoclip[BG_FindClipForWeapon(BG_AkimboSidearm(WP_AKIMBO_LUGER))] = current_ammo_clip;
			
			if (is_holding)
			{
				other->client->ps.nextWeapon = WP_AKIMBO_LUGER;
			}
			break;
		default:
			break;
...
#ifdef __EF__
		if( other->client->ps.weapon == WP_MORTAR_SET || other->client->ps.weapon == WP_MOBILE_MG42_SET || other->client->ps.weapon == WP_30CAL_SET || other->client->ps.weapon == WP_PTRS_SET) {
#else //!__EF__
		...
		if( (ent->item->giTag == WP_LUGER
			|| ent->item->giTag == WP_COLT
			|| ent->item->giTag == WP_AKIMBO_LUGER
			|| ent->item->giTag == WP_AKIMBO_COLT
			|| ent->item->giTag == WP_AKIMBO_SILENCEDLUGER
			|| ent->item->giTag == WP_AKIMBO_SILENCEDCOLT)
			&& G_CanPickupWeapon( ent->item->giTag, other ))
		{
			weapon_t primaryWeapon = G_GetSecondaryWeaponForClient( other->client );

			// rain - added parens around ambiguous &&
			if( primaryWeapon || 
				(other->client->sess.playerType == PC_SOLDIER && other->client->sess.skill[SK_HEAVY_WEAPONS] >= 4) ) {

				if( primaryWeapon && g_gametype.integer >= GT_WOLF) {
				...
				{// Bot need to let it know it has the new one...
					switch (ent->item->giTag)
					{
					case WP_COLT:
					case WP_LUGER:
					...
					case WP_AKIMBO_SILENCEDLUGER:
						if (ent->client->sess.playerWeapon2 != ent->item->giTag)
						{
							ent->client->sess.playerWeapon2 = ent->item->giTag;
							ent->beStill = level.time + 1000;
						}
						break;
					default:
						if (ent->client->sess.playerWeapon != ent->item->giTag)
						{
							ent->client->sess.playerWeapon = ent->item->giTag;
							ent->beStill = level.time + 1000;
						}
						break;
					}
                ...
                } else if( ent->item->giTag == WP_PTRS ) {
					COM_BitSet( other->client->ps.weapons, WP_PTRS_SET );
#endif //__EF__
...
#ifdef __BOT__
					if (other->r.svFlags & SVF_BOT)
					{// Give full ammo to bots!
						Add_Ammo( other, ent->item->giTag, ent->delay, qtrue );

						if (weapAlts[ ent->item->giTag ])
							Add_Ammo( other, weapAlts[ ent->item->giTag ], ent->delay, qfalse );
					}
#endif //__BOT__
				} else {
		...
		else if( G_CanPickupWeapon( ent->item->giTag, other ) ) 
		{
...
//	int			quantity = 0;
	...
	if ( ent->parent )
		if ( other->r.svFlags & SVF_BOT && !(ent->parent->r.svFlags & SVF_BOT) ) // make bots thanks for health
			G_AI_Voice( other, NULL, SAY_TEAM, "Thanks", qfalse, 3000+rand() % 1000, AIVOICE_TALK );  // rather bigger delay so they should thank after latest pack
#endif
...
#ifdef __VEHICLES__
	if( other->client->ps.eFlags & EF_VEHICLE ) {
