...
qboolean CudaInitialized = qfalse;
qboolean CudaAvailable = qfalse;
...
qboolean InitCUDA ( void )
{
	...
	if(count == 0) {
#ifdef CGAMEDLL
		CG_Printf("^1***^3\n^1***^3\n^1***^3\n^1***^3 CGAME:^5 CUDA Initialization Failed - There is GPU no valid device.\n^1***^3\n^1***^3\n^1***^3\n");
#else //!CGAMEDLL
		G_Printf("^1***^3\n^1***^3\n^1***^3\n^1***^3 GAME:^5 CUDA Initialization Failed - There is GPU no valid device.\n^1***^3\n^1***^3\n^1***^3\n");
#endif //CGAMEDLL
		return qfalse;
	}
	...
	for(i = 0; i < count; i++) {
		CUdevprop prop;
		if(cuDeviceGetProperties(&prop, i) == CUDA_SUCCESS) {
			break;
		}
	}
	if(i == count) {
#ifdef CGAMEDLL
		CG_Printf("^1***^3\n^1***^3\n^1***^3\n^1***^3 CGAME:^5 CUDA Initialization Failed - There is no device supporting CUDA.\n^1***^3\n^1***^3\n^1***^3\n");
#else //!CGAMEDLL
		G_Printf("^1***^3\n^1***^3\n^1***^3\n^1***^3 GAME:^5 CUDA Initialization Failed - There is no device supporting CUDA.\n^1***^3\n^1***^3\n^1***^3\n");
#endif //CGAMEDLL
		return qfalse;
	}
	//cuSetDevice(i);
	cuDeviceGet(&device.device, i);
	cuDeviceGetName(name, 128, device.device);

#ifdef CGAMEDLL
	CG_Printf("^4***^3\n^4***^3\n^4***^3\n^1***^3 CGAME:^5 CUDA Initialized on device [^7%s^5] - GPU Acceleration Enabled!\n^4***^3\n^4***^3\n^4***^3\n", name);
#else //!CGAMEDLL
	G_Printf("^4***^3\n^4***^3\n^4***^3\n^4***^3 GAME:^5 CUDA Initialized on device [^7%s^5] - GPU Acceleration Enabled!\n^4***^3\n^4***^3\n^4***^3\n", name);
#endif //CGAMEDLL
