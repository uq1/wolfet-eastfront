...
**  	bg_physics.c
**
**  	Major portion of vehicle physics code.
**
**		Author: STONELANCE
*/
...
float CP_SPRING_STRENGTH = 100 * (CP_FRAME_MASS / 350.0f) * CP_GRAVITY; // 125
float CP_SHOCK_STRENGTH = 12 * CP_GRAVITY; // 25
float CP_SWAYBAR_STRENGTH = 20 * CP_GRAVITY; // 20 * grav
...
//	vec3_t	avgVel;
	...
	if ( VectorNAN( source->netForce ) )
	{
		...
		pm->damage.mod = MOD_CRUSH/*MOD_HIGH_FORCES*/;
		return;
	}
...
*/
		...
		pm->damage.mod = MOD_CRUSH/*MOD_BO_SHOCKS*/;
	}
...
*/
		...
		VectorCopy(normal, pm->damage.dir);
		pm->damage.dflags = DAMAGE_NO_KNOCKBACK;
		pm->damage.mod = MOD_CRUSH/*MOD_WORLD_COLLISION*/;
	}
...
//					Com_Printf( "hit non player CONTENTS_BODY\n" );
					PM_AddVehicleTouchEnt( trace.entityNum, trace.endpos );
...
//		VectorSubtract(car->tBody.r, pm->cars[hitEnt]->sBody.r, normal);
		...
		VectorCopy(normal, pm->damage.dir);
		pm->damage.dflags = DAMAGE_NO_KNOCKBACK;
		pm->damage.mod = MOD_CRUSH/*MOD_CAR_COLLISION*/;
...
void PM_DriveMove( car_t *car, float time, qboolean includeBodies )
{
	...
	{
		memset(&car->sPoints, 0, (NUM_CAR_POINTS * sizeof(carPoint_t)));
		memset(&car->tPoints, 0, (NUM_CAR_POINTS * sizeof(carPoint_t)));
		memset(&car->sBody, 0, sizeof(carBody_t));
		memset(&car->tBody, 0, sizeof(carBody_t));
		PM_InitializeVehicle( car, pm->ps->origin, pm->ps->viewangles, pm->ps->velocity );
		//memset(&car->sBody, 0, sizeof(carBody_t));
		//memset(&car->tBody, 0, sizeof(carBody_t));
		VectorCopy(pm->ps->origin, pm->car->sBody.r);
		VectorCopy(pm->ps->velocity, pm->car->sBody.v);
		//VectorCopy(pm->ps->angularMomentum, pm->car->sBody.L);
		VectorCopy(pm->ps->velocity, pm->car->sBody.L);
		VectorCopy(pm->ps->origin, pm->car->tBody.r);
		VectorCopy(pm->ps->velocity, pm->car->tBody.v);
		//VectorCopy(pm->ps->angularMomentum, pm->car->sBody.L);
		VectorCopy(pm->ps->velocity, pm->car->tBody.L);
