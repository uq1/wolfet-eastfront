...
void NPC_MAPPING_CreateNodeLinks ( int node )
...
void NPC_MAPPING_CreateSpecialNodeFlags( int node )
...
void NPC_MAPPING_MakeLinks ( void )
{
	int loop = 0;
...
void NPC_NODES_SetObjectiveFlags( int node )
...
void NPC_NODES_CheckBeginNode (int node)
{
...
qboolean NPC_Load_AddNode(vec3_t origin, int fl, short int* ents, int objFl)
{
...
void NPC_NODES_LoadNodes( void )
{
	fileHandle_t	f;
	int				i;
	char			filename[60];
	vmCvar_t		mapname;
	short int		objNum[3] = {0, 0, 0}, objFlags, numLinks;
	int				flags;
	vec3_t			vec;
...
void NPC_FODDER_AddNode( int clientNum, int teamNum, qboolean isCoverPoint )
{
	int i;
...
int NPC_FODDER_FindClosestReachableNodeWithIgnore(gentity_t *npc, int ignore, int r, int type)
{
    int         i;
    float       closest;
	float       nonLOSclosest;
...
int NPC_FODDER_CreateRoute(gentity_t *NPC, short int *pathlist)
...
short int NPC_FODDER_GetNextNode(gentity_t *NPC)
{
...
void NPC_UcmdMoveForDir( gentity_t *self, usercmd_t *cmd, vec3_t dir )
{
...
void NPC_AI_FODDER_Humanoid_Move( gentity_t *NPC )
{
...
/*	if( NPC->s.dl_intensity == TAG_HOOK 
...
*/
	...
	if (npc_nodes[NPC->current_node].type & NPC_NODE_COVER
		|| (VectorDistance(npc_nodes[NPC->next_node].origin, NPC->r.currentOrigin) < 128 && npc_nodes[NPC->next_node].type & NPC_NODE_COVER && VectorLength(NPC->NPC_client->ps.velocity) < 16))
	{
		float cover_dist = VectorDistance(npc_nodes[NPC->current_node].origin, NPC->r.currentOrigin);
		qboolean take_cover = qfalse;

		if (VectorDistance(npc_nodes[NPC->next_node].origin, NPC->r.currentOrigin) < 128 && npc_nodes[NPC->next_node].type & NPC_NODE_COVER)
		...
		if (VectorDistance(npc_nodes[NPC->next_node].origin, NPC->r.currentOrigin) < 128 && npc_nodes[NPC->next_node].type & NPC_NODE_COVER)
			...
			NPC->NPC_CoverPoint_Last_Distance = VectorDistance(npc_nodes[NPC->current_node].origin, NPC->r.currentOrigin);
		...
		{// Using cover...
			...
			{// We want to stay still!
				...
				if ( lookOver <= 1 && NPC->NPC_CoverPoint_LookUpTimer <= level.time
					&& ((NPC->NPC_Humanoid_Type & HUMANOID_TYPE_FODDER_ATTACKER) || (NPC->NPC_Humanoid_Type & HUMANOID_TYPE_FODDER_DEFENDER)) )
	...
	if ( npc_nodes[NPC->current_node].type & NPC_NODE_JUMP 
	...
	else if ( !( npc_nodes[NPC->current_node].type & NPC_NODE_DUCK ) && !( npc_nodes[NPC->last_node].type & NPC_NODE_DUCK )
...
void NPC_FODDER_WaypointRender(void)
{
#ifdef __NPC_WP_RENDER__
	int i, n;
	int inc_checker;
	int bestindex;
	int gotbestindex;
	...
	while (i < gWPNum)
	{
		...
		{
			if (VectorDistance(g_entities[0].r.currentOrigin, npc_nodes[i].origin) < 128
...
/*		else
	...
	while (i < gWPNum)
	{
		...
		if (npc_nodes[i].origin)
		{
			if (VectorDistance(g_entities[0].r.currentOrigin, npc_nodes[i].origin) < 512
...
/*		else
	...
	while (i < gWPNum)
	{
		...
		if (npc_nodes[i].origin)
		{
			if (VectorDistance(g_entities[0].r.currentOrigin, npc_nodes[i].origin) < 1024
...
checkprint:
	...
	if (gotbestindex && bestindex != gLastPrintedIndex)
	{
		...
		G_Printf("^5NPC Node ^7%i\n^5Flags - ^3%i^5 (^7%s^5)\n^5Origin - (^3%i %i %i^5)\n", 
			(int)(bestindex), (int)npc_nodes[bestindex].type, flagstr, 
			(int)(npc_nodes[bestindex].origin[0]), (int)(npc_nodes[bestindex].origin[1]), (int)(npc_nodes[bestindex].origin[2]));
