...
void AICast_ChangeViewAngles(cast_state_t *cs, float thinktime) {
	...
	if (cs->viewangles[PITCH] > 180) cs->viewangles[PITCH] -= 360;
...
void AICast_InputToUserCommand(cast_state_t *cs, bot_input_t *bi, usercmd_t *ucmd, int delta_angles[3]) {
	...
	if (bi->actionflags & ACTION_ATTACK) {
		...
		if ((cs->triggerReleaseTime < level.time) &&
			...
			(((level.time + cs->entityNum*500)/2000)%2 || !(rand()%(1+bot_skill.integer)) || (cs->attributes[ATTACK_SKILL] < 0.5) || (cs->weaponNum != WP_LUGER) || (cs->bs->cur_ps.weaponTime == 0) || (cs->bs->cur_ps.releasedFire)) )
		{
...
//		if (COM_BitCheck( ent->client->ps.weapons, WP_KNIFE)) {
...
//	}
	...
	cs->bs->areanum = BotPointAreaNum(cs->entityNum, cs->bs->origin);
	if (cs->bs->areanum) {
...
//				ent->client->ps.eFlags |= EF_NO_TURN_ANIM;
				...
				cs->revivingTime = level.time + 500;//BG_AnimScriptEvent( &ent->client->ps, ANIM_ET_REVIVE, qfalse, qtrue );;
			} else {
...
//			ent->client->ps.eFlags |= EF_MONSTER_EFFECT2;
		...
		if (ent->health > GIB_HEALTH && cs->deathTime && cs->deathTime < (level.time - 1000)) {
			...
			if (cs->deadSinkStartTime) {
				...
				// if they are gone
				if (cs->deadSinkStartTime + 5000/*DEAD_SINK_DURATION*/ < level.time) {
					trap_DropClient( cs->entityNum, "", 0 );
					return;
				}
			}
...
//	}
	...
	// walking
	animIndex = 0;//BG_GetAnimScriptAnimation( cs->entityNum, ent->client->ps.aiState, ANIM_MT_WALK );
	if (animIndex >= 0) {
//		anim = BG_GetAnimationForIndex( cs->entityNum, animIndex );
		cs->attributes[WALKING_SPEED] = anim->moveSpeed;
	}
	// crouching
	animIndex = 0;//BG_GetAnimScriptAnimation( cs->entityNum, ent->client->ps.aiState, ANIM_MT_WALKCR );
	if (animIndex >= 0) {
//		anim = BG_GetAnimationForIndex( cs->entityNum, animIndex );
		cs->attributes[CROUCHING_SPEED] = anim->moveSpeed;
	}
	// running
	animIndex = 0;//BG_GetAnimScriptAnimation( cs->entityNum, ent->client->ps.aiState, ANIM_MT_RUN );
	if (animIndex >= 0) {
//		anim = BG_GetAnimationForIndex( cs->entityNum, animIndex );
...
//G_Printf( "AI startframe: %i\n", time );
	...
	for (i=0; i<2; i++) {
		...
		castcount = BotPointAreaNum(i, g_entities[0].s.pos.trBase);
		if (castcount) {
...
qboolean AICast_GetAvoid( cast_state_t *cs, bot_goal_t *goal, vec3_t outpos, qboolean reverse, int blockEnt )
{
	...
	for (yaw=-angleDiff*invert; yaw*invert<=maxYaw; yaw+=inc*invert) {
			...
			&&	(castmove.groundEntityNum != ENTITYNUM_NONE))
		{
			...
			{
				if (!goal || (traveltime = trap_AAS_AreaTravelTimeToGoalArea( /*cs->entityNum,*/ BotPointAreaNum(cs->entityNum, castmove.endpos), castmove.endpos, goal->areanum, cs->travelflags )) < (starttraveltime+200)) {
					bestyaw = yaw;
					bestmoved = distmoved;
	...
	for (i=0; i<pm->numtouch; i++)
	{
		...
		if (pm->touchents[i] < aicast_maxclients )
		{
			...
			if (	(ocs->bs) &&
				...
				if (ocs->leaderNum >= 0) {
					VectorCopy( g_entities[ocs->leaderNum].r.currentOrigin, ogoal.origin );
					ogoal.areanum = BotPointAreaNum(cs->entityNum, ogoal.origin);
					ogoal.entitynum = ocs->leaderNum;
...
void AICast_IdleReload( cast_state_t *cs ) {
	...
	if (cs->noReloadTime >= level.time)
		return;
	if (!((cs->bs->cur_ps.ammoclip[BG_FindClipForWeapon(cs->bs->cur_ps.weapon)] < (int)(0.75*GetAmmoTableData(cs->bs->cur_ps.weapon)->maxclip)) && cs->bs->cur_ps.ammo[BG_FindAmmoForWeapon(cs->bs->cur_ps.weapon)]))
		return;
	//
