...
/*		if (g_gametype.integer < GT_WOLF) { // JPW NERVE no attacker protect in wolf MP
			if (carrier && carrier != attacker) {
				...
				if (VectorLengthSquared(v1) < SQR(CTF_ATTACKER_PROTECT_RADIUS) ||
					...
					PrintMsg(NULL, "%s" S_COLOR_CYAN " defends the %s's flag carrier.\n",
...
gentity_t *SelectRandomTeamSpawnPoint_RTCW( int teamstate, team_t team, int spawnObjective ) {
	gentity_t	*spot;
	gentity_t	*spots[MAX_TEAM_SPAWN_POINTS];
...
gentity_t *SelectCTFSpawnPoint ( team_t team, int teamstate, vec3_t origin, vec3_t angles, int spawnObjective ) {
	...
	//if (spot)
	//	G_Printf("1. Found spawnpoint at %f %f %f.\n", spot->s.origin[0], spot->s.origin[1], spot->s.origin[2]);

	if (!spot) 
	{// UQ1: Added. RTCW compatibility?
		spot = SelectRandomTeamSpawnPoint_RTCW( teamstate, team, spawnObjective );

		//if (spot)
		//	G_Printf("2. Found spawnpoint at %f %f %f.\n", spot->s.origin[0], spot->s.origin[1], spot->s.origin[2]);
	}
	...
	if (!spot)
	{
		...
		//if (spot)
		//	G_Printf("3. Found spawnpoint at %f %f %f.\n", spot->s.origin[0], spot->s.origin[1], spot->s.origin[2]);
	}

	if (!spot) 
	{
		//return SelectSpawnPoint( vec3_origin, origin, angles ); // UQ1: Try another way...
		spot = SelectSpawnPoint( vec3_origin, origin, angles );

		//if (spot)
		//	G_Printf("4. Found spawnpoint at %f %f %f.\n", spot->s.origin[0], spot->s.origin[1], spot->s.origin[2]);
...
#ifdef __ETE__
void ControlPoint_Update( gentity_t *self, gentity_t *other, gentity_t *activator ) {
	char cs[MAX_STRING_CHARS];
...
//		self->count2 = (self->count2 & 256) + TEAM_AXIS;
	...
	self->count2 = self->s.modelindex;

	Info_SetValueForKey( cs, "t", va( "%i", self->count2 ) );
	trap_SetConfigstring( self->count, cs );
}

void ControlPoint_Register(gentity_t *self) 
{// Supremacy command point spawnpoint selection and map info!
	...
	if (numobjectives == MAX_MULTI_SPAWNTARGETS)
		...
		trap_GetConfigstring( cs_obj, cs, sizeof(cs) );
		Info_SetValueForKey( cs, "spawn_targ", va("Control Point") );
		Info_SetValueForKey( cs, "x", va( "%i", (int)self->s.origin[0] ) );
		Info_SetValueForKey( cs, "y", va( "%i", (int)self->s.origin[1] ) );
		if( level.ccLayers )
		...
		Info_SetValueForKey( cs, "t", va( "%i", self->count2 ) ); // UQ1: Team
		self->use = ControlPoint_Update;
		self->count = cs_obj;
...
void objective_Register(gentity_t *self) {
	...
	if (g_gametype.integer == GT_SUPREMACY || g_gametype.integer == GT_SUPREMACY_CAMPAIGN)
	{
		...
		return; // UQ1: Work this out with the flag code!
	}
#endif //__ETE__
...
void checkpoint_touch (gentity_t *self, gentity_t *other, trace_t *trace) {
	gclient_t	*client, *other_client;

#ifdef __NPC__
	if (self->s.eType == ET_NPC)
		client = self->NPC_client;
	else
		client = self->client;

	if (other->s.eType == ET_NPC)
		other_client = other->NPC_client;
	else
...
#endif //__NPC__
	if((other_client->ps.eFlags & EF_PLAYDEAD)) {
		return;
	}
	...
	if (other->s.eType == ET_NPC)
	{
		if ( self->count == other->s.teamNum )
			return;
	}
#endif //__NPC__

	if ( self->count == other_client->sess.sessionTeam )
		return;
...
// jpw
	...
	if (other->s.eType == ET_NPC)
	{
		self->count = other->s.teamNum;
	}
	else
#endif //__NPC__
	self->count = other_client->sess.sessionTeam;
	...
	// Gordon: reset player disguise on touching flag
	other_client->ps.powerups[PW_OPS_DISGUISED] = 0;
	// Run script trigger
	if ( self->count == TEAM_AXIS ) {
		self->health = 0;
...
// jpw
	...
	if (self->target) {
		while(1) {
			...
			if (!ent)
				break;
			if (other_client->sess.sessionTeam == TEAM_AXIS) {
...
int QDECL G_SortPlayersByPlayerRating( const void *a, const void *b ) {
	...
	arate = cla->sess.playerrating;
	brate = clb->sess.playerrating;
...
//	if (g_gametype.integer <= GT_COOP && (ent->r.svFlags & SVF_BOT))
//	{
//		return qtrue;
//	}
 ...
 * The cvar g_activeTeamBalance is a percent that decides when the 
 * teams are not balanced.  50 means the winning team is ahead in 
 * team XP by a margin of 50%.  Like 750 to 500 or 1500 to 1000. 
 * 
 * josh: 
 * We're deprecating / removing g_activeTeamBalance and replacing it as follows:
...
void G_ActiveTeamBalance() 
{
	...
	// tjw: bail out if there are < 5 mins left in the map
	if(((g_timelimit.value * 60000) -
		(level.timeCurrent - level.startTime)) < 300000) {
		return;
	}

	if(g_ATB.integer & ATB_PLAYERRATING) {
	...
	if(G_EndRoundCountdown() > -1)
		return;

	// Start of stuff g_ATB 2 doesn't care about
	...
	} else if (g_ATB.integer & ATB_PLAYERRATING) {
		if((bcount + rcount) < g_playerRating_minplayers.integer) 
			return;
		bscore = 100*G_GetWinProbability(TEAM_ALLIES, NULL, qfalse);
		rscore = 100*G_GetWinProbability(TEAM_AXIS, NULL, qfalse);
	...
	for(i = 0; i < level.numConnectedClients; i++) {
		cl = level.clients + level.sortedClients[i];
		if((bscore > rscore && cl->sess.sessionTeam == TEAM_ALLIES) ||
			(rscore > bscore && cl->sess.sessionTeam == TEAM_AXIS)) {

			winners[wcnt++] = level.sortedClients[i];
		}
		else if((bscore < rscore && cl->sess.sessionTeam == TEAM_ALLIES) ||
			(rscore < bscore && cl->sess.sessionTeam == TEAM_AXIS)) {
	...
	else if(SetTeam(ent, loseteam, qtrue, -1, -1, qfalse)) {
	...
	else if(SetTeam(ent, winteam, qtrue, -1, -1, qfalse)) {
		CP(va("cp \"Active Team Balance has moved you to "
			"the %s%s^7 team\" 1",
			(ent->client->sess.sessionTeam == TEAM_AXIS) ?
				"^1" : "^4",
