...
gentity_t *fire_tankflamechunk (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	bolt->s.eType = ET_FLAMETHROWER_CHUNK;
	bolt->r.svFlags = SVF_NOCLIENT;	
	bolt->s.weapon = WP_FLAMETHROWER/*self->s.weapon*/;
	bolt->r.ownerNum = self->s.number;
	bolt->parent = self;
	bolt->methodOfDeath = MOD_FLAMETHROWER;
	bolt->clipmask = MASK_MISSILESHOT;
	...
	VectorSet (bolt->r.maxs, 4, 4, 4);
	VectorCopy( start, bolt->s.pos.trBase );
	VectorScale(dir, FLAME_START_SPEED*2, bolt->s.pos.trDelta);

	SnapVector( bolt->s.pos.trDelta );			// save net bandwidth
	VectorCopy (start, bolt->r.currentOrigin);
	
	//bolt->think = G_RunFlamechunk;
...
gentity_t *fire_grenade (gentity_t *self, vec3_t start, vec3_t dir, int grenadeWPID) {
	...
	if(	client && client->ps.grenadeTimeLeft) {
		bolt->nextthink = level.time + client->ps.grenadeTimeLeft;
	} else {
	...
	// no self->client for shooter_grenade's
	if(client)
		client->ps.grenadeTimeLeft = 0;		// reset grenade timer
	...
	bolt->r.ownerNum	= self->s.number;
	bolt->parent		= self;
	bolt->s.teamNum		= client->sess.sessionTeam;
#ifdef __NPC__
	if (self->s.eType != ET_NPC)
		bolt->s.teamNum				= self->s.teamNum + 4;
#endif //__NPC__
...
// jpw
	...
	switch(grenadeWPID) {
		...
		case WP_AIRDROP_MARKER:
			// xkan 11/25/2002, fixed typo, classname used to be "somke_bomb"
			bolt->classname				= "airdrop_marker_canister";
			bolt->s.eFlags				= EF_BOUNCE_HALF | EF_BOUNCE;
			// rain - this is supposed to be MOD_SMOKEBOMB, not SMOKEGRENADE
			bolt->methodOfDeath			= MOD_SMOKEBOMB;
			bolt->will_explode			= qtrue;
			...
			if (self->client)
				...
				VectorCopy(self->s.angles, bolt->s.angles2);
...
// jpw
    case WP_MORTAR_SET:
      ...
      break;
		case WP_LANDMINE:
			bolt->accuracy				= 0;
			bolt->s.teamNum				= client->sess.sessionTeam + 4;
#ifdef __NPC__
			if (self->s.eType != ET_NPC)
				bolt->s.teamNum				= self->s.teamNum + 4;
#endif //__NPC__
	...
	if (projectiles >= 1) {
		...
		for (i = 0; i <= patternLen; i++) {
			...
			if (newLineFlag) {
				...
				slotCount = 0;
				for(ii = i; (g_partyPanzersPattern.string[ii] != ';') &&
					(ii < patternLen); ii++) {
...
//Masteries, panzer 2 fire
gentity_t *fire_tankptrs (gentity_t *self, vec3_t start, vec3_t dir) {
	gentity_t	*bolt;
//@TODO finish
	...
	bolt = G_Spawn();
	bolt->classname = "shell";           //Masteries
	bolt->nextthink = level.time + 20000;//Never, it´s only a huge bullet
	//bolt->think = G_ExplodeMissile;//Isn´t a missile
	bolt->accuracy = 4;
	bolt->s.eType = ET_MISSILE;
	bolt->r.svFlags = SVF_BROADCAST;	
	bolt->s.weapon = WP_PTRS_SET;//self->s.weapon;
	bolt->r.ownerNum = self->s.number;
	bolt->parent = self;
	bolt->damage = 70; 
	bolt->splashDamage = 20; 
	bolt->splashRadius = 30; 
	bolt->methodOfDeath = MOD_PTRSSHELL;
	bolt->splashMethodOfDeath = MOD_PTRSSHELL;
	bolt->clipmask = MASK_MISSILESHOT;
	bolt->s.pos.trType = TR_LINEAR;
	bolt->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;	
	VectorCopy( start, bolt->s.pos.trBase );
