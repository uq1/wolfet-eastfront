...
void G_xpsave_writeconfig() 
{
	...
	for(i=0; g_mapstats[i]; i++) {
		...
		trap_FS_Write("axis_wins        = ", 19, f);
		G_shrubbot_writeconfig_int(g_mapstats[i]->axis_wins, f);
		trap_FS_Write("allied_wins      = ", 19, f);
		G_shrubbot_writeconfig_int(g_mapstats[i]->allied_wins, f);
		trap_FS_Write("skill_weight     = ", 19, f);
		G_shrubbot_writeconfig_float(g_mapstats[i]->skill_weight, f);
		trap_FS_Write("map_weight       = ", 19, f);
		G_shrubbot_writeconfig_float(g_mapstats[i]->map_weight, f);
		trap_FS_Write("\n", 1, f);
	}
	...
	for(i=0; g_xpsaves[i]; i++) {
		...
		if(age <= g_XPSaveMaxAge_xp.integer) {
	...
	*(cnf + len) = '\0';
	...
	while(*t) {
		...
		if(xpsave_open) {
			...
			else if(!Q_stricmp(t, "allied_wins")) {
				G_shrubbot_readconfig_int(&cnf,
					&m->allied_wins);
			}
			else if(!Q_stricmp(t, "axis_wins")) {
				G_shrubbot_readconfig_int(&cnf,
					&m->axis_wins);
			}
			else if(!Q_stricmp(t, "skill_weight")) {
				G_shrubbot_readconfig_float(&cnf,
					&m->skill_weight);
			}
			else if(!Q_stricmp(t, "map_weight")) {
				G_shrubbot_readconfig_float(&cnf,
					&m->map_weight);
			}
			else {
