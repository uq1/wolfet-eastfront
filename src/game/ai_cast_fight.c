...
//	AICast_ScriptEvent( cs, "statechange", va("%s %s", animStateStr[oldstate].string, animStateStr[newaistate].string ) );
	...
	if (!(cs->aiFlags & AIFL_DENYACTION)) {
		...
		result = 0;//BG_AnimScriptStateChange( &ent->client->ps, newaistate, oldstate );
...
//				return 256;//TESLA_SUPERSOLDIER_RANGE;
//		}
//		return (256/*TESLA_RANGE*/*0.9) - 50;	// allow for bounding box
	case WP_FLAMETHROWER:
		return (256/*FLAMETHROWER_RANGE*/*0.5) - 50;	// allow for bounding box
	case WP_PANZERFAUST:
		return 8000;
	...
	case WP_KNIFE:
		...
		case AICHAR_STIMSOLDIER3:
			return 256;//TESLA_RANGE;
		case AICHAR_ZOMBIE:	// zombie flaming attack
			return 128/*ZOMBIE_FLAME_RADIUS*/-50;		// get well within range before starting
//		}
//		break;
//	case WP_KNIFE:
//		switch (cs->aiCharacter) {
...
//		}
	...
	if (cs->aiCharacter == AICHAR_WARZOMBIE && weapnum == WP_KNIFE) {
		return qtrue;
	}
	//
...
void AICast_UpdateBattleInventory(cast_state_t *cs, int enemy)
{
	...
	for (i=0; i<WP_NUM_WEAPONS; i++) {
		if ((i>=WP_KNIFE && i<=WP_KNIFE) || (g_entities[cs->bs->entitynum].client->ps.ammo[ BG_FindAmmoForWeapon(i)] > 800)) {
...
//		return;
	...
	for (i=0; i<WP_NUM_WEAPONS; i++) {
		...
		if (!battleFunc && (i == WP_KNIFE) && cs->aifuncAttack1) {
			continue;	// only choose this weapon from within AIFunc_BattleStart()
		}
		if (!battleFunc && (i == WP_KNIFE) && cs->aifuncAttack2) {
			continue;	// only choose this weapon from within AIFunc_BattleStart()
		}
		if (!battleFunc && (i == WP_KNIFE) && cs->aifuncAttack3) {
			continue;	// only choose this weapon from within AIFunc_BattleStart()
		}
	...
	if (!gotOne && (cs->weaponNum < WP_KNIFE || cs->weaponNum > WP_KNIFE) ) {
...
bot_moveresult_t AICast_CombatMove(cast_state_t *cs, int tfl)
{
	...
	if (cs->combatGoalTime > level.time)
	{
		if (VectorLength( cs->combatGoalOrigin ) > 1) {
			...
			goal.areanum = BotPointAreaNum(cs->entityNum, cs->combatGoalOrigin);
		...
		if (	(	(cs->attributes[CAMPER] < random())
			...
			if (	(cs->attributes[TACTICAL] > 0.3 + random()*0.5)
				&&	trap_AAS_RT_GetHidePos( cs->bs->origin, cs->bs->entitynum, cs->bs->areanum, cs->vislist[cs->enemyNum].visible_pos, cs->enemyNum, BotPointAreaNum(cs->enemyNum, cs->vislist[cs->enemyNum].visible_pos), cs->combatGoalOrigin )) {
...
//	vec3_t mins = {-4,-4,-4}, maxs = {4, 4, 4};
	...
	if (cs->enemyNum < 0) return qfalse;
	//
...
qboolean AICast_CanMoveWhileFiringWeapon( int weaponnum )
{
	...
	case WP_STEN:
	case WP_GARAND:
	case WP_FG42:	//----(SA)	added
	case WP_K43:	//----(SA)	added
...
qboolean AICast_GetTakeCoverPos( cast_state_t *cs, int enemyNum, vec3_t enemyPos, vec3_t returnPos )
{
	...
	if (cs->bs->areanum && trap_AAS_RT_GetHidePos( cs->bs->origin, cs->bs->entitynum, cs->bs->areanum, enemyPos, enemyNum, BotPointAreaNum(enemyNum, enemyPos), returnPos )) {
		return qtrue;
	}
