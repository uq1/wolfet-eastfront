...
//	{"startcam",						G_ScriptAction_StartCam},
	...
	{"missionfailed",					G_ScriptAction_EndRound/*G_ScriptAction_MissionFailed*/},
...
//bani
	...
	{ "create",							G_ScriptAction_Create }, // ETEnhanced
...
#ifdef __ETE__
		else if (g_gametype.integer == GT_SUPREMACY_CAMPAIGN || g_gametype.integer == GT_SUPREMACY)
...
/*
=========================================
		tankAngle -- @fixme should be in pmove
		// this is from cg version and return only pitch 
=========================================
*/
#if 1
// code down dont work well, and this is enought just for now
float tankAngle ( gentity_t *ent ) {  
	...
	// back
		...
		// back
	trap_Trace( &trb, back, NULL, NULL, back2, 
	ent->client->ps.clientNum, CONTENTS_SOLID );

		// front
	trap_Trace( &trf, front, NULL, NULL, front2, 
	ent->client->ps.clientNum, CONTENTS_SOLID );
...
float tankAngle ( gentity_t *ent/*,vec3_t slopeangles*/ ) {  
	int i;
	trace_t	 trace;
...
//	float	pitch, roll;
	...
	testangles[1] = ent->s.apos.trBase[1];
	...
	trap_Trace( &trace, start, boxMins, boxMaxs, end, ent->client->ps.clientNum, MASK_SOLID );
...
void flametank_fire( gentity_t *ent ) {
	...
	angles[0] =  tankAngle(ent)+ent->client->ps.stats[STAT_TURRET_PITCH];
	...
	m = fire_tankflamechunk (ent, muzzle, forward);
...
void tank_fire( gentity_t *ent ) {
	...
	if (ent->client && ent->client->ps.persistant[PERS_TANKUSED] == VEHICLE_TYPE_FLAMETANK)
	{
		flametank_fire(ent);
		return;
	}
...
void ptrstank_fire( gentity_t *ent ) {//Masteries, for panzer 2
	...
	m = fire_tankptrs (ent, muzzle, forward);
}
