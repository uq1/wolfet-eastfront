...
static animStringItem_t animConditionsStr[] =
{
	...
	{"GEN_BITFLAG", -1},
	{"AISTATE", -1},
	{"PRONING", -1},		// UQ1: PRONING
	{"RELAXING", -1},		// UQ1: RELAXING
	{"QUERY", -1},			// UQ1: QUERY
	{"ALERT", -1},			// UQ1: ALERT
...
static animConditionTable_t animConditionsTable[NUM_ANIM_CONDITIONS] =
{
	...
	{ANIM_CONDTYPE_VALUE,			NULL}, // UQ1: PRONING
	{ANIM_CONDTYPE_VALUE,			NULL}, // UQ1: RELAXING
	{ANIM_CONDTYPE_VALUE,			NULL}, // UQ1: QUERY
	{ANIM_CONDTYPE_VALUE,			NULL}, // UQ1: ALERT
};
...
void BG_AnimUpdatePlayerStateConditions( pmove_t *pmove )
{
	...
	if ( ps->eFlags & EF_ZOOMING ) {
		...
		&& ps->weapon != WP_SILENCED_LUGER 
		&& ps->weapon != WP_PANZERFAUST
#ifdef __EF__
		&& ps->weapon != WP_30CAL
        && ps->weapon != WP_PTRS  //Masteries
#endif //__EF__
		&& ps->weapon != WP_MOBILE_MG42) 
	{
		BG_UpdateConditionValue( ps->clientNum, ANIM_COND_WEAPON, WP_K43_SCOPE, qtrue );	// FIXME: use some needed dummy weapon, maybe dummy_mg42 or something which can be used for this..
		BG_ClearConditionBitFlag( ps->clientNum, ANIM_COND_GEN_BITFLAG, ANIM_BITFLAG_ZOOMING );
#endif //__WEAPON_AIM_NEW__
	} else {
