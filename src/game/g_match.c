...
void UpdateKillRatings(gentity_t *killer, gentity_t *victim, int mod);
float sizeAdvantage(int num_opponent, int num_self);
void UpdatePlayerRatings(team_t winner, int ratingPoints);
void UpdateMapSkillWeights(
		float winProb, 
		float skill_probability, 
		float map_probability
	);
...
void G_TopPlayerMessage(gentity_t *ent) {
	...
	for(j=0; j<level.numConnectedClients; j++) {
		gentity_t *ent = &g_entities[level.sortedClients[j]];
		if (ent->client->sess.playerrating > top) {
			top = ent->client->sess.playerrating;
			top_ent = ent;
		}
	}

	if (top_ent) {
		CP(va("chat \"^fTop Player: ^7%s^7 ^fRating: ^3%d\"",
			top_ent->client->pers.netname,
			top_ent->client->sess.playerrating));
	}
}
	...
	if( onSameTeam ) {
		...
		if(targ->health <= 0) {
			...
			if (g_killRating.integer) {
				UpdateKillRatings(attacker,targ,mod);
			}
		}
	}
...
unsigned int G_weapStatIndex_MOD(unsigned int iWeaponMOD)
{
	...
	for(i=0; i<MOD_NUM_MODS; i++) if((int)iWeaponMOD == aWeapMOD[i].iWeapon) return(aWeapMOD[i].iWS);
	return(WS_MAX);
}
...
int G_checkServerToggle(vmCvar_t *cv)
{
	...
	// special case for 2 bits
...
#ifdef __ETE__
	} else if(cv == &g_nextmap && g_gametype.integer != GT_SUPREMACY_CAMPAIGN) {
		if( *cv->string ) {
		...
		return( qtrue );
#ifdef __ETE__
	} else if(cv == &g_nextcampaign && g_gametype.integer == GT_SUPREMACY_CAMPAIGN) {
		if( *cv->string ) {
			level.server_settings |= CV_SVS_NEXTMAP;
		} else {
			level.server_settings &= ~ CV_SVS_NEXTMAP;
...
float G_GetWinProbability(team_t team, gentity_t *ent, qboolean updateWeights) {
	...
	int axisTotalRating = 0;
	int alliesTotalRating = 0;
	int numAxis = 0;
	int numAllies = 0;
	int i;
	int ratingPoints;
	...
	for( i = 0; i < level.numConnectedClients; i++ ) {
		...
		if (cl->sess.sessionTeam == TEAM_AXIS) {
			axisTotalRating += cl->sess.playerrating;
			numAxis++;
		} else if (cl->sess.sessionTeam == TEAM_ALLIES ) {
			alliesTotalRating += cl->sess.playerrating;
			numAllies++;
		} else {
			continue;
		}
	...
	if (numAxis + numAllies < g_playerRating_minplayers.integer)
		return -1;

	// No points if one team is empty
	// This can cause problems if one team has minplayers and the other 0
	// yeah, -1 is the uh, empty team error code too . . .
	if (numAxis == 0 || numAllies == 0)
		return -1;

	axisAverageRating = axisTotalRating * 1.0 / numAxis;
	alliesAverageRating = alliesTotalRating * 1.0 / numAllies;

	if (team == TEAM_AXIS) {
		// Pad wins / losses until mean stabilizes
		// Replacing the padding with real results after more maps.
		int mapPad = max(0,((g_playerRating_mapPad.integer > 0) ? 
		              g_playerRating_mapPad.integer: 1) -
		              (mapstat->axis_wins + mapstat->allied_wins)/2);

		// Calculate winning probabilities based on logistic distribution (sigmoid)
		// use 100 for standard deviation
		// Use sizeAdvantage function to add in size advantage / disadvantage
		skill_probability = 1.0 / (1.0 + pow(10.0,(alliesAverageRating -
						axisAverageRating + sizeAdvantage(numAllies,numAxis))/100.0));

		// Calculate Map Bias for team
		map_win_probability = (mapstat->axis_wins+mapPad)*1.0 / 
		                            (mapstat->axis_wins + mapstat->allied_wins +
                                mapPad*2);
	} else { // Allied team
		// Pad wins / losses until mean stabilizes
		// Replacing the padding with real results after more maps.
		...
		// use 100 for standard deviation
		// Use sizeAdvantage function to add in size advantage / disadvantage
		skill_probability = 1.0 / (1.0 + pow(10.0,(axisAverageRating - 
						alliesAverageRating + sizeAdvantage(numAxis,numAllies))/100.0));

		// Calculate Map Bias for team
		map_win_probability = (mapstat->allied_wins+mapPad)*1.0
			                          / (mapstat->axis_wins + mapstat->allied_wins +
		                            mapPad*2);
	}

	mixed_probability = mapstat->skill_weight*skill_probability+
		                  mapstat->map_weight*map_win_probability;

	// Points are Award * skill_error  
	// Top level Error is Observation - prediction
	// Observation = 1 for win, 0 for loss
	error = (1.0 - mixed_probability);

	// error weighted by skill contribution
	skill_error = mapstat->skill_weight*error;

	ratingPoints = award*skill_error;

	if (team == TEAM_ALLIES) {
		SP(va("^4Allies      ^7%8.2f     %8d\n",
				mixed_probability, ratingPoints));
		G_LogPrintf( "Skill Prob:    %8.2f    Map Prob:    %8.2f\n", skill_probability,map_win_probability);
	} else {
		SP(va("^1Axis        ^7%8.2f     %8d\n",
				mixed_probability, ratingPoints));
		G_LogPrintf( "Skill Prob:    %8.2f    Map Prob:    %8.2f\n", skill_probability,map_win_probability);
	}

	if (updateWeights) {
		UpdateMapSkillWeights(mixed_probability,
			skill_probability,
			map_win_probability);
		UpdatePlayerRatings(team, ratingPoints);
		G_LogPrintf( "Rating Points: %d\n", ratingPoints);
		AP	(va("chat \"^fWinners get ^7%d^7 ^frating points.\"",
		ratingPoints));
		AP	(va("chat \"^fLosers get ^7-%d^7 ^frating points.\"",
				ratingPoints));
	}
...
void UpdateKillRatings(gentity_t *killer, gentity_t *victim, int mod) {
	...
	prob_killer_won = 1.0 / (1.0+pow(10.0, (victim_rating-killer_rating)/400.0));

	// award * (observation - prediction)
	points = weapon_rating*(1.0 - prob_killer_won); 
	...
	// then match
	killer_rating = killer->client->sess.match_killrating;
	victim_rating = victim->client->sess.match_killrating;
	prob_killer_won = 1.0 / (pow(10.0,(victim_rating-killer_rating)/400.0)+1.0);
	...
	victim->client->sess.match_killrating -= points;

}

float sizeAdvantage(int num_opponent, int num_self) {
	float sizeWeight = 300.0f;
	return sizeWeight*log(num_opponent*1.00/num_self);
}
...
void UpdateMapSkillWeights(
	...
	// Standard update rule (Target - Output) * Learning Rate * Input
	mapstat->skill_weight += (1.0-winProb)*learning_rate*skill_probability;
	mapstat->map_weight += (1.0-winProb)*learning_rate*map_probability;
	weight_sum = mapstat->skill_weight+mapstat->map_weight;

	// normalize weights
	mapstat->skill_weight /= weight_sum;
	mapstat->map_weight /= weight_sum;
}
...
void UpdatePlayerRatings(team_t winner, int ratingPoints) {
	...
	for( i = 0; i < level.numConnectedClients; i++ ) {
		...
		// Sanity check, shouldn't get this low.
		// This is kind of a hack, but whenever I've used this stat, I've ended up
		// with the occasional weirdness without this
		if (cl->sess.playerrating <= 0)
			cl->sess.playerrating = 1600;

		if (cl->sess.sessionTeam == winner) {
			cl->sess.playerrating += ratingPoints;
		} else {
			cl->sess.playerrating -= ratingPoints;
		}
	}
