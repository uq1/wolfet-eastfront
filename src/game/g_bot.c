...
vmCvar_t	bot_min_snipe_time;			// minimum time between bot switching away from sniper zoom...
...
#ifdef __BOT_AAS__
	if (!BotAISetupClient_ORIGINAL( clientNum, &settings )) {
...
int Bot_GetWeaponForClassAndTeam( int classNum, int teamNum, const char *weaponName ) {
	...
	// Gordon: 25/10/02: adding ability to set pliers
		...
		weapon = WP_COLT;
#ifdef __EF__
	else if( !Q_stricmp( weaponName, "PPSH" ) )
		weapon = WP_PPSH;
	else if( !Q_stricmp( weaponName, "STG44" ) )
		weapon = WP_STG44;
	else if( !Q_stricmp( weaponName, "BAR" ) )
		weapon = WP_PPS;
	else if( !Q_stricmp( weaponName, "SVT40" ) )
		weapon = WP_SVT40;
	else if( !Q_stricmp( weaponName, "30CAL" ) )
		weapon = WP_30CAL;
	else if( !Q_stricmp( weaponName, "COLT" ) )
		weapon = WP_30CAL_SET;
#endif //__EF__
