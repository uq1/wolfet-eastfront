...
**  	bg_wheel_forces.c
**
**  	Utility physics functions that are used by code in bg_physics.
**
**		Author: STONELANCE
*/
...
//	float	/*friction,*/ dot;
	...
	float	skid, /*v, angle,*/ slipangle, forwardForce;//, n2, v2;
...
//	noSlipForce = (((1.0 / 3.0) * (points[i].mass) * DotProduct(accel, forward) / sec) - ((2.0 / 3.0) * points[i].netMoment / WHEEL_RADIUS));
	...
	sideForce = (1.0f / 10.f) * points[i].mass * DotProduct(accel, right) / sec;
