...
vec4_t	etePrimaryText		=	{0.5f, 0.75f, 1.0f, 1.0f};
...
int Q_irand2(int value1, int value2)
{
...
int Q_irand3(int value1, int value2)
{
	//return irand3(value1, value2);
	int result = 0;

	//srand((unsigned)time(NULL));
	result = value1 + (int) ( random()*(value2 - value1) );
...
qboolean random_seed_set = qfalse;
extern int trap_Milliseconds( void );

#ifdef CGAMEDLL
#include "../cgame/cg_local.h"
extern qboolean AIMOD_LoadMapCoordFile ( void );
extern void AIMod_GetMapBounts ( void/*vec3_t mapMins, vec3_t mapMaxs*/ );
#endif //CGAMEDLL
#ifdef QAGAME
#include "../game/g_local.h"
#endif //QAGAME

void UQ_SetRandomSeed ( void )
{
	if (!random_seed_set)
	{// UQ1: Try to find a *decent* random seed!!! OMFG THIS SUCKS!!!
		float	seed = 0;

#if defined (CGAMEDLL) || defined (QAGAME)
#ifdef QAGAME
		int		i;
		...
		for (i = MAX_CLIENTS; i < MAX_GENTITIES; i++)
		{
			...
			if (!ent->r.currentOrigin[0])
				continue;

			seed = ent->r.currentOrigin[0];
			break;
		}
		...
		if (mapcoordsValid)
		{
			if (cg.mapcoordsMaxs[0] != 0)
				seed = cg.mapcoordsMaxs[0];
			else if (cg.mapcoordsMaxs[1] != 0)
				seed = cg.mapcoordsMaxs[1];
			else if (cg.mapcoordsMaxs[2] != 0)
				seed = cg.mapcoordsMaxs[2];
			else if (cg.mapcoordsMins[0] != 0)
				seed = cg.mapcoordsMins[0];
			else if (cg.mapcoordsMins[1] != 0)
				seed = cg.mapcoordsMins[1];
			else if (cg.mapcoordsMins[2] != 0)
				seed = cg.mapcoordsMins[2];
		...
		seed = (unsigned int)time((time_t *)NULL);
#endif //CGAMEDLL
#else //!defined (CGAMEDLL) || defined (QAGAME)
		seed = (unsigned int)time((time_t *)NULL);
#endif //defined (CGAMEDLL) || defined (QAGAME)

		srand((unsigned int)seed);
		Rand_Init((unsigned int)seed); // Set main irand's seed...
		random_seed_set = qtrue;
	}
}

int Q_TrueRand ( int low, int high )
{
	//int choice = Q_irand3(0,3);
	int ret_num = 0;
...
================
*/
void MatrixMultiply2( float in1[3][3], float in2[3][3], float out[3][3]) {
	if( in1 == out || in2 == out )
	{
		float temp[3][3];
...
__shared__ vec3_t	vec3_origin = {0,0,0};
__shared__ vec3_t	axisDefault[3] = { { 1, 0, 0 }, { 0, 1, 0 }, { 0, 0, 1 } };


__shared__ vec4_t	colorBlack		=	{0, 0, 0, 1};
__shared__ vec4_t	colorRed		=	{1, 0, 0, 1};
__shared__ vec4_t	colorGreen		=	{0, 1, 0, 1};
__shared__ vec4_t	colorBlue		=	{0, 0, 1, 1};
__shared__ vec4_t	colorYellow		=	{1, 1, 0, 1};
__shared__ vec4_t	colorOrange		=	{1, 0.5, 0, 1};
__shared__ vec4_t	colorMagenta	=	{1, 0, 1, 1};
__shared__ vec4_t	colorCyan		=	{0, 1, 1, 1};
__shared__ vec4_t	colorWhite		=	{1, 1, 1, 1};
__shared__ vec4_t	colorLtGrey		=	{0.75, 0.75, 0.75, 1};
__shared__ vec4_t	colorMdGrey		=	{0.5, 0.5, 0.5, 1};
__shared__ vec4_t	colorDkGrey		=	{0.25, 0.25, 0.25, 1};
__shared__ vec4_t	colorMdRed		=	{0.5, 0, 0, 1};
__shared__ vec4_t	colorMdGreen	=	{0, 0.5, 0, 1};
__shared__ vec4_t	colorDkGreen	=	{0, 0.20, 0, 1};
__shared__ vec4_t	colorMdCyan		=	{0, 0.5, 0.5, 1};
__shared__ vec4_t	colorMdYellow	=	{0.5, 0.5, 0, 1};
__shared__ vec4_t	colorMdOrange	=	{0.5, 0.25, 0, 1};
__shared__ vec4_t	colorMdBlue		=	{0, 0, 0.5, 1};

__shared__ vec4_t	popBG			=	{0.f,0.f,0.f,0.3f};
__shared__ vec4_t	popBorder		=	{0.28f,0.28f,0.28f,1.f};
__shared__ vec4_t	popHover		=	{0.3f,0.3f,0.3f,1.f};
__shared__ vec4_t	popText			=	{1.f,1.f,1.f,1.f};
__shared__ vec4_t	popLime			=	{0.f,1.f,0.f,0.7f};
__shared__ vec4_t	popCyan			=	{0.f,1.f,1.f,0.7f};
__shared__ vec4_t	popRed			=	{1.f,0.f,0.f,0.7f};
__shared__ vec4_t	popBlue			=	{0.f,0.f,1.f,0.7f};
__shared__ vec4_t	popOrange		=	{1.f,0.63f,0.1f,0.7f};
__shared__ vec4_t	popDefaultGrey	=	{0.38f,0.38f,0.38f,1.0f};
__shared__ vec4_t	popLightGrey	=	{0.5f,0.5f,0.5f,1.0f};
__shared__ vec4_t	popDarkGrey		=	{0.33f,0.33f,0.33f,1.0f};
__shared__ vec4_t	popAlmostWhite	=	{0.83f,0.81f,0.71f,1.0f};
__shared__ vec4_t	popAlmostBlack	=	{0.16f,0.16f,0.16f,1.0f};

__shared__ vec4_t	etePrimaryText		=	{0.5f, 0.75f, 1.0f, 1.0f};

__shared__ vec4_t		clrBrown =			{0.68f,			0.68f,			0.56f,			1.f};
__shared__ vec4_t		clrBrownDk =		{0.58f * 0.75f,	0.58f * 0.75f,	0.46f * 0.75f,	1.f};
__shared__ vec4_t		clrBrownLine =		{0.0525f,		0.05f,			0.025f,			0.2f};
__shared__ vec4_t		clrBrownLineFull =	{0.0525f,		0.05f,			0.025f,			1.f};

__shared__ vec4_t		clrBrownTextLt2 =	{108*1.8/255.f,		88*1.8/255.f,	62*1.8/255.f,	1.f};
__shared__ vec4_t		clrBrownTextLt =	{108*1.3/255.f,		88*1.3/255.f,	62*1.3/255.f,	1.f};
__shared__ vec4_t		clrBrownText =		{108/255.f,			88/255.f,		62/255.f,		1.f};
__shared__ vec4_t		clrBrownTextDk =	{20/255.f,			2/255.f,		0/255.f,		1.f};
__shared__ vec4_t		clrBrownTextDk2 =	{108*0.75/255.f,	88*0.75/255.f,	62*0.75/255.f,	1.f};

__shared__ vec4_t	g_color_table[32] =
	{
		{ 0.0,	0.0,	0.0,	1.0 },	// 0 - black		0
		{ 1.0,	0.0,	0.0,	1.0 },	// 1 - red			1
		{ 0.0,	1.0,	0.0,	1.0 },	// 2 - green		2
...
__shared__ vec3_t	bytedirs[NUMVERTEXNORMALS] =
{
{-0.525731, 0.000000, 0.850651}, {-0.442863, 0.238856, 0.864188}, 
{-0.295242, 0.000000, 0.955423}, {-0.309017, 0.500000, 0.809017}, 
{-0.162460, 0.262866, 0.951056}, {0.000000, 0.000000, 1.000000}, 
...
__device__ __host__ int		Q_rand( int *seed ) {
	*seed = (69069 * *seed + 1);
	return *seed;
}

__device__ __host__ float	Q_random( int *seed ) {
	return ( Q_rand( seed ) & 0xffff ) / (float)0x10000;
}

__device__ __host__ float	Q_crandom( int *seed ) {
	return 2.0 * ( Q_random( seed ) - 0.5 );
}
...
__device__ __host__ signed char ClampChar( int i ) {
	if ( i < -128 ) {
		return -128;
	}
	if ( i > 127 ) {
...
__device__ __host__ signed short ClampShort( int i ) {
	if ( i < -32768 ) {
		return -32768;
	}
	if ( i > 0x7fff ) {
...
// this isn't a real cheap function to call!
__device__ __host__ int DirToByte( vec3_t dir ) {
	__shared__ int		i, best;
	__shared__ float	d, bestd;
...
__device__ __host__ void ByteToDir( int b, vec3_t dir ) {
	if ( b < 0 || b >= NUMVERTEXNORMALS ) {
		VectorCopy( vec3_origin, dir );
		return;
	}
...
__device__ __host__ unsigned ColorBytes3 (float r, float g, float b) {
	__shared__ unsigned	i;
...
__device__ __host__ unsigned ColorBytes4 (float r, float g, float b, float a) {
	__shared__ unsigned	i;
...
__device__ __host__ float NormalizeColor( const vec3_t in, vec3_t out ) {
...
=====================
*/
__device__ __host__ qboolean PlaneFromPoints( vec4_t plane, const vec3_t a, const vec3_t b, const vec3_t c ) {
	__shared__ vec3_t	d1, d2;
...
===============
*/
__device__ __host__ void RotatePointAroundVector( vec3_t dst, const vec3_t dir, const vec3_t point,
							 float degrees ) {
	__shared__ float	m[3][3];
	__shared__ float	im[3][3];
	__shared__ float	zrot[3][3];
	__shared__ float	tmpmat[3][3];
	__shared__ float	rot[3][3];
	__shared__ int	i;
	__shared__ vec3_t vr, vup, vf;
...
===============
*/
__device__ __host__ void RotatePointAroundVertex ( vec3_t pnt, float rot_x, float rot_y, float rot_z, const vec3_t origin ) {
	__shared__ float tmp[11];
	//float rad_x, rad_y, rad_z;
...
===============
*/
__device__ __host__ void RotateAroundDirection( vec3_t axis[3], float yaw ) {
...
__device__ __host__ void vectoangles( const vec3_t value1, vec3_t angles ) {
	__shared__ float	forward;
	__shared__ float	yaw, pitch;
...
=================
*/
__device__ __host__ void AnglesToAxis( const vec3_t angles, vec3_t axis[3] ) {
	__shared__ vec3_t	right;
...
__device__ __host__ void AxisClear( vec3_t axis[3] ) {
	axis[0][0] = 1;
	axis[0][1] = 0;
	axis[0][2] = 0;
	axis[1][0] = 0;
...
__device__ __host__ void AxisCopy( vec3_t in[3], vec3_t out[3] ) {
	VectorCopy( in[0], out[0] );
	VectorCopy( in[1], out[1] );
	VectorCopy( in[2], out[2] );
}

__device__ __host__ void ProjectPointOnPlane( vec3_t dst, const vec3_t p, const vec3_t normal )
{
	__shared__ float d;
	__shared__ vec3_t n;
...
================
*/
__device__ __host__ void MakeNormalVectors( const vec3_t forward, vec3_t right, vec3_t up) {
	__shared__ float		d;
...
__device__ __host__ void VectorRotate( vec3_t in, vec3_t matrix[3], vec3_t out )
{
	out[0] = DotProduct( in, matrix[0] );
	out[1] = DotProduct( in, matrix[1] );
	out[2] = DotProduct( in, matrix[2] );
...
** float q_rsqrt( float number )
*/
__device__ __host__ float Q_rsqrt( float number )
{
	__shared__ long i;
	__shared__ float x2, y;
	__shared__ const float threehalfs = 1.5F;
...
__device__ __host__ float Q_fabs( float f ) {
	__shared__ int tmp = (*(int*)&f) & 0x7FFFFFFF;
	return *(float*)&tmp;
}

#if id386 && !( (defined __linux__ || defined __FreeBSD__ || defined __GNUC__ ) && (defined __i386__ ) ) // rb010123
__device__ __host__ long myftol( float f ) {
	__shared__ static int tmp;
	__asm fld f
	__asm fistp tmp
	__asm mov eax, tmp
}
...
===============
*/
__device__ __host__ float LerpAngle (float from, float to, float frac)
{
	if ( to - from > 180 ) {
		to -= 360;
	}
...
__device__ __host__ void LerpPosition( vec3_t start, vec3_t end, float frac, vec3_t out) {
	__shared__ vec3_t dist;
...
=================
*/
__device__ __host__ float AngleSubtract( float a1, float a2 )
{
	__shared__ float a = a1 - a2;
...
__device__ __host__ void AnglesSubtract( vec3_t v1, vec3_t v2, vec3_t v3 )
{
	v3[0] = AngleSubtract( v1[0], v2[0] );
	v3[1] = AngleSubtract( v1[1], v2[1] );
	v3[2] = AngleSubtract( v1[2], v2[2] );
...
__device__ __host__ float AngleMod(float a)
{
	return((360.0 / 65536) * ((int)(a * (65536 / 360.0)) & 65535));
}
...
=================
*/
__device__ __host__ float AngleNormalize2Pi ( float angle ) {
	return DEG2RAD( AngleNormalize360( RAD2DEG( angle ) ) );
}
...
=================
*/
__device__ __host__ float AngleNormalize360 ( float angle ) {
	return (360.0 / 65536) * ((int)(angle * (65536 / 360.0)) & 65535);
}
...
// 
// returns angle normalized to the range [0 <= angle < 65536]
__device__ __host__ unsigned int AngleNormalizeInt(int angle)
{
	if(angle < 0)
		angle = (65536 + (angle % 65536));
	return (angle % 65536);
...
=================
*/
__device__ __host__ float AngleNormalize180 ( float angle ) {
	angle = AngleNormalize360( angle );
	if ( angle > 180.0 ) {
		angle -= 360.0;
	}
...
=================
*/
__device__ __host__ float AngleDelta ( float angle1, float angle2 ) {
	return AngleNormalize180( angle1 - angle2 );
}
...
=================
*/
__device__ __host__ void SetPlaneSignbits (cplane_t *out) {
	__shared__ int	bits, j;
...
// this is the slow, general version
__device__ __host__ int BoxOnPlaneSide2 (vec3_t emins, vec3_t emaxs, struct cplane_s *p)
{
	int		i;
	float	dist1, dist2;
	int		sides;
...
#if !(defined __linux__ && defined __i386__ && !defined C_ONLY)
#if defined __LCC__ || defined C_ONLY || !id386 || __GNUC__
__device__ __host__ int BoxOnPlaneSide (vec3_t emins, vec3_t emaxs, struct cplane_s *p)
{
	float	dist1, dist2;
	int		sides;
...
__device__ __host__ __inline __declspec( naked ) int BoxOnPlaneSide_fast (vec3_t emins, vec3_t emaxs, struct cplane_s *p)
{
	__shared__ static int bops_initialized;
	__shared__ static int Ljmptab[8];
...
=================
*/
__device__ __host__ float RadiusFromBounds( const vec3_t mins, const vec3_t maxs ) {
	__shared__ int		i;
	__shared__ vec3_t	corner;
	__shared__ float	a, b;
...
__device__ __host__ void ClearBounds( vec3_t mins, vec3_t maxs ) {
	mins[0] = mins[1] = mins[2] = 99999;
	maxs[0] = maxs[1] = maxs[2] = -99999;
}

__device__ __host__ void AddPointToBounds( const vec3_t v, vec3_t mins, vec3_t maxs ) {
	if ( v[0] < mins[0] ) {
		mins[0] = v[0];
	}
	if ( v[0] > maxs[0]) {
...
__device__ __host__ qboolean PointInBounds( const vec3_t v, const vec3_t mins, const vec3_t maxs ) {
	if ( v[0] < mins[0] ) {
		return qfalse;
	}
	if ( v[0] > maxs[0]) {
...
__device__ __host__ int VectorCompare( const vec3_t v1, const vec3_t v2 ) {
	if (v1[0] != v2[0] || v1[1] != v2[1] || v1[2] != v2[2]) {
		return 0;
	}
...
__device__ __host__ vec_t VectorNormalize( vec3_t v ) {
	__shared__ float	length, ilength;
...
// that length != 0, nor does it return length
//
__device__ __host__ void VectorNormalizeFast( vec3_t v )
{
	__shared__ float ilength;
...
__device__ __host__ vec_t VectorNormalize2( const vec3_t v, vec3_t out) {
	__shared__ float	length, ilength;
...
__device__ __host__ void _VectorMA( const vec3_t veca, float scale, const vec3_t vecb, vec3_t vecc) {
	vecc[0] = veca[0] + scale*vecb[0];
	vecc[1] = veca[1] + scale*vecb[1];
	vecc[2] = veca[2] + scale*vecb[2];
}
...
__device__ __host__ vec_t _DotProduct( const vec3_t v1, const vec3_t v2 ) {
	return v1[0]*v2[0] + v1[1]*v2[1] + v1[2]*v2[2];
}

__device__ __host__ void _VectorSubtract( const vec3_t veca, const vec3_t vecb, vec3_t out ) {
	out[0] = veca[0]-vecb[0];
	out[1] = veca[1]-vecb[1];
	out[2] = veca[2]-vecb[2];
}

__device__ __host__ void _VectorAdd( const vec3_t veca, const vec3_t vecb, vec3_t out ) {
	out[0] = veca[0]+vecb[0];
	out[1] = veca[1]+vecb[1];
	out[2] = veca[2]+vecb[2];
}

__device__ __host__ void _VectorCopy( const vec3_t in, vec3_t out ) {
	out[0] = in[0];
	out[1] = in[1];
	out[2] = in[2];
}

__device__ __host__ void _VectorScale( const vec3_t in, vec_t scale, vec3_t out ) {
	out[0] = in[0]*scale;
	out[1] = in[1]*scale;
	out[2] = in[2]*scale;
}

__device__ __host__ void CrossProduct( const vec3_t v1, const vec3_t v2, vec3_t cross ) {
	cross[0] = v1[1]*v2[2] - v1[2]*v2[1];
	cross[1] = v1[2]*v2[0] - v1[0]*v2[2];
	cross[2] = v1[0]*v2[1] - v1[1]*v2[0];
}

__device__ __host__ vec_t VectorLength( const vec3_t v ) {
	return sqrt (v[0]*v[0] + v[1]*v[1] + v[2]*v[2]);
}

__device__ __host__ vec_t VectorLengthSquared( const vec3_t v ) {
	return (v[0]*v[0] + v[1]*v[1] + v[2]*v[2]);
}

__device__ __host__ vec_t Distance( const vec3_t p1, const vec3_t p2 ) {
	__shared__ vec3_t	v;
...
__device__ __host__ vec_t DistanceSquared( const vec3_t p1, const vec3_t p2 ) {
	__shared__ vec3_t	v;
...
__device__ __host__ void VectorInverse( vec3_t v ){
	v[0] = -v[0];
	v[1] = -v[1];
	v[2] = -v[2];
}

__device__ __host__ void Vector4Scale( const vec4_t in, vec_t scale, vec4_t out ) {
	out[0] = in[0]*scale;
	out[1] = in[1]*scale;
	out[2] = in[2]*scale;
	out[3] = in[3]*scale;
...
__device__ __host__ int Q_log2( int val ) {
	__shared__ int answer;
...
*/
/*
__device__ __host__ int	PlaneTypeForNormal (vec3_t normal) {
	if ( normal[0] == 1.0 )
		return PLANE_X;
	if ( normal[1] == 1.0 )
		return PLANE_Y;
...
================
*/
__device__ __host__ void MatrixMultiply(float in1[3][3], float in2[3][3], float out[3][3]) {
	out[0][0] = in1[0][0] * in2[0][0] + in1[0][1] * in2[1][0] + in1[0][2] * in2[2][0];
	out[0][1] = in1[0][0] * in2[0][1] + in1[0][1] * in2[1][1] + in1[0][2] * in2[2][1];
	out[0][2] = in1[0][0] * in2[0][2] + in1[0][1] * in2[1][2] + in1[0][2] * in2[2][2];
	out[1][0] = in1[1][0] * in2[0][0] + in1[1][1] * in2[1][0] + in1[1][2] * in2[2][0];
...
================
*/
__device__ __host__ void VectorMatrixMultiply( const vec3_t p, vec3_t m[ 3 ], vec3_t out )
{
	out[ 0 ] = m[ 0 ][ 0 ] * p[ 0 ] + m[ 1 ][ 0 ] * p[ 1 ] + m[ 2 ][ 0 ] * p[ 2 ];
	out[ 1 ] = m[ 0 ][ 1 ] * p[ 0 ] + m[ 1 ][ 1 ] * p[ 1 ] + m[ 2 ][ 1 ] * p[ 2 ];
	out[ 2 ] = m[ 0 ][ 2 ] * p[ 0 ] + m[ 1 ][ 2 ] * p[ 1 ] + m[ 2 ][ 2 ] * p[ 2 ];
...
__device__ __host__ void AngleVectors( const vec3_t angles, vec3_t forward, vec3_t right, vec3_t up) {
	__shared__ float		angle;
	__shared__ static float		sr, sp, sy, cr, cp, cy;
	// static to help MS compiler fp bugs
...
** assumes "src" is normalized
*/
__device__ __host__ void PerpendicularVector( vec3_t dst, const vec3_t src )
{
	__shared__ int	pos;
	__shared__ int i;
	__shared__ float minelem = 1.0F;
	__shared__ vec3_t tempvec;
...
=================
*/
__device__ __host__ void GetPerpendicularViewVector( const vec3_t point, const vec3_t p1, const vec3_t p2, vec3_t up )
{
	__shared__ vec3_t	v1, v2;
...
================
*/
__device__ __host__ void ProjectPointOntoVector( vec3_t point, vec3_t vStart, vec3_t vEnd, vec3_t vProj )
{
	__shared__ vec3_t pVec, vec;
...
================
*/
__device__ __host__ void ProjectPointOntoVectorBounded( vec3_t point, vec3_t vStart, vec3_t vEnd, vec3_t vProj )
{
	__shared__ vec3_t pVec, vec;
	__shared__ int j;
...
================
*/
__device__ __host__ float DistanceFromLineSquared(vec3_t p, vec3_t lp1, vec3_t lp2) {
	__shared__ vec3_t proj, t;
	__shared__ int j;
...
================
*/
__device__ __host__ float DistanceFromVectorSquared(vec3_t p, vec3_t lp1, vec3_t lp2) {
	__shared__ vec3_t proj, t;
...
__device__ __host__ float vectoyaw( const vec3_t vec ) {
	__shared__ float	yaw;
...
=================
*/
__device__ __host__ void AxisToAngles( vec3_t axis[3], vec3_t angles ) {
	__shared__ vec3_t right, roll_angles, tvec;
...
__device__ __host__ float VectorDistance(vec3_t v1, vec3_t v2)
{
...
__device__ __host__ float VectorDistanceSquared(vec3_t v1, vec3_t v2) {
	__shared__ vec3_t dir;
...
#ifdef _MSC_VER
__device__ __host__ int rint (double x) {
	__shared__ int i;
...
__shared__ static unsigned long	holdrand = 0x89abcdef;

__device__ __host__ void Rand_Init(int seed)
{
	holdrand = seed;
}
...
__device__ __host__ float flrand(float min, float max)
{
...
__device__ __host__ float Q_flrand(float min, float max)
{
	return flrand(min,max);
}
...
__device__ __host__ int irand(int min, int max)
{
...
__device__ __host__ int Q_irand(int value1, int value2)
{
	return irand(value1, value2);
}
...
__device__ __host__ int Q_irand2(int value1, int value2)
{
	//return irand2(value1, value2);
	__shared__ int number = 0;
	__shared__ int diff = value2 - value1;
...
__device__ __host__ int Q_irand3(int value1, int value2)
{
	//return irand3(value1, value2);
...
__device__ __host__ void int_swap (int *a, int *b)
{
	__shared__ int x;
...
__device__ __host__ int random_int(int low, int high)
{
	__shared__ int rand();
	__shared__ int i,j,r;                                      /*random number*/
...
__device__ __host__ void random_permutation(int a[], int n)
{
	__shared__ int i;
...
__shared__ static unsigned long rand_next = 1;

__device__ __host__ int new_irand(void)
{
    rand_next = rand_next * 1103515245 + 12345;
    return ((unsigned int) (rand_next / 65536UL) % 32767UL);
}

__device__ __host__ void new_srand(unsigned int seed)
{
    rand_next = seed;
}
...
__device__ __host__ int		Q_rand2( int seed ) {
	seed = (69069 * seed + 1);
	return seed;
}
...
__device__ __host__ double drand48()
{
	__shared__ double x = 0;
	__shared__ double denom = RAND_MAX + 1.;
	__shared__ double need;
...
__device__ __host__ void UQ_SetRandomSeed ( void )
{
	if (!random_seed_set)
	{// UQ1: Try to find a *decent* random seed!!! OMFG THIS SUCKS!!!
		__shared__ float	seed = 0;
		...
		for (i = MAX_CLIENTS; i < MAX_GENTITIES; i++)
		{
			__shared__ gentity_t *ent = &g_entities[i];
...
__device__ __host__ int Q_TrueRand ( int low, int high )
{
	//int choice = Q_irand3(0,3);
	__shared__ int ret_num = 0;
...
================================================================================
*/
__device__ __host__ qboolean VectorNAN( const vec3_t vec ){
	if (IS_NAN(vec[0]) || IS_NAN(vec[1]) || IS_NAN(vec[2])){
		return qtrue;
	}
...
================
*/
__device__ __host__ void AnglesToOrientation( const vec3_t angles, float t[3][3] ) {
	__shared__ float		angle;
	__shared__ static float		sr, sp, sy, cr, cp, cy;
	// static to help MS compiler fp bugs
...
================
*/
__device__ __host__ void OrientationToVectors( float t[3][3], vec3_t forward, vec3_t right, vec3_t up ) {
	right[0] = t[0][0];		right[1] = t[1][0];		right[2] = t[2][0];
	forward[0] = t[0][1];	forward[1] = t[1][1];	forward[2] = t[2][1];
	up[0] = t[0][2];		up[1] = t[1][2];		up[2] = t[2][2];
...
================
*/
__device__ __host__ void MatrixMultiply2( float in1[3][3], float in2[3][3], float out[3][3]) {
	if( in1 == out || in2 == out )
	{
...
================
*/
__device__ __host__ void MatrixTranspose( float in[3][3], float out[3][3] ) {
	out[0][0] = in[0][0];
	out[0][1] = in[1][0];
	out[0][2] = in[2][0];
	out[1][0] = in[0][1];
...
================
*/
__device__ __host__ void MatrixAdd( float in1[3][3], float in2[3][3], float out[3][3] ) {
	out[0][0] = in1[0][0] + in2[0][0];
	out[0][1] = in1[0][1] + in2[0][1];
	out[0][2] = in1[0][2] + in2[0][2];
	out[1][0] = in1[1][0] + in2[1][0];
...
================
*/
__device__ __host__ void MatrixScale( float in[3][3], float s, float out[3][3] ) {
	out[0][0] = in[0][0] * s;
	out[0][1] = in[0][1] * s;
	out[0][2] = in[0][2] * s;
	out[1][0] = in[1][0] * s;
...
================
*/
__device__ __host__ void OrthonormalizeOrientation( float t[3][3] ){
	__shared__ vec3_t	x, y, z;
...
================
*/
__device__ __host__ float QuaternionLengthSquared( const vec4_t q ){
	return q[0]*q[0] + q[1]*q[1] + q[2]*q[2] + q[3]*q[3];
}
...
================
*/
__device__ __host__ float QuaternionLength( const vec4_t q ){
	return sqrt( q[0]*q[0] + q[1]*q[1] + q[2]*q[2] + q[3]*q[3] );
}
...
================
*/
__device__ __host__ float QuaternionFastNormalize( vec4_t quat )
{
	// Check if quaternion needs normalizing
	__shared__ float		len = QuaternionLengthSquared( quat );
...
================
*/
__device__ __host__ float QuaternionNormalize( vec4_t quat )
{
	__shared__ float	tmp, len;
...
================
*/
__device__ __host__ void QuaternionMultiply(const vec4_t in1, const vec4_t in2, vec4_t out){
/*
	// from racer
	w = in1[3] * in2[3] - in1[0] * in2[0] - in1[1] * in2[1] - in1[2] * in2[2];
	x = in1[3] * in2[0] + in1[0] * in2[3] + in1[1] * in2[2] - in1[2] * in2[1];
...
*/
	...
	__shared__ vec3_t temp;  // in case of mulp(a, b, a)
...
================
*/
__device__ __host__ void QuaternionRotate( const vec4_t quat, const vec3_t w, const float time, vec4_t out){
	__shared__ vec3_t	tempVec;
...
================
*/
__device__ __host__ void QuaternionSLERP(const vec4_t from, const vec4_t to, float t, vec4_t res){
/*
	// from racer
	float	theta, costheta, w1, w2, sintheta;
...
================
*/
__device__ __host__ void OrientationToQuaternion( float t[3][3], vec4_t quat ){

// from racer
	__shared__ int		i;
	__shared__ float	qw2, qx2, qy2, qz2, tmp;
...
================
*/
__device__ __host__ void AnglesToQuaternion( const vec3_t angles, vec4_t quat){
	__shared__ float	t[3][3];
...
================
*/
__device__ __host__ void QuaternionToOrientation( const vec4_t quat, float t[3][3] ){
	__shared__ float	xy, wz, xz, wy, yz, wx, x2, y2, z2;
...
================
*/
__device__ __host__ void OrientationToAngles( float t[3][3], vec3_t angles ) {
	__shared__ vec3_t	forward, right, up;
	__shared__ float	cp, as;
...
================
*/
__device__ __host__ void QuaternionToAngles( const vec4_t quat, vec3_t angles ){
	__shared__ float	t[3][3];
...
================
*/
__device__ __host__ void QuaternionL2ToOrientation( const vec4_t quat, const float l2, float t[3][3] ){
	__shared__ float	xy, wz, xz, wy, yz, wx, x2, y2, z2, s;
...
================
*/
__device__ __host__ void QuaternionToVectors( const vec4_t quat, vec3_t forward, vec3_t right, vec3_t up ) {
	__shared__ float	t[3][3];
...
===============
*/
__device__ __host__ float round_float ( float v )
{
	if ( v >= 0.5f )
	{
		return ( ceil( v) );
