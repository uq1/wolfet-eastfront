...
float SetLCVPAngle(float angle, float ideal_angle, float speed) 
{
	float move;
...
{// Just keep bot->bot_ideal_view_angles correct and call this to make it realistic...
	...
	if (NPC->npc_ideal_view_angles[PITCH] > 180) NPC->npc_ideal_view_angles[PITCH] -= 360;
	...
	for (i = 0; i < 2; i++) 
	{
		...
		NPC->npc_view_angles[i] = SetLCVPAngle(NPC->npc_view_angles[i],
										NPC->npc_ideal_view_angles[i], anglespeed);
	}
...
void LanderWaypointRender(void)
{
	int i, inc_checker;
	gentity_t *plum;
...
void NPC_LCVP_Setup_Waypoints( gentity_t *ent )
{
	int i = 0;
...
void G_SetEntitySpawnpoint( gentity_t *entity, gentity_t* LCVP, int spawnPointNum )
{
	...
	{
		G_Printf("TAG ERROR: %s does not exist on model %s (%i).\n", va("tag_spawnpoint%i", spawnPointNum), parent->model, parent->s.modelindex);
	}
...
void NPC_LCVP_Disembark_Passengers( gentity_t *ent )
...
qboolean NPC_LCVP_Check_Wait( gentity_t *ent )
...
void NPC_LCVP_Setup( gentity_t *ent )
{
	int i = 0;
	qboolean found = qfalse;
...
int LCVP_NPC_Filler_Spawn( gentity_t *CallEnt )
{
	vec3_t fwd;
	...
	NPC->playerType = Q_TrueRand(PC_GENERALINFANTRY, NUM_PLAYER_CLASSES-1);
...
void NPC_LCVP_Touch (gentity_t *self, gentity_t *other, trace_t *trace)
{
	float	ratio;
	vec3_t	v;
...
qboolean CheckLCVPsOnThisMap ( void )
...
int GetCurrentLCVPEntityNum ( void )
{
	int i = 0;
	qboolean found = qfalse;
	int entityNum = -1;
...
int GetCurrentLCVPSpawnpointNum ( int entityNum )
{
	int i = 0;
	qboolean found = qfalse;
	int spawnpoint = -1;
...
float SetDestroyerAngle(float angle, float ideal_angle, float speed) 
{
	float move;
...
{// Just keep bot->bot_ideal_view_angles correct and call this to make it realistic...
	...
	for (i = 0; i < 2; i++) 
	{
		...
		anglespeed = diff * factor;
		if (anglespeed > maxchange) anglespeed = maxchange;
		NPC->npc_view_angles[i] = SetDestroyerAngle(NPC->npc_view_angles[i],
										NPC->npc_ideal_view_angles[i], anglespeed);
	}
...
void DestroyerWaypointRender(void)
{
	int i, inc_checker;
	gentity_t *plum;
...
void NPC_Destroyer_Setup_Waypoints( gentity_t *ent )
{
	int i = 0;
	// ent->health is our first waypoint, so...
	int current_waypoint = ent->health;
...
void NPC_Destroyer_Setup( gentity_t *ent )
{
	int i = 0;
	qboolean found = qfalse;
...
void NPC_Destroyer_DoSpawn( void )
{
	gentity_t *NPC = G_Spawn();
	vec3_t spawn_angles;
	int posNum = 0;
...
void NPC_AI_Boat_Move( gentity_t *NPC )
{
...
void NPC_AI_Boat_Think( gentity_t *NPC )
{
	if ( level.intermissiontime )
