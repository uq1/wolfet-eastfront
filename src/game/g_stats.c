...
void G_AddSkillPoints( gentity_t *ent, skillType_t skill, float points ) {
	...
	oldpts = (int)(ent->client->sess.skillpoints[skill]);
	ent->client->sess.skillpoints[skill] += points;

	if ( oldpts < (int)(ent->client->sess.skillpoints[skill]) ) {
		trap_SendServerCommand(ent - g_entities, va("skpt %d %d", skill, (int)(ent->client->sess.skillpoints[skill])));
	}
