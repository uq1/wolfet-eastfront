...
{
	//VectorSet(outpos, 0, 0, 0);
	VectorCopy(g_entities[enemynum].r.currentOrigin, outpos);
	return 0;
}
...
//int pretime = Sys_MilliSeconds();
	...
	if (entnum >= 0 && entnum < level.maxclients && caststates[entnum].lastValidAreaTime[cs->aasWorldIndex] > level.time - 100) {
		...
		goal.areanum = BotPointAreaNum(entnum, pos);
...
//			ent->flags |= FL_DEFENSE_CROUCH;
	...
	case AICHAR_HELGA:
		...
		if ((cs->weaponFireTimes[WP_KNIFE] && (cs->weaponFireTimes[WP_KNIFE] > level.time - 12000)) ||
			(cs->weaponFireTimes[WP_KNIFE] && (cs->weaponFireTimes[WP_KNIFE] > level.time - 6000))) {
	...
	case AICHAR_HEINRICH:
		if (	(ent->health <= 0.25*cs->attributes[STARTING_HEALTH])
			||	(cs->weaponFireTimes[WP_KNIFE] > level.time - 6000)	// walk for period after attack
			||	(cs->weaponFireTimes[WP_KNIFE]%8000 < 3000)) {			// dont run constantly
			cs->actionFlags |= CASTACTION_WALK;
		} else {	// charging
			cs->actionFlags &= ~CASTACTION_WALK;
		}
		...
		break;
	case AICHAR_ZOMBIE:
		if (COM_BitCheck(cs->bs->cur_ps.weapons, WP_KNIFE)) {	// flaming zombie, run
			BG_UpdateConditionValue( cs->entityNum, ANIM_COND_CHARGING, 1, qfalse );
		}
		break;
	}
...
//	gclient_t *client = &level.clients[cs->entityNum];
	...
	vectoangles( vec, cs->ideal_viewangles );
	// do the roll
	duration = 500;//BG_AnimScriptEvent( &g_entities[cs->entityNum].client->ps, ANIM_ET_ROLL, qfalse, qtrue );
	//
...
//	gclient_t *client = &level.clients[cs->entityNum];
	...
	// do the roll
	duration = 500;//BG_AnimScriptEvent( &g_entities[cs->entityNum].client->ps, ANIM_ET_DIVE, qfalse, qtrue );
	//
	if (duration < 0) {	// it failed
		return NULL;
	}
...
//	gclient_t *client = &level.clients[cs->entityNum];
	...
	// do the roll
	duration = 500;//BG_AnimScriptEvent( &g_entities[cs->entityNum].client->ps, ANIM_ET_ROLL, qfalse, qfalse );
	//
	if (duration < 0) {	// it failed
		return NULL;
	}
...
/*	if ((cs->aiCharacter == AICHAR_ZOMBIE) &&
	...
	if (!moved && cs->weaponNum >= WP_LUGER && cs->weaponNum <= WP_MAX_PRIMARY && cs->attributes[TACTICAL] >= 0.1) {	
		//
	...
	if ((cs->attributes[RUNNING_SPEED] > 170) && (cs->weaponNum != WP_KNIFE) && (level.time < (cs->startBattleChaseTime + BATTLE_CHASE_ACCEL_TIME))) {
		float	ideal;
	...
	// if we are going to reach them soon, predict the attack
	{
		...
		//
			...
			destarea = BotPointAreaNum(cs->entityNum, destorg);
			simarea = BotPointAreaNum(cs->entityNum, move.endpos);
...
char *AIFunc_Battle( cast_state_t *cs )
{
	...
	*/
	...
	//
	// reload?
	if ( (cs->bs->cur_ps.weaponstate != WEAPON_RELOADING) && (cs->bs->cur_ps.ammoclip[BG_FindClipForWeapon(cs->bs->cur_ps.weapon)] < (int)(GetAmmoTableData(cs->bs->cur_ps.weapon)->uses)) ) {
		if (AICast_GotEnoughAmmoForWeapon( cs, cs->weaponNum )) {
...
recheck:
	...
	if (fabs(AngleDifference(cs->ideal_viewangles[YAW], cs->viewangles[YAW])) < 10) {
		...
		//
		if ((cs->weaponNum == WP_KNIFE) && cs->aifuncAttack1) {
			if (AICast_CheckAttack( cs, cs->enemyNum, qfalse ))
				rval = cs->aifuncAttack1( cs );
			else
				rval = AIFunc_BattleChaseStart( cs );
		} else if ((cs->weaponNum == WP_KNIFE) && cs->aifuncAttack2) {
			if (AICast_CheckAttack( cs, cs->enemyNum, qfalse ))
				rval = cs->aifuncAttack2( cs );
			else
				rval = AIFunc_BattleChaseStart( cs );
		} else if ((cs->weaponNum == WP_KNIFE) && cs->aifuncAttack3) {
			if (AICast_CheckAttack( cs, cs->enemyNum, qfalse ))
		...
		//
		if (!rval && cs->weaponNum >= WP_KNIFE && cs->weaponNum <= WP_KNIFE) {
	...
	if (!rval) {	// use the generic battle routine for all "normal" weapons
		if (cs->weaponNum >= WP_KNIFE && cs->weaponNum <= WP_KNIFE) {
