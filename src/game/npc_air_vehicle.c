...
extern qboolean G_NPCPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle );
...
float G_GetSkyHeightAtPoint( vec3_t pos )
{
	vec3_t roof;
	trace_t trace;
...
void NPC_Bomber_Spawn( vec3_t marker_origin, int marker_entitynum, gentity_t *owner )
{
	...
	vec3_t fwd, tmpangles, traceangle, roof, spawnpoint_origin, spawnpoint_angles, spawn_location, spawn_angles, exit_location;
	trace_t trace;
	...
	spawnpoint = SelectCTFSpawnPoint(owner->client->sess.sessionTeam, owner->client->pers.teamState.state, spawnpoint_origin, spawnpoint_angles, owner->client->sess.spawnObjectiveIndex);
...
void NPC_Plane_Spawn( vec3_t marker_origin )
{
	gentity_t *NPC = G_Spawn();
	...
	VectorCopy(spawn_location, NPC->s.origin);
	VectorCopy(spawn_location, NPC->npc_view_angles);
	NPC->npc_view_angles[2]-=8192;

	NPC_Setup( NPC );

	// Use a formation or not???
	{
		int form_check = Q_irand(0,9);
	...
	if (NPC->s.nextWeapon > 0)
	{
		if (NPC->s.onFireStart > 0)
		{// Pick the plane in the formation to have the smoke...
			NPC->s.onFireEnd = Q_irand(0,4);
		}
	}
...
void NPC_Plane_Cinematics ( void )
{// Call this each frame to add cinematic planes to a map...
	...
	else
	{
		...
		center_origin[2] = G_GetSkyHeightAtPoint(center_origin) - 8;
...
void NPC_AI_Air_Move( gentity_t *NPC )
{
	...
	{// Planes type NPC routing here... Just head forward for now...
		...
		if (NPC->freetime < level.time - 1000)
		{
			G_FreeEntity(NPC);
			return;
		}
		...
		/*{// Do we need AAGUN fire at this plane yet???
			...
			VectorCopy(NPC->npc_view_angles, fake_aagun->s.origin);
			G_SetOrigin(fake_aagun, NPC->npc_view_angles);
			VectorSubtract( NPC->npc_view_angles, NPC->r.currentOrigin, temp_angle );
			vectoangles( temp_angle, angles );
			angles[0]+=Q_TrueRand(-16, 16);
			angles[1]+=Q_TrueRand(-16, 16);
			angles[2]+=Q_TrueRand(-16, 16);
			G_SetAngle(fake_aagun, angles);
		...
		if (Q_TrueRand(0, 10) == 0)
		{
			vec3_t temp_org;
			
			VectorCopy(NPC->r.currentOrigin, temp_org);
			temp_org[0] += Q_TrueRand(-512, 512);
			temp_org[1] += Q_TrueRand(-512, 512);
			temp_org[2] += Q_TrueRand(-128, 128);

			flakPuff(temp_org);

			if (VectorDistance(NPC->r.currentOrigin, temp_org) < 64)
			{// Actually a hit (or close enough.. start them smoking hehehe...
				NPC->s.onFireStart = 1;
			}
		}
		return;
...
void NPC_AI_Air_Think( gentity_t *NPC )
{
