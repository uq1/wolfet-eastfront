...
*/
#ifdef __CUDA__
__device__ __host__ void G_InitGentity( gentity_t *e ) {
	e->inuse = qtrue;
	e->classname = "noclass";
	...
	if (SSE_CPU)
		sse_memset( e->goalPriority, 0, sizeof(e->goalPriority) );
	else
...
__device__ __host__ int G_GentitiesAvailable()
{
	__shared__ int free = (MAX_GENTITIES - level.num_entities);
	__shared__ int i = 0;
	__shared__ gentity_t *e = &g_entities[MAX_CLIENTS];
...
__device__ __host__ gentity_t *G_Spawn( void ) {
	__shared__ int			i, force;
	__shared__ gentity_t	*e;
	...
	__host__ trap_LocateGameData( level.gentities, level.num_entities, sizeof( gentity_t ), 
...
static qboolean G_LoadCampaignsFromFile( const char *filename ) {
	...
	while ( trap_PC_ReadToken( handle, &token ) ) {
		if( *token.string == '}' ) {
			...
			if( strstr( token.string, "wolfsp" ) || strstr( token.string, "wolfcoop" ) ) {
				g_campaigns[level.campaignCount].typeBits |= (1 << GT_SINGLE_PLAYER);
				g_campaigns[level.campaignCount].typeBits |= (1 << GT_COOP);
				g_campaigns[level.campaignCount].typeBits |= (1 << GT_NEWCOOP);
			}
			...
			if( strstr( token.string, "supremacy_campaign" ) ) {
				g_campaigns[level.campaignCount].typeBits |= (1 << GT_SUPREMACY_CAMPAIGN);
			}
#endif //__ETE__
...
#ifdef __ETE__
		// get all supremacy campaigns from .supremacy_campaign files
		numdirs = trap_FS_GetFileList("scripts", ".supremacy_campaign", bigTextBuffer, sizeof(bigTextBuffer) );
		dirptr  = bigTextBuffer;
		for(i = 0; i < numdirs; i++, dirptr += dirlen+1) 
		{
			// kw: log a warning if server has more than MAX_CAMPAIGNS
