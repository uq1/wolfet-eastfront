...
void G_StoreClientPosition( gentity_t* ent ) {
	...
	VectorCopy(ent->s.apos.trBase,
		client->clientMarkers[top].viewangles);
...
void G_AdjustSingleClientPosition( gentity_t* ent, int time, gentity_t* debugger) {
	...
	if ( client->backupMarker.time != level.time ) {
		...
		VectorCopy(client->ps.viewangles,
			client->backupMarker.viewangles);
	...
	if(i != client->topMarker) {
		...
		//roundoff error
		float frac = (float)(time - client->clientMarkers[i].time) /
			(float)(client->clientMarkers[j].time - client->clientMarkers[i].time);
		...
		TimeShiftLerp(
				client->clientMarkers[i].origin,		
				client->clientMarkers[j].origin,	
				frac,	
				ent->r.currentOrigin);
		TimeShiftLerp(
				client->clientMarkers[i].mins,
				client->clientMarkers[j].mins,
				frac,
				ent->r.mins);
		TimeShiftLerp(
				client->clientMarkers[i].maxs,
				client->clientMarkers[j].maxs,
				frac,
				ent->r.maxs);

		// These are for Head / Legs
		client->ps.viewangles[0] = LerpAngle(
				client->clientMarkers[i].viewangles[0],
				client->clientMarkers[j].viewangles[0],
				frac);
		client->ps.viewangles[1] = LerpAngle(
				client->clientMarkers[i].viewangles[1],
				client->clientMarkers[j].viewangles[1],
				frac);
		client->ps.viewangles[2] = LerpAngle(
				client->clientMarkers[i].viewangles[2],
				client->clientMarkers[j].viewangles[2],
				frac);
		// josh: Set the ints to the closest ones in time since you can't
		//       lerp them.
		if((client->clientMarkers[j].time - time)
			< (time - client->clientMarkers[i].time)) {

			client->ps.eFlags =
				client->clientMarkers[j].eFlags;
			client->ps.pm_flags =
				client->clientMarkers[j].pm_flags;
			client->ps.viewheight =
				client->clientMarkers[j].viewheight;
			...
			client->ps.eFlags =
				client->clientMarkers[i].eFlags;
			client->ps.pm_flags =
				client->clientMarkers[i].pm_flags;
			client->ps.viewheight =
				client->clientMarkers[i].viewheight;
		...
		if ( debugger && debugger->client) {
			...
			Com_sprintf( msg, sizeof(msg),
				...
				time, client->clientMarkers[i].time, client->clientMarkers[j].time,
				ent->r.currentOrigin[0], ent->r.currentOrigin[1], ent->r.currentOrigin[2],
				frac,
				client->clientMarkers[i].origin[0],
				client->clientMarkers[i].origin[1],
				client->clientMarkers[i].origin[2], 
				client->clientMarkers[j].origin[0],
				client->clientMarkers[j].origin[1],
				client->clientMarkers[j].origin[2],
		...
		VectorCopy(client->clientMarkers[j].viewangles,
...
void G_ReAdjustSingleClientPosition( gentity_t* ent ) {
	...
	if( client->backupMarker.time == level.time) {
		...
		VectorCopy(client->backupMarker.viewangles,
			client->ps.viewangles);

		client->ps.eFlags =
			client->backupMarker.eFlags;
		client->ps.pm_flags =
			client->backupMarker.pm_flags;
		client->ps.viewheight =
			client->backupMarker.viewheight;
...
void G_ResetMarkers( gentity_t* ent ) {
	...
	for( i = MAX_CLIENT_MARKERS - 1, time = level.time; i >= 0; i--, time -= period ) {
		...
		VectorCopy(client->ps.viewangles,
			client->clientMarkers[i].viewangles);
...
void G_AttachBodyParts(gentity_t* ent) {
	...
	for ( i = 0; i < MAX_GENTITIES; i++ )
	{
		...
		else
		{
			// Gordon: ok lets test everything under the sun
		 	if (!list->NPC_client)
			{// A vehicle (plane probebly).. ignore...

			}
			else if( list->inuse && 
				(list->NPC_client->sess.sessionTeam == TEAM_AXIS || list->NPC_client->sess.sessionTeam == TEAM_ALLIES) && 
				(list != ent) &&
				list->r.linked &&
				!(list->NPC_client->ps.pm_flags & PMF_LIMBO) &&
				(list->NPC_client->ps.pm_type == PM_NORMAL ||
				 list->NPC_client->ps.pm_type == PM_DEAD)
			) {
...
void G_DettachBodyParts() {
	...
	for ( i = 0; i < MAX_GENTITIES; i++ )
	{
		...
		if (list->s.eType == ET_NPC)
			client = list->NPC_client;
		...
		if( client->tempHead ) {
			G_FreeEntity( client->tempHead );
		}
		if( client->tempLeg ) {
			G_FreeEntity( client->tempLeg );
		}
	}
#else //!__NPC__
	for( i = 0; i < level.numConnectedClients; i++, list++ ) {
