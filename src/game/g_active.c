		...
		if ( ent->client->airOutTime < level.time) {
			...
			if(ent->client->ps.powerups[PW_BREATHER]) {	// take air from the breather now that we need it
				...
				if ( ent->health > 0 ) {
					...
					{
						ent->damage = (ent->maxhealth*0.1)+1;//250;
						G_Sound(ent, G_SoundIndex("sound/misc/hitsound.wav"));
					}
					else
#endif //__VEHICLES__
...
void ClientImpacts( gentity_t *ent, pmove_t *pm ) {
	...
	for (i=0 ; i<pm->numtouch ; i++) {
		...
		if (pm->touchents[i] < 0 || pm->touchents[i] > MAX_GENTITIES) return;
		...
		if ( (ent->client) /*&& !(ent->r.svFlags & SVF_BOT)*/ && other && other->client && (other->r.svFlags & SVF_BOT) ) {
/*			vec3_t dir;
...
qboolean ClientInactivityTimer( gclient_t *client ) {
	...
	if( ( g_inactivity.integer == 0 && client->sess.sessionTeam != TEAM_SPECTATOR ) || ( g_spectatorInactivity.integer == 0 && client->sess.sessionTeam == TEAM_SPECTATOR ) ) {
		...
		if(level.time > client->inactivityTime && 
			...
			!G_shrubbot_permission(&g_entities[client-level.clients], SBF_IMMUNITY)) {
			client->inactivityWarning = qfalse;
			client->inactivityTime = level.time + 60 * 1000;
...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	while( client->timeResidual >= 1000 ) {
		...
		{// In realism, health regen is much higher for everyone!
			regenRate1 = 10;
 			regenRate2 = 8;
		}
		...
		{// Non-Medic regen rates... (Slower then medics)
 			// Determine regen rate
 			switch(g_medicHealthRegen.integer) {
 				case MEDIC_REGENRATE22:
 					regenRate1 = 1;
		...
		// regenerate
#ifdef __VEHICLES__
		if( client->sess.playerType == PC_MEDIC && client->ps.eFlags & EF_VEHICLE  ) 
		{
...
#ifdef __EF__
		if( !(client->ps.eFlags & EF_DEAD) ) // UQ1: All classes now regen in EF! :)
#else //!__EF__
...
*/
		...
		case EV_TANK_FIRE:
			tank_fire( ent );
			break;
		case EV_FLAME_TANK_FIRE:
			flametank_fire( ent );
			break;
		case EV_TANK_MG_FIRE:	
			tank_mgfire( ent );
			break;
		case EV_TANK_PTRS_FIRE://Masteries, for panzer 2
			ptrstank_fire( ent );
			break;
#endif //__VEHICLES__
...
void ClientThink_real( gentity_t *ent ) {
	...
	{
		if (client->ps.stats[STAT_VEHICLE_CLASS] == VEHICLE_CLASS_APC)
		{// Need to check inrange players for ammo/health giveaways :)
			vec3_t	mins, maxs;
			float	crate_use_distance = 256.0f;
			int		touch[MAX_GENTITIES];
			int		num = 0;
			int		i = 0;

			if (ent->vehicle_apc_next_ammo_health_check > level.time + 2000)
				ent->vehicle_apc_next_ammo_health_check = level.time + 2000; // Reset timer!

			if (ent->vehicle_apc_next_ammo_health_check <= level.time)
			{
				...
				//if (num == 0)
				//	G_Printf("No entities in box (%f %f %f) (%f %f %f) (org: %f %f %f)\n", mins[0], mins[1], mins[2], maxs[0], maxs[1], maxs[2], ent->r.currentOrigin[0], ent->r.currentOrigin[1], ent->r.currentOrigin[2]);
				...
				for ( i=0 ; i<num ; i++ ) 
				{
					...
					{
						//G_Printf("%s's team is non-existant!\n", hit->client->pers.netname);
						continue;
					}

					if (!hit->client->pers.connected)
					{
						//G_Printf("%s's is not connected!\n", hit->client->pers.netname);
						continue;
					}

					if (hit->health <= 0)
					{
						//G_Printf("%s's is health is 0!\n", hit->client->pers.netname);
						continue;
					}

					if (!OnSameTeam(hit, ent))
					{
						//G_Printf("%s's is not on same team!\n", hit->client->pers.netname);
						continue;
					}
					...
					if (ClientNeedsAmmo(touch[i]))
					{
						...
						drop = Drop_Item (hit, item, 0, qfalse);
						drop->count = 1;
						//G_Printf("%s's was given ammo!\n", hit->client->pers.netname);
						continue;
					}
				}

				ent->vehicle_apc_next_ammo_health_check = level.time + 2000;
			}
		}
	}
#endif //__VEHICLES__
...
#ifdef __ETE__
#ifdef __EF__
	if (client->ps.weapon != client->sess.playerWeapon)
	{// Set back weapon...
		G_SecondaryEntityFor(ent->s.number)->s.weapon = client->sess.playerWeapon;
	}
	else
	{
		G_SecondaryEntityFor(ent->s.number)->s.weapon = client->sess.playerWeapon2;
	}
#endif //__EF__
	...
	{
		int i;
		qboolean cloaked = qfalse;

		for (i = 0; i < num_stealth_areas; i++)
		{
			if (VectorDistance(ent->r.currentOrigin, stealth_area_origin[i]) < stealth_area_radius[i])
			{
				if (!(ent->client->ps.eFlags & EF_CLOAKED))
					ent->client->ps.eFlags |= EF_CLOAKED;

				cloaked = qtrue;

				//if (ent->client->ps.eFlags & EF_CLOAKED)
				//	G_Printf("You should be cloaked!\n");
	...
	if ((ent->client->ps.eFlags & EF_AIMING) && !BG_Weapon_Is_Aimable(ent->client->ps.weapon))
	{// If we have aim switched on and this gun doesnt support it, switch it off automatically...
...
// SET TANK BOUNDING BOX
#ifdef __VEHICLES__
	if ( ent->s.eFlags & EF_VEHICLE ) 
	{
		float turretDiff;
		vec3_t mins, maxs;

		// TURRET vertical rotation
		...
		ent->client->ps.stats[STAT_TURRET_PITCH] = turretDiff; 
...
//vec3_t	tankMins = {-164, -112, -24};
		...
		VectorCopy (tankMaxs, ent->client->ps.maxs);
		VectorCopy (tankMins, ent->r.mins);
		VectorCopy (tankMaxs, ent->client->ps.maxs);
		*/
		switch (ent->client->ps.stats[STAT_VEHICLE_CLASS])
		{
		...
		ent->client->ps.stats[STAT_HEALTH] = (int)((float)((float)ent->health/(float)ent->maxhealth)*(float)100); // send health to clients
	} else if (ent->tankBox){
		ent->tankBox = qfalse;
		VectorCopy (playerMaxs, ent->r.maxs);
		VectorCopy (playerMaxs, ent->client->ps.maxs);
		VectorCopy (playerMins, ent->r.mins);
		VectorCopy (playerMins, ent->client->ps.mins);
	}
#endif //__VEHICLES__
			...
			if( ucmd->upmove > 0 ) {
				if( g_gametype.integer == GT_WOLF_LMS || client->ps.persistant[PERS_RESPAWNS_LEFT] >= 0 ) {
					...
					if(g_logOptions.integer & LOGOPTS_DIS_TAPCON) {
						limbo( ent, ( client->ps.stats[STAT_HEALTH] > GIB_HEALTH ), qfalse );						
					} else {
...
#ifdef __BOT__
		&& !(ent->r.svFlags & SVF_BOT)  // UQ1: I'm going to make bots ignore this check to reduce CPU usage...
#endif //__BOT__
...
void ClientEndFrame( gentity_t *ent ) {
	...
	if (( client->sess.sessionTeam == TEAM_SPECTATOR ) || (client->ps.pm_flags & PMF_LIMBO)) { // JPW NERVE
		...
		if(level.match_pause != PAUSE_NONE) {
			...
			client->airOutTime += time_delta;
			client->inactivityTime += time_delta;
			client->lastBurnTime += time_delta;
			client->pers.connectTime += time_delta;
			client->pers.enterTime += time_delta;
			client->pers.teamState.lastreturnedflag += time_delta;
			client->pers.teamState.lasthurtcarrier += time_delta;
			client->pers.teamState.lastfraggedcarrier += time_delta;
			client->ps.classWeaponTime += time_delta;
//			client->respawnTime += time_delta;
//			client->sniperRifleFiredTime += time_delta;
...
#if 0
	if ( !g_synchronousClients->integer && client->ps.pm_type == PM_NORMAL ) {
 ...
 */
 //unlagged - smooth client #1
	...
	if (ent->client && HasExtFlag(ent->s.number, EX_SHELLSHOCK))
	{
		if (level.time >= ent->shellShockEndTime)
		{
			...
			//G_Printf("Shellshock ended!\n");
		}
		else
		{
			entityState_t *es = SecondaryEntityState(ent->s.number);
			es->effect2Time = level.time - ent->shellShockStartTime;

			//G_Printf("Shellshock in progress!\n");
		}
	}
...
//	ent->client->ps.eFlags &= ~EF_CONNECTION;
	...
	if(g_skipCorrection.integer && !client->warped && frames > 0) {
		if (frames > 3) {
