...
#ifdef __CUDA__
#include "../game/cuda.h"
qboolean CudaInitialized;
qboolean CudaAvailable;
#endif //__CUDA__
...
char *Q_StrReplace(char *haystack, char *needle, char *new);
...
/*#ifdef __EF__
#define	MAX_WEAPONS				128	// (UQ1) and yet more! :)
#define	MAX_PS_WEAPONS		64	// (UQ1) and yet more! :)
#define	MAX_AMMOS					64
#define	MAX_WEAPONHEATS		64
#else //!__EF__*/
...
typedef struct playerState_s {
	...
	int			ammo[MAX_AMMOS];		// total amount of ammo
	int			ammoclip[MAX_AMMOS];	// ammo in clip
//#ifndef __EF__
	int			holdable[16];
...
/*#else //__EF__
	...
	int			holdable[14]; // UQ1: We cab reuse ALL of these, holdables are not used in ET!!!
	//int			holdable[14];
	int			holding;				// the current item in holdable[] that is selected (held)
#endif //__EF__*/
...
//----(SA)	added
	...
	int			weapHeat[MAX_WEAPONHEATS];	// some weapons can overheat.  this tracks (server-side) how hot each weapon currently is.
...
typedef enum {
	...
	ET_DEFENCEPOINT_HINT,	// Unique1: defence point hint.
	...
	ET_HEALTH_CRATE,
	ET_SANDBAG,
	ET_CONSTRUCTIBLE_SANDBAGS,
	ET_MG_NEST,
	ET_CONSTRUCTIBLE_MG_NEST,
	ET_TRIGGER_HURT,
#endif //__ETE__
