...
void hurt_touch( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	if (other->r.svFlags & SVF_BOT)
	{
		qboolean mark_link = qfalse;

		if (other->current_node)
		{// Mark node unreachable!
			if (!(nodes[other->current_node].type & NODE_ALLY_UNREACHABLE))
			{
				nodes[other->current_node].type |= NODE_ALLY_UNREACHABLE;
				mark_link = qtrue;
			}

			if (!(nodes[other->current_node].type & NODE_AXIS_UNREACHABLE))
			{
				nodes[other->current_node].type |= NODE_AXIS_UNREACHABLE;
				mark_link = qtrue;
			}

			if (mark_link && other->last_node)
			{// Mark the link to it!
				int loop = 0;

				for ( loop = 0; loop < nodes[other->last_node].enodenum; loop++ )
				{// Find the link we last used and mark it...
					if ( nodes[other->last_node].links[loop].targetNode == other->current_node )
					{// Mark it as Dangerous...
						BOT_MakeLinkDangerous( other, other->last_node, loop );
						break;
					}
				}

				// And reset the bot's waypointing!
				other->current_node = -1;
				other->next_node = -1;
				other->longTermGoal = -1;
			}
		}
	}
#endif //__BOT__
...
void SP_trigger_hurt( gentity_t *self ) {
	...
	trap_LinkEntity (self); // UQ1: Always linked for bots!
#endif //__ETE__
...
//	int i;
	...
	{// UQ1: No health cabinets in realism mode!
		return qfalse;
	}
