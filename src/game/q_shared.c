...
#ifdef __CUDA__
__device__ __host__ int Q_isprint( int c )
{
	if ( c >= 0x20 && c <= 0x7E )
		return ( 1 );
	return ( 0 );
...
__device__ __host__ int Q_islower( int c )
{
	if (c >= 'a' && c <= 'z')
		return ( 1 );
	return ( 0 );
...
__device__ __host__ int Q_isupper( int c )
{
	if (c >= 'A' && c <= 'Z')
		return ( 1 );
	return ( 0 );
...
__device__ __host__ int Q_isalpha( int c )
{
	if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
		return ( 1 );
	return ( 0 );
...
__device__ __host__ int Q_isnumeric( int c )
{
	if (c >= '0' && c <= '9')
		return ( 1 );
	return ( 0 );
...
__device__ __host__ int Q_isalphanumeric( int c )
{
	if(	Q_isalpha(c) ||
		Q_isnumeric(c) )
		return( 1 );
...
__device__ __host__ int Q_isforfilename( int c )
{
...
__device__ __host__ char* Q_strrchr( const char* string, int c )
{
	__shared__ char cc = c;
	__shared__ char *s;
	__shared__ char *sp=(char *)0;
...
=============
*/
__device__ __host__ void Q_strncpyz( char *dest, const char *src, int destsize ) {
	if ( !src ) {
	...
	if ( destsize < 1 ) {
		...
		//Com_Printf("Q_strncpyz: destsize (%i) < 1 (%s >> %s) - UQ1: strcpy used instead!\n", destsize, src, dest);
		//strcpy(dest, src);
		return;
	}
...
__device__ __host__ int Q_stricmpn (const char *s1, const char *s2, int n) {
...
__device__ __host__ int Q_strncmp (const char *s1, const char *s2, int n) {
	__shared__ int		c1, c2;
...
__device__ __host__ int Q_stricmp (const char *s1, const char *s2) {
	return (s1 && s2) ? Q_stricmpn (s1, s2, 99999) : -1;
}

__device__ __host__ char *Q_strlwr( char *s1 ) {
	__shared__ char*	s;
...
__device__ __host__ char *Q_strupr( char *s1 ) {
	__shared__ char* cp;
...
__device__ __host__ void Q_strcat( char *dest, int size, const char *src ) {
	__shared__ int		l1;
...
__device__ __host__ int Q_PrintStrlen( const char *string ) {
	__shared__ int			len;
	__shared__ const char	*p;
...
__device__ __host__ char *Q_CleanStr( char *string ) {
	__shared__ char*	d;
	__shared__ char*	s;
	__shared__ int		c;
...
__device__ __host__ qboolean Q_isBadDirChar( char c ) {
	__shared__ char	badchars[] = { ';', '&', '(', ')', '|', '<', '>', '*', '?', '[', ']', '~', '+', '@', '!', '\\', '/', ' ', '\'', '\"', '\0' };
	__shared__ int		i;
...
__device__ __host__ char *Q_CleanDirName( char *dirname ) {
	__shared__ char*	d;
	__shared__ char*	s;
...
__device__ __host__ void QDECL Com_sprintf( char *dest, int size, const char *fmt, ...) {
	__shared__ int		ret;
	__shared__ va_list		argptr;
...
============
*/
__device__ __host__ char	* QDECL va( char *format, ... ) {
	__shared__ va_list		argptr;
#define	MAX_VA_STRING	32000
	__shared__ static char		temp_buffer[MAX_VA_STRING];
	__shared__ static char		string[MAX_VA_STRING];	// in case va is called by nested functions
	__shared__ static int		index = 0;
	__shared__ char	*buf;
	__shared__ int len;
...
=============
*/
__device__ __host__ float	*tv( float x, float y, float z ) {
	__shared__ static	int		index;
	__shared__ static	vec3_t	vecs[8];
	__shared__ float	*v;
...
char *GetStringForID( stringID_table_t *table, int id )
{
	int	index = 0;
