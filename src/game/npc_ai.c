...
int Count_NPC_Players ( void )
{
	...
	{
		if (g_entities[loop].s.eType == ET_NPC /*&& g_entities[loop].NPC_Class != CLASS_PLANE && g_entities[loop].NPC_Class != CLASS_PLANE_BOMBER && g_entities[loop].NPC_Class != CLASS_LCVP && g_entities[loop].NPC_Class != CLASS_DESTROYER && g_entities[loop].NPC_Class != CLASS_VEHICLE*/)
		{
			npccount++;
		}
		loop++;
...
===========================================================================
*/
qboolean NPCVisible (gentity_t *self, gentity_t *other)
{
...
==============
*/
qboolean NPC_AI_inFOV(vec3_t viewangles, float fov, vec3_t angles)
{
	int i;
	float diff, angle;
...
============
*/
qboolean G_NPCPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	int			i;
	vec3_t		mins, maxs;
...
void NPC_AI_Move( gentity_t *NPC )
{
...
void NPC_AI_Think( gentity_t *NPC )
{
