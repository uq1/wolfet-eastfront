 ...
 *              //modified by Masteries 27/11/2007
*/              //engineer repairing extends to new tanks
                //line 1779
...
void Weapon_Knife( gentity_t *ent ) {
	...
	if((tr.entityNum >= MAX_CLIENTS && g_entities[tr.entityNum].s.eType != ET_CORPSE) 
#ifdef __VEHICLES__
		|| (g_entities[tr.entityNum].client && g_entities[tr.entityNum].client->ps.eFlags & EF_VEHICLE )
#endif //__VEHICLES__
...
#ifdef __VEHICLES__
	if (traceEnt->client && traceEnt->client->ps.eFlags & EF_VEHICLE)
		return;
#endif //__VEHICLES__
	...
	{
		AngleVectors (client->ps.viewangles,		pforward, NULL, NULL);
		...
		if( DotProduct( eforward, pforward ) > 0.6f )		// from behind(-ish)
		{
			...
			if ( traceEnt->health > 0 && client->sess.skill[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS] >= 4 ) {
...
void Weapon_Medic( gentity_t *ent ) {
	...
	VectorCopy( client->ps.origin, viewpos );
					...
					vec3_t velocity)
{
	...
	if (level.time - client->ps.classWeaponTime > level.medicChargeTime[client->sess.sessionTeam-1]) {
		client->ps.classWeaponTime = level.time - level.medicChargeTime[client->sess.sessionTeam-1];
	}
	
	if( client->sess.skill[SK_FIRST_AID] >= 2 ) {
		client->ps.classWeaponTime += level.medicChargeTime[client->sess.sessionTeam-1]*0.15;
	} else {
		client->ps.classWeaponTime += level.medicChargeTime[client->sess.sessionTeam-1]*0.25;
	}
...
void Weapon_MagicAmmo( gentity_t *ent )  {
	...
	//item = BG_FindItem( client->sess.skill[SK_SIGNALS] >= 1 ? "Mega Ammo Pack" : "Ammo Pack" );	
	VectorCopy( client->ps.viewangles, angles );
						  ...
						  vec3_t velocity)
{
	...
	if (level.time - client->ps.classWeaponTime > level.lieutenantChargeTime[client->sess.sessionTeam-1])
		client->ps.classWeaponTime = level.time - level.lieutenantChargeTime[client->sess.sessionTeam-1];

	if( client->sess.skill[SK_SIGNALS] >= 1 ) {
		client->ps.classWeaponTime += level.lieutenantChargeTime[client->sess.sessionTeam-1]*0.15;
	} else {
		client->ps.classWeaponTime += level.lieutenantChargeTime[client->sess.sessionTeam-1]*0.25;
	}

	item = BG_FindItem(client->sess.skill[SK_SIGNALS] >= 1 
...
qboolean ReviveEntity(gentity_t *ent, gentity_t *traceEnt)
{
	...
	int			ammo[MAX_AMMOS];		// JPW NERVE total amount of ammo
	int			ammoclip[MAX_AMMOS];	// JPW NERVE ammo in clip
...
//	gentity_t	*traceEnt,
	...
	if( client->sess.skill[SK_FIRST_AID] >= 3 )
	...
	memcpy(ammo,traceEnt->client->ps.ammo,sizeof(int)*MAX_AMMOS);
	memcpy(ammoclip,traceEnt->client->ps.ammoclip,sizeof(int)*MAX_AMMOS);
	...
	memcpy(traceEnt->client->ps.ammo,ammo,sizeof(int)*MAX_AMMOS);
	memcpy(traceEnt->client->ps.ammoclip,ammoclip,sizeof(int)*MAX_AMMOS);
	...
	trap_SendServerCommand( traceEnt-g_entities, va("cp \"You have been revived by [lof]%s[lon] [lof]%s!\n\"", client->sess.sessionTeam == TEAM_ALLIES ? rankNames_Allies[ client->sess.rank ] : rankNames_Axis[ client->sess.rank ], client->pers.netname) );
	...
	if (tr.fraction < 1.0) {
		...
		if (traceEnt->client != NULL) {
			...
			if((traceEnt->client->ps.pm_type == PM_DEAD) && 
				(traceEnt->client->sess.sessionTeam == client->sess.sessionTeam) &&
				...
				// OSP - syringe "hit"
				if(g_gamestate.integer == GS_PLAYING) client->sess.aWeaponStats[WS_SYRINGE].hits++;
				if(ent && client) G_LogPrintf("Medic_Revive: %d %d\n", ent - g_entities, traceEnt - g_entities);	// OSP
...
void Weapon_AdrenalineSyringe(gentity_t *ent) {
	...
	client->ps.powerups[PW_ADRENALINE] = level.time + 10000;
}
...
static qboolean TryConstructing( gentity_t *ent ) {
	...
	constructible = client->touchingTOI->target_ent;
	...
	if( constructible->s.eType == ET_CONSTRUCTIBLE && client->touchingTOI->chain ) {
		gentity_t *otherconstructible = NULL;

		// use the target that has the same team as the player
		if( constructible->s.teamNum != client->sess.sessionTeam ) {
			constructible = client->touchingTOI->chain;
		}
	...
	if( constructible->s.eType == ET_CONSTRUCTIBLE &&
		constructible->s.teamNum == client->sess.sessionTeam ) {
...
/*			if( constructible->parent->spawnflags & 8 ) {
			...
			if( client->touchingTOI->chain && client->touchingTOI->count2 ) {
				// find the constructible indicator and change team
				mapEntityData_t	*mEnt;
				mapEntityData_Team_t *teamList;
				gentity_t *indicator = &g_entities[client->touchingTOI->count2];
			...
			if( !constructible->count2 || constructible->grenadeFired == 1 ) {
				...
				G_UseEntity( constructible, client->touchingTOI, ent );
			}
...
/*		if( client->touchingTOI->spawnflags & 4 ) { // MESSAGE_OVERRIDE
			gentity_t* pm = G_PopupMessage( PM_CONSTRUCTION );
			pm->s.effect3Time = G_StringIndex( client->touchingTOI->spawnitem );
			pm->s.effect2Time = TEAM_AXIS;
			pm->s.density = -1; // -1 = built (custom msg)
		} else {
			gentity_t* pm = G_PopupMessage( PM_CONSTRUCTION );
			pm->s.density = client->sess.sessionTeam;
			pm->s.effect2Time = 0; // 0 = built
			pm->s.effect3Time = client->touchingTOI->s.teamNum;
		}*/
...
//				trap_UnlinkEntity( constructible->parent );
		...
		//client->ps.classWeaponTime = level.time; // Out of "ammo"
		...
		if( !(constructible->spawnflags & CONSTRUCTIBLE_INVULNERABLE) && (constructible->constructibleStats.weaponclass >= 1) ) {
			if( !constructible->count2 || constructible->grenadeFired == 1 ) {
				...
				e->s.modelindex2 = client->touchingTOI->s.teamNum;
...
#ifdef __ETE__
extern qboolean TrySandbagConstruction( gentity_t *ent );
extern qboolean Try_MG_Nest_Construction( gentity_t *ent );
#endif //__ETE__
...
void Weapon_Engineer( gentity_t *ent ) {
	...
	if( client->touchEnt ) 
	{
		if( TrySandbagConstruction( ent ) ) 
		{
			return;
		}
		if ( Try_MG_Nest_Construction( ent ) )
		{
			return;
		}
	}
	...
	AngleVectors (client->ps.viewangles, forward, right, up);
	VectorCopy( client->ps.origin, muzzleTrace );
	muzzleTrace[2] += client->ps.viewheight;
...
/// tank repairing
#ifdef __VEHICLES__
	{
			...
			|| !Q_stricmp (traceEnt->classname, "pakantitank"))
		{
			...
			{
				if ( client->ps.classWeaponTime + level.engineerChargeTime[client->sess.sessionTeam-1] < level.time ) 
				{
					client->ps.classWeaponTime = level.time - level.engineerChargeTime[client->sess.sessionTeam-1];
				}

				if( client->sess.skill[SK_EXPLOSIVES_AND_CONSTRUCTION] >= 3 ) 
				{
					client->ps.classWeaponTime += .66f * 500;
				} 
				else 
				{
					client->ps.classWeaponTime += 500;
				}

				if ( client->ps.classWeaponTime > level.time ) 
				{
				...
				traceEnt->health += (traceEnt->maxhealth*0.003);

				if ( traceEnt->health > traceEnt->maxhealth )
					traceEnt->health = traceEnt->maxhealth;
			}
		}
	}
#endif //__VEHICLES__
...
checkEngineerTrace:
	...
	if( G_EmplacedGunIsRepairable( traceEnt, ent ) ) {
		...
		if( client->sess.skill[SK_EXPLOSIVES_AND_CONSTRUCTION] >= 3 ) {
			client->ps.classWeaponTime += .66f * 150;
		} else {
			client->ps.classWeaponTime += 150;
		}
		...
		if( traceEnt->health >= 255 ) {
			...
			if( traceEnt->sound3to2 != client->sess.sessionTeam ) {
		...
		if ( traceEnt->methodOfDeath == MOD_LANDMINE ) {
			...
			if( !(tr2.surfaceFlags & SURF_LANDMINE) || (tr2.entityNum != ENTITYNUM_WORLD && (!g_entities[tr2.entityNum].inuse || g_entities[tr2.entityNum].s.eType != ET_CONSTRUCTIBLE))) {
				...
				if (client->sess.skill[SK_EXPLOSIVES_AND_CONSTRUCTION] >= 3) {
					client->ps.classWeaponTime -= .33f * 
						level.engineerChargeTime[client->sess.sessionTeam-1];
				}
				else {
					client->ps.classWeaponTime -= .5f * 
						level.engineerChargeTime[client->sess.sessionTeam-1];
				}
				client->sess.aWeaponStats[WS_LANDMINE].atts--;
				return;
//bani
// rain - #384 - check landmine team so that enemy mines can be disarmed
// even if you're using all of yours :x
			} else if ( G_CountTeamLandmines(client->sess.sessionTeam) >= g_maxTeamLandmines.integer && G_LandmineTeam(traceEnt) == client->sess.sessionTeam) {

				if(G_LandmineUnarmed(traceEnt)) {
// rain - should be impossible now
//					if ( G_LandmineTeam( traceEnt ) != client->sess.sessionTeam )
//						return;
...
//bani - #471
				...
				if(G_LandmineUnarmed(traceEnt)) {
					...
					// Give health until it is full, don't continue
					if( client->sess.skill[SK_EXPLOSIVES_AND_CONSTRUCTION] >= 2 ) {
						traceEnt->health += 24;
					} else {
						traceEnt->health += 12;
					}
					...
					traceEnt->s.teamNum = client->sess.sessionTeam;
...
/*						traceEnt->health = 255;
						...
						/*{
							...
							teamList = client->sess.sessionTeam == TEAM_AXIS ? &mapEntityData[0] : &mapEntityData[1];
							if((mEnt = G_FindMapEntityData(teamList, traceEnt-g_entities)) != NULL) {
								G_FreeMapEntityData( teamList, mEnt );
							}

							teamList = client->sess.sessionTeam == TEAM_AXIS ? &mapEntityData[1] : &mapEntityData[0];	// inverted
							if((mEnt = G_FindMapEntityData(teamList, traceEnt-g_entities)) != NULL) {
								if( teamNum != client->sess.sessionTeam ) {
									G_FreeMapEntityData( teamList, mEnt );
								} else {
			...
			if ( traceEnt->health >= 250 ) {
				...
				if( traceEnt->s.teamNum	 != client->sess.sessionTeam ) {
...
//bani - no tripmine...
		...
		if ( traceEnt->methodOfDeath == MOD_DYNAMITE ) {
			...
			if ( traceEnt->s.teamNum >= 4 ) {
				...
				// Opposing team cannot accidentally arm it
				if ( (traceEnt->s.teamNum - 4) != client->sess.sessionTeam )
					return;
...
//							return;
				...
				for ( i=0 ; i<num ; i++ ) {
					...
					if ((hit->s.eType == ET_OID_TRIGGER)) {
						...
						if ( ((hit->spawnflags & AXIS_OBJECTIVE) && (client->sess.sessionTeam == TEAM_AXIS)) || 
							 ((hit->spawnflags & ALLIED_OBJECTIVE) && (client->sess.sessionTeam == TEAM_ALLIES)) ) {
//bani
//							G_FreeEntity( traceEnt );
...
//							return;
						...
						//bani
						if ( ((hit->spawnflags & AXIS_OBJECTIVE) && (client->sess.sessionTeam == TEAM_ALLIES)) ||
							 ((hit->spawnflags & ALLIED_OBJECTIVE) && (client->sess.sessionTeam == TEAM_AXIS)) ) {
							enemyObj = qtrue;
						}
					}
				}
				...
				for ( i=0 ; i<num ; i++ ) {
					...
					if ((hit->s.eType == ET_OID_TRIGGER)) {
						...
						if ( hit->spawnflags & AXIS_OBJECTIVE ) {
							if (client->sess.sessionTeam == TEAM_ALLIES) { // transfer score info if this is a bomb scoring objective
								traceEnt->accuracy = hit->accuracy;
							}
						} else if (hit->spawnflags & ALLIED_OBJECTIVE) {
							if (client->sess.sessionTeam == TEAM_AXIS) { // ditto other team
								traceEnt->accuracy = hit->accuracy;
							}
						}

						// rain - spawnflags 128 = disabled (#309)
						if (!(hit->spawnflags & 128) && (((hit->spawnflags & AXIS_OBJECTIVE) && (client->sess.sessionTeam == TEAM_ALLIES)) ||
							 ((hit->spawnflags & ALLIED_OBJECTIVE) && (client->sess.sessionTeam == TEAM_AXIS))) ) {

							gentity_t* pm = G_PopupMessage( PM_DYNAMITE );
							pm->s.effect2Time = 0;
							pm->s.effect3Time = hit->s.teamNum;
							pm->s.teamNum = client->sess.sessionTeam;
...
//							return;
						...
						if( hit->parent ) {
							...
							if( (!(hit->parent->spawnflags & OBJECTIVE_DESTROYED)) && 
								hit->s.teamNum && (hit->s.teamNum == client->sess.sessionTeam) ) {	// ==, as it's inverse
								AddScore(traceEnt->parent, WOLF_DYNAMITE_PLANT); // give drop score to guy who dropped it
								if( traceEnt->parent && traceEnt->parent->client ) {
									G_LogPrintf("Dynamite_Plant: %d\n", traceEnt->parent - g_entities);	// OSP
								}
...
/*					if (dynamiteDropTeam == client->sess.sessionTeam)
						return;*/
						...
						for( e = 0; e < numListedEntities; e++ ) {
							...
							if (client->sess.sessionTeam == TEAM_AXIS) {
								if ( hit->s.teamNum == TEAM_AXIS && (!scored)) {
									AddScore(ent,WOLF_DYNAMITE_DIFFUSE);
									if(ent && client) G_LogPrintf("Dynamite_Diffuse: %d\n", ent - g_entities);	// OSP
									G_AddSkillPoints( ent, SK_EXPLOSIVES_AND_CONSTRUCTION, 6.f );
									G_DebugAddSkillPoints( ent, SK_EXPLOSIVES_AND_CONSTRUCTION, 6.f, "defusing enemy dynamite" );
									scored++;
								}
...
void weapon_callSecondPlane( gentity_t *ent ) {
	...
	if (ent->s.teamNum == TEAM_AXIS)
		te->s.eventParm = G_SoundIndex( "sound/weapons/airstrike/airstrike_plane_axis.wav");
	else
...
qboolean weapon_checkAirStrike( gentity_t *ent ) {
...
#ifdef __NPC__
	else if (ent->parent->NPC_client)
		client = ent->parent->NPC_client;
#endif //__NPC__
	else
		return qfalse;
	...
	if (!g_friendlyFire.integer && client && client->sess.sessionTeam == TEAM_SPECTATOR)
	{
...
//			te->s.effect1Time = 1;	// don't buffer
			...
			if( g_weapons.integer & WPF_TM_AIRSTRIKE_RESTORE_FULL ) {
				if( client->sess.skill[SK_SIGNALS] >= 2 ) {
					client->ps.classWeaponTime -= .66f * 
						level.lieutenantChargeTime[client->sess.sessionTeam-1];
				} else {
					client->ps.classWeaponTime -= 1.0f * 
						level.lieutenantChargeTime[client->sess.sessionTeam-1];
				}
...
void weapon_callAirStrike( gentity_t *ent ) {
	...
	if(
		client->sess.skill[SK_SIGNALS] >= 3 && 
...
//	te->s.effect1Time = 1;	// don't buffer
	...
	VectorSubtract( ent->s.pos.trBase, client->ps.origin, lookaxis );
	lookaxis[2] = 0;
...
void Weapon_Artillery(gentity_t *ent) {
	...
	if( client->ps.stats[STAT_PLAYER_CLASS] != PC_FIELDOPS ) {
...
/*			te = G_TempEntity( ent->s.pos.trBase, EV_GLOBAL_CLIENT_SOUND );
	...
	muzzlePoint[2] += client->ps.viewheight;
...
// i == 1->4 is regular explosives
	if( client->sess.skill[SK_SIGNALS] >= 3 ) {
		count = 9;
	} else {
		count = 5;
	}
	...
	for( i = 0; i < count; i++ ) {
		...
		bomb->parent		= ent;
		bomb->s.teamNum		= client->sess.sessionTeam;
...
// build arty falling sound effect in front of bomb drop
	...
	if( client->sess.skill[SK_SIGNALS] >= 2 ) {
		...
		client->ps.classWeaponTime += 0.66f * level.lieutenantChargeTime[client->sess.sessionTeam-1];
	} else {
		client->ps.classWeaponTime = level.time;
	}
...
#endif
		client->sess.aWeaponStats[WS_ARTILLERY].atts++;
...
void weapon_airdropMarkerExplode( gentity_t *ent ) {
	int lived = 0;
	...
	if( lived < SMOKEBOMB_GROWTIME ) {
		// Just been thrown, increase radius
		ent->s.effect1Time = 16 + lived * ((640.f-16.f)/(float)SMOKEBOMB_GROWTIME);
	} else if( lived < SMOKEBOMB_SMOKETIME/1.5 + SMOKEBOMB_GROWTIME ) {
		// Smoking
		ent->s.effect1Time = 640;
	} else if( lived < SMOKEBOMB_SMOKETIME/1.5 + SMOKEBOMB_GROWTIME + SMOKEBOMB_POSTSMOKETIME ) {
		// Dying out
...
int G_GetWeaponDamage( int weapon ) 
{
	...
	case WP_PANZERFAUST: 
		...
		break;
#ifdef __EF__
    //case WP_PTRS: //Masteries, you cannot use the antitank rifle while
	//	rval = 0;   //stand up
	//	break;
	case WP_PTRS://Testing purpose
       case WP_PTRS_SET: //Masteries, antitank rifle only can be used 
		rval = g_dmgPTRS.integer; //in prone position
		break;
#endif //__EF__
	case WP_MORTAR_SET: 
	...
	{// UQ1: In new realism mode, damage is much higher (as is health recharge)!
		if (rval <= 20)
			rval *= 1.55;
		else if (rval <= 30)
			rval *= 1.4;
...
float G_GetWeaponSpread( int weapon ) {
	switch (weapon) {
        ...
        case WP_PTRS:
		case WP_PTRS_SET:
			return 150;//Masteries, precise weapon
		case WP_30CAL:
		case WP_30CAL_SET:
#endif //__EF__
		case WP_MOBILE_MG42:
...
#define SILENCER_DAMAGE		G_GetWeaponDamage(WP_SILENCED_LUGER)
#define SILENCER_SPREAD		G_GetWeaponSpread(WP_SILENCED_LUGER)
...
#define PTRS_SPREAD	G_GetWeaponSpread(WP_PTRS)
#define	PTRS_DAMAGE	G_GetWeaponDamage(WP_PTRS)
...
//	}
//	
//	if (is_valid && client && ( client->ps.persistant[PERS_HWEAPON_USE] ) )
//	{
//		if (rand()%100 > 75)
//		{
//			gentity_t	*sfx;
...
#ifdef __ETE__
extern void CalculateSuppression ( gentity_t *attacker, vec3_t viewangles, vec3_t start, vec3_t end );
extern qboolean MyObjectVisible (gentity_t *self, gentity_t *other);
extern qboolean RoughlyVisible (gentity_t *self, gentity_t *other);

void Bullet_Wizz (gentity_t *ent, vec3_t end)
{
	// Bullet wizz sounds...
	{
		int clientNum;

		for (clientNum = 0; clientNum < MAX_CLIENTS/*MAX_GENTITIES*/; clientNum++)
		{
			...
			if (!player)
				continue;
#ifdef __NPC__
			if (!player->client && !player->NPC_client)
				continue;
#endif //__NPC__
			if (player->s.teamNum == TEAM_SPECTATOR || player->s.teamNum == TEAM_FREE || player->s.teamNum == TEAM_NUM_TEAMS)
				continue;

			distance = VectorDistance(end, player->r.currentOrigin);
			...
			*/
			...
			if( MyInFOV(player, ent, fovV, fovH) )
			{
				if (RoughlyVisible(player, ent))
				{
					G_AddEvent(player, EV_BULLET_WIZZ, ent->s.number);
					//ent->s.eventParms[0] // UQ FIXME: Add flyby distance at player point?
					//G_Printf("%s is being wizzed!\n", player->client->pers.netname);
				}
			}
		}
	}
...
void Bullet_Fire (gentity_t *ent, float spread, int damage, qboolean distance_falloff) {
	...
	switch( ent->s.weapon ) {
		...
		case WP_30CAL:
			...
			//          level 3 not level 4
			if( client->sess.skill[SK_LIGHT_WEAPONS] >= 3 )
				spread *= .65f;
			break;
#ifdef __EF__
		case WP_30CAL_SET:
		case WP_PTRS://Masteries, I think that stand up PTRS will need to remove from here
		case WP_PTRS_SET://Prone PTRS, ok!
#endif //__EF__
		case WP_MOBILE_MG42_SET:
		...
		case WP_FG42SCOPE:
			...
			if(client->sess.skill[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS] >= 4 && 
	...
	if (g_dmg.integer & COMBAT_USE_BFALL)
...
#ifdef __NPC__
	if( g_entities[ attacker->s.number ].NPC_client && g_entities[ attacker->s.number ].r.linked == qtrue ) {
		g_entities[ attacker->s.number ].r.linked = qfalse;
		waslinked = qtrue;
	}
#endif //__NPC__
...
#ifdef __VEHICLES__
		&& !(traceEnt->client->ps.eFlags & EF_VEHICLE) 
#endif //__VEHICLES__
		) {
...
//#endif //__VEHICLES__
	...
	//if (!hitClient)
		CalculateSuppression( attacker, attacker->client->ps.viewangles, start, tr.endpos );
#endif //__ETE__
...
//	float		upangle = 0, pitch;			//	start with level throwing and adjust based on angle
	...
	viewpos[2] += client->ps.viewheight;
...
gentity_t *weapon_grenadelauncher_fire (gentity_t *ent, int grenType) {
	...
	if(grenType == WP_GRENADE_LAUNCHER)
		...
		upangle *= 900;
#ifdef __EF__
	else if (grenType == WP_AIRDROP_MARKER)
		upangle *= 900;
#endif //__EF__
	...
	if (grenType == WP_LANDMINE) {
		if (client->sess.sessionTeam == TEAM_AXIS) // store team so we can generate red or blue smoke
...
#ifdef __EF__
	if( grenType == WP_AIRDROP_MARKER ) {
		m->s.effect1Time = 16;
		m->think = weapon_airdropMarkerExplode;
		m->s.teamNum = client->sess.sessionTeam;
		m->parent = ent;
	}
#endif //__EF__
	...
	if (grenType == WP_SMOKE_MARKER) {
		m->s.teamNum = client->sess.sessionTeam;	// store team so we can generate red or blue smoke
		if( client->sess.skill[SK_SIGNALS] >= 3 ) {
			m->count = 2;
	...
	//VectorAdd( m->s.pos.trDelta, client->ps.velocity, m->s.pos.trDelta );	// "real" physics
...
void Weapon_FlamethrowerFire( gentity_t *ent ) {
	...
	ent->r.svFlags |= SVF_BROADCAST;
	client->flametime = level.time + 2500;
}
...
void AddLean(gentity_t *ent, vec3_t point)
{
	...
	if(client)
	{
		if(client->ps.leanf)
		{
			...
			VectorMA(point, client->ps.leanf, right, point);
#if defined (__FRONTLINE__) || defined (__EF__)
			VectorMA(point, abs(client->ps.leanf)*(-0.33), up, point);
#endif //defined (__FRONTLINE__) || defined (__EF__)
		}
	}
}
...
void CalcMuzzlePoint ( gentity_t *ent, int weapon, vec3_t forward, vec3_t right, vec3_t up, vec3_t muzzlePoint ) 
{
	...
	if(client->ps.viewangles[PITCH] > 30 && 
		(client->ps.eFlags & EF_CROUCHING)) {
...
void CalcMuzzlePoints(gentity_t *ent, int weapon) {
	...
	VectorCopy(client->ps.viewangles, viewang);
	...
	{	// non ai's take into account scoped weapon 'sway' (just another way aimspread is visualized/utilized)
		...
		if(BG_IsScopedWeapon(weapon)) {
			...
			spreadfrac = client->currentAimSpreadScale;
...
//			they came out of the weap.
	...
	// UQ1: Store it. useful info for stuff :)
	VectorCopy(muzzleEffect, ent->muzzlePoint);
}
...
qboolean G_PlayerCanBeSeenByOthers( gentity_t *ent ) {
	...
	VectorCopy( client->ps.origin, pos[0] );
	pos[0][2] += client->ps.mins[2];
	VectorCopy( client->ps.origin, pos[1] );
	VectorCopy( client->ps.origin, pos[2] );
	pos[2][2] += client->ps.maxs[2];
	...
	for( i = 0, ent2 = g_entities; i < level.maxclients; i++, ent2++ ) {
		...
		if( ent2->health <= 0 ||
			ent2->client->sess.sessionTeam == client->sess.sessionTeam ) {
			continue;
		}
...
void G_DisguisedCovertFire(gentity_t *ent)
{
	...
	if(!(g_coverts.integer & COVERTF_KEEP_UNI_NONSIL) &&
		...
		ent->s.weapon == WP_SILENCED_LUGER ||
...
void FireWeapon( gentity_t *ent ) {
	...
	// Rafael mg42
	if (client->ps.persistant[PERS_HWEAPON_USE] && ent->active) {
		return;
	}
...
#ifdef __ETE__
	// UQ1: Use alternate spread based on weapons, aiming, zooming, and movement speed...

	if (client->ps.eFlags & EF_AIMING)
	{// Is aiming.. Check if any movement (speed based)...
		if (VectorLength(client->ps.velocity) > 0)
		{
			float velocity = VectorLength(client->ps.velocity);
			float velocity_modifier = 0.25f/*0.5*/ + (velocity / 64);

			aimSpreadScale = velocity_modifier;
		}
		else
			aimSpreadScale = 0.1f;
	}
	else if (client->ps.eFlags & EF_ZOOMING)
	{// Is zooming.. Check if any movement (speed based)...
		if (VectorLength(client->ps.velocity) > 0)
			aimSpreadScale = 0.35f/*1*/;
		else
			aimSpreadScale = 0.03f;
	}
	else if (client->ps.eFlags & EF_PRONE)
	{// Is prone.. Check if any movement (speed based)...
		if (VectorLength(client->ps.velocity) > 0)
		{
			float velocity = VectorLength(client->ps.velocity);
			float velocity_modifier = 0.35f/*0.5*/ + (velocity / 64);
	...
	{// Is crouching.. Check if any movement (speed based)...
		if (VectorLength(client->ps.velocity) > 0)
		{
			float velocity = VectorLength(client->ps.velocity);
			float velocity_modifier = 0.425f/*0.5*/ + (velocity / 64);
	...
	{// Is running or not aiming or zooming! Large spread (speed based)!
		float velocity = VectorLength(client->ps.velocity);
		float velocity_modifier = 0.65f/*1.35*/ + ((velocity / 64)*4);

		aimSpreadScale = velocity_modifier;
	}

	if (ent->supression >= 1)
	{// Less accurate when supressed!
		aimSpreadScale *= (float)(1.0f + ((100.0f / (float)ent->supression) * 0.5f) );
	}
	...
	switch( ent->s.weapon ) 
	{
		...
		default:
			//aimSpreadScale+= 0.15f;
			if ( BG_IsScopedWeapon( ent->s.weapon ) || (client->ps.eFlags & EF_ZOOMING) )
			{// No extra spread!
				
			}
			else
			{// UQ1: New method based on number of bullets in clip :)
				int		max_clip = GetAmmoTableData(ent->s.weapon)->maxclip;
				float	scale = 1.0f;
			
				if (max_clip < 1)
					max_clip = 1;

				scale = (float)((float)max_clip / 72.0f);

				if (scale >= 0.65f)
					scale = 0.65f; // UQ1: Will limit so that PPSh and MG42, etc are not too bad!
			
				if (client->ps.eFlags & EF_AIMING)
					aimSpreadScale += (scale * 0.2f);
				else if (client->ps.eFlags & EF_PRONE)
					aimSpreadScale += (scale * 0.25f);
				else if (client->ps.eFlags & EF_CROUCHING)
					aimSpreadScale += (scale * 0.35f);
				else
					aimSpreadScale += scale;
			}
			break;
	}

	if ( aimSpreadScale > 1.0 && (BG_IsScopedWeapon( ent->s.weapon ) || (client->ps.eFlags & EF_ZOOMING)) )
		aimSpreadScale = 1.0f;
	else if ( aimSpreadScale > 1.0 && client->ps.eFlags & EF_AIMING )
		aimSpreadScale = 1.0f;

	if ( aimSpreadScale > 1.25 && client->ps.eFlags & EF_PRONE )
		aimSpreadScale = 1.25f;
	else if ( aimSpreadScale > 1.5 && client->ps.eFlags & EF_CROUCHING )
		aimSpreadScale = 1.5f;
	else if ( aimSpreadScale > 2 )
		aimSpreadScale = 2.0f;	// still cap at 1.0

	//aimSpreadScale *= 0.1; // UQ1: Lower spread results... (It was too bad!)
	
	//G_Printf("EF original spread scale is %f.\n", aimSpreadScale);

	if ( BG_IsScopedWeapon( ent->s.weapon ) || (client->ps.eFlags & EF_ZOOMING) )
	{
		aimSpreadScale *= 0.05f;
		//G_Printf("Is scoped weapon!\n");
	}
	else if ( client->ps.eFlags & EF_AIMING )
	{
		aimSpreadScale *= 0.15f;
		//G_Printf("Is aiming!\n");
	}
	...
	{
		aimSpreadScale *= 0.3f;
		//G_Printf("Is crouching!\n");
	}
	else if ( !(BG_IsScopedWeapon(ent->s.weapon) || (client->ps.eFlags & EF_ZOOMING) || (client->ps.eFlags & EF_AIMING)) )
	{
		aimSpreadScale *= 0.5f;
		//G_Printf("Default spread scaling...\n");
	}

	//G_Printf("EF new spread scale is %f.\n", aimSpreadScale);
	...
	if (g_userAim.integer) {
		...
		aimSpreadScale = client->currentAimSpreadScale;
	...
	case WP_SMOKE_MARKER:
		...
		if( client->sess.skill[SK_SIGNALS] >= 2 ) {
			client->ps.classWeaponTime += .66f * level.lieutenantChargeTime[client->sess.sessionTeam-1];
		} else {
			client->ps.classWeaponTime = level.time;
		}
	...
	case WP_SATCHEL_DET:
		if( G_ExplodeSatchels( ent ) ) {
			client->ps.ammo[WP_SATCHEL_DET] = 0;
			client->ps.ammoclip[WP_SATCHEL_DET] = 0;
			client->ps.ammoclip[WP_SATCHEL] = 1;
	...
	case WP_PTRS_SET://Masteries, 
        Bullet_Fire( ent, PTRS_SPREAD*0.05f*aimSpreadScale, PTRS_DAMAGE, qfalse );
		break;
	case WP_30CAL_SET:
#endif //__EF__
...
#endif //__EF__
	case WP_MOBILE_MG42:
		if( client->ps.pm_flags & PMF_DUCKED || client->ps.eFlags & EF_PRONE ) {
		...
		break;
	case WP_PTRS://Masteries, shoot in prone position only
		if(client->ps.eFlags & EF_PRONE){
           Bullet_Fire( ent, PTRS_SPREAD*0.6f*aimSpreadScale, PTRS_DAMAGE, qfalse );
		}
		break;
	case WP_K43_SCOPE:
...
#ifdef __EF__
	case WP_SVT40:
		Bullet_Fire( ent, THOMPSON_SPREAD*aimSpreadScale*0.7, K43_DAMAGE, qtrue );
		break;
	case WP_PPSH:
		Bullet_Fire( ent, THOMPSON_SPREAD*aimSpreadScale, THOMPSON_DAMAGE*0.45, qtrue );
		break;
	case WP_STG44:
		Bullet_Fire( ent, THOMPSON_SPREAD*aimSpreadScale*1.1, THOMPSON_DAMAGE*1.1, qtrue );
		break;
	case WP_PPS:
		Bullet_Fire( ent, THOMPSON_SPREAD*aimSpreadScale*1.1, THOMPSON_DAMAGE*1.1, qtrue );
		break;
	...
	case WP_THOMPSON:
		...
		break;
	case WP_PANZERFAUST:
		if( level.time - client->ps.classWeaponTime > level.soldierChargeTime[client->sess.sessionTeam-1] ) {
			client->ps.classWeaponTime = level.time - level.soldierChargeTime[client->sess.sessionTeam-1];
		}

		if( client->sess.skill[SK_HEAVY_WEAPONS] >= 1 ) {
			client->ps.classWeaponTime += .66f * level.soldierChargeTime[client->sess.sessionTeam-1];
		} else {
			client->ps.classWeaponTime = level.time;
		}
		...
		if( client ) {
			vec3_t forward;
			AngleVectors (client->ps.viewangles, forward, NULL, NULL);
			VectorMA (client->ps.velocity, -64, forward, client->ps.velocity);
		}
		break;
	case WP_GPG40:
	case WP_M7:
		if( level.time - client->ps.classWeaponTime > level.engineerChargeTime[client->sess.sessionTeam-1] ) {
			client->ps.classWeaponTime = level.time - level.engineerChargeTime[client->sess.sessionTeam-1];
		}

		client->ps.classWeaponTime += .5f * level.engineerChargeTime[client->sess.sessionTeam-1];
	...
	case WP_MORTAR_SET:
		...
		if( client->sess.skill[SK_HEAVY_WEAPONS] >= 1 ) {
			...
			//          is more precise.
			client->ps.classWeaponTime += .33f * level.soldierChargeTime[client->sess.sessionTeam-1];
		} else {
			client->ps.classWeaponTime += .5f * level.soldierChargeTime[client->sess.sessionTeam-1];
		}
	...
	case WP_AIRDROP_MARKER:
...
#ifdef __EF__
			|| ent->s.weapon == WP_AIRDROP_MARKER
#endif //__EF__
			) {
			if( level.time - client->ps.classWeaponTime > level.covertopsChargeTime[client->sess.sessionTeam-1] ) {
				client->ps.classWeaponTime = level.time - level.covertopsChargeTime[client->sess.sessionTeam-1];
			}

			if( client->sess.skill[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS] >= 2 ) {
				client->ps.classWeaponTime += .66f * level.covertopsChargeTime[client->sess.sessionTeam-1];
			} else {
				client->ps.classWeaponTime = level.time;
			}
		}
		...
		if( ent->s.weapon == WP_LANDMINE ) {
			...
			if( client->sess.skill[SK_EXPLOSIVES_AND_CONSTRUCTION] >= 3 ) {
				...
				client->ps.classWeaponTime += .33f * level.engineerChargeTime[client->sess.sessionTeam-1];
			} else {
				client->ps.classWeaponTime += .5f * level.engineerChargeTime[client->sess.sessionTeam-1];
			}
		}
		...
		if (ent->s.weapon == WP_DYNAMITE) {
			...
			if( client->sess.skill[SK_EXPLOSIVES_AND_CONSTRUCTION] >= 3 ) {
				client->ps.classWeaponTime += .66f * level.engineerChargeTime[client->sess.sessionTeam-1];
			} else {
				client->ps.classWeaponTime = level.time;
			}
		}
	...
	if(g_gamestate.integer == GS_PLAYING)
#endif
		client->sess.aWeaponStats[BG_WeapStatForWeapon(ent->s.weapon)].atts += shots;
}
...
void G_createClusterNade( gentity_t *ent, int numNades )
{
	...
	for( it = 0; it < numNades; it++ ) {
		...
		VectorSubtract( client->ps.velocity, dir, dir );
		dir[2] = 60;
...
void G_throwKnife( gentity_t *ent )
{
	...
	if ( level.time - client->pmext.knifeThrownTime < 
				(g_throwKnifeWait.value * 1000) ) {
		CPx(ent-g_entities, va("cp \"You must wait %.2f second%s "
				"between knife throws\n\"",
				g_throwKnifeWait.value,
				(g_throwKnifeWait.value==1.f)?"":"s"));
		return;
	}

	if ( client->ps.ammoclip[BG_FindClipForWeapon(WP_KNIFE)] == 0 ) {
	...
	AngleVectors(client->ps.viewangles, velocity, NULL, NULL);
	VectorScale(velocity, 64, offset);
	offset[2] += client->ps.viewheight / 2;
	...
	VectorSet( mins, -ITEM_RADIUS, -ITEM_RADIUS, 0);
	VectorSet( maxs, ITEM_RADIUS, ITEM_RADIUS, 2 * ITEM_RADIUS);
	trap_Trace( &tr, client->ps.origin, 
			mins, maxs, org, 
			ent->s.number, MASK_SOLID);
	VectorCopy(tr.endpos, org);
	G_Sound(ent, G_SoundIndex( "sound/weapons/knife/knife_slash1.wav" ));
	traceEnt = LaunchItem(item, org, velocity, client->ps.clientNum);
	traceEnt->touch = G_touchKnife;
	traceEnt->parent = ent;
	client->ps.ammoclip[BG_FindClipForWeapon(WP_KNIFE)]--;
	client->pmext.knifeThrownTime = level.time;
	return;
}
...
#ifdef __EF__
			traceEnt->s.weapon == WP_AIRDROP_MARKER ||
#endif //__EF__
...
int G_GetWeaponFalloff(int weapon)
{
	...
	switch (weapon) {
		...
		case WP_AKIMBO_COLT:
		case WP_AKIMBO_SILENCEDCOLT: 
			// 9mm and TT-33 fall characteristics are 
