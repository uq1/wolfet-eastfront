...
int AIMOD_NAVIGATION_FindTankSpawnNode(gentity_t *bot, int r, int type)
{
    int         i;
    float       closest;
	float       nonLOSclosest;
	...
	for( i = 0; i < number_of_nodes; i++ ) // Randomize a little???
    {
		...
		if ( dist < closest && dist < 1024 && dist > 512
			 && i != bot->current_node 
			 && i != bot->next_node
			 && nodes[i].origin[2] - bot->r.currentOrigin[2] < 32 )  // Height check... never a node above us too far..
		{
...
void NPC_Tank_Spawn( vec3_t spawn_location )
{
	...
	trap_SetBrushModel( NPC, va( "*%i", NPC->s.modelindex ) );
...
void NPC_AI_Vehicle_Move( gentity_t *NPC )
{
...
void NPC_AI_Vehicle_Think( gentity_t *NPC )
{
...
void tank_think (gentity_t *self) 
...
void tank_spawn (gentity_t *ent) 
{
	...
	if ( g_knifeonly.integer != 1) 
	{
		...
		trap_SetBrushModel( tank, va( "*%i", tank->s.modelindex ) );
		tank->r.bmodel = qtrue;
		tank->sound3to2 =		-1;
