	...
	*/
	...
	for(i=0; serverinfo_cvars[i]; i++) {
		if(!Q_stricmp(serverinfo_cvars[i], var_name)) {
			...
			Info_SetValueForKey_Big(cs, var_name, value);
			if((strlen(cs) + 1) >= MAX_INFO_STRING) {
				G_Printf("WARNING: skipping SERVERINFO cvar set "
					"for %s (MAX_INFO_STRING protection)\n",
					var_name);
				return qfalse;
			}
			if((int)(level.csLenTotal + strlen(cs) + 1 - level.csLen[0])
				>= MAX_GAMESTATE_CHARS) {

				G_Printf("WARNING: skipping SERVERINFO cvar set "
					"for %s (MAX_GAMESTATE_CHARS protection)\n",
					var_name);
				return qfalse;
			}
	...
	/*
	cs[0] = '\0';
	for(i=0; systeminfo_cvars[i]; i++) {
		if(!Q_stricmp(systeminfo_cvars[i], var_name)) {
			trap_GetConfigstring(CS_SYSTEMINFO, cs, sizeof(cs));
			Info_SetValueForKey_Big(cs, var_name, value);
			if(strlen(cs) >= MAX_INFO_STRING) {
				G_Printf("WARNING: skipping SYSTEMINFO cvar set "
					"for %s (MAX_INFO_STRING protection)\n",
					var_name);
				return qfalse;
			}
			if((int)(level.csLenTotal + strlen(cs) - level.csLen[1])
				>= MAX_GAMESTATE_CHARS) {
			...
			level.csLenTotal += (int)(strlen(cs) - level.csLen[1]);
			level.csLen[1] = strlen(cs);
			return qtrue;
		}
	}
	*/
...
void trap_SetConfigstring( int num, const char *string ) {
	...
	if((int)(level.csLenTotal + strlen(string) + 1 - level.csLen[num])
		>= MAX_GAMESTATE_CHARS) {
	...
	level.csLenTotal += (int)(strlen(string) - level.csLen[num] + 1);
	level.csLen[num] = (strlen(string) + 1);
...
void trap_SetUserinfo( int num, const char *buffer ) {
	...
	trap_GetUserinfo(num, curr_ui, 1024);

	if (strcmp(curr_ui, buffer)) // UQ1: only send if changed!
	{
		//G_Printf("%s != %s\n", curr_ui, buffer);
...
#ifdef __ETE__
	// UQ1: Added, for footstep type overriding on specific maps...
	if (snowstep_kludge_map)
	{
		if (results->surfaceFlags & SURF_GRASS)
		{
			results->surfaceFlags &= ~SURF_GRASS;

			if (!(results->surfaceFlags & SURF_SNOW))
				results->surfaceFlags |= SURF_SNOW;
		}
		
		if (results->surfaceFlags & SURF_GRAVEL)
		{
			results->surfaceFlags &= ~SURF_GRAVEL;
...
#ifdef __VEHICLES__
	//if (contentmask == MASK_SHOT || contentmask == (MASK_SHOT | MASK_WATER))
	{
		if (results->entityNum && results->entityNum < ENTITYNUM_MAX_NORMAL)
		{
			if (g_entities[results->entityNum].s.eType == ET_VEHICLE_TURRET)
			{// Hit a tank turret! Return the owner!
				//G_Printf("Trace hit a vehicle turret!\n");
				results->entityNum = g_entities[results->entityNum].parent->s.number;
			}
		}
	}
#endif //__VEHICLES__
...
#ifdef __VEHICLES__
	if(hitEnt && hitEnt->client && (hitEnt->client->ps.eFlags & EF_VEHICLE)) return;
#endif //__VEHICLES__
