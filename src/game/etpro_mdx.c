...
void mdx_gentity_to_grefEntity(gentity_t *ent, grefEntity_t *refent, int lerpTime)
{
	...
	if (ent->s.eType == ET_PLAYER) {
		...
		character = BG_GetCharacter( ent->s.teamNum, ent->NPC_PlayerType );
#endif //__NPC__
	} else {
...
)
{
	...
	// UQ1: Added these 2 checks for NPCs...
	if (frame < 0)
		frame = 0;
...
void mdx_PlayerAngles( gentity_t *ent, vec3_t legsAngles, vec3_t torsoAngles, vec3_t headAngles, qboolean doswing )
{
	...
	if (client)
	{
		torsoAngles[ROLL] += client->ps.leanf *1.8 ;
		headAngles[ROLL] += client->ps.leanf *1.8 ;
	}
