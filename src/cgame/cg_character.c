...
qboolean CG_RegisterCharacter( const char *characterFile, bg_character_t *character )
{
	...
	// UQ1: Register stealth skin if we have one!
	//filename = va( "%s_stlth_%s.skin", buf, characterDef.skin );
	//character->stlth_skin = trap_R_RegisterSkin( filename );
...
extern int num_game_characters;
...
#ifdef __NPC__
	if( cent && cent->currentState.eType == ET_NPC_CORPSE ) {
		if( cent->currentState.onFireStart >= 0 && cent->currentState.onFireStart < num_game_characters )
			return cgs.gameCharacters[ cent->currentState.onFireStart ];
		else {
			if( cent->currentState.modelindex < 4 && cent->currentState.modelindex2 <= PC_COVERTOPS)
		...
		/*team = cent->currentState.teamNum == TEAM_AXIS ? TEAM_ALLIES : TEAM_AXIS;
		cls = cent->currentState.density;
		return BG_GetCharacter( team, cls );*/

		// DOH!
		team = TEAM_AXIS;
		cls = PC_SOLDIER;
		cent->currentState.eFlags |= EF_NODRAW; // Hide it!
		return BG_GetCharacter( team, cls );
	}
#endif //__NPC__
...
#ifdef __NPC__
bg_character_t *CG_NPC_GetCharacter( centity_t *cent )
{
	int		team, cls;
	clientInfo_t	*ci;
	...
	ci->character = BG_GetCharacter( cent->currentState.teamNum, cent->currentState.density );

    return ci->character;//BG_GetCharacter( cent->currentState.teamNum, cent->currentState.density );
}
#endif //__NPC__
