...
static qboolean uq_is_atmospheric_kludge_map = qfalse;
static qboolean uq_atmospheric_kludge_checked = qfalse;

void UQ_Reset_Kludge ( void )
{
	kludgeChecked = qfalse;
	kludgeResult = 0;
}

qboolean UQ_Atmospheric_Kludge ( void )
{
	char bigTextBuffer2[100000];

	if (uq_is_atmospheric_kludge_map)
	{
		return qtrue;
	}

	if (!uq_atmospheric_kludge_checked)
	{// UQ1: Load from file!
		int len = 0;
		fileHandle_t file;

		len = trap_FS_FOpenFile(va("maps/%s.atmospheric", cgs.rawmapname), &file, FS_READ);

		uq_atmospheric_kludge_checked = qtrue;

		if (len > 0)
		{
			char		*text_p;
			char		*token;
			char		atospheric_selections[255][255];
			int			num_atospheric_selections = 0;
			int			choice = 0;

			uq_is_atmospheric_kludge_map = qtrue;

			trap_FS_Read( bigTextBuffer2, len, file );
			bigTextBuffer2[len] = 0;
			trap_FS_FCloseFile( file );

			// parse the text
			text_p = bigTextBuffer2;

			CG_Printf( "^1*** ^3%s^5: Successfully loaded map atmospheric info from file ^7/maps/%s.atmospheric^5.\n", GAME_VERSION, cgs.rawmapname);
			...
			while( 1 ) 
			{
				...
				strcpy(atospheric_selections[num_atospheric_selections], token);
				CG_Printf("^1*** ^3%s^5: Parsed atmospheric [%i] - \"%s\"\n", GAME_VERSION, num_atospheric_selections, atospheric_selections[num_atospheric_selections]);
				num_atospheric_selections++;
			}

			if (num_atospheric_selections <= 0)
			{// None listed... No kludge for this map!
				return ( kludgeResult = qfalse );
			}

			// Select and use one!
			choice = Q_TrueRand(0, num_atospheric_selections-1);
			CG_EffectParse( ("%s", atospheric_selections[choice]) );

			CG_Printf("^1*** ^3%s^5: Selected atmospheric [%i] - \"%s\"\n", GAME_VERSION, choice, atospheric_selections[choice]);
			return ( kludgeResult = qtrue );
		}
		else
		{
			CG_Printf( "^1*** ^3%s^5: No atmospheric info for this map. If you want atmospherics, create ^7/maps/%s.atmospheric^5.\n", GAME_VERSION, cgs.rawmapname);
		}
	}
...
qboolean CG_AtmosphericKludge()
{
	...
	if( !Q_stricmp( cgs.mapname, "maps/xposed.bsp" ) )
	{
	...
	if( !Q_stricmp( cgs.mapname, "maps/battery.bsp" ) )
	{
		int choice = Q_TrueRand(0,1);
	...
	if( !Q_stricmp( cgs.mapname, "maps/goldrush.bsp" ) )
	{
	...
	if( !Q_stricmp( cgs.mapname, "maps/stalingrad.bsp" ) )
	{
		int choice = Q_TrueRand(0,2);
	...
	if( !Q_stricmp( cgs.mapname, "maps/leningrad.bsp" ) )
	{
		int choice = Q_TrueRand(0,2);
	...
	if( !Q_stricmp( cgs.mapname, "maps/moscow_forest.bsp" ) )
	{
		int choice = Q_TrueRand(0,2);
	...
	if( !Q_stricmp( cgs.mapname, "maps/warsaw_poland.bsp" ) )
	{
		int choice = Q_TrueRand(0,2);
...
#ifdef __CUDA__
__device__ __host__ static qboolean CG_SnowParticleCheckVisible( cg_atmosphericParticle_t *particle )
{
	// Check the snowflake is visible and still going, wrapping if necessary.

	__shared__ float moved;
	__shared__ vec2_t distance;
//	int msec = trap_Milliseconds();
...
//		checkvisibletime += trap_Milliseconds() - msec;
	...
	moved = (cg.time - cg_atmFx.lastRainTime) * 0.001;	// Units moved since last frame
	VectorMA( particle->pos, moved, particle->delta, particle->pos );
	__host__ if( particle->pos[2] < BG_GetSkyGroundHeightAtPoint( particle->pos ) ) {
//		checkvisibletime += trap_Milliseconds() - msec;
		return CG_SetParticleActive( particle, ACT_NOT );
	}
	...
	distance[0] = particle->pos[0] - cg.refdef_current->vieworg[0];
	distance[1] = particle->pos[1] - cg.refdef_current->vieworg[1];
 	if( (distance[0] * distance[0] + distance[1] * distance[1]) > MAX_ATMOSPHERIC_DISTANCE*MAX_ATMOSPHERIC_DISTANCE /*Square( MAX_ATMOSPHERIC_DISTANCE )*/ )
 	{
		// ydnar: just nuke this particle, let it respawn
		return CG_SetParticleActive( particle, ACT_NOT );
