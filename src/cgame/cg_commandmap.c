...
CG_ScissorEntIsCulled ( mapEntityData_t *mEnt, mapScissor_t *scissor )
{
	...
	else
	{
		...
		if ( distSquared > Square( 0.5f * 1/*cg_compassTolerance.value*/ * (scissor->br[0] - scissor->tl[0])) )
		{
			return ( qtrue );
		}
	}
...
CG_TransformAutomapEntity ( void )
{
	...
	for ( i = 0; i < mapEntityCount; i++ )
	{
		...
		mEnt->automapTransformed[0] = ( mEnt->x - cg.mapcoordsMins[0] ) * cg.mapcoordsScale[0] * 100 * cg_mapzoom.value;
		mEnt->automapTransformed[1] = ( mEnt->y - cg.mapcoordsMins[1] ) * cg.mapcoordsScale[1] * 100 * cg_mapzoom.value;
	}
}
...
//		return; // UQ1: No grid on single player maps!
	...
	else
	{
		...
		for ( coord_char[0] = ('A' - 1); grid_x < dim_x[1]; grid_x += step[0], coord_char[0]++ )
		{
			if ( coord_char[0] >= 'A' )
			{
				text_width = CG_Text_Width_Ext( coord_char, 0.2f, 0, &cgs.media.tahoma30 );
				text_height = CG_Text_Height_Ext( coord_char, 0.2f, 0, &cgs.media.tahoma30 );
				CG_Text_Paint_Ext( (x + grid_x) - (.5f * step[0]) - (.5f * text_width),
								   y + dim_y[0] + textOrigin[1] + 1.5f * text_height, 0.2f, 0.2f, colorBlack,
								   coord_char, 0, 0, 0, &cgs.media.tahoma30 );
			}
		...
		for ( coord_int = -1; grid_y < dim_y[1]; grid_y += step[1], coord_int++ )
		{
			if ( coord_int >= 0 )
			{
				...
				text_width = CG_Text_Width_Ext( "0", 0.2f, 0, &cgs.media.tahoma30 );
				text_height = CG_Text_Height_Ext( coord_char, 0.2f, 0, &cgs.media.tahoma30 );
...
)
{
	...
	switch ( mEnt->type )
	{
		...
		case ME_PLAYER_DM:
			...
			else
			{
				...
				{
					CG_DrawRotatedPic( icon_pos[0]-1-32+6, icon_pos[1]-1-32+6, 64, 64,
						cgs.media.hudFOVLine, (0.5 - ( mEnt->yaw - 180.f ) / 360.f) - ((cg_fov.value/2)/360.f) );
					CG_DrawRotatedPic( icon_pos[0]-1-32+6, icon_pos[1]-1-32+6, 64, 64,
						cgs.media.hudFOVLine, (0.5 - ( mEnt->yaw - 180.f ) / 360.f) + ((cg_fov.value/2)/360.f) );
				...
				else if ( CG_IsOnSameFireteam( cg.clientNum, mEnt->data) )
				{
					...
					if ( !scissor )
					{
						CG_Text_Paint_Ext( string_pos[0], string_pos[1], 0.2f, 0.2f, colorWhite, ci->name, 0, 0,
										   ITEM_TEXTSTYLE_SHADOWED, &cgs.media.tahoma30 );
					}
	...
	#define CONST_ICON_EXPANDED_SIZE	48.f
			...
			)
			{
				...
				w2 = CG_Text_Width_Ext( name, 0.2f, 0, &cgs.media.tahoma30 );
				CG_CommandMap_SetHighlightText( name, icon_pos[0] - (w2 * 0.5f), icon_pos[1] - 8 );
			}
...
int CG_DrawCrateInfo ( int px, int py, int pw, int ph, qboolean draw, mapScissor_t *scissor, int expand );
...
)
{
	...
	if ( scissor )
	{
		...
		if ( scissor->br[0] >= scissor->tl[0] )
		{
			...
			if ( cgs.ccLayers )
			{
				trap_R_DrawStretchPic( sc_x, sc_y, sc_w, sc_h, s0, t0, s1, t1, cgs.media.commandCentreMapShaderTrans[cgs.ccSelectedLayer] );
			}
			else
			{
				trap_R_DrawStretchPic( sc_x, sc_y, sc_w, sc_h, s0, t0, s1, t1, cgs.media.commandCentreMapShaderTrans[0] );
			}
		...
		CG_DrawRect_FixedBorder(x-1, y-1, w+2, h+2, 2, popBorder);
	}
	else
	{
	...
	//CG_DrawCrateInfo( x, y, w, h, qtrue, scissor, exspawn );
...
int
CG_DrawCrateInfo ( int px, int py, int pw, int ph, qboolean draw, mapScissor_t *scissor, int expand )
{
	int		i;
	char	buffer[64];
	vec2_t	point;
...
#define SPAWN_SIZEUPTIME	1000.f
	...
	for ( i = 1; i < MAX_GENTITIES; i++ )
	{
		...
		if (cent->currentState.eType != ET_AMMO_CRATE && cent->currentState.eType != ET_HEALTH_CRATE)
			continue;

		VectorCopy(cent->currentState.origin, CoordsTransformed);
		CG_TransformToCommandMapCoord( &CoordsTransformed[0], &CoordsTransformed[1] );

		if ( cgs.ccLayers )
		{
			if ( CG_CurLayerForZ( (int) cent->currentState.origin[2]) != cgs.ccSelectedLayer )
			{
				break;
			}
		}
		...
		if ( scissor )
		{
			point[0] = ( (CoordsTransformed[0] - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0] ) *
				pw *
				scissor->zoomFactor;
			point[1] = ( (CoordsTransformed[1] - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1] ) *
				ph *
				scissor->zoomFactor;
		}
		else
		{
			point[0] = px + ( ((CoordsTransformed[0] - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0]) * pw );
			point[1] = py + ( ((CoordsTransformed[1] - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1]) * ph );
		}
		...
		if (cent->currentState.eType == ET_AMMO_CRATE)
		{
			if ((draw && i == expand) || ( !expanded &&	BG_RectContainsPoint( point[0] - FLAGSIZE_NORMAL * 0.5f, point[1] - FLAGSIZE_NORMAL * 0.5f, FLAGSIZE_NORMAL, FLAGSIZE_NORMAL, cgDC.cursorx, cgDC.cursory)))
			{
				if ( draw )
				{
					...
					CG_DrawPic( point[0] - FLAG_LEFTFRAC * size, point[1] - FLAG_TOPFRAC * size, size, size,
								cgs.media.commandCentreAmmoCrateShader );
				}
				else
				{
					if ( !scissor )
					{
						float	w;
						Com_sprintf( buffer, sizeof(buffer), "Ammo Crate" );
						w = CG_Text_Width_Ext( buffer, 0.2f, 0, &cgs.media.tahoma30 );
		...
		{// A health crate
			if ((draw && i == expand) || ( !expanded &&	BG_RectContainsPoint( point[0] - FLAGSIZE_NORMAL * 0.5f, point[1] - FLAGSIZE_NORMAL * 0.5f, FLAGSIZE_NORMAL, FLAGSIZE_NORMAL, cgDC.cursorx, cgDC.cursory)))
			{
				if ( draw )
				{
					...
					CG_DrawPic( point[0] - FLAG_LEFTFRAC * size, point[1] - FLAG_TOPFRAC * size, size, size,
								cgs.media.commandCentreHealthCrateShader );
				}
				else
				{
					if ( !scissor )
					{
						float	w;
						Com_sprintf( buffer, sizeof(buffer), "Health Crate" );
						w = CG_Text_Width_Ext( buffer, 0.2f, 0, &cgs.media.tahoma30 );
						CG_CommandMap_SetHighlightText( buffer, point[0] - (w * 0.5f), point[1] - 8 );
					}
...
#define SPAWN_SIZEUPTIME	1000.f
	for ( i = 1; i < cg.spawnCount; i++ )
	{
		...
		if ((cgs.gametype == GT_SUPREMACY || cgs.gametype == GT_SUPREMACY_CAMPAIGN)
			&& cg.spawnTeams[i] == TEAM_FREE)
		{// Neutral flag!
			if
			(
				(draw && i == expand) || ( !expanded &&	BG_RectContainsPoint( point[0] - FLAGSIZE_NORMAL * 0.5f, point[1] - FLAGSIZE_NORMAL * 0.5f, FLAGSIZE_NORMAL, FLAGSIZE_NORMAL, cgDC.cursorx, cgDC.cursory)))
			{
				if ( draw )
				{
					float	size = FLAGSIZE_EXPANDED;
					...
					CG_DrawPic( point[0] - FLAG_LEFTFRAC * size, point[1] - FLAG_TOPFRAC * size, size, size,
								cgs.media.commandCentreSpawnShader[2] );
				}
				else
				{
					if ( !scissor )
			...
			if ( draw )
			{
				...
				if ( !scissor )
				{
					...
					CG_Text_Paint_Ext( point[0] + FLAGSIZE_NORMAL * 0.25f, point[1], 0.2f, 0.2f, colorWhite, buffer, 0,
									   0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.tahoma30 );
				}
			}
		}
	}
