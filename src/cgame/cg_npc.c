...
//28/11/2007
//line 1319 Masteries add stug3 in an OR condition, like jagpanther style turret
...
extern void CG_RunLerpFrameRate( clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, centity_t *cent, int recursion );
...
extern bg_character_t *CG_NPC_GetCharacter( centity_t *cent );
...
==============
*/
static void CG_NPCBreathPuffs( centity_t *cent, refEntity_t *head ) {
	clientInfo_t	*ci;
	vec3_t			up, forward;
	int				contents;
...
===============
*/
static void CG_NPCFloatSprite( centity_t *cent, qhandle_t shader, int height ) {
	int				rf;
	refEntity_t		ent;
...
===============
*/
static void CG_NPCSprites( centity_t *cent ) {
	int		team;
...
static qboolean CG_NPCShadow( centity_t *cent, float *shadowPlane )
{
	vec3_t			end;
	trace_t			trace;
	float			dist, distFade;
	...
	*shadowPlane = trace.endpos[2] + 1;
	...
	if ( cg_shadows.integer != 1 || using_uq_fog) {	// no mark for stencil or projection shadows
		return qtrue;
	}
...
===============
*/
static void CG_NPCSplash( centity_t *cent ) {
	vec3_t		start, end;
	trace_t		trace;
	int			contents;
	polyVert_t	verts[4];
...
===============
*/
static void CG_NPCAnimation( centity_t *cent, refEntity_t *body ) {
	clientInfo_t	*ci;
	int				clientNum;
	...
	//character = CG_NPC_GetCharacter(cent);//CG_CharacterForClientinfo( ci, cent );
...
==================
*/
static void CG_NPCSwingAngles( float destination, float swingTolerance, float clampTolerance,
...
=================
*/
static void CG_NPCAddPainTwitch( centity_t *cent, vec3_t torsoAngles ) {
	int		t;
	float	f;
	int		duration;
	float	direction;
...
static void CG_NPCAngles( centity_t *cent, vec3_t legs[3], vec3_t torso[3], vec3_t head[3] ) {
	...
	if( cent->currentState.eFlags & EF_DEAD || cent->currentState.eFlags & EF_MOUNTEDTANK ) {
		...
		{// torso
			CG_NPCSwingAngles( torsoAngles[YAW], 0, clampTolerance, 0.06f, &cent->pe.torso.yawAngle, &cent->pe.torso.yawing );
		...
		{
			// torso
			CG_NPCSwingAngles( torsoAngles[YAW], 25, clampTolerance, cg_swingSpeed.value, &cent->pe.torso.yawAngle, &cent->pe.torso.yawing );
			...
			if( BG_GetConditionBitFlag( ci->clientNum, ANIM_COND_MOVETYPE, ANIM_MT_IDLE) ) 
			{
				...
				CG_NPCSwingAngles( legsAngles[YAW], 20, clampTolerance, 0.5*cg_swingSpeed.value, &cent->pe.legs.yawAngle, &cent->pe.legs.yawing );
			} 
			...
			//if	( BG_GetConditionValue( ci->clientNum, ANIM_COND_MOVETYPE, qfalse ) & ((1<<ANIM_MT_STRAFERIGHT)|(1<<ANIM_MT_STRAFELEFT)) )
				cent->pe.legs.yawing = qfalse; // set it if they really need to swing
				legsAngles[YAW] = headAngles[YAW];
				CG_NPCSwingAngles( legsAngles[YAW], 0, clampTolerance, cg_swingSpeed.value, &cent->pe.legs.yawAngle, &cent->pe.legs.yawing );
			} 
			...
			{
				CG_NPCSwingAngles( legsAngles[YAW], 40, clampTolerance, cg_swingSpeed.value, &cent->pe.legs.yawAngle, &cent->pe.legs.yawing );
			}
		}
	...
	if ( cent->currentState.eFlags & EF_VEHICLE) {  
		...
		// back
		CG_Trace( &trb, back, NULL, NULL, back2, 
		cent->currentState.number, CONTENTS_SOLID );

		// front
		CG_Trace( &trf, front, NULL, NULL, front2, 
		cent->currentState.number, CONTENTS_SOLID );
...
#ifdef __UNUSED__
static void CG_NPC_Vehicle( centity_t *cent ) {
	refEntity_t			ent;
	refEntity_t			ent2;
	orientation_t		or;
	entityState_t		*s1;
...
void CG_NPCAttachBitsToTank( centity_t* tank, refEntity_t* mg42base, refEntity_t* mg42upper, refEntity_t* mg42gun, refEntity_t* player, refEntity_t* flash, vec_t* playerangles, const char* tagName, qboolean browning ) {
	refEntity_t	ent;
	vec3_t angles;
	int i;
...
void CG_NPC_Vehicle_PostProcess( centity_t* cent ) {
	...
	CG_NPCAttachBitsToTank( cent, &mg42base, &mg42upper, &mg42gun, &player, &flash, angles, "tag_player", cent->currentState.density & 8 ? qtrue : qfalse );
...
// We might want this tag code outside of NPC usage, so I will temporarily disable the ifdef around it...
#endif //__NPC__
...
extern void CG_Tank(centity_t *cent);

void CG_NPCTank(centity_t *cent) {
#ifdef __VEHICLES__
	CG_Tank(cent);
#endif //__VEHICLES__
}
...
void CG_RenderPlane2 ( vec3_t org, vec3_t angles, int modelindex, qboolean smoke )
{
	refEntity_t		model;
	vec3_t			playerOrigin;
	...
	model.hModel = cgs.gameModels[modelindex];
...
//	if (cent->currentState.nextWeapon <= 0 
//		|| (cent->currentState.nextWeapon > 0 && cent->currentState.onFireEnd == 0))
	if (cent->currentState.nextWeapon <= 0)
	...
	{// Add smoke effects...
		...
		*/
		...
		if (cent->highlightTime == 1 && cent->entityPS)
		{// Gun0
#ifndef __OLD_PARTICLE_SYSTEM__
			CG_SetAttachmentTag( &cent->entityPS->attachment, model, model.hModel, "tag_gun1" );
#else //!__OLD_PARTICLE_SYSTEM__
			CG_SetParticleSystemTag( cent->entityPS, model, model.hModel, "tag_gun1" );
			//CG_SetParticleSystemTag( cent->entityPS, model, model.hModel, "tag_prop" );
#endif //__OLD_PARTICLE_SYSTEM__
		}
		else if (cent->highlightTime == 3 && cent->entityPS2)
		{// Gun1
#ifndef __OLD_PARTICLE_SYSTEM__
			CG_SetAttachmentTag( &cent->entityPS2->attachment, model, model.hModel, "tag_gun02" );
#else //!__OLD_PARTICLE_SYSTEM__
			CG_SetParticleSystemTag( cent->entityPS2, model, model.hModel, "tag_gun02" );
#endif //__OLD_PARTICLE_SYSTEM__
		}
		else if (cent->highlightTime == 2 && cent->entityPS)
		{// Prop
#ifndef __OLD_PARTICLE_SYSTEM__
			CG_SetAttachmentTag( &cent->entityPS->attachment, model, model.hModel, "tag_prop" );
#else //!__OLD_PARTICLE_SYSTEM__
			CG_SetParticleSystemTag( cent->entityPS, model, model.hModel, "tag_prop" );
#endif //__OLD_PARTICLE_SYSTEM__
		}
		else if (cent->entityPS && cent->entityPS2)
		{// Guns 1 and 2
#ifndef __OLD_PARTICLE_SYSTEM__
			CG_SetAttachmentTag( &cent->entityPS->attachment, model, model.hModel, "tag_gun1" );
			//CG_SetAttachmentTag( &cent->entityPS->attachment, model, model.hModel, "tag_prop" );
...
#else //!__OLD_PARTICLE_SYSTEM__
			//vec3_t  up = { 0.0f, 0.0f, 1.0f };
			//CG_SetParticleSystemNormal( cent->entityPS2, up );
			//CG_AttachToPoint( &cent->entityPS2->attachment );
			CG_SetAttachmentCent( &cent->entityPS2->attachment, cent );
			CG_AttachToTag( &cent->entityPS2->attachment );
#endif //__OLD_PARTICLE_SYSTEM__
		}
	}
	...
	if (cent->currentState.nextWeapon > 0)
	{// UQ1: Add formation... 
		vec3_t org, forward, right/*, temp_org*/;
		
		AngleVectors ( cent->lerpAngles, forward, right, NULL);
		VectorCopy(cent->lerpOrigin, org);
		VectorMA (org, -256, forward, org);
		VectorMA (org, -256, right, org);
		CG_RenderPlane2( org, cent->lerpAngles, cent->currentState.modelindex, qfalse );
...
//	float			gumsflappin = 0;	// talking amplitude
	...
	{// A LCVP lander boat model...
		refEntity_t		model;
		bg_character_t	*character = BG_GetCharacter( TEAM_ALLIES, PC_SOLDIER );
	...
	{// A Destroyer boat model...
		...
		// Add the turret gun...
		{
			...
			CG_PositionEntityOnTag( &prop, &model, "tag_cannon", 0, &offset/*NULL*/ );
			trap_R_AddRefEntityToScene( &prop );
		...
		// Add the flag...
		{
			...
			//CG_PositionEntityOnTag( &prop, &model, "tag_flag", 0, /*&offset*/NULL );
			trap_R_AddRefEntityToScene( &prop );
		}
		...
		// Add the radar...
		{
			...
			//CG_PositionEntityOnTag( &prop, &model, "tag_radar", 0, /*&offset*/NULL );
			trap_R_AddRefEntityToScene( &prop );
		}
...
/*		// Add wake/mist...
		{
			...
			VectorSet( offset, 0, 96, -48 );
			VectorCopy(CG_GetTagOrigin(model, "tag_cannon", &offset), spot);

			// Add the propeller mist/bubbles...
			if (!cent->voiceChatSpriteTime)
			{
				cent->entityPS4 = CG_SpawnNewParticleSystem( cgs.media.mistTrailPS );
			
				// We're set up...
				cent->voiceChatSpriteTime = 1;
			}

			//CG_SetParticleSystemTag( cent->entityPS4, model, model.hModel, "tag_cannon" );
			//CG_SetParticleSystemCent( cent->entityPS4, cent );
			//CG_AttachParticleSystemToTag( cent->entityPS4 );
			CG_SetParticleSystemOrigin( cent->entityPS4, spot );
			CG_SetParticleSystemCent( cent->entityPS4, cent );
			CG_AttachParticleSystemToOrigin( cent->entityPS4 );
		}*/
...
#endif __UNUSED__
	if ( cent->currentState.modelindex2 == CLASS_PLANE )
	{// A bomber plane model...
		CG_RenderPlane( cent );
		return;
	}
	...
	if (!ci->name || !ci->name[0] || strlen(ci->name) <= 0)
	{// Names are disabled and we have not set the name yet. Set it now...
		strcpy(ci->name, va("NPC"));
	}
#endif //__NPC_NAMES__
	...
	ci->team = team;
//	character = CG_CharacterForClientinfo( ci, cent );
	ci->cls = PC_SOLDIER;//cent->currentState.density;//cent->currentState.modelindex2;//PC_SOLDIER;
	character = BG_GetCharacter( ci->team, ci->cls );//BG_GetCharacter( ci->team, ci->cls );

	if (cent->currentState.eType == ET_NPC_CORPSE)
	{
		cls = PC_SOLDIER;//cent->currentState.onFireStart;
		ci->cls =cls;
		team = cent->currentState.teamNum;
		ci->team = team;
...
//	character = CG_NPC_GetCharacter(cent);
...
/*	if( cg_entities[cent->currentState.clientNum].currentState.dl_intensity == TAG_HOOK ) 
	{// Need to join to a spawn tag!
		...
		Q_strncpyz( cg_entities[cent->currentState.clientNum].tagName, va("tag_spawnpoint%i", cg_entities[cent->currentState.clientNum].currentState.otherEntityNum2), sizeof(cg_entities[cent->currentState.clientNum].tagName) );
	...
	{// Need to join to a driver tag!
		cg_entities[cent->currentState.clientNum].tagParent = cg_entities[cent->currentState.clientNum].currentState.otherEntityNum;
		Q_strncpyz( cg_entities[cent->currentState.clientNum].tagName, "tag_driver", sizeof(cg_entities[cent->currentState.clientNum].tagName) );
		cg_entities[cent->currentState.clientNum].currentState.eFlags |= EF_MOUNTEDTANK;
	...
	{// Need to join to a gunner tag!
		...
		Q_strncpyz( cg_entities[cent->currentState.clientNum].tagName, "tag_gunner1", sizeof(cg_entities[cent->currentState.clientNum].tagName) );
		cg_entities[cent->currentState.clientNum].currentState.eFlags |= EF_MOUNTEDTANK;
	...
	{// Need to join to a gunner tag!
		cg_entities[cent->currentState.clientNum].tagParent = cg_entities[cent->currentState.clientNum].currentState.otherEntityNum;
		Q_strncpyz( cg_entities[cent->currentState.clientNum].tagName, "tag_gunner2", sizeof(cg_entities[cent->currentState.clientNum].tagName) );
		cg_entities[cent->currentState.clientNum].currentState.eFlags |= EF_MOUNTEDTANK;
	...
	{// Need to release the tag!
		...
		Q_strncpyz( cg_entities[cent->currentState.clientNum].tagName, "", sizeof(cg_entities[cent->currentState.clientNum].tagName) );
		cg_entities[cent->currentState.clientNum].currentState.eFlags &= ~EF_MOUNTEDTANK;
	...
	{// Attached to an NPC...
		if (!Q_stricmp( cg_entities[cent->currentState.clientNum].tagName, "tag_driver" ))
		{
			refEntity_t refent = cg_entities[cg_entities[cent->currentState.clientNum].tagParent].pe.bodyRefEnt;
			VectorCopy(CG_GetTagOrigin( refent, cg_entities[cent->currentState.clientNum].tagName, NULL ), playerOrigin);
			VectorCopy(cg_entities[refent.entityNum].lerpAngles, cg_entities[cent->currentState.clientNum].currentState.angles);
			playerOrigin[2]+=18;
		}
		else
		{
	...
	*/
	...
	body.shadowPlane = 0;//shadowPlane;
	...
	head.shadowPlane = 0;//shadowPlane;
