 ...
 *        //modified by masteries; added some tricks about vehicles
*/        //28/11/2007
...
#define	CORONA_FLASH_TIME	200
#define	MUZZLE_FLASH_TIME	200
...
// UQ1: Stealth System...
extern qhandle_t			stlth_skin;
extern qhandle_t			stlth_shader;
// UQ1: Shadow System...
extern qhandle_t			shadow_skin;
extern qhandle_t			shadow_shader;
...
typedef struct attachment_s
{
  ...
  //AT_PARTICLE
  struct particlesys_particle_s *particle;
} attachment_t;
...
#define MAX_PS_PARTICLES             MAX_PARTICLE_EJECTORS*5
...
typedef struct particlesys_particle_s
{
...
typedef struct clientInfo_s {
	...
	qboolean		ccSelected;
	int				fireteam;
	int				medals[SK_NUM_SKILLS+1];
	int				skill[SK_NUM_SKILLS+1];
	int				skillpoints[SK_NUM_SKILLS+1];	// filled OOB by +wstats
...
//	qboolean		extrapolated;	// false if origin / angles is an interpolation
	...
	float			player_scale;
	qboolean		player_scaled;
#ifdef __VEHICLES__
	vec3_t			last_tank_angles;
	qboolean		last_tank_angles_initialized;
	vec3_t			last_tank_turret_angles;
	qboolean		last_tank_turret_angles_initialized;
	vec3_t			flameTankTurretOrigin;
	vec3_t			flameTankTurretAngles;
#endif //__VEHICLES__

#ifdef __PARTICLE_SYSTEM__
	// New Particle System...
	particleSystem_t	*entityPS;
	particleSystem_t	*explosionPS;
	...
	int snipe_render_view_height_addition;
	int snipe_render_view_height_next_addition;
	int next_snipe_render_view_height_addition_time;
} centity_t;
...
typedef struct {
	...
	int healthbar_vertical;
	int staminabar[3];		// x, y, width
	int staminabar_vertical;
	int chargebar[3];		// x, y, width
	int chargebar_vertical;
...
#ifdef __ETE__
	qboolean	current_commander_orders_select;
	int			current_commander_order_setting;
	int			current_commander_order_time;
	qboolean	current_commander_crosshair_spin;
	vec3_t		current_commander_order_location;
	centity_t	*current_commander_order_enemy;
	int			current_commander_order_enemy_vis_time;
#endif //__ETE__
...
#ifdef __ETE__
	qhandle_t	coverspotMoveShader;
	qhandle_t	coverspotShader;
#endif //__ETE__
...
//	qhandle_t	selectShader;
	...
	qhandle_t	reticleShaderOverlay;
...
//----(SA)	cursor hints
	...
	qhandle_t	commandCentreSpawnShader[3];
	qhandle_t	commandCentreAmmoCrateShader;
	qhandle_t	commandCentreHealthCrateShader;
...
#ifdef __VEHICLES__
	qhandle_t	tankModel[VEHICLE_NUM_TYPES];	// jaquboss
	qhandle_t	tankTracksModel[VEHICLE_NUM_TYPES];	// jaquboss
	qhandle_t	tankTurretModel[VEHICLE_NUM_TYPES];
	qhandle_t	tankBarrelModel[VEHICLE_NUM_TYPES];
	qhandle_t	tankFlashModel;
	qhandle_t	tankShader[6];
#endif //__VEHICLES__
...
#ifdef __ETE__
	sfxHandle_t	sfx_grenexp2;
	sfxHandle_t	sfx_grenexpDist2;
	sfxHandle_t	sfx_grenexp3;
	sfxHandle_t	sfx_grenexpDist3;
	sfxHandle_t	sfx_grenexp4;
	sfxHandle_t	sfx_grenexpDist4;
#endif //__ETE__
...
//	sfxHandle_t	sparkSounds;
	...
	sfxHandle_t	tankSmokeSound;
	sfxHandle_t tankPtrsSound;//Masteries, can be used for ptrs infantry too
#endif
	...
	qhandle_t	skillPics[SK_NUM_SKILLS+1];
	...
	qhandle_t	heartBeatSnd;
	qhandle_t	breathAim;
	qhandle_t	breathSnipe;
	qhandle_t	breathAfraid;
	qhandle_t	breathRunning;
...
#ifdef __VEHICLES__
	qhandle_t	hMG42Base;	//	trap_R_RegisterModel( "models/mapobjects/tanks_sd/mg42nestbase.md3" );
#endif //__VEHICLES__

	// Gordon: medals
	qhandle_t		medals[SK_NUM_SKILLS+1];
...
//	skill_testicon
	...
	qhandle_t		limboWeaponCard4;
#endif //__EF__

#ifdef __SHELLSHOCK__
	qhandle_t		shellShockViewShader;
#endif //__SHELLSHOCK__

#ifdef __BLOODY_VIEW__
	qhandle_t		troubledViewShader;
#endif //__BLOODY_VIEW__

	qhandle_t		muzzleCorona;
	...
	qhandle_t		weaponIcons[MAX_WEAPONS][2]; // UQ1: For death icons...
	...
	qhandle_t		hudFOVLine;

#ifdef __NPC__
	qhandle_t		hPropeller;
#endif //__NPC__
#ifdef __PARTICLE_SYSTEM__
	qhandle_t		flameTrailPS;
	qhandle_t		grenadeParticlePS;
	qhandle_t		onFirePS;
#endif //__PARTICLE_SYSTEM__
} cgMedia_t;
...
typedef struct {
	int weapHits[WS_MAX];
	int weapShots[WS_MAX];
	int weapKills[WS_MAX];
	int weapDeaths[WS_MAX];
	int weapHS[WS_MAX];
	int dmgGiven;
	int dmgReceived;
	int teamDamage;
...
typedef struct {
	int weap[WS_MAX*2];
	int weapShots[WS_MAX*2];
	int weapHits[WS_MAX*2];
	int weapKills[WS_MAX*2];
	int clientID[WS_MAX*2];
	int cWeapons;
	int fadeTime;
	int show;
	int requestTime;
...
void CG_DrawStringExtSmall( int x, int y, const char *string, const float *setColor, 
		qboolean forceColor, qboolean shadow, int charWidth, int charHeight, int maxChars );
// END JOSEPH
...
void CG_FirePtrsTank( centity_t *cent);//Masteries, panzer2
...
void CG_ParseETEnhancedinfo( void );
void CG_ParseETESkillInfo( void );
...
// done.
void		trap_R_AddFlashEffectToScene ( vec3_t org, float radius, float intensity, float r, float g, float b, qhandle_t hShader, int flags, centity_t *cent );
void		trap_R_AddLightToScene (/* const*/ vec3_t org, float radius, float intensity, float r, float g, float b, qhandle_t hShader, int flags );
void		trap_R_AddLightToScene2( /*const*/ vec3_t org, float radius, float intensity, float r, float g, float b, qhandle_t hShader, int flags );
...
void CG_FitTextToWidth_SingleLine_Ext( char* instr, float scale, float w, int size, fontInfo_t* font);
...
#ifdef __ETE__
// UQ1: Secondary entites for players.. Located in cg_players.c
...
void        CG_SetAttachmentParticle( attachment_t *a, particlesys_particle_t *p );
...
void                CG_AddParticleSystemTrails( void );
...
void CG_AddRealLoopingSound( const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx, int range, int volume, int soundTime );
