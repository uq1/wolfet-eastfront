...
#endif //__ETE__
/*
void UQ_FlashEffect( vec3_t org, float radius, float intensity, float r, float g, float b, qhandle_t hShader, int flags, centity_t *cent )
{
	int			t;
	int			x, y;
	float		v, len;
	...
	t = cg.time - cent->muzzleFlashTime;

	if ( t <= 0 || t >= MUZZLE_FLASH_TIME ) 
	{
		return;
	}
	...
	if(!behind && !toofar) {
...
#ifdef __ETE__
		if(tr.fraction == 1 || VectorDistance(tr.endpos, org) <= 64)
			visible = qtrue;
#else //!__ETE__
		if(tr.fraction == 1)
			visible = qtrue;
		...
		if ( visible )
		{
			...
			v = ( 1.0f - ((float)t / MUZZLE_FLASH_TIME )) * ((1.0f - ( len / 800.0f )) * 2.0f + 0.35f);
			...
			if (CG_WorldCoordToScreenCoord( org, &x, &y ))
			{
				...
				CG_DrawPic( x - ( v * 300 ), y - ( v * 300 ), v * 600, v * 600, cgs.media.muzzleCorona);

				CG_Printf("Corona drawn at %f x %f - size is %f.\n", x - ( v * 300 ), y - ( v * 300 ), v * 600);
			}
		}
	}
}*/
...
//	CG_Printf("dot: %f\n", dot);
	...
	if(!behind && !toofar) {
		...
		trap_R_AddCoronaToScene( org, r, g, b, radius, cent->currentState.number, visible);
	}
}
...
{
	float rad = radius * 128.0f;

	UQ_FlashEffect( org, radius, intensity, r, g, b, hShader, flags, cent );

	syscall( CG_R_ADDLIGHTTOSCENE, org, PASSFLOAT( rad ), PASSFLOAT( intensity ), PASSFLOAT( r),
