...
static void CG_EntityEffects( centity_t *cent ) {
	...
	if (cent->currentState.eType == ET_PLAYER
		...
		&& (HasExtFlag(cg.snap->ps.clientNum, EX_SHELLSHOCK)))
	{
		...
		int		volume		= 32000 * (1.0f - (elapsed / duration));

		// ydnar: allow looped sounds to start at trigger time
		if( cent->soundTime == 0 )
			cent->soundTime = trap_S_GetCurrentSoundTime();

		CG_AddRealLoopingSound(cent->lerpOrigin, vec3_origin, cgs.media.shellShockSnd, 1250, volume, cent->soundTime );
	}
	else
	...
	if (/*g_realism.integer 
		...
		//&& !((cg.snap->ps.pm_flags & PMF_FOLLOW) && (cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR))
		&& cg.snap->ps.stats[STAT_HEALTH] < 100 
		&& cg.snap->ps.stats[STAT_HEALTH] > 0)
	{// We are hurt, play the hurt sound...
		float	volume = (cg.snap->ps.stats[STAT_HEALTH] / cg.snap->ps.stats[STAT_MAX_HEALTH]) * 1024;
		...
		CG_AddRealLoopingSound(cent->lerpOrigin, vec3_origin, cgs.media.heartBeatSnd, 1250, volume, cent->soundTime );
	}
	else if (!(cg.snap->ps.eFlags & EF_VEHICLE)
		&& cg.pmext.sprintTime < SPRINTTIME/* - 10000*/
		&& cg.snap->ps.stats[STAT_HEALTH] > 0)
	{// Sprinting, play the sprint sound...
		float volume = 133 - ((cg.pmext.sprintTime / ((float)SPRINTTIME/* - 10000*/)) * 133);

		// ydnar: allow looped sounds to start at trigger time
		if( cent->soundTime == 0 )
			cent->soundTime = trap_S_GetCurrentSoundTime();

		CG_AddRealLoopingSound(cent->lerpOrigin, vec3_origin, cgs.media.breathRunning, 1250, volume, cent->soundTime );
	}
	else if (!(cg.snap->ps.eFlags & EF_VEHICLE)
		&& (cg_entities[cg.snap->ps.clientNum].currentState.density > 0)
		&& cg.snap->ps.stats[STAT_HEALTH] > 0)
	{// Supressed, play the supression sound...
		float	volume = (float)((cg_entities[cg.snap->ps.clientNum].currentState.density)*0.01) * 255;

		// ydnar: allow looped sounds to start at trigger time
		if( cent->soundTime == 0 )
			cent->soundTime = trap_S_GetCurrentSoundTime();

		CG_AddRealLoopingSound(cent->lerpOrigin, vec3_origin, cgs.media.breathAfraid, 1250, volume, cent->soundTime );
	}
	else if ( cent->currentState.loopSound )
	{
		...
		if (cent->currentState.eType == ET_SPEAKER) {
...
#ifdef __NPC__
		} else if ( cent->currentState.eType == ET_NPC && cent->currentState.modelindex2 == CLASS_PLANE ) {
			vec3_t	forward, plane_dir, plane_angles, plane_sound_org;
			float	plane_real_distance = VectorDistance(cg.refdef.vieworg, cent->lerpOrigin);

			if (plane_real_distance > 2048)
			{// UQ1: Hack to make the sound travel further!
				VectorSubtract(cent->lerpOrigin, cg.refdef.vieworg, plane_dir);
				vectoangles(plane_dir, plane_angles);

				VectorCopy(cg.refdef.vieworg, plane_sound_org);
				AngleVectors (plane_angles, forward, NULL, NULL);
				VectorMA( plane_sound_org, 256+(plane_real_distance*0.25), forward, plane_sound_org );
			}
			else if (plane_real_distance > 1024)
			{// UQ1: Hack to make the sound travel further!
				VectorSubtract(cent->lerpOrigin, cg.refdef.vieworg, plane_dir);
				vectoangles(plane_dir, plane_angles);
				...
				VectorCopy(cg.refdef.vieworg, plane_sound_org);
				AngleVectors (plane_angles, forward, NULL, NULL);
				VectorMA( plane_sound_org, 128+(plane_real_distance*0.15), forward, plane_sound_org );
			}
			else if (plane_real_distance > 512)
			{// UQ1: Hack to make the sound travel further!
				VectorSubtract(cent->lerpOrigin, cg.refdef.vieworg, plane_dir);
				vectoangles(plane_dir, plane_angles);
				...
				VectorCopy(cg.refdef.vieworg, plane_sound_org);
				AngleVectors (plane_angles, forward, NULL, NULL);
				VectorMA( plane_sound_org, 64+(plane_real_distance*0.15), forward, plane_sound_org );
			}
			/*else if (plane_real_distance > 256)
			{// UQ1: Hack to make the sound travel further!
				VectorSubtract(cent->lerpOrigin, cg.refdef.vieworg, plane_dir);
				vectoangles(plane_dir, plane_angles);

				VectorCopy(cent->lerpOrigin, plane_sound_org);
				AngleVectors (plane_angles, forward, NULL, NULL);
				VectorMA( plane_sound_org, 64, forward, plane_sound_org );
			}*/
			else
			{
				//VectorCopy(cent->lerpOrigin, plane_sound_org);
			...
			//trap_S_AddLoopingSound( plane_sound_org, vec3_origin, cgs.gameSounds[ cent->currentState.loopSound ], 255, cent->soundTime );
			trap_S_AddRealLoopingSound( plane_sound_org, vec3_origin, cgs.gameSounds[ cent->currentState.loopSound ], 1250, 255, cent->soundTime );
#endif //__NPC__
		} else {
	...
	else if( cent->soundTime )
	{
		...
		if (next_aiming_sound > cg.time + 14000)
			next_aiming_sound = 0;

		if (!(cg.snap->ps.eFlags & EF_VEHICLE)
			&& (cg.snap->ps.eFlags & EF_AIMING) 
			&& next_aiming_sound <= cg.time
			&& cg.snap->ps.stats[STAT_HEALTH] > 0)
		{// Started Aiming, play the start aiming sound...
			trap_S_StartSoundVControl( cent->lerpOrigin, cg.snap->ps.clientNum, CHAN_LOCAL_SOUND, cgs.media.breathAim, 133 );
			next_aiming_sound = cg.time + 5000;
		}
		else if (!(cg.snap->ps.eFlags & EF_VEHICLE)
			&& ( cg.snap->ps.weapon == WP_FG42SCOPE || cg.snap->ps.weapon == WP_GARAND_SCOPE || cg.snap->ps.weapon == WP_K43_SCOPE)
			&& next_aiming_sound <= cg.time
			&& cg.snap->ps.stats[STAT_HEALTH] > 0)
		{// Started Sniping, play the start aiming sound...
			trap_S_StartSoundVControl( cent->lerpOrigin, cg.snap->ps.clientNum, CHAN_LOCAL_SOUND, cgs.media.breathSnipe, 133 );
			next_aiming_sound = cg.time + 14000;
		}
	}
	else
	{
...
//			CG_Printf( "Gamemodel: oldframe: %i frame: %i lerp: %f\n", ent.oldframe, ent.frame, ent.backlerp );
	...
	if (cent->currentState.onFireEnd == 1)
	{// UQ1: MG Nest - Under construction...
		if (cent->currentState.eType == ET_MG42_BARREL)
			ent.customShader = cgs.media.genericConstructionShader;
		if (cent->currentState.weapon == 1) // It's pole/tripod...
			ent.customShader = cgs.media.genericConstructionShader;
	}
...
void CG_Vehicle_PostProcess( centity_t* cent );
extern qboolean CG_SpawnVehicleSmokeSprite( centity_t *cent, float dist );
...
============================
*/
static void CG_Vehicle( centity_t *cent ) {
	refEntity_t			ent;
	entityState_t		*s1;
	vec3_t				angles, slope;
	...
	CG_RegisterTank( s1->effect1Time );
...
*/	
	...
	// player model
	if (s1->number == cg.snap->ps.clientNum) {
		ent.renderfx |= RF_THIRD_PERSON;	// only draw from mirrors, HUH???? 
	}

	// rotate it !
	VectorCopy( cent->lerpAngles, angles);

	CG_tank_slope ( cent, slope ); 
...
/*
	if (cent->currentState.dl_intensity <= 20)
	{// UQ1: Add smoke!
		//CG_SpawnVehicleSmokeSprite( cent, 128.0f );
		...
		CG_SetParticleSystemTag( cent->entityPS, ent, ent.hModel, "tag_turret" );
		...
		trap_S_AddLoopingSound( cent->lerpOrigin, vec3_origin, cgs.media.tankSmokeSound, 16 + Q_TrueRand(0, 64), 0 );
	...
	/*{
		...
		AnglesToAxis( angles, mg42base.axis );

		if (s1->number == cg.snap->ps.clientNum) {
			mg42gun.renderfx |= RF_THIRD_PERSON;	// only draw from mirrors, HUH???? 
			mg42base.renderfx |= RF_THIRD_PERSON;	// only draw from mirrors, HUH???? 
		}

		VectorCopy( cent->lerpOrigin, mg42gun.origin);
		VectorCopy( cent->lerpOrigin, mg42gun.oldorigin);
		mg42gun.origin[2]+=96;
		mg42gun.oldorigin[2]+=96;

		VectorCopy(mg42gun.origin, mg42base.origin);
		VectorCopy(mg42gun.oldorigin, mg42base.oldorigin);
		mg42base.origin[2]-=32;
		mg42base.oldorigin[2]-=32;

		trap_R_AddRefEntityToScene (&mg42base);
		trap_R_AddRefEntityToScene (&mg42gun);

		//VectorCopy(mg42gun.origin, end);
		//end[2]+=64;
		//CG_RailTrail2( NULL, mg42gun.origin, end );
	}*/

	//CG_Vehicle_PostProcess( cent );
}
...
static animation_t multi_tankAnims[] = {
	...
	{ 0,	"",	65,		10,		0,		1000/15,	1000/15	},	// shooting
};
...
//	CG_Printf("dot: %f\n", dot);
	...
	if(!behind && !toofar) {
...
#ifdef __ETE__
		if(tr.fraction == 1 || VectorDistance(tr.endpos, cent->lerpOrigin) <= 64)
			visible = qtrue;
#else //!__ETE__
		if(tr.fraction == 1)
			visible = qtrue;
		...
		trap_R_AddLightToScene2( cent->lerpOrigin, (float)cent->currentState.density * 4, (float)1.0f, (float)r/255.0f, (float)g/255.0f, (float)b/255.0f, 0, REF_JUNIOR_DLIGHT); //REF_DIRECTED_DLIGHT REF_JUNIOR_DLIGHT REF_FORCE_DLIGHT
		...
		if( s1->torsoAnim ) {
			if( cg.time >= cent->lerpFrame.frameTime ) {
				...
				while( cg.time >= cent->lerpFrame.frameTime ) {
					...
					if (s1->weapon <= 0)
						cent->lerpFrame.frameTime++; // UQ1: Fix for freeze!
...
#ifdef __ETE__
	if (cent->currentState.eType == ET_SECONDARY)
		return;
#endif //__ETE__
...
extern void CG_Ammo_Crate( centity_t *cent ); // UQ1: cg_unique_entities.c
extern void CG_Health_Crate( centity_t *cent ); // UQ1: cg_unique_entities.c
extern void CG_SandBag ( centity_t *cent ); // UQ1: cg_unique_entities.c
extern void CG_Constructible_Sandbags( centity_t *cent ); // UQ1: cg_unique_entities.c
extern void CG_MG_Nest ( centity_t *cent ); // UQ1: cg_unique_entities.c
extern void CG_Constructible_MG_Nest( centity_t *cent ); // UQ1: cg_unique_entities.c
#endif //__ETE__
...
/*	case ET_CONSTRUCTIBLE_MARKER:
	...
	case ET_CONSTRUCTIBLE_SANDBAGS:
		CG_Constructible_Sandbags( cent );
		break;
	case ET_MG_NEST:
		CG_MG_Nest( cent );
		break;
	case ET_CONSTRUCTIBLE_MG_NEST:
		CG_Constructible_MG_Nest( cent );
		break;
#endif //__ETE__
...
#ifdef __CUDA__
__device__ __host__ qboolean CG_AddCEntity_Filter( centity_t* cent ) {
	__shared__ int eflags = cent->currentState.eFlags;

	if(cent->processedFrame == cg.clientFrame && cg.mvTotalClients < 2) {
		return qtrue;
	}

	if( eflags & EF_PATH_LINK ) {
		return CG_AddLinkedEntity(cent, qfalse, cg.time);
	} 
	
	if( eflags & EF_TAGCONNECT ) {
...
#ifdef __CUDA__
__device__ __host__ void CG_AddPacketEntities( void ) {
	__shared__ int					num;
	__shared__ playerState_t		*ps;
	//int					clcount;
	...
	{
		__shared__ centity_t	*cent = &cg_entities[cg.snap->entities[num].number];
		cent->valid = qtrue;
	}

	for ( num = 0; num < MAX_GENTITIES; num++ )
	{
		__shared__ centity_t	*cent = &cg_entities[num];
