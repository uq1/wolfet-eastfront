...
//		}
	...
	else if (p->type == P_FLAT_SCALEUP)
	{
		...
		width2 = p->width + ( ratio * ( p->endwidth - p->width) );
		height2 = p->height + ( ratio * ( p->endheight - p->height) );

		if (width2 > p->endwidth)
			width2 = p->endwidth;

		if (height2 > p->endheight)
			height2 = p->endheight;

		sinR = height2 * sin(DEG2RAD(p->roll)) * ROOT_2;
		cosR = width2 * cos(DEG2RAD(p->roll)) * ROOT_2;
