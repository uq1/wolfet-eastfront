...
#define	BLOOD_SPURT_COUNT	4
	...
	if ( entityNum != cg.snap->ps.clientNum )
	{
...

		{
			...
			VectorSubtract( bOrigin, vhead, vec2 );
			len = VectorLength (vec2);
...
qboolean CG_SpawnVehicleSmokeSprite( centity_t *cent, float dist ) {
#ifdef __VEHICLES__
	smokesprite_t *smokesprite = AllocSmokeSprite();
//	int volume = 16 + ((cent->currentState.effect1Time/640.f)*(100-16));
	...
	if( smokesprite ) {
		...
		CG_Printf("Drawing tank smoke at %f %f %f!\n", cent->origin2[0], cent->origin2[1], cent->origin2[2]);
		CG_Printf("lerporigin at %f %f %f!\n", cent->lerpOrigin[0], cent->lerpOrigin[1], cent->lerpOrigin[2]);

		smokesprite->smokebomb = cent;
		VectorCopy( cent->lerpOrigin, smokesprite->pos );
		smokesprite->pos[2] += 32;
		//VectorCopy(CG_GetTagOrigin(cent->refEnt, "tag_cannon", &offset), smokesprite->pos);
		//VectorCopy( cent->origin2, smokesprite->pos );		
		VectorCopy( bytedirs[rand()%NUMVERTEXNORMALS], smokesprite->dir );
		smokesprite->dir[2] *= .5f;
		smokesprite->size = 16.f;
	...
	trap_S_AddLoopingSound( cent->lerpOrigin, vec3_origin, cgs.media.tankSmokeSound, 255, 0 );
...
/*
	...
	if( cent->currentState.effect1Time > 16 ) {
		...
		trap_S_AddLoopingSound( cent->lerpOrigin, vec3_origin, cgs.media.tankSmokeSound, volume, 0 );
