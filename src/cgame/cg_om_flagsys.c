...
static animation_t multi_flagpoleAnims2[] = {
	...
	{ 0,	"",	0,		15,		0,		5000/15,	5000/15	},	// (axis flag rising)
	{ 0,	"",	490,	15,		0,		5000/15,	5000/15	},	// (american flag rising)
	{ 0,	"",	20,		211,	211,	1000/15,	1000/15	},	// (axis flag raised)
	{ 0,	"",	255,	211,	211,	1000/15,	1000/15	},	// (american flag raised)
	{ 0,	"",	235,	15,		0,		5000/15,	5000/15	},	// (axis switching to american)
	{ 0,	"",	470,	15,		0,		5000/15,	5000/15	},	// (american switching to axis)
	{ 0,	"",	510,	15,		0,		5000/15,	5000/15	},	// (axis flag falling)
...
===============
*/
void Flag_RunLerpFrame( centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, float speedScale ) {
	int			f;
	animation_t	*anim;
...
==============
*/
static void CG_FlagSetAnim(centity_t *cent, lerpFrame_t *lf, int newAnim) 
...
void CG_Flag( centity_t *cent ) {
	...
	else if (ps->stats[STAT_FLAG_CAPTURE] > 0
	...
	else if (ps->stats[STAT_CAPTURE_ENTITYNUM] > 0
		&& VectorDistance(cg.refdef.vieworg, cg_entities[ps->stats[STAT_CAPTURE_ENTITYNUM]].lerpOrigin) > (float) cg_entities[captureEntityNum].currentState.effect1Time*0.5)
	{
		cg.captureFlagPercent = 0;
		cg.capturingFlag = qfalse;
	}
	...
	{// s.frame is the animation number
		if ( cent->om_flag_frame == WCP_ANIM_NOFLAG )
		{
			...
			trap_S_StartSound (model.origin, cent->currentState.number, CHAN_AUTO,	trap_S_RegisterSound("sound/movers/doors/door6_open.wav", qfalse));
		}
		else if ( cg.time < cent->lerpFrame.frameTime 
			&& cent->om_flag_frame == WCP_ANIM_RAISE_AXIS )
		{
			model.frame = WCP_ANIM_RAISE_AXIS;
		}
		else
	...
	{// s.frame is the animation number
		if ( cent->om_flag_frame == WCP_ANIM_NOFLAG )
		{
			...
			&& cent->om_flag_frame == WCP_ANIM_RAISE_AMERICAN )
		{
			model.frame = WCP_ANIM_RAISE_AMERICAN;
		}
		else
	...
	{// s.frame is the animation number
		...
		else if (cent->om_flag_frame == WCP_ANIM_AXIS_FALLING)
		{
			...
			&& cent->om_flag_frame == WCP_ANIM_AMERICAN_RAISED )
		{
			model.frame = WCP_ANIM_AMERICAN_FALLING;
			...
			&& cent->om_flag_frame == WCP_ANIM_AXIS_RAISED )
		{
			model.frame = WCP_ANIM_AXIS_FALLING;
