	...
	if( !editSpeakerActive ) {
		...
		CG_Text_Paint_Ext( 8, y, .2f, .2f, colour,
						va( "Current amount of speakers in map: %i (inpvs: %i max in map: %i)",
														...
														numSpeakersInPvs,
														256 ),
						0, 0, ITEM_TEXTSTYLE_SHADOWED, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
...
void trap_S_StartLocalSoundEx( sfxHandle_t sfx, int channelNum, int volume );
...
int CG_RealVolumeSound(int volume)
{
//#ifdef __SHELLSHOCK__
//#ifdef __SHELLSHOCK_SOUND__
//	if ((cg.snap->ps.extFlags & EX_SHELLSHOCK) && (cg_shellshock.integer & (SHELLSHOCK_ENABLED|SHELLSHOCK_SOUND)))
//	{
...
int CG_RealAttenuationSound(int volume)
{
#ifdef __CAMERA__
	if (cg.drawForCamera)
		return volume;
...
void CG_StartSound( vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfx )
{
	...
	trap_S_StartSoundVControl( origin, entityNum, entchannel, sfx, 127 );
}

void CG_StartSoundVControl( vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfx, int volume )
{
#ifdef __CAMERA__
	if (cg.drawForCamera)
		return;
	...
	trap_S_StartSoundVControl( origin, entityNum, entchannel, sfx, CG_RealVolumeSound(volume) );
}

void CG_StartSoundExVControl( vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfx, int flags, int volume )
{
#ifdef __CAMERA__
	if (cg.drawForCamera)
		return;
	...
	trap_S_StartSoundExVControl( origin, entityNum, entchannel, sfx, flags, CG_RealVolumeSound(volume) );
}

void CG_StartSoundEx( vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfx, int flags )
{
#ifdef __CAMERA__
	if (cg.drawForCamera)
		return;
	...
	CG_StartSoundExVControl( origin, entityNum, entchannel, sfx, flags, 127 );
}
...
void CG_StartLocalSound( sfxHandle_t sfx, int channelNum )
{
	...
	trap_S_StartLocalSoundEx( sfx, channelNum,  CG_RealVolumeSound(127));
}

void CG_ClearLoopingSounds( void )
{
#ifdef __CAMERA__
	if (cg.drawForCamera)
		return;
...
void CG_ClearSounds( qboolean killmusic )
{
#ifdef __CAMERA__
	if (cg.drawForCamera)
		return;
	...
	trap_S_ClearSounds( killmusic );
}

void CG_AddLoopingSound( const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx, int volume, int soundTime )
{
#ifdef __CAMERA__
	if (cg.drawForCamera)
		return;
	...
	trap_S_AddLoopingSound( origin, velocity, sfx, CG_RealVolumeSound(volume), soundTime );
}
...
void CG_AddRealLoopingSound( const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx, int range, int volume, int soundTime )
{
	...
	trap_S_AddRealLoopingSound( origin, velocity, sfx, range, CG_RealVolumeSound(volume), soundTime );
}

void CG_StopStreamingSound(int entityNum)
{
#ifdef __CAMERA__
	if (cg.drawForCamera)
		return;
	...
	trap_S_StopStreamingSound(entityNum);
}

int	CG_StartStreamingSound( const char *intro, const char *loop, int entnum, int channel, int attenuation )
{
#ifdef __CAMERA__
	if (cg.drawForCamera)
		return 0;
	...
	return trap_S_StartStreamingSound( intro, loop, entnum, channel, CG_RealAttenuationSound(attenuation) );
}
