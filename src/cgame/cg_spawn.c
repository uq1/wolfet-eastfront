...
====================
*/
qboolean CG_ParseSpawnVarsEx( int handle ) {
	pc_token_t	token;
	char		keyname[MAX_TOKEN_CHARS];
...
/*void
AIMOD_SaveMapCoordFile ( void )
{
	fileHandle_t	f;
	vmCvar_t		mapname;

	CG_Printf( "^3*** ^3%s: ^7Saving map coord table.\n", GAME_VERSION );
	
	trap_Cvar_Register( &mapname, "mapname", "", CVAR_SERVERINFO | CVAR_ROM );

	trap_FS_FOpenFile( va( "maps/%s.mcf", mapname.string), &f, FS_WRITE );
	if ( !f )
	{
		CG_Printf( "^1*** ^3CLIENT^5: Error opening map coord file ^7maps/%s.mcf^5 for save!!!\n", mapname.string );
		return;
	}

	trap_FS_Write( &cg.mapcoordsMins, sizeof(vec3_t), f );
	trap_FS_Write( &cg.mapcoordsMaxs, sizeof(vec3_t), f );
...
AIMOD_LoadMapCoordFile ( void )
{
	...
	trap_FS_FOpenFile( va( "/%s/maps/%s.mcf", fs_game.string, mapname.string), &f, FS_READ );
	if ( !f )
	{
		CG_Printf( "^1*** ^3CLIENT^5: Reading from ^7maps/%s.mcf^3 failed^5!!!\n", mapname.string );
		return qfalse;
	}

	trap_FS_Read( &cg.mapcoordsMins, sizeof(vec3_t), f );
	trap_FS_Read( &cg.mapcoordsMaxs, sizeof(vec3_t), f );

	trap_FS_FCloseFile( f );							//close the file

	cg.mapcoordsValid = qtrue;

	CG_Printf( "^1*** ^3CLIENT^5: Map coordinates loaded from ^7maps/%s.mcf^5!!!\n", mapname.string );
...
AIMOD_SaveMapCoordFile ( void )
{
	...
	//CG_Printf( "Map mins %f %f %f.\n", cg.mapcoordsMins[0], cg.mapcoordsMins[1], cg.mapcoordsMins[2]);
	//CG_Printf( "Map maxs %f %f %f.\n", cg.mapcoordsMaxs[0], cg.mapcoordsMaxs[1], cg.mapcoordsMaxs[2]);

	fwrite( &cg.mapcoordsMins[0], sizeof(float), 1, pOut );
	fwrite( &cg.mapcoordsMins[1], sizeof(float), 1, pOut );
	fwrite( &cg.mapcoordsMins[2], sizeof(float), 1, pOut );
	fwrite( &cg.mapcoordsMaxs[0], sizeof(float), 1, pOut );
	fwrite( &cg.mapcoordsMaxs[1], sizeof(float), 1, pOut );
	fwrite( &cg.mapcoordsMaxs[2], sizeof(float), 1, pOut );
...
AIMOD_LoadMapCoordFile ( void )
{
	...
	if ( (pIn = fopen( va("%s/maps/%s.mcf", fs_game.string, mapname.string), "rb")) == NULL )
	{
		...
		{
			CG_Printf( "^1*** ^3CLIENT^5: Reading from ^7%s/maps/%s.mcf^3 failed^5!!!\n", fs_game.string, mapname.string );
			return qfalse;
		}
	}

	fread( &cg.mapcoordsMins[0], sizeof(float), 1, pIn );
	fread( &cg.mapcoordsMins[1], sizeof(float), 1, pIn );
	fread( &cg.mapcoordsMins[2], sizeof(float), 1, pIn );
	fread( &cg.mapcoordsMaxs[0], sizeof(float), 1, pIn );
	fread( &cg.mapcoordsMaxs[1], sizeof(float), 1, pIn );
	fread( &cg.mapcoordsMaxs[2], sizeof(float), 1, pIn );

	fclose( pIn );							//close the file

	cg.mapcoordsValid = qtrue;

	CG_Printf( "^1*** ^3CLIENT^5: Map coordinates loaded from ^7%s/maps/%s.mcf^5.\n", fs_game.string, mapname.string );
...
void SP_worldspawn( void ) {
	...
	BG_InitLocationNames( cgs.rawmapname );
...
void CG_ParseEntitiesFromString( void ) {
	...
	else if (cgs.gametype == GT_SINGLE_PLAYER || cgs.gametype == GT_COOP || cgs.gametype == GT_NEWCOOP)
