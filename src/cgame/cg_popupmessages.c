...
void CG_DrawPMItemsBig( void ) {
	...
	w = CG_Text_Width_Ext( cg_pmWaitingListBig->message, 0.22f, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
	CG_Text_Paint_Ext( 640 - 4 - w, y + 56, 0.22f, 0.24f, colourText, cg_pmWaitingListBig->message, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
}
	...
	switch( cent->currentState.effect1Time ) {
		case PM_DYNAMITE:
			switch( cent->currentState.effect2Time ) {
				case 0:
					return va( "^5Planted at ^5%s.", CG_ConfigString( CS_OID_TRIGGERS + cent->currentState.effect3Time ) );
				case 1:
					return va( "^5Defused at ^5%s.", CG_ConfigString( CS_OID_TRIGGERS + cent->currentState.effect3Time ) );
			}
			break;
		case PM_CONSTRUCTION:
			switch( cent->currentState.effect2Time ) {
				...
				case 0:
					return va( "^5%s ^5has been constructed.", CG_ConfigString( CS_OID_TRIGGERS + cent->currentState.effect3Time ) );
			}
			break;
		case PM_DESTRUCTION:
			switch( cent->currentState.effect2Time ) {
				case 0:
					return va( "^5%s ^5has been damaged.", CG_ConfigString( CS_OID_TRIGGERS + cent->currentState.effect3Time ) );
				case 1:
					return va( "^5%s ^5has been destroyed.", CG_ConfigString( CS_OID_TRIGGERS + cent->currentState.effect3Time ) );
			}
			break;
		case PM_MINES:
			...
			return va( "^5Spotted by %s^5 at %s", cgs.clientinfo[cent->currentState.effect3Time].name, BG_GetLocationString( cent->currentState.origin ) );
		case PM_OBJECTIVE:
			switch( cent->currentState.density ) {
				case 0:
					return va( "^5%s ^5have stolen %s^5!", cent->currentState.effect2Time == TEAM_ALLIES ? "Allies" : "Axis", CG_ConfigString( CS_STRINGS + cent->currentState.effect3Time ));
				case 1:
					return va( "^5%s ^5have returned %s^5!", cent->currentState.effect2Time == TEAM_ALLIES ? "Allies" : "Axis", CG_ConfigString( CS_STRINGS + cent->currentState.effect3Time ));
			}
			break;
		case PM_TEAM:
			switch( cent->currentState.density ) {
				case 0: // joined
					{
						...
						return va( "^5%s^5 has joined the %s^5!", cgs.clientinfo[cent->currentState.effect3Time].name, teamstr );
					}
				case 1:
					return va( "^5%s^5 disconnected", cgs.clientinfo[cent->currentState.effect3Time].name );
			}
	}
