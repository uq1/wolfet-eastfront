...
static void CG_Obituary( entityState_t *ent ) {
	...
	qboolean	normalShader = qfalse; // UQ1: Weapon icon death pics..
	...
	if (ent->density == ET_NPC)
	{
		if (ent->teamNum == TEAM_AXIS)
			strcpy(targetName, va("^8A german NPC"));
		else if (ent->teamNum == TEAM_ALLIES)
			strcpy(targetName, va("^1A russian NPC"));
		else
			strcpy(targetName, va("^5An NPC"));
	}
	else
#endif //__NPC_NAMES__
#endif //__NPC__
	...
	strcat( targetName, S_COLOR_CYAN );
	...
	if ( !ca ) {
#ifdef __NPC__
		if (ent->effect1Time == ET_NPC)
		{
#ifndef __NPC_NAMES__
			if (ent->effect2Time == TEAM_AXIS)
				strcpy(attackerName, va("^8a german NPC"));
			else if (ent->effect2Time == TEAM_ALLIES)
				strcpy(attackerName, va("^1a russian NPC"));
			else
				strcpy(attackerName, va("^5an NPC"));
#endif //__NPC_NAMES__
		}
		else
		{
		...
		strcat( attackerName, S_COLOR_CYAN );
...
//			}
        ...
        case MOD_PTRSSHELL://Masteries, if you die by an antitank rifle...
			message = "was perforated by";
			message2 = "'s antitank rifle shell";
			break;
		...
		if( (ca && ci && ci->team == ca->team) && mod != MOD_SWAP_PLACES ) { // jaquboss tell thruth about place swaps
		...
		if (message) {
			message = CG_TranslateString( message );
			if ( message2 ) {
				int weaponNum = cg_entities[attacker].currentState.weapon;
				...
				if (!normalShader)
				{// UQ1: Get weapon's icon for death picture...
					if (cgs.media.weaponIcons[weaponNum][0])
					{
						CG_AddPMItem( PM_DEATH, va( "%s %s %s%s", targetName, message, attackerName, message2 ), cgs.media.weaponIcons[weaponNum][0] );
						//CG_Printf("%s - should have used weapon icon (%i[0])! - WeaponNum %i. Attacker %i.\n", va( "%s %s %s%s", targetName, message, attackerName, message2 ), cgs.media.weaponIcons[weaponNum][0], weaponNum, attacker);
					}
					else
					{
						CG_AddPMItem( PM_DEATH, va( "%s %s %s%s", targetName, message, attackerName, message2 ), cgs.media.weaponIcons[weaponNum][1] );
						//CG_Printf("%s - should have used weapon icon (%i[1])! - WeaponNum %i. Attacker %i.\n", va( "%s %s %s%s", targetName, message, attackerName, message2 ), cgs.media.weaponIcons[weaponNum][1], weaponNum, attacker);
					}
				}
				else
				{
...
#ifdef __VEHICLES__
extern void CG_FlamethrowerTankFlame( centity_t *cent, vec3_t origin );
#endif //__VEHICLES__

#ifdef __ETE__
extern void CG_AddSnowFootStepParticles ( centity_t *cent, refEntity_t body );
#endif //__ETE__
...
#ifdef __ETE__
			{
				centity_t *cent2 = &cg_entities[es->number];
				CG_AddSnowFootStepParticles( cent2, cent2->refEnt );
			}
#endif //__ETE__
		}
		break;
...
// jpw
	...
	case EV_FLAME_TANK_FIRE:
		CG_FlamethrowerTankFlame( cent, cent->currentState.origin );
		break;
	case EV_TANK_FIRE:
		CG_FireWeapon( cent );
		break;
	case EV_TANK_PTRS_FIRE://Masteries, for panzer2
		CG_FirePtrsTank( cent );
		break;
#endif //__VEHICLES__
...
/*	case EV_FLAMEBARREL_BOUNCE:
...
#ifdef __PARTICLE_SYSTEM__
			if (es->weapon != WP_KNIFE && es->weapon != WP_PTRS && es->weapon != WP_PTRS_SET)
				CG_DrawGrenadeParticles( cent, position );
#endif //__PARTICLE_SYSTEM__
		}
		else
		{
...
#ifdef __ETE__
	case EV_BULLET_WIZZ:
		CG_WizzSound(es->number, es->eventParm);
		DEBUGNAME("EV_BULLET_WIZZ");
		break;
#endif //__ETE__
	...
	case EV_PAIN:
		...
		if ( cent->currentState.number != cg.snap->ps.clientNum ) {
			...
			trap_S_StartSound( NULL, es->number, CHAN_VOICE, 
				CG_CustomSound( es->number, va("sound/player/pain/pain%i.wav", Q_TrueRand(1,4)) ) );
#endif //__ETE__
		}
		break;
	...
	case EV_DEATH3:
		...
		// UQ1: I want sounds!
		trap_S_StartSound( NULL, es->number, CHAN_VOICE, 
				CG_CustomSound( es->number, va("sound/player/death/death%i.wav", Q_TrueRand(1,2)) ) );
#else //!__ETE__
		trap_S_StartSound( NULL, es->number, CHAN_VOICE, 
...
//		trap_S_StopStreamingSound( es->number );
	...
	case EV_SCOREPLUM:
		...
		{// A Land Vehicle Waypoint.. Show a tank icon above it!
			if (cent->currentState.density == 1) // Big!
				CG_ScorePlum( cent->currentState.otherEntityNum, cent->lerpOrigin, cent->currentState.time, es->weapon, qtrue, cgs.media.ccTankIcon );
			else
				CG_ScorePlum( cent->currentState.otherEntityNum, cent->lerpOrigin, cent->currentState.time, es->weapon, qfalse, cgs.media.ccTankIcon );
		}
		else if (cent->currentState.density == 1) // Big!
			CG_ScorePlum( cent->currentState.otherEntityNum, cent->lerpOrigin, cent->currentState.time, es->weapon, qtrue, 0 );
		else
			CG_ScorePlum( cent->currentState.otherEntityNum, cent->lerpOrigin, cent->currentState.time, es->weapon, qfalse, 0 );
		break;
