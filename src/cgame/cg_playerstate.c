...
extern qboolean CG_CanSetupMobileMG42Here ( centity_t *cent );
...
#ifdef __ADVANCED_VEHICLES__
	Init_Vehicles();
	cg.car = vehicles[cg.predictedPlayerState.clientNum];
...
void CG_TransitionPlayerState( playerState_t *ps, playerState_t *ops )
{
	...
	if( !(ps->eFlags & EF_PRONE) 
		&& !(cg.weaponSelect == WP_MOBILE_MG42_SET && CG_CanSetupMobileMG42Here(NULL)) 
		&& ops->eFlags & EF_PRONE ) {
...
#ifdef __EF__
	if( !(ps->eFlags & EF_PRONE) 
		&& !(cg.weaponSelect == WP_30CAL_SET && CG_CanSetupMobileMG42Here(NULL)) 
		&& ops->eFlags & EF_PRONE ) {
    ...
    if( !(ps->eFlags & EF_PRONE) 
		&& !(cg.weaponSelect == WP_PTRS_SET && CG_CanSetupMobileMG42Here(NULL)) 
		&& ops->eFlags & EF_PRONE ) {
		if( cg.weaponSelect == WP_PTRS_SET )
			CG_FinishWeaponChange( cg.weaponSelect, ps->nextWeapon );
	}
#endif //__EF__
