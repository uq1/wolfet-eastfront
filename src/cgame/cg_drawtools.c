...
// UQ1: Added new health-specific bar... Flash on low health!
#define HEALTH_BAR_BORDERSIZE 1

int crosshair_health_bar_flashtimer = 0;

void CG_Crosshair_HealthBar(float x, float y, float w, float h, float *startColor, float *endColor, const float *bgColor, float frac, int flags) {
	vec4_t	backgroundcolor = {1, 1, 1, 0.25f}, colorAtPos;	// colorAtPos is the lerped color if necessary
	int indent = HEALTH_BAR_BORDERSIZE;
...
//			CG_FillRectGradient ( x, y, w, h * frac, startColor, endColor, 0 );
		...
		crosshair_health_bar_flashtimer++;

		if(flags&BAR_LERP_COLOR) 
		{
			if (frac < 0.25)
			{
				if (crosshair_health_bar_flashtimer > 10)
				{
					CG_FillRect ( x, y, w*frac, h, colorAtPos );

					if (crosshair_health_bar_flashtimer > 20)
						crosshair_health_bar_flashtimer = 0;
				}
			}
			else
			{
...
int health_bar_flashtimer = 0;

void CG_HealthBar(float x, float y, float w, float h, float *startColor, float *endColor, const float *bgColor, float frac, int flags) {
	vec4_t	backgroundcolor = {1, 1, 1, 0.25f}, colorAtPos;	// colorAtPos is the lerped color if necessary
	int indent = HEALTH_BAR_BORDERSIZE;
...
//			CG_FillRectGradient ( x, y, w, h * frac, startColor, endColor, 0 );
		...
		health_bar_flashtimer++;

		if(flags&BAR_LERP_COLOR) 
		{
			if (frac < 0.25)
			{
				if (health_bar_flashtimer > 10)
				{
					CG_FillRect ( x, y, w*frac, h, colorAtPos );

					if (health_bar_flashtimer > 20)
						health_bar_flashtimer = 0;
				}
			}
			else
			{
...
void CG_VerticalPercentBar( float x, float y, float width, float height, float percent ) {
	vec4_t	bgcolor = {0.5f, 0.5f, 0.5f, 0.3f},
			color = {1.0f, 1.0f, 1.0f, 0.3f};
	CG_FilledBar(x, y, width, height, color, NULL, bgcolor, percent, BAR_BG|BAR_NOHUDALPHA|BAR_VERT);
}
...
==============
*/
void CG_FillRectGradientUQ( float x, float y, float width, float height, const float *color, const float *gradcolor, int gradientType ) {
	trap_R_SetColor( color );
...
void CG_VerticalPercentBarNoBorder( float x, float y, float width, float height, float percent ) {
	...
	CG_FilledBar(x, (y + height) - (height*percent), width, height, color, /*bgcolor*/NULL, NULL, percent, BAR_NOHUDALPHA|BAR_VERT);
}
		...
		qboolean forceColor, qboolean shadow, int charWidth, int charHeight, int maxChars ) 
{
	...
	if (shadow)
		CG_Text_Paint_Ext( x, y, 0.25f, 0.25f, color, string, 0, maxChars, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.tahoma16 );
	else
		CG_Text_Paint_Ext( x, y, 0.25f, 0.25f, color, string, 0, maxChars, 0, &cgs.media.tahoma16 );
}
...
/*	vec4_t		color;
	...
	if (shadow)
		CG_Text_Paint_Ext( x, y, 0.25f, 0.25f, color, string, 0, maxChars, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.tahomabd24 );
	else
		CG_Text_Paint_Ext( x, y, 0.25f, 0.25f, color, string, 0, maxChars, 0, &cgs.media.tahomabd24 );
}
...
void CG_DrawSmallString( int x, int y, const char *s, float alpha ) {
	...
	CG_DrawStringExtSmall( x, y, s, color, qfalse, qfalse, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 0 );
}

void CG_DrawSmallStringColor( int x, int y, const char *s, vec4_t color ) {
	//CG_DrawStringExt( x, y, s, color, qtrue, qfalse, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 0 );
	CG_DrawStringExtSmall( x, y, s, color, qtrue, qfalse, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 0 );
}
