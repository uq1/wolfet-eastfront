...
void CG_ParseETEnhancedinfo( void ) {
	...
	info = CG_ConfigString( CS_ETE_INFO );

	cgs.misc = atoi(Info_ValueForKey(info, "g_misc"));
	cgs.doubleJumpHeight = atof(Info_ValueForKey(info, "g_doubleJumpHeight"));
	cgs.staminaRecharge = atof(Info_ValueForKey(info, "g_staminaRecharge"));
	...
	cgs.throwableKnives = atoi(Info_ValueForKey(info, "g_throwableKnives"));
}

void CG_ParseETESkillInfo() {
	const char* info;
	info = CG_ConfigString( CS_ETE_SKILLLEVELS );
	BG_InitSkillLevelStructure(SK_BATTLE_SENSE, Info_ValueForKey(info, "skill_battlesense"));
	BG_InitSkillLevelStructure(SK_EXPLOSIVES_AND_CONSTRUCTION, Info_ValueForKey(info, "skill_engineer"));
	BG_InitSkillLevelStructure(SK_FIRST_AID, Info_ValueForKey(info, "skill_medic"));
	BG_InitSkillLevelStructure(SK_SIGNALS, Info_ValueForKey(info, "skill_fieldops"));
	BG_InitSkillLevelStructure(SK_LIGHT_WEAPONS, Info_ValueForKey(info, "skill_lightweapons"));
	BG_InitSkillLevelStructure(SK_HEAVY_WEAPONS, Info_ValueForKey(info, "skill_soldier"));
	BG_InitSkillLevelStructure(SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, Info_ValueForKey(info, "skill_covertops"));
}
...
//		if(cg_announcer.integer > 0) trap_S_StartLocalSound( cgs.media.countPrepare, CHAN_ANNOUNCER );
		...
		if(cg.warmupCount >= 0) {
			Pri("^3All players ready!^5\nMatch starting...\n");
			CPri("^3All players ready!^5\nMatch starting...");
		}
	}
...
qboolean using_uq_fog = qfalse;

#ifdef __SHELLSHOCK__
#ifdef __SHELLSHOCK_FOG__
extern void CG_SetFogShellShock();

float	map_fog_ne = 0, map_fog_fa = 0, map_fog_r = 0, map_fog_g = 0, map_fog_b = 0, map_fog_density = 0;
int		map_fog_time = 0;
...
static void CG_ConfigStringModified( void ) {
	...
	if ( num == CS_MUSIC ) {
		...
		CG_ParseOIDInfo( num );
	} else if(num == CS_ETE_INFO) {
		CG_ParseETEnhancedinfo();
	} else if(num == CS_ETE_SKILLLEVELS) {
...
void CG_topshotsParse_cmd(qboolean doBest)
{
	...
	while(iWeap) {
		if(ts->cWeapons < WS_MAX * 2) {
			ts->weap[ts->cWeapons] = iWeap - 1;
			ts->clientID[ts->cWeapons] = atoi(CG_Argv(iArg++));
			ts->weapHits[ts->cWeapons] = atoi(CG_Argv(iArg++));
			ts->weapShots[ts->cWeapons] = atoi(CG_Argv(iArg++));
			ts->weapKills[ts->cWeapons] = atoi(CG_Argv(iArg++));
			iArg++;
			ts->cWeapons++;
		}
...
void CG_parseWeaponStatsGS_cmd(void)
{
	...
	if(weaponMask != 0) {
		for(i=WS_KNIFE; i<WS_MAX; i++) {
			if(weaponMask & (1 << i)) {
				gs->cWeapons++;
				gs->weapHits[i] = atoi(CG_Argv(iArg++));
				gs->weapShots[i] = atoi(CG_Argv(iArg++));
				gs->weapKills[i] = atoi(CG_Argv(iArg++));
				gs->weapDeaths[i] = atoi(CG_Argv(iArg++));
				gs->weapHS[i] = atoi(CG_Argv(iArg++));

				if( gs->weapShots[i] > 0 || gs->weapHits[i] > 0 || gs->weapKills[i] > 0 || gs->weapDeaths[i] > 0 ) {
					gs->fHasStats = qtrue;
				}
			}
		}
		...
		if(gs->fHasStats) {
			gs->dmgGiven = atoi(CG_Argv(iArg++));
			gs->dmgReceived = atoi(CG_Argv(iArg++));
			gs->teamDamage = atoi(CG_Argv(iArg++));
		}
	}
...
#ifdef __ETE__
	txt_dump(    va("Skills         Level/Points%s\n", ((cgs.gametype == GT_SUPREMACY_CAMPAIGN) ? "  Medals" : "")));
#endif //__ETE__

	if(fFull) {
		txt_dump(va("---------------------------%s\n", ((cgs.gametype == GT_WOLF_CAMPAIGN) ? "--------" : "")));
#ifdef __ETE__
		txt_dump(va("---------------------------%s\n", ((cgs.gametype == GT_SUPREMACY_CAMPAIGN) ? "--------" : "")));
#endif //__ETE__
	} else {
		txt_dump("\n");
	}
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp( cmd, "skpt" ) ) {
		...
		ci->skillpoints[atoi(CG_Argv(1))] = atoi(CG_Argv(2));
		return;
	}
...
#ifdef __NPC__
#ifdef __NPC_NAMES__
	if ( !strcmp( cmd, "npc_name" ) ) {
		int entNum;
		clientInfo_t	*ci;
		char arg1[64], arg2[64], arg3[64], arg4[64];

		strcpy(arg1, va("%s", CG_Argv(0)));
		strcpy(arg2, va("%s", CG_Argv(1)));
		strcpy(arg3, va("%s", CG_Argv(2)));
		strcpy(arg4, va("%s", CG_Argv(3)));

		//CG_Printf("CG_Argv(0) = %s. CG_Argv(1) = %s. CG_Argv(2) = %s. CG_Argv(3) = %s.\n", arg1, arg2, arg3, arg4);

		entNum = atoi(arg2);
		ci = &cgs.clientinfo[entNum];
		cg_entities[entNum].NPC_client = *ci;
		strcpy(ci->name, va("%s %s", arg3, arg4));
		ci->team = cg_entities[entNum].currentState.teamNum;
		return;
	}
#endif //__NPC_NAMES__
#endif //__NPC__
