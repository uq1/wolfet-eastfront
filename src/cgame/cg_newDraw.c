...
void CG_FitTextToWidth_SingleLine_Ext( char* instr, float scale, float w, int size, fontInfo_t *font) {
	...
	for( s = buffer; *s; s++, p++ ) {
		*p = *s;
		if(CG_Text_Width_Ext( instr, scale, 0, font ) > w) {
			*p = '\0';
			return;
		}
	}
...
void CG_DrawPlayerWeaponIcon( rectDef_t *rect, qboolean drawHighlighted, int align, vec4_t *refcolor ) {
	...
	qhandle_t	icon;
	float		scale = 0.0f, halfScale = 0.0f;
	vec4_t		hcolor;
...
#ifdef __VEHICLES__
	if (cg.predictedPlayerEntity.currentState.eFlags & EF_VEHICLE)
	{
		icon = cgs.media.ccTankIcon;
		realweap = VERYBIGEXPLOSION;
	}
	else
#endif //__VEHICLES__
...
#ifdef __VEHICLES__
void CG_DrawTankMGHeat(rectDef_t *rect, int align) {
	vec4_t	color = {1, 0, 0, 0.2f}, color2 = {1, 0, 0, 0.5f};
	int flags = 0;
	float heat = ((float)cg.snap->ps.curWeapHeat / 255.0f);
...
//	flags|=32;		// BAR_BGSPACING_X0Y5	- different style
	...
	//CG_Printf("Heat is %f (%i).\n", heat, cg.snap->ps.curWeapHeat);

	CG_FilledBar(rect->x, rect->y, rect->w, rect->h, color, color2, NULL, heat, flags);
	//CG_FillRect( rect->x, rect->y, rect->w, rect->h, popBG );
	//CG_FilledBar( rect->x, rect->y, rect->w, rect->h, popRed, popBlue, NULL, heat, flags );
