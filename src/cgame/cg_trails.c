...
*/
#ifdef __CUDA__
__shared__ static vec3_t vforward, vright, vup;
#define	MAX_TRAIL_VERTS		2048
__shared__ static	polyVert_t	verts[MAX_TRAIL_VERTS];
__shared__ static	polyVert_t	outVerts[MAX_TRAIL_VERTS*3];

__device__ __host__ void CG_AddTrailToScene( trailJunc_t *trail, int iteration, int numJuncs )
{
	__shared__ int		k, i, n, l, numOutVerts;
	__shared__ polyVert_t	mid;
	__shared__ float	mod[4];
	__shared__ float	sInc, s;
	__shared__ trailJunc_t	*j, *jNext;
	__shared__ vec3_t	fwd, up, p, v;
	// clipping vars
#define	TRAIL_FADE_CLOSE_DIST	64.0
#define	TRAIL_FADE_FAR_SCALE	4.0
	__shared__ vec3_t	viewProj;
	__shared__ float	viewDist, fadeAlpha;

	// add spark shader at head position
	if (trail->flags & TJFL_SPARKHEADFLARE) {
		__shared__ polyBuffer_t* pPolyBuffer = CG_PB_FindFreePolyBuffer( cgs.media.sparkFlareShader, 4, 6 );

		if(pPolyBuffer) {
			__shared__ int pos = pPolyBuffer->numVerts;
...
//	}
	...
	if (trail->flags & TJFL_FIXDISTORT) {
		...
		if (!(trail->flags & TJFL_NOPOLYMERGE)) {
			__host__ trap_R_AddPolysToScene( trail->shader, 3, &outVerts[0], numOutVerts/3 );
		} else {
			__shared__ int k2;
			for (k2=0; k<numOutVerts/3; k2++) {
				__host__ trap_R_AddPolyToScene( trail->shader, 3, &outVerts[k2*3] );
			}
		}
	}
	else
	{
		...
		if (!(trail->flags & TJFL_NOPOLYMERGE)) {
			__host__ trap_R_AddPolysToScene( trail->shader, 4, &verts[0], i/4 );
		} else {
			__shared__ int k2;
			for (k2=0; k2<i/4; k2++) {
				__host__ trap_R_AddPolyToScene( trail->shader, 4, &verts[k2*4] );
			}
		}
	}
	...
	if (trail->flags & TJFL_CROSSOVER) {
		if (iteration < 2) {
			__host__ CG_AddTrailToScene( trail, iteration + 1, numJuncs );
		}
	}
}
#else //!__CUDA__
...
//	}
	...
	if (trail->flags & TJFL_FIXDISTORT) {
		...
		if (!(trail->flags & TJFL_NOPOLYMERGE)) {
			...
			int k2;
			for (k2=0; k<numOutVerts/3; k2++) {
				trap_R_AddPolyToScene( trail->shader, 3, &outVerts[k2*3] );
			}
		}
	}
	else
	{
		...
		if (!(trail->flags & TJFL_NOPOLYMERGE)) {
			...
			int k2;
			for (k2=0; k2<i/4; k2++) {
				trap_R_AddPolyToScene( trail->shader, 4, &verts[k2*4] );
			}
		}
	}
