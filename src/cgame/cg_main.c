...
*/             //added the models for new tanks, line 1881
               //and for Artillery
...
#pragma export off
	...
	/*{
		...
		trap_Cvar_Register( &com_hunkmegs, "com_hunkmegs", "256", CVAR_ARCHIVE | CVAR_LATCH );
		if (com_hunkmegs.integer < 256)
		{
			Com_Printf( "Restarting to set com_hunkmegs higher...\n" );
			trap_Cvar_Set( "com_hunkmegs", "256" );
			//trap_SendConsoleCommand( EXEC_NOW, "vid_restart\n" );
	...
	case CG_DRAW_ACTIVE_FRAME:
		CG_DrawActiveFrame( arg0, arg1, arg2, qfalse );
		return 0;
...
#ifdef __BOT__
vmCvar_t		cg_autoWaypoint;
vmCvar_t		cg_waypoint_render;
#endif //__BOT__

vmCvar_t cg_realview;
vmCvar_t cg_realview_dist;

qhandle_t			stlth_skin;
qhandle_t			stlth_shader;
qhandle_t			shadow_skin;
qhandle_t			shadow_shader;

// UQ1: memory manager...
vmCvar_t			cg_hunkMegs;
...
//	{ &cg_draw2D, "cg_draw2D", "1", CVAR_CHEAT }, // JPW NERVE changed per atvi req to prevent sniper rifle zoom cheats
...
#ifdef __ETE__
	{ &cg_coronafardist, "cg_coronafardist", "4096", CVAR_ARCHIVE },
#else //!__ETE__
	...
	{ &cg_blinktime, "cg_blinktime", "0", CVAR_ARCHIVE},		 //----(SA)	added
...
//	{ &cg_smoothClients, "cg_smoothClients", "0", CVAR_USERINFO | CVAR_ARCHIVE},
	...
	// ETE
	{ &cg_locationMode, "cg_locationMode", "1", CVAR_ARCHIVE },
	{ &cg_mapzoom, "cg_mapzoom", "5.159", CVAR_ARCHIVE },

#ifdef __MUSIC_ENGINE__
	{ &s_radioStation, "s_radioStation", "http://scfire-ntc-aa08.stream.aol.com:80/stream/1050", CVAR_SERVERINFO | CVAR_ARCHIVE },
	...
	{ &cg_realview_dist, "cg_realview_dist", "32", CVAR_ARCHIVE /*| CVAR_CHEAT*/ },
	{ &cg_realview, "cg_realview", "1", CVAR_ARCHIVE /*| CVAR_CHEAT*/ },

#ifdef __BOT__
	{ &cg_autoWaypoint, "cg_autoWaypoint", "1", CVAR_ARCHIVE },
	{ &cg_waypoint_render, "cg_waypoint_render", "0", CVAR_ARCHIVE },
#endif //__BOT__

	//{ &cg_hunkMegs, "cg_hunkMegs", "80", CVAR_ARCHIVE },
	{ &cg_hunkMegs, "cg_hunkMegs", "128", CVAR_ARCHIVE },

	{ &cg_sniperx, "cg_sniperx", "0", CVAR_ARCHIVE },
	{ &cg_snipery, "cg_snipery", "0", CVAR_ARCHIVE },
	{ &cg_sniperw, "cg_sniperw", "0", CVAR_ARCHIVE },
	{ &cg_sniperh, "cg_sniperh", "0", CVAR_ARCHIVE },

	{ &g_realism, "g_realism", "0", CVAR_SERVERINFO | CVAR_ARCHIVE },
};
...
void CG_RestoreProfile(void) 
{
	...
	Com_sprintf(bakPath, sizeof(bakPath),
		"profiles/%s/%s.eteprofile", profile, CONFIG_NAME);
...
*/
	...
	cgs.media.tankSmokeSound =				trap_S_RegisterSound( "sound/weapons/grenade/gren_smoke.wav", qfalse );
	cgs.media.tankPtrsSound =               trap_S_RegisterSound( "sound/weapons/ptrs/ptrs.wav", qfalse );//Masteries, ptrs fire sound.
#endif
...
#ifdef __ETE__
	cgs.media.sfx_grenexp2 =			trap_S_RegisterSound( "sound/weapons/grenade/gren_expl_2.wav", qfalse );
	cgs.media.sfx_grenexpDist2 =		trap_S_RegisterSound( "sound/weapons/grenade/gren_expl_far2.wav", qfalse );
	cgs.media.sfx_grenexp3 =			trap_S_RegisterSound( "sound/weapons/grenade/gren_expl_3.wav", qfalse );
	cgs.media.sfx_grenexpDist3 =		trap_S_RegisterSound( "sound/weapons/grenade/gren_expl_far3.wav", qfalse );
	cgs.media.sfx_grenexp4 =			trap_S_RegisterSound( "sound/weapons/grenade/gren_expl_4.wav", qfalse );
	cgs.media.sfx_grenexpDist4 =		trap_S_RegisterSound( "sound/weapons/grenade/gren_expl_far4.wav", qfalse );
#endif //__ETE__
	...
	trap_S_RegisterSound( "sound/weapons/airstrike/airstrike_plane_axis.wav", qfalse );
...
#ifndef __ORIGINAL_PLAYER_SKINS__
extern void CG_LoadAllCharacters ( void );
#endif //__ORIGINAL_PLAYER_SKINS__

// UQ1: Set the default minimum hunkmegs value here...
#define DEFAULT_HUNKMEGS 128

void CG_SetHunkMegs (void) 
{
	char	com_hunkMegs[MAX_CVAR_VALUE_STRING];

	trap_Cvar_VariableStringBuffer("com_hunkMegs", com_hunkMegs, sizeof(com_hunkMegs));
	
	if ( atoi(com_hunkMegs) < DEFAULT_HUNKMEGS )
	{// com_hunkMegs is too low. Update and restart the client...
		// UQ1: Game values...
		char	cl_currentServerIP[MAX_CVAR_VALUE_STRING];
		char	fs_basePath[MAX_CVAR_VALUE_STRING];
		char	fs_game[MAX_CVAR_VALUE_STRING];

		// UQ1: Graphics values...
		char	r_mode[MAX_CVAR_VALUE_STRING];
		char	r_depthbits[MAX_CVAR_VALUE_STRING];
		char	r_fullscreen[MAX_CVAR_VALUE_STRING];

		// UQ1: Command line...
		char	*cmd;

		// UQ1: Game values...
		trap_Cvar_VariableStringBuffer("cl_currentServerIP", cl_currentServerIP, sizeof(cl_currentServerIP));
...
#ifdef _WIN32
		// UQ1: Set the windows command line...
		cmd = va("%s/et.exe", fs_basePath);
#else //!_WIN32
		// UQ1: Set the *nix command line...
		cmd = va("%s/et", fs_basePath);
#endif //_WIN32

		// UQ1: Graphics values...
		trap_Cvar_VariableStringBuffer("r_mode", r_mode, sizeof(r_mode));
		trap_Cvar_VariableStringBuffer("r_depthbits", r_depthbits, sizeof(r_depthbits));
		trap_Cvar_VariableStringBuffer("r_fullscreen", r_fullscreen, sizeof(r_fullscreen));

		if ( !cl_currentServerIP[0] || !strcmp(cl_currentServerIP, "") )
		{// A local server, start the map locally with the gametype set...
			execl(cmd, cmd, "+set", "fs_game", fs_game, "+set", "com_hunkmegs", va("%i", DEFAULT_HUNKMEGS), "+set", "r_mode", r_mode, "+set", "r_depthbits", r_depthbits, "+set", "r_fullscreen", r_fullscreen, "+set", "g_gametype", va("%i", cgs.gametype), "+map", cgs.rawmapname, NULL);
		}
		else
		{// A remote server, reconnect to the server...
			execl(cmd, cmd, "+set", "fs_game", fs_game, "+set", "com_hunkmegs", va("%i", DEFAULT_HUNKMEGS), "+set", "r_mode", r_mode, "+set", "r_depthbits", r_depthbits, "+set", "r_fullscreen", r_fullscreen, "+connect", cl_currentServerIP, NULL);
		}

		// UQ1: Ok. We have forked our second instance, now kill this one...
		exit(0);
	}
}

void CG_RegisterTank ( int type )
{// UQ1: Moved to dynamic loading as required to save memory!
#ifdef __VEHICLES__
#ifdef __VEHICLES_DELAYED_MODEL_LOADING__
	if (cgs.media.tankModel[type])
		return; // Already registerred!
	
	switch (type)
	{
	case VEHICLE_TYPE_CHURCHILL:
		// Churchill...
		cgs.media.tankModel[VEHICLE_TYPE_CHURCHILL] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/churchhill_notracks.md3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_CHURCHILL] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/churchhill_tracks.md3");
		cgs.media.tankTurretModel[VEHICLE_TYPE_CHURCHILL] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/churchhill_turret2.md3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_CHURCHILL] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/churchhill_barrel.md3"); 
		break;

	case VEHICLE_TYPE_PANZER:
		// Panzer...
		cgs.media.tankModel[VEHICLE_TYPE_PANZER] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/jagdpanther_notracks.md3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_PANZER] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/jagdpanther_tracks.md3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_PANZER] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/jagdpanther_temperate_turret.md3"); 
		break;

	case VEHICLE_TYPE_KV1:
		// KV1...
		cgs.media.tankModel[VEHICLE_TYPE_KV1] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/kv1_notracks.md3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_KV1] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/kv1_tracks.md3");
		cgs.media.tankTurretModel[VEHICLE_TYPE_KV1] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/kv1_turret2.md3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_KV1] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/kv1_barrel.md3");
		break;

	case VEHICLE_TYPE_T34:
		// T34...
		cgs.media.tankModel[VEHICLE_TYPE_T34] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_notracks.md3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_T34] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_tracks.md3");
		cgs.media.tankTurretModel[VEHICLE_TYPE_T34] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_turret2.md3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_T34] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_barrel.md3");
		break;

	case VEHICLE_TYPE_BT7:
		// BT7...
		cgs.media.tankModel[VEHICLE_TYPE_BT7] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/bt7_notracks.md3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_BT7] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/bt7_tracks.md3");
		cgs.media.tankTurretModel[VEHICLE_TYPE_BT7] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/bt7_turret2.md3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_BT7] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/bt7_barrel.md3");
		break;

	case VEHICLE_TYPE_FLAMETANK:
		// Flametank (t34)...
		cgs.media.tankModel[VEHICLE_TYPE_FLAMETANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_notracks.md3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_FLAMETANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_tracks.md3");
		cgs.media.tankTurretModel[VEHICLE_TYPE_FLAMETANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_turret2.md3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_FLAMETANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_barrel.md3");
		break;

	case VEHICLE_TYPE_PANZERIV:
		// Panzer VI (tiger)...
		cgs.media.tankModel[VEHICLE_TYPE_PANZERIV] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzeriv_notracks.md3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_PANZERIV] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzeriv_tracks.md3");
		cgs.media.tankTurretModel[VEHICLE_TYPE_PANZERIV] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzeriv_turret2.md3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_PANZERIV] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzeriv_barrel.md3");
		break;

	case VEHICLE_TYPE_TIGERTANK:
/*
		// UQ1: Disabled! tiger_notracks and tiger_full have too many verts!!!
		cgs.media.tankModel[VEHICLE_TYPE_TIGERTANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/tiger_notracks.md3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_TIGERTANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/tiger_tracks.md3");
		cgs.media.tankTurretModel[VEHICLE_TYPE_TIGERTANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/tiger_turret2.md3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_TIGERTANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/tiger_barrel.md3");
*/
		cgs.media.tankModel[VEHICLE_TYPE_TIGERTANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzeriv_notracks.md3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_TIGERTANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzeriv_tracks.md3");
		cgs.media.tankTurretModel[VEHICLE_TYPE_TIGERTANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzeriv_turret2.md3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_TIGERTANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzeriv_barrel.md3");
		break;

	case VEHICLE_TYPE_STUG3:
		// Stug3...
		cgs.media.tankModel[VEHICLE_TYPE_STUG3] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/stug3_notracks.MD3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_STUG3] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/stug3_tracks.MD3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_STUG3] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/stug3_temperate_turret.MD3");
		break;

	case VEHICLE_TYPE_FLAK88:
		// Flak88...
		cgs.media.tankModel[VEHICLE_TYPE_FLAK88] = trap_R_RegisterModel ("models/mapobjects/artillery/flak88_body.md3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_FLAK88] = trap_R_RegisterModel ("models/mapobjects/artillery/flak88_barrel.md3");
		break;

	case VEHICLE_TYPE_PAK:
		// Pak...
		cgs.media.tankModel[VEHICLE_TYPE_PAK] = trap_R_RegisterModel ("models/mapobjects/artillery/pak_body.md3");
		cgs.media.tankBarrelModel[VEHICLE_TYPE_PAK] = trap_R_RegisterModel ("models/mapobjects/artillery/pak_barrel.md3");
		break;

	case VEHICLE_TYPE_AXIS_APC:
		// Add Me :) Axis APC...
		cgs.media.tankModel[VEHICLE_TYPE_AXIS_APC] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/m3_notracks.md3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_AXIS_APC] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/m3_tracks.md3");
		break;

	case VEHICLE_TYPE_ALLIED_APC:
		// M3 Russian APC...
		cgs.media.tankModel[VEHICLE_TYPE_ALLIED_APC] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/m3_notracks.md3");
		cgs.media.tankTracksModel[VEHICLE_TYPE_ALLIED_APC] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/m3_tracks.md3");
		break;
	default:
		break;
	}

	// UQ1: We need to concerve model memory!!!
	if (!cgs.media.tankFlashModel)
		cgs.media.tankFlashModel = cgs.media.mg42muzzleflash;
#endif //__VEHICLES_DELAYED_MODEL_LOADING__
#endif //__VEHICLES__
}
...
static void CG_RegisterGraphics( void ) {
	...
	if ( cgs.arenaInfoLoaded ) {
		CG_LoadingString( va("Map: %s", cgs.arenaData.longname) );
	} else {
		CG_LoadingString( va("Map: %s", cgs.rawmapname) );
	}
...
" );
	...
	// UQ1: Stealth System...
	stlth_skin = trap_R_RegisterSkin( "models/players/temperate/allied/cvops/body_stlth_cvops.skin" );
	stlth_shader = trap_R_RegisterShader( "stealth" );

	// UQ1: Shadow System...
//	shadow_skin = trap_R_RegisterSkin( "models/players/temperate/allied/cvops/body_shadow_cvops.skin" );
//	shadow_shader = trap_R_RegisterShader( "playershadow" );
...
//	cgs.media.cursor = trap_R_RegisterShaderNoMip( "ui/assets/mvcursor.tga" );
	...
	cgs.media.tankShader[4] = trap_R_RegisterShaderNoMip( "models/mapobjects/tanks_sd/bits_backward" );
	cgs.media.tankShader[5] = trap_R_RegisterShaderNoMip( "models/mapobjects/tanks_sd/jag_tracks_alt_backward" );
#endif //__VEHICLES__
	...
	cgs.media.reticleShaderSimple	= trap_R_RegisterShader( "gfx/misc/adv_scope" );
	cgs.media.reticleShaderOverlay	= trap_R_RegisterShader( "gfx/misc/adv_scope_trans" );
	...
	cgs.media.stealthShader			= trap_R_RegisterShaderNoMip( "gfx/2d/stealthshader.tga" );
...
//	cgs.media.hintPlrUnknownShader	= trap_R_RegisterShader( "gfx/2d/hintPlrUnknown" );	
	...
	cgs.media.shellShockViewShader	= trap_R_RegisterShaderNoMip( "gfx/2d/shellshock" );
#endif //__SHELLSHOCK__

#ifdef __BLOODY_VIEW__
	cgs.media.troubledViewShader	= trap_R_RegisterShaderNoMip( "gfx/2d/bloodyview" );
#endif //__BLOODY_VIEW__

	cgs.media.muzzleCorona	= trap_R_RegisterShaderNoMip( "gfx/2d/bloodyview" );//trap_R_RegisterShader( "muzzle_corona3" );
...
//	cgs.media.commandCentreWoodShader		= trap_R_RegisterShaderNoMip( "ui/assets2/commandMap" );
	...
	cgs.media.blackmask = trap_R_RegisterShaderNoMip( "blackgrad" );
	...
	cgs.media.commandCentreSpawnShader[2] = trap_R_RegisterShaderNoMip( "gfx/limbo/cm_flagneutral" );
	cgs.media.compassConstructShader =		trap_R_RegisterShaderNoMip( "sprites/construct.tga" );
	cgs.media.commandCentreAmmoCrateShader = trap_R_RegisterShaderNoMip( "gfx/limbo/cm_ammocrate" );
	cgs.media.commandCentreHealthCrateShader = trap_R_RegisterShaderNoMip( "gfx/limbo/cm_healthcrate" );
...
*/
	...
	// UQ1: Not used!
	/*
	cgs.media.mortarload[11] = trap_R_RegisterModel( "models/common/mortar12.md3" );
	cgs.media.mortarload[10] = trap_R_RegisterModel( "models/common/mortar11.md3" );
	cgs.media.mortarload[9] = trap_R_RegisterModel( "models/common/mortar10.md3" );
	cgs.media.mortarload[8] = trap_R_RegisterModel( "models/common/mortar9.md3" );
	cgs.media.mortarload[7] = trap_R_RegisterModel( "models/common/mortar8.md3" );
	cgs.media.mortarload[6] = trap_R_RegisterModel( "models/common/mortar7.md3" );
	cgs.media.mortarload[5] = trap_R_RegisterModel( "models/common/mortar6.md3" );
	cgs.media.mortarload[4] = trap_R_RegisterModel( "models/common/mortar5.md3" );
	cgs.media.mortarload[3] = trap_R_RegisterModel( "models/common/mortar4.md3" );
	cgs.media.mortarload[2] = trap_R_RegisterModel( "models/common/mortar3.md3" );
	cgs.media.mortarload[1] = trap_R_RegisterModel( "models/common/mortar2.md3" );
	cgs.media.mortarload[0] = trap_R_RegisterModel( "models/common/mortar.md3" );
	
	cgs.media.bulletload[2] = trap_R_RegisterModel( "models/common/bullets2.md3" );
	cgs.media.bulletload[1] = trap_R_RegisterModel( "models/common/bullets1.md3" );
	cgs.media.bulletload[0] = trap_R_RegisterModel( "models/common/bullets.md3" );
	*/
...
//	cgs.media.shardCeramic2 = trap_R_RegisterModel( "models/shards/ceramic2.md3" );
...
#ifdef __VEHICLES__
#ifndef __VEHICLES_DELAYED_MODEL_LOADING__
	// UQ1: Now loaded as required instead! -- UQ1: Disabled that as its not working all the time for some reason :(
	CG_LoadingString("Vehicle/Artillery Models");
	...
	// Flametank (t34)... UQ1: Concerve model memory...
	cgs.media.tankModel[VEHICLE_TYPE_FLAMETANK] = cgs.media.tankModel[VEHICLE_TYPE_T34];
	cgs.media.tankTracksModel[VEHICLE_TYPE_FLAMETANK] = cgs.media.tankTracksModel[VEHICLE_TYPE_T34];
	cgs.media.tankTurretModel[VEHICLE_TYPE_FLAMETANK] = cgs.media.tankTurretModel[VEHICLE_TYPE_T34];
	cgs.media.tankBarrelModel[VEHICLE_TYPE_FLAMETANK] = cgs.media.tankBarrelModel[VEHICLE_TYPE_T34];
	//cgs.media.tankModel[VEHICLE_TYPE_FLAMETANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_notracks.md3");
	//cgs.media.tankTracksModel[VEHICLE_TYPE_FLAMETANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_tracks.md3");
	//cgs.media.tankTurretModel[VEHICLE_TYPE_FLAMETANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_turret2.md3");
	//cgs.media.tankBarrelModel[VEHICLE_TYPE_FLAMETANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/t34_barrel.md3");
	...
	cgs.media.tankBarrelModel[VEHICLE_TYPE_TIGERTANK] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/tiger_barrel.md3");
/*
	cgs.media.tankModel[VEHICLE_TYPE_TIGERTANK] = cgs.media.tankModel[VEHICLE_TYPE_PANZERIV];
	cgs.media.tankTracksModel[VEHICLE_TYPE_TIGERTANK] = cgs.media.tankTracksModel[VEHICLE_TYPE_PANZERIV];
	cgs.media.tankTurretModel[VEHICLE_TYPE_TIGERTANK] = cgs.media.tankTurretModel[VEHICLE_TYPE_PANZERIV];
	cgs.media.tankBarrelModel[VEHICLE_TYPE_TIGERTANK] = cgs.media.tankBarrelModel[VEHICLE_TYPE_PANZERIV];
*/
	...
	// Add Me :) Allied APC...  -Masteries says: don´t worry I am working in german transport hehe
	cgs.media.tankModel[VEHICLE_TYPE_ALLIED_APC] = cgs.media.tankModel[VEHICLE_TYPE_AXIS_APC];
	cgs.media.tankTracksModel[VEHICLE_TYPE_ALLIED_APC] = cgs.media.tankTracksModel[VEHICLE_TYPE_AXIS_APC];
    
    // Panzer II...
    cgs.media.tankModel[VEHICLE_TYPE_PANZER2] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzer2_notracks.md3");
	cgs.media.tankTracksModel[VEHICLE_TYPE_PANZER2] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzer2_tracks.md3");
	cgs.media.tankTurretModel[VEHICLE_TYPE_PANZER2] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzer2_turret2.md3");
	cgs.media.tankBarrelModel[VEHICLE_TYPE_PANZER2] = trap_R_RegisterModel ("models/mapobjects/tanks_sd/panzer2_barrel.md3");
...
//	cgs.media.hflakWeaponSnd =		trap_S_RegisterSound( "sound/vehicles/misc/20mm_fire.wav", qfalse );
...
#ifdef __SHELLSHOCK__
	cgs.media.shellShockSnd =		trap_S_RegisterSound( "sound/misc/shellshock.wav", qfalse );
#endif //__SHELLSHOCK__

	cgs.media.heartBeatSnd =		trap_S_RegisterSound( "sound/misc/heartbeat.wav", qfalse );
	cgs.media.breathAim =			trap_S_RegisterSound( "sound/player/breathing/breath_aim.wav", qfalse );
	cgs.media.breathSnipe =			trap_S_RegisterSound( "sound/player/breathing/sniper_breath.wav", qfalse );
	cgs.media.breathAfraid =		trap_S_RegisterSound( "sound/player/breathing/breath_afraid.wav", qfalse );
	cgs.media.breathRunning =		trap_S_RegisterSound( "sound/player/breathing/breath_run.wav", qfalse );
	...
	for ( i = 0 ; i < 5 ; i++ ) {
		...
		Com_sprintf( name2, sizeof(name2), "blood_dot%i", i+1 );
		cgs.media.bloodDotShaders[i] = trap_R_RegisterShader( name2 );
	}
	...
	for ( i = 1 ; i < cgs.numInlineModels ; i++ ) {
		...
		Com_sprintf( name2, sizeof(name2), "*%i", i );
		cgs.inlineDrawModel[i] = trap_R_RegisterModel( name2 );
...
#ifdef __VEHICLES__
	cgs.media.hMG42Base =			trap_R_RegisterModel( "models/mapobjects/weapons/mg42b.md3" );
#endif //__VEHICLES__
	...
	//trap_R_RegisterFont( "ariblk", 27, /*&cgs.media.limboFont1*/&cgs.media.tahomabd24 );
	//trap_R_RegisterFont( "ariblk", 16, /*&cgs.media.limboFont1*/&cgs.media.tahomabd24_lo );	
	//trap_R_RegisterFont( "courbd", 30, /*&cgs.media.limboFont2*/&cgs.media.tahoma30 );

	//trap_R_RegisterFont( "tahoma", 16, &cgs.media.tahoma16 );
	trap_R_RegisterFont( "tahoma", 36, &cgs.media.tahoma16 );
	///*cgs.media.limboFont1*/cgs.media.tahomabd24_lo = cgs.media.tahoma16;
	//trap_R_RegisterFont( "tahoma", 30, &cgs.media.tahoma30 );
	trap_R_RegisterFont( "tahoma", 36, &cgs.media.tahoma30 );
	//cgs.media.limboFont2 = cgs.media.tahoma30;
	//trap_R_RegisterFont( "tahomabd", 24, &cgs.media.tahomabd24 );
	trap_R_RegisterFont( "tahomabd", 36, &cgs.media.tahomabd24 );
	//cgs.media.limboFont1 = cgs.media.tahomabd24;
	...
	cgs.media.limboWeaponCard4 =				trap_R_RegisterShaderNoMip( "gfx/limbo/weaponcard04"	);
#endif //__EF__
	...
	cgs.media.hudFOVLine = 					trap_R_RegisterShaderNoMip("icons/miscellaneous/fov_line");
...
static void CG_FeederSelection(float feederID, int index) {
	...
	team = (feederID == FEEDER_REDTEAM_LIST) ? TEAM_AXIS : TEAM_ALLIES;
	count = 0;
...
void CG_CheckForSnowFootstepKludge ( void )
{
	if( !Q_stricmp( cgs.mapname, "maps/mp_trenchtoast.bsp" ) )
...
extern void UQ_Reset_Kludge ( void );
...
//	int startat = trap_Milliseconds();
...
#ifdef __ETE__
	// UQ1: Needs to be after server info for gametype and map name info...
	CG_SetHunkMegs();
#endif //__ETE__
	...
	cgs.dumpStatsTime = 0;
 	...
 	CG_ParseETEnhancedinfo();
	CG_ParseETESkillInfo();

#ifdef __ETE__
	// UQ1: Check if we need an override for this map (for maps converted to snow)!
	CG_CheckForSnowFootstepKludge();
#endif //__ETE__
...
extern void	BG_MemoryClose();
extern qboolean BG_IsMemoryInitialized();
...
{
	if (cg_gameType.integer == GT_SINGLE_PLAYER || cgs.gametype == GT_COOP || cgs.gametype == GT_NEWCOOP)
		return qtrue;
...
centity_t *SecondaryEntity( int entityNum )
{
	return CG_SecondaryEntityFor(entityNum);
}

entityState_t *SecondaryEntityState( int entityNum )
{
	return &CG_SecondaryEntityFor(entityNum)->currentState;
}

qboolean HasExtFlag( int entityNum, int flag )
{
	if (SecondaryEntity(entityNum)->currentState.eFlags & flag)
		return qtrue;
