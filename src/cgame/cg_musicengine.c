...
extern void			*B_Alloc ( int size/*, char *procedure*/ );
#ifdef CGAMEDLL
...
extern int			Q_TrueRand ( int value1, int value2 );
...
{				// Load music playlist from external file.
	...
	buf2 = (char *) B_Alloc( 1 + 1/*, "CG_LoadMyMusic"*/ );
	...
	f = fopen( va( "%s", loadPath), "r" );
	if ( (buf = (char *) B_Alloc( len + 1/*, "CG_LoadMyMusic"*/)) == 0 )
...
CG_MyMusicPlay ( void )
{
	...
	if ( MyMusicTotal > 0 )
	{
		selection = Q_TrueRand(0, MyMusicTotal-1);//rand() % MyMusicTotal;	//Q_TrueRand(0, MyMusicTotal);
	}
...
void CG_RealDrawMusicInformation( char *music_title, char *music_description ) {
	...
	float		title_size = 0.12f;//0.19f;
	float		description_size = 0.08f;//0.19f;
	...
	// STATION TITLE
	w = CG_Text_Width_Ext( va("%s", music_title_short), title_size, 0, &cgs.media.tahoma16 );
	pos_x += (32 - (w * 0.5f));

	CG_Text_Paint_Ext( pos_x, pos_y + 11, title_size, title_size, tclr, va("^3%s", music_title_short), 0, 0, 0, &cgs.media.tahoma16 );
	...
	// STATION TITLE
	w = CG_Text_Width_Ext( va("%s", music_description_short), description_size/*title_size*/, 0, &cgs.media.tahoma16 );
	//w *= (title_size/description_size);//(description_size/title_size) + 8;
	w *= ((description_size/title_size) * 2);
	pos_x += (32 - (w * 0.5f));

	CG_Text_Paint_Ext( pos_x, pos_y + 11 + 6, description_size, description_size, tclr, va("^7%s", music_description_short), 0, 0, 0, &cgs.media.tahoma16 );
}
...
void CG_DrawMusicInformation( void ) 
{
	...
	{
		char artist[128] = "";
		TAG_ID3 *meta2 = BASS_ChannelGetTags(chan,BASS_TAG_ID3);
