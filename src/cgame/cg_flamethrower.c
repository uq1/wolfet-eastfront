...
#ifdef __VEHICLES__
void CG_FireFlameTankChunks( centity_t *cent, vec3_t origin, vec3_t angles, float speedScale, qboolean firing )
{
	...
	if (	(centInfo->lastClientFrame == cent->currentState.frame) &&
		...
		timeInc = 1000.0 * (firing ? 1.0 : 0.5) * ((FLAME_CHUNK_DIST*2) / (FLAME_START_SPEED * speedScale));
...
void CG_FireFlameChunks( centity_t *cent, vec3_t origin, vec3_t angles, float speedScale, qboolean firing )
{
	...
	{
		CG_FireFlameTankChunks( cent, origin, angles, speedScale, firing );
		return;
	}
#endif //__VEHICLES__
...
#define	FLAME_SPRITE_START_BLUE_SCALE	0.2
		...
		if (!f->ignitionOnly &&
			...
			float alpha2, lifeFrac;
			qboolean skip2 = qfalse;
			...
			alpha2 = (1.0 - lifeFrac)*1.4;
			if (alpha2 > 1.0)
				alpha2 = 1.0;

			if (!skip2) {
				// draw the sprite
				CG_AddFlameSpriteToScene (f, lifeFrac, alpha2 );
			}
			// update the sizeRate
			f->sizeRate = GET_FLAME_SIZE_SPEED( f->sizeMax );
		}
