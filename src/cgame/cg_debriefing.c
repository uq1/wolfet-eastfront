...
#define DB_MASTER_FONT &/*cgs.media.limboFont2*/cgs.media.tahoma30
...
void CG_Debriefing_ChatBox_Draw( panel_button_t* button ) {
	...
	if( cgs.teamLastChatPos != cgs.teamChatPos ) {
		...
		for( i = cgs.teamChatPos - 1; i >= cgs.teamLastChatPos; i-- ) {
			...
			CG_Text_Paint_Ext( button->rect.x + 12, button->rect.y - (cgs.teamChatPos - i - 1)*lineHeight, 0.2f, 0.2f, colorWhite, cgs.teamChatMsgs[i % chatHeight], 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
		}
	}
}
...
//	CG_FillRect( button->rect.x, button->rect.y, button->rect.w, button->rect.h, colorRed );
	...
	for( i = 0; i + cgs.dbPlayerListOffset < MAX_CLIENTS && i < 24; i++ ) {
		...
		CG_FitTextToWidth_SingleLine_Ext(buffer, button->font->scalex, 158, sizeof(buffer), button->font->font);
		CG_Text_Paint_Ext( DB_NAME_X, y, button->font->scalex, button->font->scaley, button->font->colour, buffer, 0, 0, 0, button->font->font );
...
void CG_Debriefing_PlayerName_Draw( panel_button_t* button ) {
	...
	Q_strncpyz(buffer, ci->name, sizeof(buffer));
	CG_FitTextToWidth_SingleLine_Ext(buffer, button->font->scalex, 182, sizeof(buffer), button->font->font);
	CG_Text_Paint_Ext( button->rect.x, button->rect.y, button->font->scalex, button->font->scaley, button->font->colour, buffer, 0, 0, ITEM_TEXTSTYLE_SHADOWED, button->font->font );
}
...
#ifdef __ETE__
	if( cg_gameType.integer == GT_SUPREMACY_CAMPAIGN ) {
		for( i = 0; i < cgs.campaignData.mapCount; i++ ) {
...
void CG_PanelButtonsRender_Button_Ext( rectDef_t* r, const char* text ) {
	...
	if( text ) {
		float w = CG_Text_Width_Ext( text, 0.2f, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
		CG_Text_Paint_Ext( r->x + ((r->w+2) - w) * 0.5f, r->y + 11, 0.19f, 0.19f, hilight ? clrTxt_hi : clrTxt, text, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
	}
}
...
void CG_PanelButtonsRender_Window_Ext( rectDef_t* r, const char* text, int align, int innerheight, float fontscale, int yofs ) {
	...
	if( text ) {
		...
		if( align == ITEM_ALIGN_CENTER ) {
			float w = CG_Text_Width_Ext( text, fontscale, 0, /*&cgs.media.limboFont1*/&cgs.media.tahomabd24 );
			x = r->x + (r->w - w) * 0.5f;
		...
		CG_Text_Paint_Ext( x, r->y + yofs, fontscale, fontscale, clrTxtBck, text, 0, 0, 0, /*&cgs.media.limboFont1*/&cgs.media.tahomabd24 );
	}
}
...
void CG_Debreifing2_MissionTitle_Draw( panel_button_t* button ) {
	...
	if( cg_gameType.integer == GT_WOLF_STOPWATCH ) {
...
#ifdef __ETE__
	} else if( cg_gameType.integer == GT_SUPREMACY_CAMPAIGN ) {		
		...
		CG_Text_Paint_Ext( button->rect.x + (button->rect.w-w)*0.5f, button->rect.y + 16, 0.25f, 0.25f, clrTxtBck, s, 0, 0, 0, /*&cgs.media.limboFont1*/&cgs.media.tahomabd24 );
#endif //__ETE__
	...
	CG_Text_Paint_Ext( x, button->rect.y + 16, 0.25f, 0.25f, clrTxtBck, s, 0, 0, 0, /*&cgs.media.limboFont1*/&cgs.media.tahomabd24 );
}
...
void CG_Debreifing2_Awards_Draw( panel_button_t* button ) {
	...
	for( i = 0; i < NUM_ENDGAME_AWARDS; i++ ) {
		...
		CG_Text_Paint_Ext( button->rect.x + 28, y + 11, 0.19f, 0.19f, clrTxtBck, awardNames[ i ], 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );

		Q_strncpyz(buffer, cgs.dbAwardNames[i], sizeof(buffer));
		CG_FitTextToWidth_SingleLine_Ext(buffer, 0.19f, 203, sizeof(buffer), &cgs.media.tahoma30);
		CG_Text_Paint_Ext( button->rect.x + 28 + 180, y + 11, 0.19f, 0.19f, clrTxtBck, buffer, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
		y += 16;
	}
}
...
void CG_Debreifing2_Maps_Draw( panel_button_t* button ) {
	...
	if( cg_gameType.integer == GT_WOLF_CAMPAIGN ) {
		...
		CG_Text_Paint_Ext( button->rect.x + 4, button->rect.y + 11, 0.19f, 0.19f, clrTxtBck, va( "Campaign: %s", cgs.campaignData.campaignName ), 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
		...
		for( i = 0; i < cgs.campaignData.mapCount; i++ ) {
			...
			CG_Text_Paint_Ext( button->rect.x + 8, y + 11, 0.19f, 0.19f, clrTxtBck, va( "%i. %s", i+1, cgs.campaignData.arenas[i].longname ), 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
			
			if( i <= cgs.currentCampaignMap ) {
				str = CG_Debreifing2_WinStringForTeam( CG_Debriefing_FindWinningTeamForPos( i + 1 ) );
				
				w = CG_Text_Width_Ext( str, 0.2f, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
				CG_Text_Paint_Ext( button->rect.x + button->rect.w - w - 8, y + 11, 0.19f, 0.19f, clrTxtBck, str, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
			}
...
void CG_Debreifing2_Mission_Draw( panel_button_t* button ) {
	...
	if( cg_gameType.integer == GT_WOLF_CAMPAIGN ) {
		...
		if( cgs.campaignData.mapTC[0][0] && cgs.campaignData.mapTC[1][0] ) {
			...
			CG_DrawPicST( button->rect.x, button->rect.y, button->rect.w, button->rect.h, cgs.campaignData.mapTC[0][0] / 1024.f, cgs.campaignData.mapTC[0][1] / 1024.f, cgs.campaignData.mapTC[1][0] / 1024.f, cgs.campaignData.mapTC[1][1] / 1024.f, trap_R_RegisterShaderNoMip( "gfx/loading/camp_worldmap" ) );
			...
			for( i = cgs.campaignData.mapCount-1; i >= 0; i-- ) {
				...
				w = CG_Text_Width_Ext( cgs.campaignData.arenas[i].longname, 0.2f, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
...
//					CG_Text_Paint_Ext( x - w - 10, y + 3, 0.2f, 0.2f, colorWhite, cgs.campaignData.arenas[i].longname, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
//				} else {
//					CG_Text_Paint_Ext( x + 10, y + 3, 0.2f, 0.2f, colorWhite, cgs.campaignData.arenas[i].longname, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
//				}
 				// CHRUKER: b049 - Correct placement of the map name
 				if( x + 14 + w > button->rect.x + button->rect.w ) {
 					// x - pinhwidth (12) - pin left margin (3) - w => x - w - 15
 					CG_Text_Paint_Ext( x - w - 15, y + 3, 0.2f, 0.2f, colorWhite, cgs.campaignData.arenas[i].longname, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
 				} else {
 					// x + pinhwidth (12) + pin right margin (0) => x + 12
 					CG_Text_Paint_Ext( x + 12, y + 3, 0.2f, 0.2f, colorWhite, cgs.campaignData.arenas[i].longname, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
 				} // b049
			}
	...
	if( cgs.arenaData.mappos[0] && cgs.arenaData.mappos[1] ) {
		...
		w = CG_Text_Width_Ext( cgs.arenaData.longname, 0.2f, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
		...
		switch( CG_Debriefing_FindWinningTeam() ) {
 		...
 		if( x + 14 + w > button->rect.x + button->rect.w ) {
 			// x - pinhwidth (12) - pin left margin (3) - w => x - w - 15
 			CG_Text_Paint_Ext( x - w - 15, y + 3, 0.2f, 0.2f, colorWhite, cgs.arenaData.longname, 0, 0, 0, &cgs.media.tahoma30 );
 		} else {
 			// x + pinhwidth (12) + pin right margin (0) => x + 12
 			CG_Text_Paint_Ext( x + 12, y + 3, 0.2f, 0.2f, colorWhite, cgs.arenaData.longname, 0, 0, 0, &cgs.media.tahoma30 );
 		} // b049
	} else {
		CG_DrawPic( button->rect.x, button->rect.y, button->rect.w, button->rect.h, trap_R_RegisterShaderNoMip( "menu/art/unknownmap" ) );
	}
	...
	for( j = 0; j < 2; j++ ) {
		for( i = 0; i <= SK_NUM_SKILLS; i++ ) {
			...
			if( *str ) {
				...
				CG_Text_Paint_Ext( button->rect.x + 100 + skillPositions[ i ] - (w*0.5f), button->rect.y + (j*11), 0.2f, 0.2f, clrTxtBck, str, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
			}
		}
	}
}
...
void CG_Debriefing2TeamSkillXP_Draw( panel_button_t* button ) {
	...
	switch( team ) {
		case TEAM_AXIS:
			CG_Text_Paint_Ext( button->rect.x, button->rect.y + 11, scale, scale, clrTxtBck, "Axis", 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
			break;
		default:
			CG_Text_Paint_Ext( button->rect.x, button->rect.y + 11, scale, scale, clrTxtBck, "Allies", 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
			break;
	}
	...
	for( i = 0; i <= SK_NUM_SKILLS; i++ ) {
		...
		w = CG_Text_Width_Ext( str, scale, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );

		CG_Text_Paint_Ext( button->rect.x + 100 + skillPositions[ i ] - (w*0.5f), button->rect.y + 11, scale, scale, clrTxtBck, str, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
	}
}
