...
#define FT_LOC_COORDS 0
#define FT_LOC_LOCNAMES 1
#define FT_LOC_COORDS_LOCNAMES 2
#define FT_BASE_WIDTH 174
...
void CG_DrawFireTeamOverlay( rectDef_t* rect ) {
	...
	for(i = 0; i < MAX_FIRETEAM_MEMBERS; i++) {
		...
		if ( cg_locationMode.integer == FT_LOC_LOCNAMES ) {
			s = va("^3%s", BG_GetLocationName(cg_entities[ci->clientNum].lerpOrigin));
		} else if ( cg_locationMode.integer == FT_LOC_COORDS_LOCNAMES ) {
			s = va("^3(%s) %s", BG_GetLocationString(loc), BG_GetLocationName(cg_entities[ci->clientNum].lerpOrigin));
		} else { // last resort, just do coordinates
			s = va("^3(%s)", BG_GetLocationString(loc));
		}

		w = CG_Text_Width_Ext( s, .2f, 0, &cgs.media.tahoma30 );
		if ( w > locWidth ) {
			locWidth = w;
		}
	...
	CG_DrawRect( x, y, FT_BASE_WIDTH + locWidth, h, 1, borderColor);
	CG_FillRect( x + 1, y + 1, FT_BASE_WIDTH + locWidth  - 2, h - 2, bgColor);

	x += 2;
	y += 2;

	CG_FillRect( x, y, FT_BASE_WIDTH + locWidth - 4, 12, clr1 );

	sprintf( buffer, "Fireteam: %s", bg_fireteamNames[f->ident] );
	Q_strupr( buffer );
	CG_Text_Paint_Ext( x + 3, y + FT_BAR_HEIGHT, .19f, .19f,
		tclr, buffer,
		0, 0, 0, /*cgs.media.limboFont2*/&cgs.media.tahomabd24 );
	...
	for(i = 0; i < MAX_FIRETEAM_MEMBERS; i++) {
		...
		if( ci->selected ) {
			CG_FillRect( x, y + FT_BAR_YSPACING, FT_BASE_WIDTH + locWidth - 4, FT_BAR_HEIGHT, clr3 );
		} else {
			CG_FillRect( x, y + FT_BAR_YSPACING, FT_BASE_WIDTH + locWidth - 4, FT_BAR_HEIGHT, clr2 );
		}

		x += 4;
				
		CG_Text_Paint_Ext( x, y + FT_BAR_HEIGHT, .2f, .2f, 
			tclr, BG_ClassLetterForNumber( ci->cls ),
			0, 0, ITEM_TEXTSTYLE_SHADOWED, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );	
		x += 10;
		
		CG_Text_Paint_Ext( x, y + FT_BAR_HEIGHT, .2f, .2f,
			tclr, ci->team == TEAM_AXIS ? miniRankNames_Axis[ci->rank] : miniRankNames_Allies[ci->rank], 
			0, 0, ITEM_TEXTSTYLE_SHADOWED, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );	
		x += 22;

		Q_strncpyz(buffer, ci->name, sizeof(buffer));
		CG_FitTextToWidth_SingleLine_Ext(buffer, .2f, 107, sizeof(buffer), &cgs.media.tahoma30);
...
/*		if( isLeader ) {
		...
		if( ci->health > 80 ) {
			CG_Text_Paint_Ext(x, y + FT_BAR_HEIGHT,  .2f, .2f,
				tclr, va("%i", ci->health < 0 ? 0 : ci->health ),
				0, 0, ITEM_TEXTSTYLE_SHADOWED, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
		} else if( ci->health > 0 ) {
			CG_Text_Paint_Ext(x, y + FT_BAR_HEIGHT,  .2f, .2f,
				colorYellow, va("%i", ci->health < 0 ? 0 : ci->health ),
				0, 0, ITEM_TEXTSTYLE_SHADOWED, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
		} else {
			CG_Text_Paint_Ext(x, y + FT_BAR_HEIGHT, .2f, .2f,
				colorRed, va("%i", ci->health < 0 ? 0 : ci->health ),
				0, 0, ITEM_TEXTSTYLE_SHADOWED, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
		}
		...
		//x = rect->x + ( FT_BASE_WIDTH + locWidth - 4 - CG_Text_Width_Ext( s, .2f, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 ) );
		//x = FT_BASE_WIDTH + locWidth - 4 - CG_Text_Width_Ext( s, .2f, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
