 ...
 *              //Masteries 5/12/2007  some changes in muzzle line 8101
*/
...
/*#ifdef __PARTICLE_SYSTEM__
	if (cgs.media.flameTrailPS <= 0)
		cgs.media.flameTrailPS = CG_RegisterParticleSystem( "planeFlameTrailPS" );

	if (!ent->entityPS)
	{
		ent->entityPS = CG_SpawnNewParticleSystem( cgs.media.flameTrailPS );
		CG_SetParticleSystemTag( ent->entityPS, ent->refEnt, ent->refEnt.hModel, "tag_flash" );
		CG_SetParticleSystemCent( ent->entityPS, ent );
		CG_AttachParticleSystemToTag( ent->entityPS );
	}
	...
	for ( ; t <= ent->trailTime; t += step )
	{
		...
		le = CG_SmokePuff( origin, dir, 45 + rnd * 120, rnd * 0.5 + 0.5, rnd * 0.5 + 0.5, rnd * 0.5 + 0.5, rnd * 1.0,
						   9800 + (rand() % 2800), t, 0, 0, cgs.media.smokeParticleShader );
	}
	...
	while( 1 ) {
		...
		if( !Q_stricmp( token.string, "tag" ) ) {
			...
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				...
				partModel->useCharacterSkin = qtrue; // UQ1: @FIXME - Add actual support!
	...
	while( 1 ) {
		...
		if( !Q_stricmp( token.string, "model" ) ) {
			...
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				...
				weaponInfo->weaponModel[viewType].useCharacterSkin = qtrue; // UQ1: @FIXME - Add actual support!
#endif //defined (__FRONTLINE__) || defined (__EF__)
#ifdef __PARTICLE_SYSTEM__
		} else if( !Q_stricmp( token.string, "flashPS" ) ) {
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				return CG_RW_ParseError( handle, "expected flashPS filename" );
			} else {
				weaponInfo->flashPS = CG_RegisterParticleSystem( filename );
			}
#endif //__PARTICLE_SYSTEM__
...
#ifdef __EF__
static qboolean CG_RW_ParseBrassModel( int handle, weaponInfo_t *weaponInfo )
{
	char		filename[MAX_QPATH];
	...
	if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
		return CG_RW_ParseError( handle, "expected model filename" );
			...
			//weaponInfo->brassModel = trap_R_RegisterShader( filename );
	}
...
//					CG_Error( "Couldn't register weapon %i (failed to parse %s)", weaponNum, filename );
			...
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				...
				cgs.media.weaponIcons[weaponInfo->weaponNumber][0] = weaponInfo->weaponIcon[0];
				//CG_Printf("Weapon %i registered icon 0 as %s.\n", weaponInfo->weaponNumber, filename);
			}
			...
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				...
				cgs.media.weaponIcons[weaponInfo->weaponNumber][1] = weaponInfo->weaponIcon[1];
				//CG_Printf("Weapon %i registered icon 1 as %s.\n", weaponInfo->weaponNumber, filename);
			}
		/*} else if( !Q_stricmp( token.string, "ammoIcon" ) ) {
			...
			// UQ1: @FIXME -- Add this...
			if( !CG_RW_ParseBrassModel( handle, weaponInfo ) )
				return qfalse;
#endif //__EF__
...
void CG_RegisterWeapon( int weaponNum, qboolean force ) {
	...
	switch( weaponNum ) {
		...
		case WP_SILENCED_LUGER:										filename = "silenced_luger.weap"; break;
		case WP_SILENCED_COLT:								filename = "silenced_colt.weap"; break;
		case WP_K43:										filename = "k43.weap"; break;
		case WP_K43_SCOPE:										filename = "k43_scope.weap"; break;
		...
		case WP_PPS:													filename = "bar.weap"; break;
		case WP_SVT40:												filename = "svt40.weap"; break;
		case WP_30CAL:
		case WP_30CAL_SET:										filename = "30cal.weap"; break;
        case WP_PTRS:
		case WP_PTRS_SET:										filename = "ptrs.weap"; break;
		case WP_AIRDROP_MARKER:									filename = "airdrop_marker.weap"; break;
#endif //__EF__

		default:															CG_Printf( S_COLOR_RED "WARNING: trying to register weapon %i but there is no weapon file entry for it.\n", weaponNum ); return;
	}

	weaponInfo->weaponNumber = weaponNum;
...
extern int PM_ReloadAnimForWeapon ( int weapon );
qboolean forced_client_reloading = qfalse;
...
static void CG_RunWeapLerpFrame( clientInfo_t *ci, weaponInfo_t *wi, lerpFrame_t *lf, int newAnimation, float speedScale ) {
	...
	{
		if((lf->animationNumber) == PM_ReloadAnimForWeapon(cg.snap->ps.nextWeapon)
			&& (lf->frame < (lf->animation->numFrames + lf->animation->firstFrame) - 1)) 
		{// UQ1: Never change during a reload!!!
			//forced_client_reloading = qtrue;
		}
		else if((newAnimation&~ANIM_TOGGLEBIT) == PM_RaiseAnimForWeapon(cg.snap->ps.nextWeapon)) 
		{
			CG_ClearWeapLerpFrame(wi, lf, newAnimation );	// clear when switching to raise (since it should be out of view anyway)
			//forced_client_reloading = qfalse;
		} 
		/*else if((newAnimation&~ANIM_TOGGLEBIT) == PM_ReloadAnimForWeapon(cg.snap->ps.nextWeapon))
		{
			CG_ClearWeapLerpFrame(wi, lf, newAnimation );	// clear when switching to raise (since it should be out of view anyway)
			//forced_client_reloading = qtrue;
		}*/
	...
	if ( cg.time >= lf->frameTime ) {
		...
		//CG_Printf("lf->frame is %i. (anim->numFrames + anim->firstFrame) - 1 is %i. numframes is %i.\n", lf->frame, (anim->numFrames + anim->firstFrame) - 1, anim->numFrames);
		//CG_Printf("Num frames is %i.\n", anim->numFrames);
...
/*	if ( cg.time >= lf->frameTime ) {
		...
		if ( cg.time < lf->animationTime ) 
		{
			...
			lf->frameTime = cg.time + (anim->frameLerp/speedScale);		// initial lerp
		} else {
			lf->frame++;
			lf->frameTime = cg.time + (anim->frameLerp/speedScale);
		}

		if (lf->frame >= (anim->numFrames + anim->firstFrame) - 1)
		{
			//CG_Printf("lf->frame is %i. (anim->numFrames + anim->firstFrame) - 1 is %i. numframes is %i.\n", lf->frame, (anim->numFrames + anim->firstFrame) - 1, anim->numFrames);
			if (forced_client_reloading)
			{
				lf->frame = 0;
				CG_ClearWeapLerpFrame(wi, lf, newAnimation );	// clear when switching to raise (since it should be out of view anyway)
				forced_client_reloading = qfalse;
...
static void CG_CalculateWeaponPosition( vec3_t origin, vec3_t angles ) {
...
#if defined (__FRONTLINE__) || defined (__EF__)
	if( !(cg.predictedPlayerState.eFlags & EF_AIMING) && cg.predictedPlayerState.weapon != WP_MORTAR_SET 
        ...
        && cg.predictedPlayerState.weapon != WP_PTRS_SET
...
#ifdef __EF__
		} else if ( cg.predictedPlayerState.eFlags & EF_MG42_ACTIVE ) {
#else //!__EF__
		} else if ( cg.predictedPlayerState.eFlags & EF_BROWNING_ACTIVE || cg.predictedPlayerState.eFlags & EF_MG42_ACTIVE ) {
#endif //__EF__
	...
	// idle drift
#ifdef __EF__
	if( (!(cg.predictedPlayerState.eFlags & EF_MOUNTEDTANK)) && (cg.predictedPlayerState.weapon != WP_MORTAR_SET) && (cg.predictedPlayerState.weapon != WP_MOBILE_MG42_SET) && (cg.predictedPlayerState.weapon != WP_30CAL_SET) && (cg.predictedPlayerState.weapon != WP_PTRS_SET) ) {
#else //!__EF__
...
#if defined (__FRONTLINE__)
	if ( cent->currentState.constantLight !=0 && !(cent->currentState.eFlags & EF_AIMING) ) {
		return;
	}
#endif //defined (__FRONTLINE__)

#ifdef __VEHICLES__
	if (!((cent->currentState.eFlags & EF_VEHICLE) && cent->currentState.effect1Time == VEHICLE_TYPE_FLAMETANK) 
		&& cent->currentState.weapon != WP_FLAMETHROWER) {
		return;
	}

	if ((cent->currentState.eFlags & EF_VEHICLE) && cent->currentState.effect1Time == VEHICLE_TYPE_FLAMETANK)
	{// Flame tanks use the barrel's flash origin...
		CG_FireFlameChunks( cent, cent->flameTankTurretOrigin, cent->flameTankTurretAngles, 1.0, qtrue );
	}
	else
#endif //__VEHICLES__
	{
...
#ifdef __VEHICLES__
void CG_FlamethrowerTankFlame( centity_t *cent, vec3_t origin )
{
	CG_FlamethrowerFlame(cent, origin);
}
#endif //__VEHICLES__
...
#if defined (__FRONTLINE__) || defined (__EF__)
// draw player weapon on his back
	...
	weaponInfo_t	*backweapon;
		...
		backweap = CG_SecondaryEntityFor(cent->currentState.number)->currentState.weapon;
		...
		if ( backweap ) {   
			if ( 
                ...
                && weaponNum != WP_PTRS_SET
				&& backweap != WP_KNIFE
				&& backweap != WP_LUGER
				&& backweap != WP_COLT
				&& backweap != WP_AKIMBO_COLT
				&& backweap != WP_AKIMBO_LUGER
				&& backweap != WP_AKIMBO_SILENCEDCOLT
				&& backweap != WP_AKIMBO_SILENCEDLUGER
				&& backweap != WP_SILENCED_LUGER
				&& backweap != WP_SILENCED_COLT
#endif //defined (NWEAPS) || defined (__EF__)
			...
			{   
				...
				if ( (character->accModels[ACC_BACK]  || backweap == WP_MORTAR || backweap == WP_FLAMETHROWER) && backweap != WP_MOBILE_MG42 
#if defined (NWEAPS) || defined (__EF__)
					&& backweap != WP_30CAL
                    && backweap != WP_PTRS
#endif //defined (NWEAPS) || defined (__EF__)
...
// looping sounds
		...
		//	cent->pe.lightningFiring = qfalse;
			if (  /*cent->currentState.eFlags & EF_FIRING*/ cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME*2  ) 
			{//3
				if( cg_entities[cg_entities[cg_entities[ cent->currentState.number ].tagParent].tankparent].currentState.density & 8 ) 
				{ // should we use a browning?//4
...
// endif
	...
	if( cent->currentState.eFlags & EF_MG42_ACTIVE || cent->currentState.eFlags & EF_AAGUN_ACTIVE ) 
	{
		...
		{
#ifndef __EF__
			if( ( cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME*2 ) && cent->currentState.eFlags & EF_BROWNING_ACTIVE)
			{ 
				trap_S_AddLoopingSound( cent->lerpOrigin,  vec3_origin, cgs.media.browningloop, 255, 0 );
				cent->pe.lightningFiring = qtrue;
			} 
...
#endif //__EF__
			if ( ( cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME*2 ) && (cent->currentState.eFlags & EF_MG42_ACTIVE)) 
			{
				trap_S_AddLoopingSound( cent->lerpOrigin,  vec3_origin, cgs.media.mg42loop, 255, 0 );
				cent->pe.lightningFiring = qtrue;
			}
	...
	if( ps ) {
		...
		CG_WeaponAnimation( ps, weapon, &gun.oldframe, &gun.frame, &gun.backlerp);	
...
#endif //__EF__
		if (weapon->weaponModel[W_FP_MODEL].skin[team]) {
	...
	if( ps && !cg.renderingThirdPerson && cg.predictedPlayerState.weapon == WP_MORTAR_SET && cg.predictedPlayerState.weaponstate != WEAPON_RAISING ) {
		...
		angles2[YAW] = angles2[ROLL] = 0.f;
		angles2[PITCH] = -.4f * AngleNormalize180( cg.pmext.mountedWeaponAngles[PITCH] - ps->viewangles[PITCH] );

		AnglesToAxis( angles2, gun.axis );
...
#if defined (__FRONTLINE__) || defined (__EF__)
	if ( !ps || cg.renderingThirdPerson ) { // jaquboss , old wont work
		refEntity_t	brass;
...
#ifdef __EF__
			} else if( weaponNum == WP_30CAL_SET ) {
            } else if( weaponNum == WP_PTRS_SET ) {  //Masteries, doesn´t include any code inside{} ??
#endif //__EF__
			}
			...
			if( barrel.hModel ) {
				...
				if( drawpart ) {
					...
					team_t team = ps->persistant[PERS_TEAM];
						...
						if( cent->currentState.powerups & (1 << PW_OPS_DISGUISED) )  { // i dunno why this wasnt here 
							team = team == TEAM_AXIS ? TEAM_ALLIES : TEAM_AXIS;
					...
					if ( weapon->partModels[W_FP_MODEL][i].useCharacterSkin  ){
						bg_character_t	*character2;

						character2 = CG_CharacterForClientinfo( ci, cent );
						barrel.customSkin = character2->weaponskin;

						if (!barrel.customSkin)
...
#endif	 //defined (__FRONTLINE__) || defined (__EF__)
					if (cent->currentState.eFlags & EF_CLOAKED && ((cent->currentState.eFlags & EF_PRONE ) || (cent->currentState.eFlags & EF_CROUCHING)))
						barrel.customShader	= stlth_shader;
					...
					if( weaponNum == WP_SATCHEL_DET && i == W_PART_1 ) {
						...
						if (cent->currentState.eFlags & EF_CLOAKED && ((cent->currentState.eFlags & EF_PRONE ) || (cent->currentState.eFlags & EF_CROUCHING)))
							satchelDetPart.customShader	= stlth_shader;
...
//			}
	...
	// 3rd person attachements
		...
		if( weaponNum == WP_M7 || weaponNum == WP_GPG40/* || weaponNum == WP_CARBINE || weaponNum == WP_KAR98*/ ) {
			...
			CG_AddWeaponWithPowerups( &barrel, cent->currentState.powerups, ps, cent );
#ifdef __EF__
		} else if( (weaponNum == WP_30CAL) || (weaponNum == WP_PTRS) ) {
			barrel.hModel = weapon->modModels[0];
			barrel.frame = 1;
			CG_PositionEntityOnTag( &barrel, &gun, "tag_bipod", 0, NULL);
			CG_AddWeaponWithPowerups( &barrel, cent->currentState.powerups, ps, cent );
		} else if( (weaponNum == WP_30CAL_SET) || (weaponNum == WP_PTRS_SET) ) {
			barrel.hModel = weapon->modModels[0];
			barrel.frame = 0;
			CG_PositionEntityOnTag( &barrel, &gun, "tag_bipod", 0, NULL);
			CG_AddWeaponWithPowerups( &barrel, cent->currentState.powerups, ps, cent );
...
#define BARREL_SMOKE_TIME 1000
		...
		if ( ps || cg.renderingThirdPerson || !isPlayer ) {
#ifdef __EF__
			if( weaponNum == WP_PPSH && ( nonPredictedCent->currentState.eFlags & EF_FIRING) )
			{
				if(Q_irand(0,2) == 2) 
				{
					float alpha;
			...
			else if( weaponNum == WP_STEN || weaponNum == WP_MOBILE_MG42 || weaponNum == WP_MOBILE_MG42_SET || weaponNum == WP_30CAL || weaponNum == WP_30CAL_SET  || weaponNum == WP_PTRS || weaponNum == WP_PTRS_SET ) {
#else //!__EF__
...
if ( ps || cg.renderingThirdPerson || !isPlayer  ) {
		if( weaponNum == WP_KAR98 ||
			...
			if ( cg.time - cent->muzzleFlashTime < 100 )
				...
				CG_ParticleImpactSmokePuffExtended(cgs.media.smokeParticleShader, flash.origin, 600, 8, 20, 30, 0.30f, 15.f);
		}
	...
	if ( ps || cg.renderingThirdPerson || !isPlayer) {
		if( weaponNum == WP_MORTAR_SET ||
			...
			weaponNum == WP_PANZERFAUST)
			if ( cg.time - cent->muzzleFlashTime < 100 )
				
				CG_ParticleImpactSmokePuffExtended(cgs.media.smokeParticleShader, flash.origin, 1000, 8, 20, 30, 0.50f, 20.f);
		}
#endif //defined (__FRONTLINE__) || defined (__EF__)
...
#ifndef __FLASH_CORONAS__
			// RF, changed this so the muzzle flash stays onscreen for long enough to be seen
			if ( cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME ) {
			//if (firing) {	// Ridah
				trap_R_AddRefEntityToScene( &flash );
			}
#else //__FLASH_CORONAS__
#define FLASH_CORONA_TIME 500
#define FLASH_CORONA_SIZE 1//100

			//if ( cent->corona_flash_time > cg.time )
			if ( cg.time - cent->muzzleFlashTime < CORONA_FLASH_TIME )
			{// Continue drawing flash corona...
				if ( weapon->flashDlightColor[0] || weapon->flashDlightColor[1] || weapon->flashDlightColor[2] )
				{// UQ1: I want to draw a corona as well...
					float frac = (float)(Q_TrueRand(30,80) * 0.01);//((cent->corona_flash_time - cg.time) / FLASH_CORONA_TIME);

					frac *= 2;

					//CG_Printf("1 - frac %f.\n", frac);
					
					trap_R_AddFlashEffectToScene( flash.origin, 
						frac,//(float)((float)FLASH_CORONA_SIZE*frac),
						1.25 + (rand() & 31) / 128, weapon->flashDlightColor[0], weapon->flashDlightColor[1], weapon->flashDlightColor[2], 0, 0 /*REF_JUNIOR_DLIGHT*/, cent);
				...
				else
				{
					...
					trap_R_AddFlashEffectToScene( flash.origin, 
						frac,//(float)((float)FLASH_CORONA_SIZE*frac),
						1.25 + (rand() & 31) / 128, 1.0f, 0.5f, 0.0f, 0, 0 /*REF_JUNIOR_DLIGHT*/, cent);

					if ( cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME )
						trap_R_AddRefEntityToScene( &flash );
				}
			}
#endif //__FLASH_CORONAS__
		}
	}
	...
	if( weaponNum == WP_FLAMETHROWER )  { 
		...
		vec3_t	gorg, porg, norm;
		float	gdist;
		trace_t	tr;

		trap_R_AddLightToScene2( flash.origin, 48, 25 + (rand() & 130) / 32, 0, 0, .75 , 0, 0 );
		...
		if(gdist < cg_coronafardist.integer || (isPlayer && !cg.renderingThirdPerson)) { 
			if ( cent->currentState.constantLight == 0 || cent->currentState.eFlags & EF_AIMING){  
				if (cent->currentState.eFlags & EF_FIRING ) {  
					trap_R_AddCoronaToScene( flash.origin, 0.8, 0.8, 0.2, 6, cent->currentState.number, visible);
				} else {  
					trap_R_AddCoronaToScene( flash.origin, 0.2, 0.2, 1, 3, cent->currentState.number, visible);
				}   
			} else {  
			trap_R_AddCoronaToScene( flash.origin, 0.2, 0.2, 1, 3, cent->currentState.number, visible);
					}  
	...
	if ( ps || cg.renderingThirdPerson || !isPlayer )
	{
		...
		if( firing && !(cent->currentState.eFlags & (EF_PRONE_MOVING | EF_DEAD)) )
		{
			...
			case WP_NONE:									// 0
			case WP_KNIFE:									// 1
			case WP_GRENADE_LAUNCHER:		// 4
			//WP_PANZERFAUST:				// 5
			//WP_FLAMETHROWER:				// 6
			case WP_GRENADE_PINEAPPLE:		// 9
			case WP_STEN:									// 10	// silenced sten sub-machinegun
			case WP_MEDIC_SYRINGE:					// 11	// JPW NERVE -- broken out from CLASS_SPECIAL per Id request
			case WP_AMMO:								// 12	// JPW NERVE likewise
			case WP_ARTY:									// 13
			case WP_SILENCED_LUGER:							// 14	// used to be sp5
			case WP_DYNAMITE:							// 15
			case WP_SMOKETRAIL:						// 16
			case WP_MAPMORTAR:						// 17	
			case VERYBIGEXPLOSION:					// 18	// explosion effect for airplanes
			case WP_MEDKIT:								// 19
			case WP_BINOCULARS:						// 20
			case WP_PLIERS:									// 21
			case WP_SMOKE_MARKER:				// 22	// Arnout: changed name to cause less confusion
			case WP_LANDMINE:							// 26
			case WP_SATCHEL:								// 27
			case WP_SATCHEL_DET:						// 28
			case WP_TRIPMINE:							// 29
			case WP_SMOKE_BOMB:					// 30
	//		case WP_DUMMY_MG42:					// 34 // Gordon: for storing heat on mounted mg42s...
			case WP_MORTAR:								// 35
			case WP_LOCKPICK:							// 36	// Mad Doc - TDF lockpick
#ifdef __EF__
			case WP_SILENCED_COLT:					// 46
			case WP_MORTAR_SET:						// 50
			case WP_MEDIC_ADRENALINE:			// 51
			case WP_AKIMBO_SILENCEDCOLT:	// 52
			case WP_AKIMBO_SILENCEDLUGER:// 53
			case WP_AIRDROP_MARKER:
#else //!__EF__
// Gordon: ONLY secondaries below this mark, as they are checked >= WP_GPG40 && < WP_NUM_WEAPONS
			case WP_SILENCED_COLT:					// 41
			case WP_MORTAR_SET:						// 45
			case WP_MEDIC_ADRENALINE:			// 46
			case WP_AKIMBO_SILENCEDCOLT:	// 47
			case WP_AKIMBO_SILENCEDLUGER:// 48
#endif //__EF__
				break;
			default:
				if (!(weapon->flashDlightColor[0] || weapon->flashDlightColor[1] || weapon->flashDlightColor[2]))
				{
					MAKERGB( weapon->flashDlightColor, 1, 0.5, 0 );
				}
				break;
			}
			...
			if ( weapon->flashDlightColor[0] || weapon->flashDlightColor[1] || weapon->flashDlightColor[2] )
			{
				trap_R_AddLightToScene( flash.origin, 128, 1.25 + (rand() & 31) / 128, weapon->flashDlightColor[0], weapon->flashDlightColor[1], weapon->flashDlightColor[2] , 0, REF_JUNIOR_DLIGHT ); // light up the models as well
				...
				if ( tr.fraction == 1 )
				{
					if ( gdist < 8192 )
					{
						...
						//						 weapon->flashDlightColor[2], 6, cent->currentState.number, qtrue );

						// UQ1: I want to draw a corona as well...
						//trap_R_AddCoronaToScene( origin, rgb[0], rgb[1], rgb[2], 2, cg.predictedPlayerEntity.currentState.number, qtrue);
						CG_AddFlashLighting( flash.origin, weapon->flashDlightColor );
					}
				}
			}
#else //__FLASH_CORONAS__
			if ( cg.time - cent->muzzleFlashTime < CORONA_FLASH_TIME || !flash.hModel )
			{
			...
			{// UQ1: I want to draw a corona as well...
				...
				cent->corona_flash_time = cg.time + FLASH_CORONA_TIME;
				cent->muzzleFlashTime = cg.time;

				frac = (( cent->corona_flash_time - cg.time) / FLASH_CORONA_TIME);

				trap_R_AddFlashEffectToScene( flash.origin, 
					(float)((float)FLASH_CORONA_SIZE*frac),
					1.25 + (rand() & 31) / 128, weapon->flashDlightColor[0], weapon->flashDlightColor[1], weapon->flashDlightColor[2], 0, 0 /*REF_JUNIOR_DLIGHT*/, cent);

				if(Q_TrueRand(0,5) == 1) 
				{// And add a little smoke as well :)
					float alpha;
					alpha = 0.5f;
					alpha *= 0.25f;		// .25 max alpha
					CG_ParticleImpactSmokePuffExtended(cgs.media.smokeParticleShader, flash.origin, 1000, 8, 20, 30, alpha, 8.f);
		...
		{
			vec3_t angles2;
			AxisToAngles( flash.axis, angles2 );
			weaponNum = BG_FindAmmoForWeapon(WP_FLAMETHROWER);

			if (ps)
			{
				if (ps->ammoclip[weaponNum])
					CG_FireFlameChunks( cent, flash.origin, angles2, 1.0, qfalse );
			}
			else 
			{
				CG_FireFlameChunks( cent, flash.origin, angles2, 1.0, qfalse );
...
float CG_GetAds( int weapon , int axis ) {
...
/*	if ( cg_weapons[weapon].ADSPos[axis] ) {
	...
	if (axis == 0){ // X
		switch(weapon){
#if defined (NEWAPS) || defined (__EF__)
#ifdef __EF__
	case WP_STG44:
#else //!__EF__
	case WP_MP44:
	...
	case WP_30CAL:
		return -10;
		break;
    case WP_PTRS_SET://Masteries. To do, fix this to the correct value
	case WP_PTRS:
		return -10;
		break;
#endif //defined (NWEAPS) || defined (__EF__)
	...
	case WP_30CAL_SET:
		return -4.4;
		break;
    case WP_PTRS://Masteries, to do, fix this to their correct value
		return 13.3;
		break;
	case WP_PTRS_SET:
		return -4.4;
	...
	case WP_30CAL:
		return 6.8;
		break;
    case WP_PTRS_SET: //Masteries, to do, fix this to their correct value
		return 11.2;
		break;
	case WP_PTRS:
		return 6.8;
...
#if defined (__FRONTLINE__)
		||  ps->eFlags & EF_MG42_ACTIVE || ps->eFlags & EF_BROWNING_ACTIVE 
#else //!defined (__FRONTLINE__)
		...
		) 
	{
		...
		{ // should we use a browning?
			...
			for( i = 0; i < 3; i++ ) { //for 
				...
				if( ps->eFlags & EF_AIMING) { //aiming 
					f2 = ( cg.time - cg.zoomTime ) / (float)300;

						if ( f2 > 1.0 ) { 
							gunoff_w[i] = adspos[i];
						} else { 
							gunoff_w[i] = gunoff[i] + f2 * ( adspos[i] - gunoff[i] ) ;
						} 
					...
					f2 = ( cg.time - cg.zoomTime ) / (float)300;
						...
						if ( f2 > 1.0 ) { 
							...
							gunoff_w[i] = adspos[i] + f2 * ( gunoff[i] - adspos[i]);
						}  
		...
		if ( !(ps->eFlags & EF_MOUNTEDTANK ) ) { // not a tank
				gunoff[1] = adspos[1];
				if (!(ps->eFlags & EF_AIMING) ) { // not aiming
					if (ps->eFlags & EF_MG42_ACTIVE ) { // mg42
...
#ifdef __FLASH_CORONAS__
			// UQ1: I want to draw a corona as well...
			trap_R_AddCoronaToScene( flash.origin, 0.8f, 0.8f, 0.1f, 2, cg.predictedPlayerEntity.currentState.number, qtrue);
#endif //__FLASH_CORONAS__
		}
		return;
	}
...
#define ADS_DELAY 300
		...
		for ( j=0; j < 3; j++){
			...
			if( ps->eFlags & EF_AIMING ) {		
				gun[j] = CG_GetAds(ps->weapon , j ) - gunpos[j];
				f = ( cg.time - cg.zoomTime ) / (float)ADS_DELAY;

					if ( f > 1.0 ) {
						gunoff_w[j] = gun[j];
					} else {
						gunoff_w[j] = gunoff[j] + f * ( gun[j] - gunoff[j] );
					}
				...
				f = ( cg.time - cg.zoomTime ) / (float)ADS_DELAY;
					...
					if ( f > 1.0 ) {
						...
						gunoff_w[j] = gun[j] + f * ( gunoff[j] - gun[j]);
					}
		...
		gunoff[0] =  gunoff_w[0] + gunpos[0] + cg_gun_x.value;
		gunoff[1] =  gunoff_w[1] + gunpos[1] + cg_gun_y.value;
		gunoff[2] =  gunoff_w[2] + gunpos[2] + cg_gun_z.value;

		if ( gunoff[0] == CG_GetAds(ps->weapon , 0 ) && gunoff[1] == CG_GetAds(ps->weapon , 1 )	&& gunoff[2] == CG_GetAds(ps->weapon , 2 )	){
...
#define AIMING
/*
		if (ps->eFlags & EF_AIMING && ps->weapon == WP_LUGER)
		{// Special case for WP_LUGER...
			hand.renderfx = RF_DEPTHHACK | RF_FIRST_PERSON | RF_MINLIGHT;
		...
		else if (ps->eFlags & EF_AIMING && ps->weapon == WP_COLT)
		{// Special case for WP_COLT...
			hand.renderfx = RF_DEPTHHACK | RF_FIRST_PERSON | RF_MINLIGHT;
		...
		else if (ps->eFlags & EF_AIMING && ps->weapon == WP_THOMPSON)
		{// Special case for WP_THOMPSON...
			hand.renderfx = RF_DEPTHHACK | RF_FIRST_PERSON | RF_MINLIGHT;
			...
			gunoff[1] = 7.7f;
			gunoff[2] = 5.0f;
			//gunoff[2] = cg_gun_z.value;//4.6f;
		}
		else if (ps->eFlags & EF_AIMING && ps->weapon == WP_MP40)
		{// Special case for WP_MP40...
			hand.renderfx = RF_DEPTHHACK | RF_FIRST_PERSON | RF_MINLIGHT;
		...
		else if (ps->eFlags & EF_AIMING && ps->weapon == WP_STEN)
		{// Special case for WP_STEN...
			hand.renderfx = RF_DEPTHHACK | RF_FIRST_PERSON | RF_MINLIGHT;
		...
		else if (ps->eFlags & EF_AIMING && ps->weapon == WP_PANZERFAUST)
		{// Special case for WP_PANZERFAUST...
			hand.renderfx = RF_DEPTHHACK | RF_FIRST_PERSON | RF_MINLIGHT;
		...
		else if (ps->eFlags & EF_AIMING && ps->weapon == WP_MOBILE_MG42)
		{// Special case for WP_MOBILE_MG42...
			hand.renderfx = RF_DEPTHHACK | RF_FIRST_PERSON | RF_MINLIGHT;
		...
		else if (ps->eFlags & EF_AIMING && ps->weapon == WP_CARBINE)
		{// Special case for WP_CARBINE...
			hand.renderfx = RF_DEPTHHACK | RF_FIRST_PERSON | RF_MINLIGHT;
		...
		else if (ps->eFlags & EF_AIMING && ps->weapon == WP_KAR98)
		{// Special case for WP_KAR98...
			hand.renderfx = RF_DEPTHHACK | RF_FIRST_PERSON | RF_MINLIGHT;
		...
		else if (ps->eFlags & EF_AIMING && ps->weapon == WP_FG42)
		{// Special case for WP_FG42...
			hand.renderfx = RF_DEPTHHACK | RF_FIRST_PERSON | RF_MINLIGHT;
...
#define ADS_DELAY 300
		...
		switch ( ps->weapon ) 
		{ // just for now, PoP will have something better 
			case WP_LUGER:
				adspos[0] = -4.0f;
				adspos[1] = 9.0f;
				adspos[2] = 8.6f;
		...
		if( ps->eFlags & EF_AIMING) 
		{  // to even better fit old code, again will be gone later ..
			switch ( ps->weapon ) 
			{ 
				case WP_LUGER:
					angles[0]+=-4.0f;
...
void CG_CheckClassSpecificTools( clientInfo_t *ci ) 
{
	int	pc;
	qboolean add_binocs = qfalse;
	pc = ci->cls;
	
	if(ci->skill[SK_BATTLE_SENSE] >= 1)
		add_binocs = qtrue;
  
	switch(pc) 
	{
		case PC_ENGINEER:
			if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_DYNAMITE ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_DYNAMITE );

			if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_PLIERS ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_PLIERS );

			if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_LANDMINE ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_LANDMINE );
			break;
		case PC_COVERTOPS:
			add_binocs = qtrue;
			if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_SMOKE_BOMB ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_SMOKE_BOMB );

			if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_SATCHEL ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_SATCHEL );

			if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_SATCHEL_DET ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_SATCHEL_DET );

			if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_AIRDROP_MARKER ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_AIRDROP_MARKER );

			break;
		case PC_FIELDOPS:
			add_binocs = qtrue;
			if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_AMMO ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_AMMO );

			if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_SMOKE_MARKER ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_SMOKE_MARKER );
			break;
		case PC_MEDIC:
			if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_MEDKIT ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_MEDKIT );

			if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_MEDIC_SYRINGE ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_MEDIC_SYRINGE );

			if( ci->skill[SK_FIRST_AID] >= 4 )
			{
				if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_MEDIC_ADRENALINE ) ) )
	...
	if(add_binocs) 
	{
		if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_BINOCULARS ) ) )
				COM_BitSet(cg.predictedPlayerState.weapons, WP_BINOCULARS );

		if (!(cg.predictedPlayerState.stats[STAT_KEYS] & ( 1 << INV_BINOCS )))
				cg.predictedPlayerState.stats[STAT_KEYS] |= ( 1 << INV_BINOCS );
  	}
...
/*	if (ci->cls == PC_MEDIC)
	{// UQ1: Hack to fix WP_MEDKIT not being transmitted for some reason...
		if (!(COM_BitCheck( cg.predictedPlayerState.weapons, WP_MEDKIT ) ) )
			COM_BitSet(cg.predictedPlayerState.weapons, WP_MEDKIT );
	}*/
/*
#ifdef __EF__
	// UQ1: Hack to fix some class weapons not being transmitted for some reason...
	CG_CheckClassSpecificTools(ci);
#endif //__EF__
*/
	if( BG_PlayerMounted( cg.predictedPlayerState.eFlags ) ) {
		//CG_Printf("%i (%s) - Player is mounted!\n", i, WeaponTableStrings[i].name);
		return qfalse;
	}

	// check for weapon
	if (! (COM_BitCheck( cg.predictedPlayerState.weapons, i ) ) ) {
		//CG_Printf("%i (%s) - Failed bit check!\n", i, WeaponTableStrings[i].name);
		return qfalse;
	}

	if(!CG_WeaponHasAmmo(i))
	{
		//CG_Printf("%i (%s) - Weapon has no ammo!\n", i, WeaponTableStrings[i].name);
		return qfalse;
	}
	...
	/*{
		...
		CG_Printf("Have the following weapons:\n");

		for (j = 0; j < WP_NUM_WEAPONS; j++)
		{
			if ((COM_BitCheck( cg.predictedPlayerState.weapons, j ) ) ) 
			{
				CG_Printf("%i (%s) ", j, WeaponTableStrings[j].name);
				if (CG_WeaponHasAmmo(j))
					CG_Printf("[has ammo] ");
			}
		}
...
int CG_WeaponIndex( int weapnum, int *bank, int *cycle) {
	...
	if( cg_panzerhack.integer && (
		...
		weapBanksMultiPlayer[3][1] = 0;
		weapBanksMultiPlayer[3][0] = 0;
#else //!__EF__
		weapBanksMultiPlayer[2][6] = WP_MP40;
		weapBanksMultiPlayer[2][7] = WP_THOMPSON;
		...
		weapBanksMultiPlayer[3][0] = WP_THOMPSON;
		weapBanksMultiPlayer[3][1] = WP_MP40;
#else //!__EF__
		weapBanksMultiPlayer[2][6] = 0;
		weapBanksMultiPlayer[2][7] = 0;
...
void CG_ListWeaponBanks ( void )
{
	int bnk, cyc;

	for(bnk = 0; bnk < MAX_WEAP_BANKS_MP; bnk++) 
	{
		CG_Printf("Bank: %i: ", bnk);
		...
		for(cyc = 0; cyc < MAX_WEAPS_IN_BANK_MP; cyc++) 
		{
				...
				if (weapBanksMultiPlayer[bnk][cyc] == 0)
					...
					CG_Printf("%i (%s) ", cyc, WeaponTableStrings[weapBanksMultiPlayer[bnk][cyc]].name);
		}
...
void CG_ListWeapons ( void )
{
	int bnk;
	
	for(bnk = 0; bnk < MAX_WEAPONS; bnk++) 
	{
		if (! (COM_BitCheck( cg.snap->ps.weapons/*cg.predictedPlayerState.weapons*/, bnk ) ) ) 
		{
			continue;
		}

		CG_Printf("Weapon: %i: ", bnk);
		CG_Printf("(%s) ", WeaponTableStrings[bnk].name);
		CG_Printf("\n");
	}
}
...
static int getNextWeapInBank( int bank, int cycle ) 
{
	...
	if (cycle >= MAX_WEAPS_IN_BANK_MP)
		cycle = 0;

	//CG_Printf("Trying cycle %i.\n", cycle);
...
static int getNextWeapInBankBynum( int weapnum ) 
{
	...
	{
		//CG_Printf("No weaponindex for %i.\n", weapnum);
		return weapnum;
	}
...
void CG_FinishWeaponChange(int lastweap, int newweap) {
	...
	switch( newweap ) {
	case WP_LUGER:
		if((cg.pmext.silencedSideArm & 1) && lastweap != WP_SILENCED_LUGER) {
			newweap = WP_SILENCED_LUGER;
...
qboolean CG_CanSetupMobileMG42Here ( centity_t *cent )
{// UQ1: Testing to see if we can mount mg42 on objects while standing...
	...
	{
		if( cg.predictedPlayerState.eFlags & EF_CROUCHING )
			muzzlePoint[2] += 10;
		else
			muzzlePoint[2] += 24;
	}
...
void CG_AltWeapon_f(void)
{
	...
	if( cg.weaponSelect == WP_MORTAR ) {
		...
		if( !(cg.predictedPlayerState.eFlags & EF_PRONE) && !CG_CanSetupMobileMG42Here(NULL) ) {
			return;
		}
#ifdef __EF__
	} else if( cg.weaponSelect == WP_30CAL ) {
		if( !(cg.predictedPlayerState.eFlags & EF_PRONE) && !CG_CanSetupMobileMG42Here(NULL) ) {
			return;
		}
	} else if( cg.weaponSelect == WP_PTRS ) {
		if( !(cg.predictedPlayerState.eFlags & EF_PRONE) && !CG_CanSetupMobileMG42Here(NULL) ) {
			return;
		}
#endif //__EF__
...
#ifdef __WEAPON_AIM__
	if ( BG_Weapon_Is_Aimable(original) 
		&& (original == WP_LUGER && cg.predictedPlayerState.stats[STAT_PLAYER_CLASS] == PC_COVERTOPS))
	{
#ifdef __BOT__
		if (CG_BOT_FireTeam_Check_Orders())
		{// Check for fireteam orders.. Skip secondary fire code if needed..
			return;
		}
#endif //__BOT__
	}
	else if ( BG_Weapon_Is_Aimable(original) 
		&& (original == WP_COLT && cg.predictedPlayerState.stats[STAT_PLAYER_CLASS] == PC_COVERTOPS))
	{
#ifdef __BOT__
		if (CG_BOT_FireTeam_Check_Orders())
		{// Check for fireteam orders.. Skip secondary fire code if needed..
...
#ifdef __BOT__
		if (!CG_BOT_FireTeam_Check_Orders())
		{// Transmit AIM message to the server so other clients know...
			trap_SendConsoleCommand( "aim\n" );
		}
		else
		{// Skip secondary fire code if giving orders..
			return;
		}
#else //!__BOT__
		// Transmit AIM message to the server so other clients know...
	...
	if ( original == WP_MOBILE_MG42 && !(cg.predictedPlayerState.eFlags & EF_PRONE) && CG_CanSetupMobileMG42Here(NULL) )
	{
		// And transmit to the server so other clients know...
		trap_SendConsoleCommand( "setupmg42\n" );
		return;
	}
#ifdef __EF__
	else if ( original == WP_30CAL && !(cg.predictedPlayerState.eFlags & EF_PRONE) && CG_CanSetupMobileMG42Here(NULL) )
	{
		// And transmit to the server so other clients know...
		trap_SendConsoleCommand( "setupbrowning\n" );
		return;
	}
//Masteries
#ifdef __SCREW_THIS__
// UQ1: Bah! Can't be botherred with this mounted PTRS right now... Too much work...
    else if ( original == WP_PTRS && !(cg.predictedPlayerState.eFlags & EF_PRONE) && CG_CanSetupMobileMG42Here(NULL) )
	{
		// And transmit to the server so other clients know...
		trap_SendConsoleCommand( "setupptrs\n" );
		return;
	}
	...
	if( ( cg.snap->ps.weaponstate == WEAPON_RAISING || cg.snap->ps.weaponstate == WEAPON_DROPPING ) &&
		( (original == WP_GPG40 || num == WP_GPG40 || original == WP_M7 || num == WP_M7) ||
		  (original == WP_SILENCED_LUGER || num == WP_SILENCED_LUGER || original == WP_SILENCED_COLT || num == WP_SILENCED_COLT) ||
          ...
          (original == WP_PTRS_SET || num == WP_PTRS_SET) ||
#endif //__EF__
...
void CG_NextWeap(qboolean switchBanks) {
	...
	//CG_Printf("DEBUG: NextWeapon.\n");

	num = curweap = cg.weaponSelect;

#ifdef __EF__
	if( curweap == WP_MORTAR_SET || curweap == WP_MOBILE_MG42_SET || curweap == WP_30CAL_SET  || curweap == WP_PTRS_SET )
#else //!__EF__
				...
				switch( num ) {
					case WP_CARBINE:
						if ((found = CG_WeaponSelectable( WP_M7 ))) {
							if (CG_WeaponSelectable( WP_CARBINE ))
								num = WP_M7;
						}
						break;
					case WP_KAR98:
						if ((found = CG_WeaponSelectable( WP_GPG40 ))) {
							if (CG_WeaponSelectable( WP_KAR98 ))
								num = WP_GPG40;
						}
						break;
				}
...
void CG_LastWeaponUsed_f( void ) {
	...
	//CG_Printf("DEBUG: LastWeaponUsed.\n");
...
#ifdef __EF__
	if( cg.weaponSelect == WP_MORTAR_SET || cg.weaponSelect == WP_MOBILE_MG42_SET || cg.weaponSelect == WP_30CAL_SET  || cg.weaponSelect == WP_PTRS_SET ) {
#else //!__EF__
...
void CG_WeaponBank_f(void) {
	int	num = 0, /*i,*/ curweap = 0, initial_weap = 0;
	int curbank = 0, curcycle = 0, bank = 0, cycle = 0;
	qboolean	skip_check = qfalse;

	if (!cg.snap)
		return;

	//CG_Printf("DEBUG: WeaponBank.\n");
...
/*	CG_WeaponIndex(curweap, &curbank, &curcycle);		// get bank/cycle of current weapon
	...
	// UQ1: OMFG!!! That code is a joke! Let's do this smart!
	CG_WeaponIndex(curweap, &curbank, &curcycle);		// get bank/cycle of current weapon

	if( bank != curbank ) 
	{
		cycle = cg.lastWeapSelInBank[bank];

		if (cycle < 0 || cycle >= WP_NUM_WEAPONS || !CG_WeaponSelectable( cycle ))
		{// Need to initialize...
			cycle = weapBanksMultiPlayer[bank][0];
			cg.lastWeapSelInBank[bank] = cycle;
		}

		initial_weap = cycle;

		if (CG_WeaponSelectable( cycle ))
		{
			num = cycle;
			skip_check = qtrue;
		}
	...
	while (!skip_check)
	{
		...
		cycle = getNextWeapInBankBynum(cycle);

		if (cycle == original || cycle == initial_weap)
		{// Have no weapons for this bank!
			return;
		}
	...
	if( ( cg.snap->ps.weaponstate == WEAPON_RAISING || cg.snap->ps.weaponstate == WEAPON_DROPPING ) &&
		( (curweap == WP_GPG40 || num == WP_GPG40 || curweap == WP_M7 || num == WP_M7) ||
		  (curweap == WP_SILENCED_LUGER || num == WP_SILENCED_LUGER || curweap == WP_SILENCED_COLT || num == WP_SILENCED_COLT) ||
		  (curweap == WP_MORTAR_SET || num == WP_MORTAR_SET ) ) )
		return;

	CG_FinishWeaponChange(curweap, num);

	cg.lastWeapSelInBank[bank] = num;
}
...
void CG_OutOfAmmoChange( qboolean allowforceswitch ) {
	...
	if( allowforceswitch ) {
		if( cg.weaponSelect == WP_SMOKE_BOMB ) {
			...
			return;
		} else if( cg.weaponSelect == WP_PTRS_SET ) {
...
void CG_DrawHitBoxOnHit( centity_t *cent ) {
	...
	if ( trace.fraction < 1.0f && (trace.contents & CONTENTS_BODY) ) {
		...
		if ( cg.nextSnap ) {
			centity_t *c2 = &cg_entities[trace.entityNum];
			...
			CG_Printf("^3Int: time: %d, j: %d, k: %d, origin: %0.2f %0.2f %0.2f\n",
					...
					c2->lerpOrigin[0], c2->lerpOrigin[1], c2->lerpOrigin[2]);
...
void CG_FirePtrsTank( centity_t *cent ) {//Masteries, this function is only for audio purpose?
		trap_S_StartSound( NULL, cent->currentState.number, CHAN_WEAPON, cgs.media.hWeaponSnd );
		cent->muzzleFlashTime2 = cg.time;
	}
...
void CG_FireWeapon( centity_t *cent ) {
	...
	if( cent->currentState.eFlags & EF_VEHICLE ) 
	{
		...
		{
			trap_S_StartSound( NULL, cent->currentState.number, CHAN_WEAPON, cgs.media.flameBlowSound );
			trap_S_StartSound( NULL, cent->currentState.number, CHAN_ITEM, cgs.media.flameStreamSound );
		}
		else//Masteries, probably panzer2 ptrs sound effect needs to go here
		...
		if ( c > 0 ) {
			...
			if ( firesound[c] ) {
				...
				if(fireEchosound && fireEchosound[c])	// check for echo
				{
					...
					cent2 = &cg_entities[ent->number];
					VectorCopy(cent2->currentState.pos.trBase, gorg);
					VectorCopy(cg.refdef_current->vieworg, porg);
					VectorSubtract(gorg, porg, norm);
					gdist = VectorNormalize(norm);
...
// check for water/dirt spurt
	...
	case WP_MORTAR_SET:
		...
		if( weapon == WP_SATCHEL ) {
			...
			{
				sfx =		cgs.media.sfx_grenexp2;
				sfx2 =		cgs.media.sfx_grenexpDist2;
			}
			else if (choice == 2)
			{
				sfx =		cgs.media.sfx_grenexp3;
				sfx2 =		cgs.media.sfx_grenexpDist3;
			}
			else
			{
				sfx =		cgs.media.sfx_grenexp4;
				sfx2 =		cgs.media.sfx_grenexpDist4;
			}
#endif //__ETE__
		} else {
			sfx =		cgs.media.sfx_rockexp;
...
#ifdef __PARTICLE_SYSTEM__
		CG_DrawGrenadeParticles( cent, origin );
#endif //__PARTICLE_SYSTEM__
		break;
	case WP_KNIFE:
...
==============
*/
void AddLean(playerState_t *ps, vec3_t up, vec3_t point)
{
	if(ps->leanf)
	{
		vec3_t	right;
		AngleVectors(ps->viewangles, NULL, right, NULL);
		VectorMA(point, ps->leanf, right, point);
#if defined (__FRONTLINE__) || defined (__EF__)
		VectorMA(point, abs(ps->leanf)*(-0.33), up, point);
#endif //defined (__FRONTLINE__) || defined (__EF__)
	}
}
...
//	int			anim;
		...
		) 
	{
		...
		if( cg.snap->ps.eFlags & EF_MG42_ACTIVE ) {
			...
			AngleVectors ( cg.snap->ps.viewangles, forward2, NULL, NULL );
			VectorMA ( mg42->currentState.pos.trBase, 40, forward2, muzzle );	// wsa -36, made 40 to be in sync with the actual muzzleflash drawing
			...
			AngleVectors ( cg.snap->ps.viewangles, forward2, right2, up2 );
			...
			BG_AdjustAAGunMuzzleForBarrel( muzzle, forward2, right2, up2, (aagun->currentState.modelindex2 + 3) % 4 );
...
#ifdef __EF__
			} else if( (cg.snap->ps.weapon == WP_30CAL_SET) || (cg.snap->ps.weapon == WP_PTRS_SET) ) {
				VectorMA( muzzle, 36, forward, muzzle );
#endif //__EF__
			} else {
				VectorMA( muzzle, 14, forward, muzzle );
...
//		int			num;
		...
		// ydnar: this is silly--the entity is a mg42 barrel, so just use itself
			...
			if( cent->currentState.eType == ET_MG42_BARREL )
			{
				...
				AngleVectors( cent->lerpAngles, forward2, NULL, NULL );
				VectorMA( muzzle, 40, forward2, muzzle );
				muzzle[ 2 ] += DEFAULT_VIEWHEIGHT;
			}
		...
		VectorCopy( tank->mountedMG42Flash.origin, muzzle );
#ifdef __VEHICLES__  //some changes here by Masteries, not completed
	} else if( cent->currentState.eFlags & EF_VEHICLE ) {
				VectorCopy( cent->currentState.pos.trBase, muzzle );
				...
				} else if (cent->currentState.effect1Time == VEHICLE_TYPE_PANZER){
				VectorMA( muzzle, 32, right, muzzle );
				} else if ( cent->currentState.effect1Time == VEHICLE_TYPE_KV1) {
				VectorMA( muzzle, -16, right, muzzle );
				} else if ( cent->currentState.effect1Time == VEHICLE_TYPE_FLAMETANK) {
				VectorMA( muzzle, -16, right, muzzle );
				} else if ( cent->currentState.effect1Time == VEHICLE_TYPE_PANZERIV) {
				VectorMA( muzzle, -16, right, muzzle );
				} else if ( cent->currentState.effect1Time == VEHICLE_TYPE_TIGERTANK) {
				VectorMA( muzzle, -16, right, muzzle );
				} else if ( cent->currentState.effect1Time == VEHICLE_TYPE_PANZER2) {
				VectorMA( muzzle, -16, right, muzzle );
				} else { // VEHICLE_TYPE_STUG3
                VectorMA( muzzle, 32, right, muzzle );
				}
				muzzle[ 2 ] += 48;
#endif //__VEHICLES__
		...
		AngleVectors( cent->currentState.apos.trBase, forward, right, up );
#ifdef __EF__
		if( cent->currentState.eFlags & EF_PRONE || (cent->currentState.weapon == WP_MOBILE_MG42_SET && CG_CanSetupMobileMG42Here(cent))  || (cent->currentState.weapon == WP_30CAL_SET && CG_CanSetupMobileMG42Here(cent)) || (cent->currentState.weapon == WP_PTRS_SET && CG_CanSetupMobileMG42Here(cent)) ) {
#else //!__EF__
		if( cent->currentState.eFlags & EF_PRONE || (cent->currentState.weapon == WP_MOBILE_MG42_SET && CG_CanSetupMobileMG42Here(cent)) ) {
#endif //__EF__
			muzzle[2] += PRONE_VIEWHEIGHT;
			if( cent->currentState.weapon == WP_MOBILE_MG42_SET )
				VectorMA( muzzle, 36, forward, muzzle );
#ifdef __EF__
			else if( (cent->currentState.weapon == WP_30CAL_SET) || (cent->currentState.weapon == WP_PTRS_SET) )
				VectorMA( muzzle, 36, forward, muzzle );
#endif //__EF__
			else
				VectorMA( muzzle, 14, forward, muzzle );
...
// jpw
		...
		if( CG_CalcMuzzlePoint( sourceEntityNum, start ) || cg.snap->ps.persistant[PERS_HWEAPON_USE]) {
			if( waterfraction ) {
				...
				CG_MissileHitWall( fromweap, 2, end2, dir2, 0 );
