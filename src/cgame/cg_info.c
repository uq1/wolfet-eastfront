...
#define FONT_HEADER			/*&cgs.media.limboFont1*/&cgs.media.tahomabd24
#define FONT_SUBHEADER		&/*cgs.media.limboFont1*/cgs.media.tahoma16
#define FONT_TEXT			&/*cgs.media.limboFont2*/cgs.media.tahoma30
...
//		vec4_t hdrColor2	= COLOR_HDR2;	// text
		...
		ci = &cgs.clientinfo[gs->nClientID];
		...
		if(gs->cWeapons == 0) {
			...
			for(i = WP_KNIFE; i < WS_MAX; i++) {
				if ( gs->weapShots[i] == 0 && gs->weapHits[i] == 0 && gs->weapKills[i] == 0 && gs->weapDeaths[i] == 0 )
					continue;
				x = GS_X + 4;
				y += tSpacing;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, aWeaponInfo[i].pszName, 0.0f, 0, tStyle, tFont);
				x += 66;
				if ( gs->weapShots[i] > 0 || gs->weapHits[i] > 0 ) {
					CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, 
						va("%.1f",((gs->weapShots[i] == 0) ? 0.0 : 
							(float)(gs->weapHits[i]*100.0/(float)gs->weapShots[i]))),
						0.0f, 0, tStyle, tFont);
				}
				x += 53;
				if ( gs->weapShots[i] > 0 || gs->weapHits[i] > 0 ) {
					CG_Text_Paint_Ext(x, y, tScale, tScale, tColor,
						va("%d/%d",gs->weapHits[i], gs->weapShots[i]), 0.0f, 0, tStyle, tFont);
				}
				x += 62;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, va("%d", gs->weapKills[i]), 0.0f, 0, tStyle, tFont);
				x += 29;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, va("%d", gs->weapDeaths[i]), 0.0f, 0, tStyle, tFont);
				x += 40;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, 
					((aWeaponInfo[i].fHasHeadShots) ? va("%d", gs->weapHS[i]) : ""), 0.0f, 0, tStyle, tFont);
			}
			...
			if(gs->fHasStats) {
				...
				y += tSpacing;
				y += tSpacing;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, "Damage Given:", 0.0f, 0, tStyle, tFont);
				CG_Text_Paint_Ext(x+90, y, tScale, tScale, tColor, va("%d", gs->dmgGiven), 0.0f, 0, tStyle, tFont);
				y += tSpacing;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, "Damage Received:", 0.0f, 0, tStyle, tFont);
				CG_Text_Paint_Ext(x+90, y, tScale, tScale, tColor, va("%d", gs->dmgReceived), 0.0f, 0, tStyle, tFont);
				y += tSpacing;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, "Team Damage:", 0.0f, 0, tStyle, tFont);
				CG_Text_Paint_Ext(x+90, y, tScale, tScale, tColor, va("%d", gs->teamDamage), 0.0f, 0, tStyle, tFont);
			}
		}
		...
		y += 1 + tSpacing;
		CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, 
			va("%s", ((ci->team == TEAM_AXIS) ? 
				rankNames_Axis : rankNames_Allies)[ci->rank]),
			0.0f, 0, tStyle, tFont);
		x += 82;
		CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, va("%d", gs->xp), 0.0f, 0, tStyle, tFont);
		...
		if(gs->cSkills == 0) {
			...
			for(i = SK_BATTLE_SENSE; i < SK_NUM_SKILLS; i++) {
				if ( ci->skillpoints[i] == 0 )
					continue;
				x = GS_X + 4;
				y += tSpacing;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, skillNames[i], 0.0f, 0, tStyle, tFont);
				x += 84;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, va("%d",ci->skill[i]), 0.0f, 0, tStyle, tFont);
				x += 40;
				if(ci->skill[i] < NUM_SKILL_LEVELS - 1) {
					CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, va("%d/%d", ci->skillpoints[i], skillLevels[i][ci->skill[i]+1]), 0.0f, 0, tStyle, tFont);
				} else {
					CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, va("%d", ci->skillpoints[i]), 0.0f, 0, tStyle, tFont);
				}
#ifdef __ETE__
				if( cg_gameType.integer == GT_WOLF_CAMPAIGN || cg_gameType.integer == GT_SUPREMACY_CAMPAIGN ) {
#else //!__ETE__
...
#endif //__ETE__
					x += 86;
					CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, va("%d",ci->medals[i]), 0.0f, 0, tStyle, tFont);
				}
			}
...
#define TS_X	-5		// spacing from right
#define TS_Y	-75		// spacing from bottom
#define TS_W	308
	...
	if(cgs.topshots.show == SHOW_OFF) {
		...
		if(ts->cWeapons == 0) {
			...
			for(i = 0; i < ts->cWeapons; i++) {
				...
				y += tSpacing;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor,
					aWeaponInfo[ts->weap[i]].pszName,
					0.0f, 0, tStyle, tFont);
				x += 60;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, 
					va("%.1f", (ts->weapShots[i] > 0) ? (float)(ts->weapHits[i] * 100) / (float)ts->weapShots[i] : 0.0f ), 
					0.0f, 0, tStyle, tFont);
				x += 53;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor,
					va("%d/%d",ts->weapHits[i],ts->weapShots[i]),
					0.0f, 0, tStyle, tFont);
				x += 62;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor,
					va("%d",ts->weapKills[i]),
				...
				x += 32;
				CG_Text_Paint_Ext(x, y, tScale, tScale, tColor, 
					cgs.clientinfo[ts->clientID[i]].name,
					0.0f, 17, tStyle, tFont);
			}
		}
	}
}
