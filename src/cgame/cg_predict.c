...
*/
#ifdef __CUDA__
__device__ __host__ static void CG_ClipMoveToEntities ( const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end,
							int skipNumber, int mask, int capsule, trace_t *tr ) {
	__shared__ int			i, x, zd, zu;
	__shared__ trace_t		trace;
	__shared__ entityState_t	*ent;
	__shared__ clipHandle_t 	cmodel;
	__shared__ vec3_t		bmins, bmaxs;
	__shared__ vec3_t		origin, angles;
	__shared__ centity_t	*cent;
	...
	for ( i = 0 ; i < cg_numSolidEntities ; i++ ) {
		...
		if ( ent->solid == SOLID_BMODEL ) {
			...
			__host__ cmodel = trap_CM_InlineModel( ent->modelindex );
...
//			VectorCopy( cent->lerpOrigin, origin );
			...
			__host__ cmodel = trap_CM_TempBoxModel( bmins, bmaxs );
		...
		if (capsule) {
			__host__ trap_CM_TransformedCapsuleTrace ( &trace, start, end,
				mins, maxs, cmodel,  mask, origin, angles);
		}
		else {
			__host__ trap_CM_TransformedBoxTrace ( &trace, start, end,
				mins, maxs, cmodel,  mask, origin, angles);
		}
...
__device__ __host__ static void CG_ClipMoveToEntities_FT ( const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int skipNumber, int mask, int capsule, trace_t *tr ) {
	__shared__ int			i, x, zd, zu;
	__shared__ trace_t		trace;
	__shared__ entityState_t	*ent;
	__shared__ clipHandle_t 	cmodel;
...
//			VectorCopy( cent->lerpOrigin, origin );
			...
			__host__ cmodel = trap_CM_TempCapsuleModel( bmins, bmaxs );
...
*/
#ifdef __CUDA__
__device__ __host__ int		CG_PointContents( const vec3_t point, int passEntityNum ) {
	__shared__ int			i;
	__shared__ entityState_t	*ent;
	__shared__ centity_t	*cent;
	__shared__ clipHandle_t cmodel;
	__shared__ int			contents;

	__host__ contents = trap_CM_PointContents (point, 0);
	...
	for ( i = 0 ; i < cg_numSolidEntities ; i++ ) {
		...
		__host__ contents |= trap_CM_TransformedPointContents( point, cmodel, cent->lerpOrigin, cent->lerpAngles );
...
//	vec3_t			mins, maxs; // TTimo: unused
	...
	for ( i = 0 ; i < cg_numTriggerEntities ; i++ ) {
		...
		if( ent->eType == ET_CONSTRUCTIBLE || 
			...
			if(cs) {
				CG_ObjectivePrint( va( "^5You are near %s", cs ), SMALLCHAR_WIDTH );
			}
...
//	AnglesToQuaternion(ps->viewangles, cg.car->sBody.q);
	...
	VectorCopy(ps->velocity, cg.car->sBody.L);
...
extern void Init_Vehicles ( void );
#endif //__ADVANCED_VEHICLES__
...
//	int useCommand = 0;
	...
	if ( !cg.validPPS ) {
		...
		cg.car = vehicles[cg.snap->ps.clientNum];
