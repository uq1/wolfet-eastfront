...
#ifdef __BOT__
void CG_DrawPlumIcon( vec3_t position, int shader ) 
{
	int				rf = 0;
	refEntity_t		ent;
...
void CG_AddGoalNode( localEntity_t *le ) {
	refEntity_t	*re;
	vec3_t		origin, delta, dir, vec, up = {0, 0, 1};
	float		c, len;
	int			score;
...
/*		case LE_ZOMBIE_SPIRIT:
	...
	{
		//CG_ScorePlum( cg.clientNum, cg.current_commander_order_location, 0, 1 /*type*/, qtrue, cgs.media.crosshairShader[ cg_drawCrosshair.integer % NUM_CROSSHAIRS ] );
	}
#endif //__BOT__
}
