...
//				CG_Text_Paint(x, y + h, w->fontScale, textColor, (char*)w->lineText[j], 0.0f, 0, 0);
				CG_Text_Paint_Ext(x, y + h, w->fontScaleX, w->fontScaleY, textColor,
								  (char*)w->lineText[j], 0.0f, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30);
			}
...
void CG_windowNormalizeOnText(cg_window_t *w)
{
	...
	for(i=0; i<w->lineCount; i++) {
		if(w->effects & WFX_TRUETYPE) {
			tmp = CG_Text_Width_Ext((char*)w->lineText[i], w->fontScaleX, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30);
		} else {
	...
	for(i=0; i<w->lineCount; i++) {
		if(w->effects & WFX_TRUETYPE) {
			w->lineHeight[i] = CG_Text_Height_Ext((char*)w->lineText[i], w->fontScaleY, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30);
		} else {
