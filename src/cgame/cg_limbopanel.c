...
panel_button_text_t weaponPanelFilterFont = {
	...
	{ 1.0f, 1.0f, 1.0f, 0.6f },
	0, 0,
	&/*cgs.media.limboFont1*/cgs.media.tahoma16,
};
...
void CG_LimboPanel_RenderObjectiveText( panel_button_t* button ) {
	...
	CG_FitTextToWidth_Ext( buffer, button->font->scalex, button->rect.w - 16, sizeof(buffer), &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
	...
	while(*p) {
		if(*p == '\n') {
			*p++ = '\0';
			CG_Text_Paint_Ext( button->rect.x + 4, y, button->font->scalex, button->font->scaley, button->font->colour, s, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
			y += 8;
			s = p;
		} else {
			p++; 
	...
	if( cg_gameType.integer != GT_WOLF_LMS && CG_LimboPanel_GetTeam() != TEAM_SPECTATOR ) {
		...
		w = CG_Text_Width_Ext( ofTxt, 0.2f, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
		
		x = button->rect.x + button->rect.w - w - 4;

		CG_Text_Paint_Ext( x, button->rect.y + button->rect.h - 2, 0.2f, 0.2f, colorBlack, ofTxt, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
	}
...
void CG_DrawPlayerHead( rectDef_t *rect, bg_character_t* character, bg_character_t* headcharacter, float yaw, float pitch, qboolean drawHat, hudHeadAnimNumber_t animation, qhandle_t painSkin, int rank, qboolean spectator ) {
	...
	if( drawHat ) {
		...
		if (cg_entities[cg.clientNum].currentState.eFlags & EF_CLOAKED && (( cg_entities[cg.clientNum].currentState.eFlags & EF_PRONE ) || ( cg_entities[cg.clientNum].currentState.eFlags & EF_CROUCHING)) )
		{
			hat.customSkin	= stlth_skin;
			hat.customShader = stlth_shader;
		}
...
void CG_DrawPlayerHeadShadow( rectDef_t *rect, bg_character_t* character, bg_character_t* headcharacter, float yaw, float pitch, qboolean drawHat, hudHeadAnimNumber_t animation, qhandle_t painSkin, int rank, qboolean spectator ) {
	float			len;
	vec3_t			origin;
	vec3_t			mins, maxs, angles;
	float			x, y, w, h;
	...
	if( drawHat ) {
		...
		hat.customSkin	= shadow_skin;
		hat.customShader = shadow_shader;
...
void CG_LimboPanel_WeaponPanel_DrawWeapon( rectDef_t* rect, weapon_t weap, qboolean highlight, const char* ofTxt, qboolean disabled ) {
	...
	int width = CG_Text_Width_Ext( ofTxt, 0.2f, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
...
//	CG_DrawRect( rect->x, rect->y, rect->w, rect->h, 1, colorWhite );
	...
	CG_Text_Paint_Ext( x, rect->y +rect->h - 2, 0.2f, 0.2f, colorBlack, ofTxt, 0, 0, 0, &/*cgs.media.limboFont2*/cgs.media.tahoma30 );
}
...
void CG_LimboPanel_GetWeaponCardIconData( weapon_t weap, qhandle_t* shader, float* w, float* h, float* s0, float* t0, float* s1, float* t1 ) {
	...
	switch( weap ) {
		...
		case WP_PTRS:
		case WP_PTRS_SET:
			*shader = cgs.media.limboWeaponCard4;
			break;
#endif //__EF__
	...
	switch( weap ) {
		...
		default: // shouldn't happen
			...
			*t1 = 1.0;
			break;
#ifdef __EF__
		// UQ1: Weaponcard3 - Hopefully will be same placement as weaponcard1...
		case WP_PPSH:
			*t0 = 0/8.f;
			*t1 = 1/8.f;
			break;
		...
		case WP_30CAL_SET:
			...
			*t1 = 5/8.f;
			break;
        case WP_PTRS://Masteries -- UQ1: Moved to new weaponcard 4...
		case WP_PTRS_SET:
			*t0 = 0/8.f;
			*t1 = 1/8.f;
			break;
	...
	if( slot == 1 ) {
		...
		if( cgs.clientinfo[cg.clientNum].skill[SK_HEAVY_WEAPONS] >= 4 && CG_LimboPanel_GetClass() == PC_SOLDIER ) {
			if( cgs.clientinfo[cg.clientNum].skill[SK_LIGHT_WEAPONS] >= 4 ) {
				if( number == 2 ) {
#ifdef __EF__
					return CG_LimboPanel_GetTeam() == TEAM_AXIS ? WP_MP40 : WP_PPSH;
#else //!__EF__
		...
		if( number == 0 ) {
			if( CG_LimboPanel_GetClass() == PC_COVERTOPS ) {
				return CG_LimboPanel_GetTeam() == TEAM_AXIS ? WP_SILENCED_LUGER : WP_SILENCED_COLT;
			} else {
