...
//	struct bot_settings_s *settings;
	...
	if( bs->inuse ) {
		G_Printf("client %d already setup\n", client);
		return qfalse;
	}
...
//	Q_strncpyz( settings->characterfile, va("wolfbot"), sizeof(va("wolfbot")) );

	skill = Q_irand(1, 5);

	//load the bot character
	bs->character = trap_BotLoadCharacter(va("bots/wolfbot.bot"), skill);
	if (!bs->character) {
		G_Printf("couldn't load skill %i from %s\n", skill, va("wolfbot"));
		return qfalse;
	}
...
//	memcpy(&bs->settings, settings, sizeof(bot_settings_t));
	...
	if (errnum != BLERR_NOERROR) 
	{
		G_Printf("Item weights load failed!\n");
		trap_BotFreeGoalState(bs->gs);
		return qfalse;
	}
	//allocate a weapon state
	...
	if (errnum != BLERR_NOERROR) 
	{
		G_Printf("Weapons weights load failed!\n");
		trap_BotFreeGoalState(bs->gs);
		trap_BotFreeWeaponState(bs->ws);
		return qfalse;
	}
	...
	if (errnum != BLERR_NOERROR) 
	{
		G_Printf("Chat file load failed!\n");
		trap_BotFreeChatState(bs->cs);
		trap_BotFreeGoalState(bs->gs);
		trap_BotFreeWeaponState(bs->ws);
		return qfalse;
...
//	g_entities[client].AASbotstate = bs;
...
int BotAILoadMap( int restart ) {
	...
	if (!restart) {
		...
		trap_BotLibLoadMap( va("%s_b0", mapname.string) );
		trap_BotLibLoadMap( va("%s_b1", mapname.string) );
	} else {
...
int BotAIThinkFrame(int time) {
	...
	static int lastbot;
//	int startTime = 0, totalProfileTime;
