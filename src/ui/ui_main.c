...
//
// MyMusic List Loading...
//

typedef struct MyMusic_s	// Sounds Table Structure
{
	char	musicFile[MAX_TOKEN_CHARS];		// Sound name.
	char	station_name[MAX_TOKEN_CHARS];	// Radio Station Name
} MyMusic_t;				// MyMusic Table Typedef
MyMusic_t	MyMusic[MAX_TOKEN_CHARS];
...
static void UI_LoadRadioStations ( void )
{
...
static int UI_MapCountByGameType(qboolean singlePlayer) {
	...
	if( game == GT_SUPREMACY_CAMPAIGN ) {
		for (i = 0; i < uiInfo.campaignCount; i++) {
			if( uiInfo.campaignList[i].typeBits & (1 << GT_SUPREMACY) || uiInfo.campaignList[i].typeBits & (1 << GT_SUPREMACY_CAMPAIGN) ) {
				c++;
			}
		}
	} 
...
static void UI_BuildServerDisplayList(qboolean force) {
	...
	for (i = 0; i < count; i++) {
		...
		if (ping >/*=*/ 0 || ui_netSource.integer == AS_FAVORITES) {
			...
			// UQ1: Test if it is an EF server... Only show them (for now)...
			if ((strcmp(va(Info_ValueForKey(info, "game")), "ef") 
				&& strcmp(va(Info_ValueForKey(info, "game")), "ef-new")
				&& strcmp(va(Info_ValueForKey(info, "game")), "eastfront")
				&& strcmp(va(Info_ValueForKey(info, "game")), "East Front")))
			{// ef-new is for my local debug compiles...
				//Com_Printf("%s is not east front.\n", va(Info_ValueForKey(info, "game")));
...
/*		if( uiInfo.serverStatus.currentServerCinematic >= 0 ) {
	  ...
	  if ( index < uiInfo.numFoundPlayerServers-1) {
		...
		trap_Cvar_Set( "s_radioStationOverride", va("%s", MyMusic[index].musicFile) );
	}
}
...
void _UI_Init( qboolean inGameLoad ) {
  ...
  //UI_Load();
	...
	uiInfo.campaignMap = trap_R_RegisterShaderNoMip( "gfx/loading/camp_worldmap.tga" );
...
//	{ &ui_actualNetGameType, "ui_actualNetGametype", "5", CVAR_ARCHIVE },		// NERVE - SMF - hardwired for now
	...
	{ NULL, "s_radioStationOverride", "", CVAR_ARCHIVE },
...
#ifdef __ETE__
	if( i == uiInfo.campaignCount || !((campaign->typeBits & (1 << GT_WOLF) || campaign->typeBits & (1 << GT_SUPREMACY) || campaign->typeBits & (1 << GT_SUPREMACY_CAMPAIGN)) ) ) {
#else //!__ETE__
...
#ifdef __ETE__
void UI_ListSupremacyCampaigns_f( void ) {
	int i, mpCampaigns;
