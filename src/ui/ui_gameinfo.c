		...
		/*} else if( !Q_stricmp( token.string, "objectives" ) ) {
			...
			if( !trap_PC_ReadToken( handle, &token ) ) {
				...
				if( strstr( token.string, "wolfsp" ) || strstr( token.string, "wolfcoop" ) ) {
					...
					uiInfo.mapList[uiInfo.mapCount].typeBits |= (1 << GT_NEWCOOP);
				}
				...
				if( strstr( token.string, "supremacy_campaign" ) ) {
					uiInfo.mapList[uiInfo.mapCount].typeBits |= (1 << GT_SUPREMACY_CAMPAIGN);
				}
#endif //__ETE__
			}
...
*/
	...
	dirptr  = dirlist;
	for(i = 0;
		i < numdirs && uiInfo.mapCount < MAX_MAPS;
		i++, dirptr += dirlen+1) {
	...
	while ( trap_PC_ReadToken( handle, &token ) ) {
		if( *token.string == '}' ) {
			...
			if( strstr( token.string, "wolfsp" ) || strstr( token.string, "wolfcoop" ) ) {
				...
				uiInfo.campaignList[uiInfo.campaignCount].typeBits |= (1 << GT_NEWCOOP);
			}
			if( strstr( token.string, "wolfmp" ) ) {
			...
			if( strstr( token.string, "supremacy_campaign" ) ) {
				uiInfo.campaignList[uiInfo.campaignCount].typeBits |= (1 << GT_SUPREMACY_CAMPAIGN);
			}
#endif //__ETE__
			if( strstr( token.string, "wolflms" ) ) {
			...
			while( *ptr ) {
				...
				*mapnameptr = '\0';
				...
				// kw: fixed a crash issue when a campaign had too many maps
				if(uiInfo.campaignList[uiInfo.campaignCount].mapCount >= MAX_MAPS_PER_CAMPAIGN) {
					trap_Print( va( "^3Warning: too many maps in campaign: %s\n", filename ) );
					trap_PC_FreeSource( handle );
					return;
				}
...
#ifdef __ETE__
	// get all campaigns from .campaign files
	numdirs = trap_FS_GetFileList( "scripts", ".supremacy_campaign", dirlist, sizeof(dirlist) );
	dirptr  = dirlist;
	for (i = 0; i < numdirs && uiInfo.campaignCount < MAX_CAMPAIGNS; i++, dirptr += dirlen+1) {
		dirlen = strlen(dirptr);
		strcpy(filename, "scripts/");
