...
{
	etetRectangle *itemRect = &item->window.rectClient;
	etetRectangle *tipRect = &item->toolTipData->window.rectClient;
	...
	if( Int_Parse(args, &useprofile) ) {
		...
		if( useprofile && cl_profileStr[0] ) {
			...
			Com_sprintf(bakPath, sizeof(bakPath),
				"profiles/%s/%s.eteprofile", cl_profileStr, CONFIG_NAME);
...
static bind_t g_bindings[] = {
	...
	{ "+sprint",		K_SHIFT,	-1,	K_SHIFT,		-1,	-1,	-1 },
...
#ifdef __BOT__
	{ "cmdtoggle",		K_CTRL,		-1,	K_F10,			-1,	-1,	-1 },
#else //!__BOT__
...
void Item_OwnerDraw_Paint(itemDef_t *item) {
	...
	if (DC->ownerDrawItem) {
		...
		menuDef_t *parent2 = (menuDef_t*)item->parent;
		Fade(&item->window.flags, &item->window.foreColor[3], parent2->fadeClamp, &item->window.nextTime, parent2->fadeCycle, qtrue, parent2->fadeAmount);
		...
		if (item->window.flags & WINDOW_HASFOCUS && item->window.flags & WINDOW_FOCUSPULSE) {
			lowLight[0] = 0.8 * parent2->focusColor[0]; 
			lowLight[1] = 0.8 * parent2->focusColor[1]; 
			lowLight[2] = 0.8 * parent2->focusColor[2]; 
			lowLight[3] = 0.8 * parent2->focusColor[3]; 
			LerpColor(parent2->focusColor,lowLight,color,0.5+0.5*sin(DC->realTime / PULSE_DIVISOR));
		} else if (item->textStyle == ITEM_TEXTSTYLE_BLINK && !((DC->realTime/BLINK_DIVISOR) & 1)) {
			lowLight[0] = 0.8 * item->window.foreColor[0]; 
			lowLight[1] = 0.8 * item->window.foreColor[1]; 
			lowLight[2] = 0.8 * item->window.foreColor[2]; 
		...
		if (item->cvarFlags & (CVAR_ENABLE | CVAR_DISABLE) && !Item_EnableShowViaCvar(item, CVAR_ENABLE)) {
			memcpy(color, parent2->disableColor, sizeof(vec4_t));
		}
