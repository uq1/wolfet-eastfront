...
Set_Best_AI_weapon ( gentity_t *bot )
{
	...
	(
		...
		((bot->client->ps.weapon == WP_PLIERS || ucmd->weapon == WP_PLIERS) && bot->beStillEng > level.time)
	)
	{
	...
	if ( bot->movetarget && bot->movetarget->health && bot->movetarget->client )
	{															// Switch to best weapon.
		if ( !AI_Medic_Time[bot->client->ps.clientNum] )
		{
			AI_Medic_Time[bot->client->ps.clientNum] = 0;
		}

		if ( JobType( bot) > BOT_JOB_NONE )						// We have a job to do.
		{
			if
			(
				JobType( bot) == BOT_JOB_SYRINGE &&
				bot->movetarget->health <= 0 &&
				!(bot->movetarget->client->ps.pm_flags & PMF_LIMBO) &&
				VectorDistance( bot->r.currentOrigin, bot->movetarget->r.currentOrigin) < 96
			)
			{
				if ( !g_realmedic.integer )
				{												// jaquboss , realmedic support
					ucmd->weapon = WP_MEDIC_SYRINGE;
					next_weapon_change[bot->s.clientNum] = level.time + 2000;
				}
				else if
					(
						JobType( bot) == BOT_JOB_MEDKIT &&
						bot->movetarget->health > 0 &&
						bot->movetarget->health < bot->movetarget->client->ps.stats[STAT_MAX_HEALTH] - 10 &&
						!(bot->movetarget->client->ps.pm_flags & PMF_LIMBO) &&
						VectorDistance( bot->r.currentOrigin, bot->movetarget->r.currentOrigin) < 96
					)
				{
					...
					(
						...
						ClientOutOfAmmo( bot->movetarget->s.clientNum) &&
						bot->movetarget->health > 0 &&
						!(bot->movetarget->client->ps.pm_flags & PMF_LIMBO) &&
						VectorDistance( bot->r.currentOrigin, bot->movetarget->r.currentOrigin) < 96
					)
				...
				{												// Switch to best weapon.
					if ( AIMOD_HaveAmmoForWeapon( &bot->client->ps, bot->weaponchoice) )
					{
					...
					{
						ucmd->weapon = WP_AKIMBO_SILENCEDCOLT;
					}
					else if
						(
					...
					{
						ucmd->weapon = WP_AKIMBO_SILENCEDLUGER;
					}
					else if
						(
					...
					{
						ucmd->weapon = WP_SILENCED_COLT;
					}
					else if ( AIMOD_HaveAmmoForWeapon( &bot->client->ps, WP_LUGER) )
					{
						ucmd->weapon = WP_LUGER;
					}
					else if ( AIMOD_HaveAmmoForWeapon( &bot->client->ps, WP_COLT) )
					{
						ucmd->weapon = WP_COLT;
					}
					else
		...
		//bot->s.weapon = bot->client->ps.weapon;
		return;
	}

	next_weapon_change[bot->s.clientNum] = level.time + 5000;	// 5 seconds before we change again.
	need_ammo[clientNum] = qfalse;
	if
	(
		bot->enemy &&
		(
			bot->weaponchoice == WP_MEDIC_SYRINGE ||
			bot->client->ps.weapon == WP_MEDIC_SYRINGE ||
			ucmd->weapon == WP_MEDIC_SYRINGE ||
			bot->weaponchoice == WP_PLIERS ||
	...
	{															// Switch back to a gun if we require combat.
		if ( AIMOD_HaveAmmoForWeapon( &bot->client->ps, bot->weaponchoice) )
		{
			ucmd->weapon = bot->weaponchoice;
		}
	...
	{															// Need to check range and maybe switch to pistol.
		...
		if ( !AIMOD_HaveAmmoForWeapon( &bot->client->ps, WP_FLAMETHROWER) )
		{
	...
	// Now the snipers!
	else if ( bot->weaponchoice == WP_K43 )
	{
		if ( !COM_BitCheck( bot->client->ps.weapons, WP_K43_SCOPE) )
		{
			AddWeaponToPlayer( bot->client, WP_K43_SCOPE, GetAmmoTableData( WP_K43_SCOPE)->defaultStartingAmmo,
	...
	else if ( bot->weaponchoice == WP_FG42 )
	{
		if ( !COM_BitCheck( bot->client->ps.weapons, WP_FG42SCOPE) )
		{
			AddWeaponToPlayer( bot->client, WP_FG42SCOPE, GetAmmoTableData( WP_FG42SCOPE)->defaultStartingAmmo,
	...
	/*else if (bot->client->ps.ammo[bot->client->ps.weapon] == 0 && bot->client->ps.ammoclip[bot->client->ps.weapon] == 0)
	{// Need to check range and maybe switch to pistol.
		if (bot->client->ps.weapon == WP_LUGER || bot->client->ps.weapon == WP_COLT) 
		{
			G_Voice( bot, NULL, SAY_TEAM, "NeedAmmo", qfalse );
			bot->client->pers.cmd.weapon = WP_KNIFE;
			//ucmd->weapon = WP_KNIFE;
		}
		else
		{
			...
			{
				if (bot->client->ps.weapon != WP_LUGER
					&& (bot->client->ps.ammo[WP_LUGER] > 0 || bot->client->ps.ammoclip[WP_LUGER] > 0))
				{
					bot->client->pers.cmd.weapon = WP_LUGER;
					//ucmd->weapon = WP_LUGER;
					G_Voice( bot, NULL, SAY_TEAM, "NeedAmmo", qfalse );
				}
			}
			...
			{
				if (bot->client->ps.weapon != WP_COLT
					&& (bot->client->ps.ammo[WP_COLT] != 0 && bot->client->ps.ammoclip[WP_COLT] != 0))
				{
					bot->client->pers.cmd.weapon = WP_COLT;
					//ucmd->weapon = WP_COLT;
					G_Voice( bot, NULL, SAY_TEAM, "NeedAmmo", qfalse );
				}
			}
	...
	if ( !COM_BitCheck( bot->client->ps.weapons, ucmd->weapon) )
	{															// Something is wrong... Pick a secondary weapon we have...
		if
		(
			COM_BitCheck( bot->client->ps.weapons, WP_AKIMBO_SILENCEDCOLT) &&
			AIMOD_HaveAmmoForWeapon( &bot->client->ps, WP_AKIMBO_SILENCEDCOLT)
...
//extern void TAB_StandardBotAI(bot_state_t *bs, float thinktime, usercmd_t *ucmd);

//int MAX_BOT_THINKFRAME = 100;
void
SOD_AI_BotThink ( gentity_t *bot )
{
	...
	//	int think_time = trap_Milliseconds() - level.frameTime;
	//int cpu_think_level = 100 - bot_cpu_use.integer; // This cvar should be a %...
	if ( level.intermissiontime )
	{							// Don't think in level intermission time.
		return;
	}

	//	MAX_BOT_THINKFRAME = bot_cpu_use.integer;
	if ( bot->health )
	{
		if ( bot->health <= 0 )
		{
	...
	/*	if (bot->nextthink)
	{
		if (bot->nextthink > level.time)
		{
			AIMOD_AI_FastThink(bot);
			return;
		}
	}*/
	client = bot->client;

	/*	if (bot_cpu_use.integer >= 1 && bot_cpu_use.integer < 100 && think_time > bot_cpu_use.integer)
	{// We have thought too much this frame.. Think next frame instead!
		bot->nextthink = level.time;

		AIMOD_AI_FastThink(bot);

		if (bot->client->ps.persistant[PERS_SCORE])
		{
			if (bot->client->ps.persistant[PERS_SCORE] < 0) // Bots should never go negative.
	...
	/*	if (bot_cpu_use.integer >= 1 && bot_cpu_use.integer < 100 && think_time > MAX_BOT_THINKFRAME)
	{// Only delay thinks if the bot_cpu_use level is below the maximum (100).
		int cpu_think_delay = 500 - (bot_cpu_use.integer*5); // This cvar should be a %...
		bot->nextthink = level.time + cpu_think_delay; // Default this to 100 (FRAMETIME)
	}
	else*/
	...
	//	if (bot->s.weapon != bot->client->ps.weapon)
	//		bot->s.weapon = bot->client->ps.weapon;
}
...
/*
int AIMOD_AI_SetMedkitAction( gentity_t *bot, usercmd_t *ucmd ) 
{
	int i;
...
//	qboolean revive = qfalse;
	...
	for (i = 0; i < MAX_CLIENTS; i++) 
	{
		...
		if (OnSameTeam(bot, tempent)) 
		{
			...
			if (((need_medic[tempent->s.clientNum] == qtrue && tempent->health < tempent->client->ps.stats[STAT_MAX_HEALTH] - 10)
				|| tempent->health < tempent->client->ps.stats[STAT_MAX_HEALTH] - 10 )
				&& !(tempent->client->ps.pm_flags & PMF_LIMBO) ) {
...
// Prototype fast think procedure
void
AIMOD_AI_FastThink ( gentity_t *bot )
{																// Ultra-Fast think to keep lagometer and choppiness down low...
	char		buf[1024], *args;
	qboolean	showServerCommand = qfalse;
	weapon_t	weapon;

	//	int closest_node = 0;
	//	int classSpecificAction = 0;
	int			pc;												// Player Class.
	usercmd_t	*ucmd = &bot->client->pers.cmd;
	if ( bot->s.number > MAX_CLIENTS || bot->s.number < 0 )
	{
		return;													// Stop non bots ever getting here...
	}
	...
	while ( trap_BotGetServerCommand( bot->client->ps.clientNum, buf, sizeof(buf)) && showServerCommand )
	{
	...
	{															// Init all bot timers...
		...
		//bot->wander_timeout = level.time - 1;
		//bot->suicide_timeout = level.time - 1;
//		bot->shooting_timeout = level.time - 1;
...
#ifdef __NPC__
		bot->next_enemy_check = level.time;
		bot->next_fire_time = level.time;
#endif //__NPC__
	...
	//-------------------------------------------------------
	// Fast attack handling... lol.. really fast...
	if ( bot->bot_last_ucmd )
	{

		//if (bot->bot_last_ucmd->buttons == BUTTON_ATTACK)
		//	bot->client->pers.cmd.buttons = BUTTON_ATTACK;
		ucmd->buttons = bot->bot_last_ucmd->buttons;
		ucmd->doubleTap = bot->bot_last_ucmd->doubleTap;
		ucmd->flags = bot->bot_last_ucmd->flags;
		ucmd->forwardmove = bot->bot_last_ucmd->forwardmove;
		ucmd->identClient = bot->bot_last_ucmd->identClient;
		ucmd->rightmove = bot->bot_last_ucmd->rightmove;

		//ucmd->serverTime = bot->bot_last_ucmd->serverTime;
		ucmd->serverTime = level.time;
		ucmd->upmove = bot->bot_last_ucmd->upmove;
		ucmd->wbuttons = bot->bot_last_ucmd->wbuttons;

		//		ucmd->weapon = bot->client->ps.weapon;
	}
	else
	{
		bot->bot_last_ucmd = 0;
	...
	// Send through fast move methods...
	if ( bot->enemy && bot->enemy->health > 0 && bot->bot_enemy_visible_time > level.time )
	{
		if (bot->client->ps.weaponstate == WEAPON_READY || client->ps.weaponstate == WEAPON_RELAXING)
...
/* */
int
Get_Bot_Think_Offset ( void )
{
	if ( level.frameTime != this_frame_time )
	{
		this_frame_time = level.frameTime;
		num_this_frame = 1;
	}

	num_this_frame++;
	return ( num_this_frame );
}
...
//		bot->shooting_timeout = level.time - 1;
	...
	{// Reschedule for next frame...
		int offset = Get_Bot_Think_Offset();

		bot->nextthink = level.time + offset;
		//G_Printf("Bot %s think offset set to %i.\n", bot->client->pers.netname, offset);
		bot->bot_last_good_move_time--;
	...
	/*	if( bot->s.dl_intensity == TAG_HOOK || bot->s.dl_intensity == TAG_HOOK_DRIVER || bot->s.dl_intensity == TAG_HOOK_GUNNER1 || bot->s.dl_intensity == TAG_HOOK_GUNNER2 )
	{// Hooked to an LCVP vehicle.. Don't do anything until we can spawn...
		// set approximate ping and show values
		//-------------------------------------------------------
		bot->client->ps.ping = 75 + floor (random () * 25) + 1;
		//bot->client->ps.ping = 10;
	...
	if ( bot->health > 0 )
	{
		...
		//		if (!bot->enemy || bot->enemy->health <= 0 /*&& time - level.frameTime < MAX_BOT_THINKFRAME*/)
		...
		(
			...
			!bot->followtarget &&
			!bot->team_leader &&
			time - level.frameTime < MAX_BOT_THINKFRAME
		)
		{													// COOP modes.. Follow another player???
			...
			{												// Look for a real player for this bot to follow...
				...
				{											// Loop through our clients and find a real player...
					...
					if ( VectorDistance( bot->client->ps.origin, mt->r.currentOrigin) > best_dist )
					{
						continue;
					}

					// Found one...
					best_dist = VectorDistance( bot->client->ps.origin, mt->r.currentOrigin );
				...
				if ( best_followtarget )
				{
					bot->followtarget = best_followtarget;
					bot_follow_time[bot->s.clientNum] = level.time + 60000;
				}
			}
		}*/
...
#endif //__WEAPON_AIM__
#ifdef __BOT_ENEMY_CHASE__
		if ( bot->enemy && bot->bot_enemy_visible_time < level.time && pc == PC_ENGINEER )
		{													// Engineers dont bother to follow... More important things to do...
			bot->enemy = NULL;

			//bot->bot_enemy_visible_time = 0;
			after_enemy[bot->s.clientNum] = 0;
		}
		else if ( bot->movetarget && bot->enemy && bot->bot_enemy_visible_time < level.time && pc == PC_MEDIC )
		{													// Medics dont bother to follow... If they have more important things to do...
			bot->enemy = NULL;
		...
		{													// Do some pathing...
			if ( bot->enemy->client && (bot->enemy->client->ps.pm_flags & PMF_LIMBO) )
			{
				bot->enemy = NULL;
				after_enemy[bot->s.clientNum] = 0;
			}
			else if ( after_enemy[bot->s.clientNum] < level.time )
			{												// Set a new path to them...
				memset( bot->pathlist, NODE_INVALID, MAX_PATHLIST_NODES );

				//if (!bot->current_node || bot->current_node <= 0 || bot->current_node > number_of_nodes)
				bot->current_node = AIMOD_NAVIGATION_FindClosestReachableNodes( bot, NODE_DENSITY, NODEFIND_ALL );
				if ( bot->enemy->r.svFlags & SVF_BOT && bot->enemy->current_node )
				{
					bot->longTermGoal = bot->enemy->current_node;
				}
				else if ( bot->enemy->r.svFlags & SVF_BOT && bot->enemy->last_node )
				{
					bot->longTermGoal = bot->enemy->last_node;
				}
				else
				{											// Have to actually find one for players...
					bot->longTermGoal = AIMOD_NAVIGATION_FindClosestReachableNodes( bot->enemy, NODE_DENSITY,
																					NODEFIND_ALL );
				}

				// So we can offset thinking for fps...
				bot->bot_route_think_delay = level.time + 4000;
				if ( bot->longTermGoal > 0 )
				{											// Create a path to the enemy!
					VectorCopy( bot->enemy->r.currentOrigin, bot->bot_enemy_stored_pos );
					bot->pathsize = CreatePathAStar( bot, bot->current_node, bot->longTermGoal, bot->pathlist );
					bot->next_node = BotGetNextNode( bot ); //move to current node first, since it's where our path starts from
					bot->node_timeout = level.time + 4000;
					after_enemy[bot->s.clientNum] = level.time + ( bot->pathsize * 2000 );	//level.time + 30000; // 30 sec counter to find our enemy...
				}
				else
				{												// If our path failed us... Reset enemy state...
					bot->enemy = NULL;
					after_enemy[bot->s.clientNum] = 0;
				}
			}
			else if ( after_enemy[bot->s.clientNum] > level.time )
			{													// Set a new path to them when needed...
				if ( VectorDistance( bot->bot_enemy_stored_pos, bot->enemy->r.currentOrigin) > 256 )
				{												// Find a new path if they have moved too far from our path target...
					memset( bot->pathlist, NODE_INVALID, MAX_PATHLIST_NODES );
...
#endif //__BOT_ENEMY_CHASE__

		// Now check following (followtarget) status...
/*		if ( !bot->followtarget )
		{																						// If we already have an enemy, don't look again...
			bot->followtarget = NULL;
			bot_follow_time[bot->s.clientNum] = 0;
		}
		else if ( bot->followtarget->client && (bot->followtarget->client->ps.pm_flags & PMF_LIMBO) )
		{
			bot->followtarget = NULL;
			bot_follow_time[bot->s.clientNum] = 0;
		}
		...
		{																						// Have a follow target.
			if ( bot->bot_followtarget_visible_time < level.time )
			{																					// Set visibility time...
				if ( MyVisible( bot, bot->followtarget) )
				{
					bot->bot_followtarget_visible_time = level.time + 1500;
				}
				else
				{
					bot->bot_followtarget_visible_time = 0;
				}
			}
			else
			{
		...
		(
			bot->followtarget &&
			bot->bot_followtarget_visible_time < level.time
		)
		{																						// Do some pathing...
			if ( bot->followtarget->health <= 0 )
			{
				bot->followtarget = NULL;
				bot_follow_time[bot->s.clientNum] = 0;
			}
			...
			else if ( bot_follow_time[bot->s.clientNum] < level.time )
			{																					// Set a new path to them...
				memset( bot->pathlist, NODE_INVALID, MAX_PATHLIST_NODES );

				//if (!bot->current_node || bot->current_node <= 0 || bot->current_node > number_of_nodes)
				bot->current_node = AIMOD_NAVIGATION_FindClosestReachableNodes( bot, NODE_DENSITY, NODEFIND_ALL );
				if ( bot->followtarget->r.svFlags & SVF_BOT && bot->followtarget->current_node )
				{
					bot->longTermGoal = bot->followtarget->current_node;
				}
				else if ( bot->followtarget->r.svFlags & SVF_BOT && bot->followtarget->last_node )
				{
					bot->longTermGoal = bot->followtarget->last_node;
				}
				else
				{																				// Have to actually find one for players...
					bot->longTermGoal = AIMOD_NAVIGATION_FindClosestReachableNodes( bot->followtarget, NODE_DENSITY,
																					NODEFIND_ALL );
				}
				...
				if ( bot->longTermGoal > 0 )
				{																				// Create a path to the enemy!
					VectorCopy( bot->followtarget->r.currentOrigin, bot->bot_followtarget_stored_pos );
					bot->pathsize = CreatePathAStar( bot, bot->current_node, bot->longTermGoal, bot->pathlist );
					bot->next_node = BotGetNextNode( bot );										//move to current node first, since it's where our path starts from
					bot->node_timeout = level.time + 4000;
					bot_follow_time[bot->s.clientNum] = level.time + ( bot->pathsize * 2000 );	//level.time + 30000; // 30 sec counter to find our enemy...
				}
			}
			else if
				(
					bot_follow_time[bot->s.clientNum] > level.time &&
					VectorDistance( bot->bot_followtarget_stored_pos, bot->followtarget->r.currentOrigin) > 256
				)
			{												// Set a new path to them when needed...
				memset( bot->pathlist, NODE_INVALID, MAX_PATHLIST_NODES );
		...
		// Now check movetarget status...
/*		if ( !bot->movetarget )
		{													// If we already have an enemy, don't look again...
			bot->movetarget = NULL;
			bot_movetarget_time[bot->s.clientNum] = 0;
		}
		else if ( bot->movetarget->client && (bot->movetarget->client->ps.pm_flags & PMF_LIMBO) )
		{
			bot->movetarget = NULL;
			bot_movetarget_time[bot->s.clientNum] = 0;
		}
		...
		{													// Have a movetarget.
			if ( bot->bot_movetarget_visible_time < level.time && time - level.frameTime < MAX_BOT_THINKFRAME )
			{												// Set visibility time...
				if ( MyVisible( bot, bot->movetarget) )
				{
					bot->bot_movetarget_visible_time = level.time + 1500;
				}
				else
				{
					bot->bot_movetarget_visible_time = 0;
				}
			}
			else
			{
		...
		(
			bot->movetarget &&
			bot->bot_movetarget_visible_time < level.time
		)
		{													// Do some pathing...
			if ( bot->movetarget->client && (bot->movetarget->client->ps.pm_flags & PMF_LIMBO) )
			{
				bot->movetarget = NULL;
				bot_movetarget_time[bot->s.clientNum] = 0;
			}
			else if ( bot_movetarget_time[bot->s.clientNum] < level.time )
			{												// Set a new path to them...
				memset( bot->pathlist, NODE_INVALID, MAX_PATHLIST_NODES );

				//if (!bot->current_node || bot->current_node <= 0 || bot->current_node > number_of_nodes)
				bot->current_node = AIMOD_NAVIGATION_FindClosestReachableNodes( bot, NODE_DENSITY, NODEFIND_ALL );
				if ( bot->movetarget->r.svFlags & SVF_BOT && bot->movetarget->current_node )
				{
					bot->longTermGoal = bot->movetarget->current_node;
				}
				else if ( bot->movetarget->r.svFlags & SVF_BOT && bot->movetarget->last_node )
				{
					bot->longTermGoal = bot->movetarget->last_node;
				}
				else
				{											// Have to actually find one for players...
					bot->longTermGoal = AIMOD_NAVIGATION_FindClosestReachableNodes( bot->movetarget, NODE_DENSITY,
																					NODEFIND_ALL );
				}
				...
				if ( bot->longTermGoal > 0 )
				{											// Create a path to the enemy!
					VectorCopy( bot->movetarget->r.currentOrigin, bot->bot_movetarget_stored_pos );
					bot->pathsize = CreatePathAStar( bot, bot->current_node, bot->longTermGoal, bot->pathlist );
					bot->next_node = BotGetNextNode( bot ); //move to current node first, since it's where our path starts from
					bot->node_timeout = level.time + 4000;
					bot_movetarget_time[bot->s.clientNum] = level.time + ( bot->pathsize * 2000 );	//level.time + 30000; // 30 sec counter to find our enemy...
				}
				else
				{												// If our path failed us... Reset enemy state...
					bot->enemy = NULL;
			...
			{													// Set a new path to them when needed...
				if ( VectorDistance( bot->bot_movetarget_stored_pos, bot->movetarget->r.currentOrigin) > 256 )
				{												// Find a new path if they have moved too far from our path target...
					memset( bot->pathlist, NODE_INVALID, MAX_PATHLIST_NODES );
...
/*			else if ( bot->movetarget )
			{			// UniqueMod medics/fldops/engineer.. Go to another entity.
				if
				(
					!(
						pc == PC_MEDIC &&
						bot->bot_movetarget_visible_time > level.time // Let medics revive/heal if they have a job..
						&&
			...
			{			// UniqueMod followers.. Following another entity.
				if ( bot->enemy && bot->bot_enemy_visible_time > level.time && bot->bot_runaway_time < level.time )
				{		//AIMOD_MOVEMENT_Attack ( bot, ucmd );
					AIMOD_MOVEMENT_AttackMove( bot, ucmd );
				}
				else
			...
			{			// Attack handling...
#ifdef __NPC__
				if ( bot->playerType == PC_ENGINEER && bot->health > 80 )
				{		// Engineer's should prioritize movement! Just keep moving to our goal..
					//AIMOD_MOVEMENT_Move( bot, ucmd );
					AIMOD_MOVEMENT_AttackMove( bot, ucmd );
				}
				else
...
#endif //__NPC__
/*				else if ( oldenemy != bot->enemy )
				{		// Delay firing...
					if (
#ifdef __NPC__
					bot->enemy->s.eType != ET_NPC &&
#endif //__NPC__
					bot->enemy->client->ps.powerups[PW_OPS_DISGUISED] )
					{
						enemy_fire_time[bot->s.clientNum] = level.time + ( 5000 - (bot->skillchoice * 200) );
					}
					else
					{
						enemy_fire_time[bot->s.clientNum] = level.time + ( 1000 - (bot->skillchoice * 200) );
					}
				}*/
				else if
					(
						//enemy_fire_time[bot->s.clientNum] <= level.time &&
						bot->bot_enemy_visible_time > level.time &&
						bot->bot_runaway_time < level.time
					)
				{		// Attack
		...
		/*#ifdef __ADVANCED_BOT_CHECKS__
		if ( bot->health > 0 && bot->health < 50 && bot->bot_enemy_visible_time < level.time)
		{// Bot has no health... Head for closest health cabinet, or dropped pack...
			int loop = 0;
			int best = -1;
			float best_dist = 9999.9f;
			...
			if (!bot->current_node)
				bot->current_node = AIMOD_NAVIGATION_FindClosestReachableNode( bot, NODE_DENSITY, NODEFIND_ALL );

			bot->longTermGoal = AIMOD_NAVIGATION_FindClosestReachableNodeTo( g_entities[best].r.currentOrigin, NODE_DENSITY, NODEFIND_ALL );
			...
			{// Create a path to the enemy!
				//bot->pathsize = CreatePathAStar(bot, bot->current_node, bot->longTermGoal, bot->pathlist);
				bot->pathsize = CreateDumbRoute(bot->current_node, bot->longTermGoal, bot->pathlist);
				bot->next_node = BotGetNextNode(bot);		//move to current node first, since it's where our path starts from
				bot->node_timeout = level.time + 4000;
			}
		}
		else if ( pc != PC_FIELDOPS && bot->s.weapon != WP_PLIERS 
			&& bot->client->ps.ammoclip[bot->weaponchoice] == 0 && bot->client->ps.ammo[bot->weaponchoice] == 0 
			&& time - level.frameTime < MAX_BOT_THINKFRAME && bot->bot_enemy_visible_time < level.time)
		{// Bot has no ammo for main weapon... Head for closest ammo cabinet, or ammo pack dropped nearby... Or same gun...
			int loop = 0;
			int best = -1;
			float best_dist = 9999.9f;
			...
			for (loop = MAX_CLIENTS; loop < MAX_GENTITIES; loop++)
			{
				...
				if (test->item)
				{
					if (test->item->giType != IT_AMMO && !(test->item->giType == IT_WEAPON && test->item->giTag == bot->weaponchoice) )
						continue;
				}
				else if ( Q_stricmp( test->classname, "misc_cabinet_supply" ) )
					continue;
		...
		{				// Waypoint navigation...
			...
			/*	if (bot->client->sess.sessionTeam == TEAM_AXIS
				&& next_idle_talk[bot->s.number] <= level.time 
				&& next_overall_idle_talk_time < level.time)
			{// Idle bot talking...
				//gentity_t *te;

				G_AddEvent( bot, EV_GENERAL_SOUND, G_SoundIndex( va("sound/idle/idle%i.wav", rand()%28)) );
				// sound
				next_idle_talk[bot->s.number] = level.time + 10000 + rand()%10000; // 10 secs at least between talk for each bot.
				next_overall_idle_talk_time = level.time + 5000 + rand()%5000; // At least 5 secs between talks...
			}*/
		}
	}
