...
{				// Load bot first names from external file.
	...
	//	int num1 = 0, num2 = 0;
	int				num = 0;
	loadPath = va( "americanFirstNames.cfg" );
...
{				// Load bot last names from external file.
	...
	//	int num1 = 0, num2 = 0;
	int				num = 0;
	loadPath = va( "americanLastNames.cfg" );
	len = trap_FS_FOpenFile( loadPath, &f, FS_READ );
	if ( !f )
	{
...
{				// Load bot first names from external file.
	...
	//	int num1 = 0, num2 = 0;
	int				num = 0;
	loadPath = va( "germanFirstNames.cfg" );
	len = trap_FS_FOpenFile( loadPath, &f, FS_READ );
	if ( !f )
	{
...
{				// Load bot last names from external file.
	...
	//	int num1 = 0, num2 = 0;
	int				num = 0;
	loadPath = va( "germanLastNames.cfg" );
	len = trap_FS_FOpenFile( loadPath, &f, FS_READ );
	if ( !f )
	{
		G_Printf( "^1*** ^3WARNING^5: No (american) bot last names file (^7%s^5). Using default name list.\n", loadPath );
 ...
 */
extern int	Count_Bot_Players ( void );
extern int	nextchat[MAX_CLIENTS];
extern void RandomSelectPOPSpawnpoint ( gentity_t *ent );
...
SetBotWeapons ( gentity_t *bot )
{
	...
	{
		if ( pc == PC_COVERTOPS)
		{// UQ1: Special case, we want them to always use sniper rifles!
			if (client->ps.teamNum == TEAM_AXIS)
			{
				if (Q_irand(0,5) == 0) // More team specific ones..
					client->sess.latchPlayerWeapon = client->sess.playerWeapon = WP_FG42;
				else
					client->sess.latchPlayerWeapon = client->sess.playerWeapon = WP_K43;
			}
			else
			{
				if (Q_irand(0,5) == 0) // More team specific ones..
					...
					client->sess.latchPlayerWeapon = client->sess.playerWeapon = WP_GARAND;
			}
		}
		else
		{
	...
	AddWeaponToPlayer( client, client->sess.playerWeapon, GetAmmoTableData(client->sess.playerWeapon)->defaultStartingAmmo, GetAmmoTableData(client->sess.playerWeapon)->defaultStartingClip, qtrue );

	if (weapAlts[client->sess.playerWeapon])
	{
		if (!(client->sess.playerWeapon == WP_KAR98 && pc != PC_ENGINEER)) // Don't give grenade to non-engineers...
			AddWeaponToPlayer( client, weapAlts[client->sess.playerWeapon], GetAmmoTableData(weapAlts[client->sess.playerWeapon])->defaultStartingAmmo, GetAmmoTableData(weapAlts[client->sess.playerWeapon])->defaultStartingClip, qtrue );
	}
	...
	else
	{
		...
		{
			if (client->sess.sessionTeam == TEAM_AXIS)
				client->sess.latchPlayerWeapon2 = WP_AKIMBO_LUGER;
			else
				client->sess.latchPlayerWeapon2 = WP_AKIMBO_COLT;
		}
		else if (client->sess.sessionTeam == TEAM_AXIS)
			client->sess.latchPlayerWeapon2 = WP_LUGER;
		else
			client->sess.latchPlayerWeapon2 = WP_COLT;
	}
	...
	/*if (weapAlts[client->sess.playerWeapon2])
	{
		AddWeaponToPlayer( client, weapAlts[client->sess.playerWeapon2], GetAmmoTableData(weapAlts[client->sess.playerWeapon2])->defaultStartingAmmo, GetAmmoTableData(weapAlts[client->sess.playerWeapon2])->defaultStartingClip, qtrue );
	}*/
...
/* */
int
SOD_SPAWN_ChooseClass_Standard ( void )
{
	int choice = Q_TrueRand( 0, 100 );
	int num_eng = Count_Eng_Bot_Players();
	int num_med = Count_Medic_Bot_Players();
	int num_fld = Count_FldOps_Bot_Players();

	//return ( PC_COVERTOPS );

	if ( num_eng < bot_minplayers.integer / 5/*4*/ )
	{
		return ( PC_ENGINEER ); // Mostly engineers to help complete objectives faster.
	}
	else if ( num_med < bot_minplayers.integer / 5/*4*/ )
	{
		return ( PC_MEDIC );	// Plenty of medics!
	}
	else if ( num_fld < bot_minplayers.integer / 5/*4*/ )
	{
...
SOD_SPAWN_SaveBots ( void )
{
	...
	for ( i = level.maxclients; i > 0; i-- )
	{
		...
		if ((g_gametype.integer <= GT_COOP ||  g_gametype.integer == GT_NEWCOOP) && sodbot->inuse && sodbot->is_bot)
		    trap_DropClient( i, " (bot) was removed between maps.", 0 );
...
AIMOD_SPAWN_SpawnBot ( char *team, char *name, int skill )
{
	...
	if ( (team != NULL) && (strcmp( team, "blue") == 0) )
		uniquebot->client->sess.sessionTeam = TEAM_ALLIES;
	else
		uniquebot->client->sess.sessionTeam = TEAM_AXIS;

	if (g_gametype.integer == GT_SUPREMACY || g_gametype.integer == GT_SUPREMACY_CAMPAIGN)
	{// Pick a spawnpoint for the bot!
		...
		{// We need to find the most forward flag point to spawn at... FIXME: Selectable spawnpoints in UI...
			if (flag_list[test_flag].flagentity)
			{
				if (flag_list[test_flag].num_spawnpoints > 0
					&& flag_list[test_flag].flagentity->s.modelindex == uniquebot->client->sess.sessionTeam)
				{// This is our flag...
		...
		SetPlayerSpawn( uniquebot, flag_choice, qtrue );
	}
}
...
*/
			...
			if (sp_bot_team[i] == TEAM_ALLIES)
			{
				if (SP_SPAWN_AddBotShort("blue", name, iskill, i, NULL))
					G_Printf("^1*** ^3%s^5: ^5Spawned ^4Allied ^5bot ^7%s^5 (ident %i) with skill level ^7%d^5 at %f %f %f.\n", GAME_VERSION, name, i, iskill, sp_bot_positions[i][0], sp_bot_positions[i][1], sp_bot_positions[i][2]);
			}
			else
			{
				if (SP_SPAWN_AddBotShort("red", name, iskill, i, NULL))
					G_Printf("^1*** ^3%s^5: ^5Spawned ^2Axis ^5bot ^7%s^5 (ident %i) with skill level ^7%d^5 at %f %f %f.\n", GAME_VERSION, name, i, iskill, sp_bot_positions[i][0], sp_bot_positions[i][1], sp_bot_positions[i][2]);
			}
		}
...
//				Q_strncpyz(name, va("%s", PickName(TEAM_ALLIES)), sizeof(name));*/
			...
			//	G_Printf("^1*** ^3%s^5: ^5Spawned bot ^7%s^5 (ident %i) with skill level ^7%d^5 at %f %f %f.\n", GAME_VERSION, name, i, iskill, sp_bot_positions[i][0], sp_bot_positions[i][1], sp_bot_positions[i][2]);
			if (sp_bot_team[i] == TEAM_ALLIES)
			{
				if (SP_SPAWN_AddBotShort("blue", name, iskill, i, NULL))
					G_Printf("^1*** ^3%s^5: ^5Spawned ^4Allied ^5bot ^7%s^5 (ident %i) with skill level ^7%d^5 at %f %f %f.\n", GAME_VERSION, name, i, iskill, sp_bot_positions[i][0], sp_bot_positions[i][1], sp_bot_positions[i][2]);
...
//	G_Printf("cast weapon: %i.\n", cs->weaponNum);
	...
	sp_bot_team[sp_bot_max] = TEAM_AXIS;
...
//	G_Printf("cast weapon: %i.\n", cs->weaponNum);
	...
	sp_bot_team[sp_bot_max] = TEAM_ALLIES;
