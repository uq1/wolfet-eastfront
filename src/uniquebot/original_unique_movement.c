...
AIMOD_NeedAmmoForWeapon ( playerState_t *ps, weapon_t wep )
{
	...
	if ( ps->ammo[cur_wep] <= maxammo * 0.75 /*0.25*/ )
	{
		return ( qtrue );
	}
...
int SP_NodeVisible ( vec3_t org1, vec3_t org2, int ignore ); // below
...
{																// Fallback navigation. returns if it was used or not as qtrue/qfalse.
	...
	AngleVectors( bot->s.angles, forward, NULL, NULL );
	VectorMA( bot->r.currentOrigin, 96, forward, forward_pos );
	vis = SP_NodeVisible( bot->r.currentOrigin, forward_pos, bot->s.number );
...
TravelTime ( gentity_t *bot )
{
	...
	if (travel_time < 2000)
		travel_time = 2000;
		*/
	float	travel_time = 6000;//4000;//10000;
	return ( travel_time );
}
...
typedef struct aiFireTeam_s
{
	...
	int			goal_node;
	int			pathsize;
	short int	longterm_pathlist[MAX_NODES];
} aiFireTeam_t;
...
int					num_aiFireTeams = 1;
aiFireTeam_t		aiFireTeams[16];

void Bot_Copy_Pathlist ( short int *pathlist, short int *new_pathlist, int size )
{
	int i;

	for (i = 0; i < size; i++)
	{
		new_pathlist[i] = pathlist[i];
	}
}

void Bot_Create_aiFireTeam(gentity_t *bot)
{
	G_Printf("Bot %s created AI fireteam %i.\n", bot->client->pers.netname, num_aiFireTeams);

	aiFireTeams[num_aiFireTeams].leader = bot;
	aiFireTeams[num_aiFireTeams].members[aiFireTeams[num_aiFireTeams].num_members] = bot;
	aiFireTeams[num_aiFireTeams].num_members++;
	num_aiFireTeams++;
}

void Bot_Join_aiFireTeam(gentity_t *bot, int teamNum)
{
	G_Printf("Bot %s joined AI fireteam %i (leader %s).\n", bot->client->pers.netname, teamNum, aiFireTeams[teamNum].leader->client->pers.netname);

	aiFireTeams[teamNum].members[aiFireTeams[teamNum].num_members] = bot;
	aiFireTeams[teamNum].num_members++;

	bot->aiFireTeam = teamNum;
}

void Bot_Leave_aiFireTeam(gentity_t *bot)
{
	int				i;
	aiFireTeam_t	old_team[16];
	int				num_members = aiFireTeams[bot->aiFireTeam].num_members;

	memcpy(&aiFireTeams[bot->aiFireTeam].members, &old_team, aiFireTeams[bot->aiFireTeam].num_members);

	aiFireTeams[bot->aiFireTeam].num_members = 0;

	for (i = 0; i < num_members; i++)
	{
		if (old_team[bot->aiFireTeam].members[i] != bot)
		{
			aiFireTeams[bot->aiFireTeam].members[aiFireTeams[bot->aiFireTeam].num_members] = old_team[bot->aiFireTeam].members[i];
			aiFireTeams[bot->aiFireTeam].num_members++;
		}
	}

	G_Printf("Bot %s left AI fireteam %i.\n", bot->client->pers.netname, bot->aiFireTeam);

	bot->aiFireTeam = -1;
}

#define MAX_AI_FIRETEAM_BOTS 5

void Bot_Find_New_aiFireTeam(gentity_t *bot)
{
	int i = 0;
	qboolean teamFTfound = qfalse;

	bot->aiFireTeam = -1;

	for (i = 1; i < num_aiFireTeams; i++)
	{
		if (OnSameTeam(bot, aiFireTeams[i].leader) && aiFireTeams[i].num_members <= MAX_AI_FIRETEAM_BOTS)
		{
			teamFTfound = qtrue;
			break;
		}
	...
	if (!teamFTfound)
	{// Make one!
		Bot_Create_aiFireTeam(bot);
		return;
	}

	for (i = 1; i < num_aiFireTeams; i++)
	{// Find a leader!
		if (aiFireTeams[i].num_members > MAX_AI_FIRETEAM_BOTS)
			continue; // Too many already!

		if (OnSameTeam(bot, aiFireTeams[i].leader))
		{
			if (VectorDistance(bot->r.currentOrigin, aiFireTeams[i].leader->r.currentOrigin) < 512)
			{
				if (MyVisible(bot, aiFireTeams[i].leader))
				{// This one should do!
					if (bot->aiFireTeam >= 0)
					{
						Bot_Leave_aiFireTeam(bot);
					}

					Bot_Join_aiFireTeam(bot, i);
					return;
				}
			}
		}
	...
	// If we failed, create our own!
	Bot_Create_aiFireTeam(bot);
}
#endif //__AI_FIRETEAMS__
...
Bot_Set_New_Path ( gentity_t *bot, usercmd_t *ucmd )
{
	...
	{// Make them patrol this area for a bit...
		bot->bot_patrol_time = level.time + 5000;
	}

	bot->bot_next_goalfind_time = level.time + 4000;

#ifdef __AI_FIRETEAMS__
	if (!bot->aiFireTeam)
	{
		Bot_Find_New_aiFireTeam(bot);
	}

	if (bot->aiFireTeam
		&& (!aiFireTeams[bot->aiFireTeam].leader 
		|| aiFireTeams[bot->aiFireTeam].leader->client->pers.connected != CON_CONNECTED 
		|| !OnSameTeam(aiFireTeams[bot->aiFireTeam].leader, bot)))
	{// Lost our leader! Take over command :)
		aiFireTeams[bot->aiFireTeam].leader = bot;
	}

	if (bot->aiFireTeam && bot != aiFireTeams[bot->aiFireTeam].leader)
	{// Have an ai fireteam, and a leader, use the team's path...
		/*if (aiFireTeams[bot->aiFireTeam].pathsize > 0)
		{
			int i;

			for (i = aiFireTeams[bot->aiFireTeam].pathsize; i > 0; i++)
			{
				if (VectorDistance(bot->r.currentOrigin, nodes[aiFireTeams[bot->aiFireTeam].pathlist[i]].origin) < 256)
				{
					if (NodeVisible(bot->r.currentOrigin, nodes[aiFireTeams[bot->aiFireTeam].pathlist[i]].origin, bot->client->ps.clientNum))
					{// This one is reachable...
						bot->current_node = aiFireTeams[bot->aiFireTeam].pathlist[i];
					
						if (i > 0)
							bot->last_node = aiFireTeams[bot->aiFireTeam].pathlist[i-1];
						else
							bot->last_node = bot->current_node;

						//memcpy(&aiFireTeams[bot->aiFireTeam].pathlist, &bot->pathlist, sizeof(int)*aiFireTeams[bot->aiFireTeam].pathsize);
						Bot_Copy_Pathlist( aiFireTeams[bot->aiFireTeam].pathlist, bot->pathlist, aiFireTeams[bot->aiFireTeam].pathsize );

						bot->pathsize = aiFireTeams[bot->aiFireTeam].pathsize - (aiFireTeams[bot->aiFireTeam].pathsize - i)-1;
		...
		if (aiFireTeams[bot->aiFireTeam].leader->longTermGoal == bot->current_node)
			return; // Wait!

		if (aiFireTeams[bot->aiFireTeam].leader->longTermGoal < 0)
			return; // Wait!

		// Find a route to our leader!
		...
		bot->pathsize = CreatePathAStar( bot, bot->current_node, aiFireTeams[bot->aiFireTeam].leader->longTermGoal, bot->pathlist );
		bot->next_node = BotGetNextNode( bot );				//move to this node first, since it's where our path starts from
		bot->node_timeout = level.time + TravelTime( bot ); //6000;//4000;
		bot->longTermGoal = aiFireTeams[bot->aiFireTeam].leader->longTermGoal;
		//if (bot->pathsize <= 0)
		//	Bot_Find_New_aiFireTeam(bot);
		//else

		if (bot->pathsize > 0)
			G_Printf("Bot %s AI fireteam %i (following leader %s) %i to %i. OK!\n", bot->client->pers.netname, bot->aiFireTeam, aiFireTeams[bot->aiFireTeam].leader->client->pers.netname, bot->current_node, aiFireTeams[bot->aiFireTeam].leader->longTermGoal);
		else
			G_Printf("Bot %s AI fireteam %i (following leader %s) %i to %i. FAILED!\n", bot->client->pers.netname, bot->aiFireTeam, aiFireTeams[bot->aiFireTeam].leader->client->pers.netname, bot->current_node, aiFireTeams[bot->aiFireTeam].leader->longTermGoal);
	...
	//	}
	else if ( BAD_WP_Height( bot->r.currentOrigin, nodes[bot->current_node].origin) )
	{
...
/*	if (bot->aiFireTeam && aiFireTeams[bot->aiFireTeam].leader == bot)
	{
		aiFireTeams[bot->aiFireTeam].pathsize = bot->pathsize;
		//memcpy(&bot->pathlist, &aiFireTeams[bot->aiFireTeam].pathlist, sizeof(int)*bot->pathsize);
		Bot_Copy_Pathlist( bot->pathlist, aiFireTeams[bot->aiFireTeam].pathlist, bot->pathsize );
	}
*/
...
/*	if
	(
		bot->client->sess.playerWeapon > WP_KNIFE &&
		bot->client->ps.weapon != bot->client->sess.playerWeapon &&
		bot->client->ps.weapon != WP_FG42SCOPE &&
		bot->client->ps.weapon != WP_K43_SCOPE &&
		bot->client->ps.weapon != WP_GARAND_SCOPE &&
		AIMOD_HaveAmmoForWeapon( &bot->client->ps, bot->client->sess.playerWeapon)
	)
	{
		if (bot->client->ps.weapon != bot->client->sess.playerWeapon)
		{
			BOT_ChangeWeapon(bot, bot->client->sess.playerWeapon);
			next_weapon_change[bot->s.clientNum] = level.time + 3000;
		}
	...
	)
	{
		...
		{														// Need an instant change here!
			...
			if ( bot->client->ps.weapon == WP_GARAND_SCOPE )
			{
				...
				{
					BOT_ChangeWeapon(bot, WP_GARAND);
					next_weapon_change[bot->s.clientNum] = level.time + 3000;
				}
			}
...
AIMOD_LoadCoverPoints ( void )
{
	...
	if ( (pIn = fopen( va("%s/nodes/%s.cpf", fs_game.string, mapname.string), "rb")) == NULL )
	{
		if ( (pIn = fopen( va("nodes/%s.cpf", fs_game.string, mapname.string), "rb")) == NULL )
		{
			G_Printf( "^1*** ^3%s^5: Reading coverpoints from ^7%s/nodes/%s.cpf^3 failed^5!!!\n", GAME_VERSION, fs_game.string, mapname.string );
			return qfalse;
		}
	}

	fread( &num_axis_cover_spots, sizeof(short int), 1, pIn );
	for ( i = 0; i < num_axis_cover_spots; i++ )
	{
		fread( &axis_cover_spots[i], sizeof(vec3_t), 1, pIn );
	}

	fread( &num_allied_cover_spots, sizeof(short int), 1, pIn );
	for ( i = 0; i < num_allied_cover_spots; i++ )
	{
		fread( &allied_cover_spots[i], sizeof(vec3_t), 1, pIn );
	}

	fclose( pIn );

	G_Printf( "^1*** ^3%s^5: Successfully loaded %i cover points from file ^7%s/nodes/%s.cpf^5.\n", GAME_VERSION, num_axis_cover_spots, fs_game.string, mapname.string);
...
void AIMOD_Generate_Cover_Spots ( void )
{
	...
	{// Need to make some from waypoint list if we can!
		...
		if (number_of_nodes > 0)
		{
			...
			if (bot_debug.integer)
				G_Printf("^4*** ^3BOT DEBUG^5: Generated ^7%i^5 cover spots for each team.\n", num_allied_cover_spots);
...
//	else if (bot->client->ps.weaponstate != WEAPON_READY)
//	{
//		ucmd->forwardmove = 0;
//		ucmd->rightmove = 0;
//		ucmd->upmove = 0;
...
#ifdef __BOT_USE_PRONE__
				if ( visible( bot, bot->enemy) )
				{
					bot->client->ps.eFlags |= EF_PRONE;
				}
				else
#endif //__BOT_USE_PRONE__
				{
					bot->client->ps.pm_flags |= PMF_DUCKED;
				}
...
/*		else if ( bot->skillchoice >= 4 && dist > 800 )
	...
	// Aim
	//-------------------------------------------------------
	VectorCopy( bot->firelocation/*bot->enemy->r.currentOrigin*/, target );
	...
	else if ( VectorDistance( bot->bot_ideal_view_angles, bot->client->ps.viewangles) <= 16 )
	{
		if ( bot->skillchoice == 0 )
		{
			if ( firedelay < 0.10 )
...
//		bot->s.dl_intensity = 0;	// Debug sprite above head!
		//bot->s.density = 0;			// Distance from cover!
		//bot->bot_coverspot_displaying = qfalse;
...
//			bot->s.dl_intensity = 0; // Debug sprite above head!
		...
		else if (bot->bot_coverspot_enemy->health <= 0 || bot->bot_coverspot_enemy->client->ps.stats[STAT_HEALTH] <= 0)
		{// He is dead!!!
			// Initialize! Will find a new spot next frame!
			bot->bot_coverspot_time = 0;
			VectorSet(bot->bot_coverspot, 0, 0, 0);
...
//			bot->s.dl_intensity = 999; // Debug sprite above head!
			...
			/*if (!bot->bot_coverspot_displaying)
			{
				float	dist = VectorDistance(bot->bot_coverspot, bot->r.currentOrigin);
				float	original_dist;
				int		percentage = 100;

				VectorCopy(bot->r.currentOrigin, bot->bot_coverspot_original_position);
				original_dist = VectorDistance(bot->bot_coverspot, bot->bot_coverspot_original_position);

				if (dist < original_dist)
				{
					float diff = original_dist - dist;

					percentage = (diff * original_dist)*0.01;
				}
				else
				{
					percentage = 100;
				...
				bot->s.density = percentage; // Distance from cover!
			}
			else
			{
				float	dist = VectorDistance(bot->bot_coverspot, bot->r.currentOrigin);
				float	original_dist = VectorDistance(bot->bot_coverspot, bot->bot_coverspot_original_position);
				int		percentage = 100;
			...
			bot->bot_coverspot_displaying = qtrue;*/
		}
		else
...
//			bot->s.dl_intensity = 1000; // Debug sprite above head!
			//bot->s.density = 1; // Distance from cover!
			//bot->bot_coverspot_displaying = qtrue;
		}
	}
	else
...
AIMOD_MOVEMENT_Move ( gentity_t *bot, usercmd_t *ucmd )
{
	...
	{// Patrol this spot (camp) for a while!
		if (!bot->bot_initialized)
		{
			bot->bot_initialized = qtrue;
...
//			}
	...
	if ( bot->client->sess.playerType == PC_ENGINEER || bot->client->sess.playerType == PC_COVERTOPS )
...
/*	if (!SP_NodeVisible(bot->r.currentOrigin, nodes[bot->current_node].origin, bot->s.number))
	...
	if ( !onLadder && BAD_WP_Height( bot->r.currentOrigin, nodes[bot->current_node].origin) )
...
/*	
	...
	else
	{
		...
		else if ( bot->client->ps.eFlags & EF_CLOAKED )
		{
			if ( !(bot->client->ps.eFlags & EF_PRONE) && !(bot->client->ps.pm_flags & PMF_DUCKED) )
			{													// We should only vischeck this once at the first attack time...
				ucmd->upmove = -127;							// Crouch?
...
//	}
	...
	{													// Slowly set view angles...
		...
		(
			...
			bot->client->sess.playerWeapon != WP_K43_SCOPE &&
			bot->client->sess.playerWeapon != WP_FG42 &&
			bot->client->sess.playerWeapon != WP_FG42SCOPE /*&& !(bot->client->ps.eFlags & EF_AIMING)*/ &&
		...
		)
		{
			...
			(
				...
				bot->client->sess.playerWeapon != WP_K43_SCOPE &&
				bot->client->sess.playerWeapon != WP_FG42 &&
				bot->client->sess.playerWeapon != WP_FG42SCOPE &&
...
#else //!__OLD_ATTACK_STUFF__
	// Set up ideal view angles for this enemy...
//	VectorSubtract( bot->enemy->r.currentOrigin, bot->r.currentOrigin, bot->move_vector );
//	vectoangles( bot->move_vector, bot->bot_ideal_view_angles );
//	BotSetViewAngles( bot, 100 );
#endif //__OLD_ATTACK_STUFF__
...
/*	if ( VectorDistance( bot->bot_ideal_view_angles, bot->client->ps.viewangles) > 16 )
	...
	if ( next_arty[bot->s.clientNum] < level.time && bot->client->ps.stats[STAT_PLAYER_CLASS] == PC_FIELDOPS )
	{
		if (VectorDistance( bot->r.currentOrigin, bot->enemy->r.currentOrigin) > 768 && Can_Arty( bot, bot->enemy->r.currentOrigin))
		{
			VectorSubtract( bot->enemy->r.currentOrigin, bot->r.currentOrigin, bot->move_vector );
			bot->move_vector[2] -= VectorDistance( bot->r.currentOrigin, bot->enemy->r.currentOrigin ) / 100;
			vectoangles( bot->move_vector, angles );
