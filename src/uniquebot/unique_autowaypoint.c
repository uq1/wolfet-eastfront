...
AW_NodeVisible ( vec3_t org1, vec3_t org2, int ignore )
{
	...
	trap_Trace( &tr, newOrg2, mins, maxs, newOrg, ignore, CONTENTS_PLAYERCLIP | MASK_SHOT | MASK_OPAQUE | CONTENTS_LAVA /*MASK_WATER*/ );
...
AIMOD_LoadMapCoordFile ( void )
{
	...
	if ( (pIn = fopen( va("%s/maps/%s.mcf", fs_game.string, mapname.string), "rb")) == NULL )
	{
		if ( (pIn = fopen( va("maps/%s.mcf", mapname.string), "rb")) == NULL )
		{
			G_Printf( "^1*** ^3SERVER^5: Reading from ^7%s/maps/%s.mcf^3 failed^5!!!\n", fs_game.string, mapname.string );
			return qfalse;
		}
	}

	//fread( &level.mapcoordsMins, sizeof(vec3_t), 1, pIn );
	//fread( &level.mapcoordsMaxs, sizeof(vec3_t), 1, pIn );
	fread( &level.mapcoordsMins[0], sizeof(float), 1, pIn );
	fread( &level.mapcoordsMins[1], sizeof(float), 1, pIn );
	fread( &level.mapcoordsMins[2], sizeof(float), 1, pIn );
	fread( &level.mapcoordsMaxs[0], sizeof(float), 1, pIn );
	fread( &level.mapcoordsMaxs[1], sizeof(float), 1, pIn );
	fread( &level.mapcoordsMaxs[2], sizeof(float), 1, pIn );

	fclose( pIn );							//close the file

	G_Printf( "^1*** ^3SERVER^5: Map coordinates loaded from ^7%s/maps/%s.mcf^5.\n", fs_game.string, mapname.string );
	//G_Printf( "Map mins %f %f %f.\n", level.mapcoordsMins[0], level.mapcoordsMins[1], level.mapcoordsMins[2]);
	//G_Printf( "Map maxs %f %f %f.\n", level.mapcoordsMaxs[0], level.mapcoordsMaxs[1], level.mapcoordsMaxs[2]);

	return qtrue;
}

void
AIMOD_SaveMapCoordFile ( vec3_t mapMins, vec3_t mapMaxs )
{
	FILE			*pOut;
	vmCvar_t		mapname;

	G_Printf( "^3*** ^3%s: ^7Saving map coord table.\n", GAME_VERSION );
	
	trap_Cvar_Register( &mapname, "mapname", "", CVAR_SERVERINFO | CVAR_ROM );
	trap_Cvar_Register( &fs_game, "fs_game", "", CVAR_SERVERINFO | CVAR_ROM );

	if ( (pOut = fopen( va("%s/maps/%s.mcf", fs_game.string, mapname.string), "wb")) == NULL )
	{
		G_Printf( "^1*** ^3SERVER^5: Error opening map coord file ^7maps/%s.mcf^5 for save!!!\n", mapname.string );
		return; // bail
	}

	G_Printf( "SAVE: Map mins %f %f %f.\n", mapMins[0], mapMins[1], mapMins[2]);
	G_Printf( "SAVE: Map maxs %f %f %f.\n", mapMaxs[0], mapMaxs[1], mapMaxs[2]);

	fwrite( &mapMins[0], sizeof(float), 1, pOut );
	fwrite( &mapMins[1], sizeof(float), 1, pOut );
	fwrite( &mapMins[2], sizeof(float), 1, pOut );
	fwrite( &mapMaxs[0], sizeof(float), 1, pOut );
	fwrite( &mapMaxs[1], sizeof(float), 1, pOut );
	fwrite( &mapMaxs[2], sizeof(float), 1, pOut );

	fclose( pOut );							//close the file		
}
...
AIMod_GetMapBounts ( vec3_t mapMins, vec3_t mapMaxs )
{
	...
	AIMOD_SaveMapCoordFile( mapMins, mapMaxs );
}
...
AIMod_Generate_Temp_Waypoints ( vec3_t mapMins, vec3_t mapMaxs, intvec3_t nodelist[MAX_MAP_SIZE] )
{
	...
	for ( i = 0; i <= temp_nodes; i++ )
	{
		...
		{
			intVectorCopy( temp_nodelist[i], nodelist[nnodes] );
			nnodes++;
		}
	}
...
/* */
long int
AIMod_Prune_Nodes ( intvec3_t temp_nodelist[MAX_MAP_SIZE], long int nnodes, intvec3_t out_nodes[MAX_MAP_SIZE] )
{
	int			i, j;
...
AIMod_Advanced_Prune_Nodes ( intvec3_t sort_nodes[MAX_MAP_SIZE], long int sort_total, vec3_t final_nodes[MAX_MAP_SIZE] )
{
	...
	{			// Sort them into the final array...
		...
		{		// Lower them all to ground level...
			...
			if ( AW_NodeVisible( org1, org3, -1) < 1 )
	...
	while ( 1 )
	{
		if ( numOpen != 0 )							//if there are still items in the open list
		{
			...
			for ( i = 0; i < temporary_nodes[atNode].enodenum; i++ )							//loop through all the links for this node
			{
				...
				//if (AW_NodeVisible(temporary_nodes[atNode].origin, temporary_nodes[newnode].origin, -1) < 1)
				//	continue;
...
AIMOD_CreateWaypointLinks ( int node, int temp2_total )
{
	...
	for ( loop = 0; loop <= temp2_total; loop++ )
	{
		...
		if ( VectorDistance( temporary_nodes[loop].origin, temporary_nodes[node].origin) <= 256/*512*/ /*256*/ )
		{
			int visCheck = AW_NodeVisible( temporary_nodes[loop].origin, tmp, -1 );
...
/*
#ifdef _WIN32
extern void SetupProgressWindow ( void );
extern void Progressbar_Start( int range );
extern void Progressbar_Update( void );
#endif //__WIN32
*/
...
*/
		...
		else*/
		{
			...
			VectorAdd( mins, arealist[i], mins );
			VectorAdd( maxs, arealist[i], maxs );
			nnodes = AIMod_Generate_Temp_Waypoints( mins, maxs, nodelist );
		}

		//
		// Pruning the extras...
		//
		sort_total = AIMod_Prune_Nodes( nodelist, nnodes, sort_nodes );
...
extern void ShowLinkInfo ( int wp );

void 
AIMod_CreateNewRoute ( gentity_t *ent )
{
	int				i = 0, tries = 0;
	int				wp = AIMOD_NAVIGATION_FindClosestReachableNode( ent, NODE_DENSITY, NODEFIND_ALL, ent );
	...
	trap_Printf( va( "Finding bot objectives for %s at node number %i (%f %f %f).\n", ent->client->pers.netname,
		wp, nodes[wp].origin[0], nodes[wp].origin[1], nodes[wp].origin[2]) );

	ent->current_node = wp;

	ShowLinkInfo(wp);

	i = Q_irand(0, num_bot_objectives-1);

	while (1)
	{
		gentity_t	*goal = &g_entities[BotObjectives[i]];

		if (tries >= num_bot_objectives*2)
		{
		...
		ent->pathsize = CreatePathAStar( ent, ent->current_node, ent->longTermGoal, ent->pathlist );

		if (ent->pathsize < 0 && tries < num_bot_objectives*2)
		{
			i = Q_irand(0, num_bot_objectives-1);
			tries++;
			continue;
...
AIMod_ShowNodeLinks ( gentity_t *ent )
{
	...
	for ( i = 0; i < nodes[ent->current_node].enodenum; i++ )
	{
		if
		(
			AW_NodeVisible( nodes[ent->current_node].origin, nodes[nodes[ent->current_node].links[i].targetNode].origin,
						 ent->s.number) >= 1
		)
		{
