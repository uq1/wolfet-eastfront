...
extern int		avoid_node_timer[MAX_NODES];
...
Can_Airstrike ( gentity_t *ent/*, vec3_t origin*/ )
{
	...
	for ( i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (!OnSameTeam(ent, test))
			continue;

		if (VectorDistance(ent->r.currentOrigin, test->r.currentOrigin) < 1024)
		{
			return ( qfalse );
		}
	}
...
Can_Arty ( gentity_t *ent, vec3_t origin )
{
	...
	for ( i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (VectorDistance(origin, test->r.currentOrigin) < 1024)
		{
			return ( qfalse );
		}
	}
...
extern int AIMod_CountNodeReachableGoals ( gentity_t *bot, int node );
extern int TankNodeVisible ( vec3_t org1, vec3_t org2, vec3_t mins, vec3_t maxs, int ignore );
int Tank_PositionVisible ( vec3_t org1, vec3_t org2, int ignore );

int
AIMOD_NAVIGATION_FindClosestReachableNode_Backup ( gentity_t *bot, int r, int type, gentity_t *master )
{
	// UQ1: This is a bit slower then I would have hoped :(
	int i;
	int upto = 0;
	int best_nodes[MAX_NODES];
	qboolean best_nodes_visible[MAX_NODES];
	int best_node = -1;
	...
	{
		float	best_weight = 1024.0f;//128.0f;
...
#ifdef __VEHICLES__
		if ( master->client && (master->client->ps.eFlags & EF_VEHICLE) )
		{
			best_weight = 2048.0f;								//64.0f;
		}
#endif //__VEHICLES__
		...
		if (type && type == NODEFIND_TANK && !(nodes[i].type & NODE_LAND_VEHICLE))
		{// Check node type!
			continue;
		}

		if (master->client 
			&& master->client->ps.teamNum == TEAM_AXIS 
		...
		if (master->client 
			&& master->client->ps.teamNum == TEAM_ALLIES 
		...
		if ( master->client && (master->client->ps.eFlags & EF_VEHICLE) )
		{
			if ( master->client && (master->client->ps.eFlags & EF_VEHICLE) && !(nodes[i].type & NODE_LAND_VEHICLE) )
			{
				continue;										// Tank driving bots should only use tank nodes!
			}
		}
		else if ( master->client && !(master->client->ps.eFlags & EF_VEHICLE) )
		{
			if ( master->client && (master->client->ps.eFlags & EF_VEHICLE) && (nodes[i].type & NODE_LAND_VEHICLE) )
			{
	...
	//
	// Default pass.. Try visble and exposive checks...
	//
	...
	{															// Return the closest one that is visible, checking visibilities only as far as needed for CPU time saving...
		...
		// UQ1: Added explosive check!
		//if (AIMOD_MOVEMENT_Explosive_Near_Waypoint(best_nodes[i])
		//	&& VectorDistance(nodes[best_nodes[i]].origin, master->r.currentOrigin) > 380)
		//	continue;

		VectorCopy(bot->r.currentOrigin, temp_org);
		temp_org[2]+=16;

#ifdef __VEHICLES__
		if ( master->client && (master->client->ps.eFlags & EF_VEHICLE) && bot != master )
		{														// Tanks don't need to actually be visible to the objective!
			return ( best_nodes[i] );
		}
		else													
#endif //__VEHICLES__
		if ( NodeVisible( temp_org, nodes[best_nodes[i]].origin, bot->s.number) )
		{
			best_nodes_visible[i] = qtrue;

			if (AIMod_CountNodeReachableGoals( master, best_nodes[i] ) > 4)
				return ( best_nodes[i] );
		}
	}
...
#endif //__VEHICLES__
		if ( best_nodes_visible[i] )
		{
			if (AIMod_CountNodeReachableGoals( master, best_nodes[i] ) > 2)
				return ( best_nodes[i] );
		}
	}
	...
	{															// Return the closest one that is visible, checking visibilities only as far as needed for CPU time saving...
		...
		if ( best_nodes_visible[i] )
		{
			if (AIMod_CountNodeReachableGoals( master, best_nodes[i] ) > 0)
				return ( best_nodes[i] );
		}
	}

	//
	// Second pass.. Try visble checks only...
	//
...
//	if (upto > 0)
//	{// Try node without visibility???
//		return ( best_nodes[Q_TrueRand(0, upto)] );
//	}
...
AIMOD_NAVIGATION_FindClosestReachableNode ( gentity_t *bot, int r, int type, gentity_t *master )
{
...
#ifdef __VEHICLES__
	&& !(master->client && (master->client->ps.eFlags & EF_VEHICLE))
#endif //__VEHICLES__		
	)
	{															// Already have some for this goal/objective...
		...
		if (bot->client 
			...
			&& (nodes[best_node].type & NODE_ALLY_UNREACHABLE))
		{
			if ( bot->num_in_range_nodes >= 16 )
			{
				best_node = bot->in_range_nodes[Q_TrueRand( 0, 15 )];
			...
			&& (nodes[best_node].type & NODE_AXIS_UNREACHABLE))
		{
			if ( bot->num_in_range_nodes >= 16 )
			{
				best_node = bot->in_range_nodes[Q_TrueRand( 0, 15 )];
	...
	for ( i = 0; i < number_of_nodes; i++ )
	{
		...
		if (selection_upto > 16)
				break; // UQ1: limit the max number for speed!

		if (nodes[i].enodenum <= 0 || nodes[i].enodenum > MAX_NODELINKS)
			continue;

#ifdef __VEHICLES__
		if ( master->client && (master->client->ps.eFlags & EF_VEHICLE) )
		{
			best_weight = 1024.0f;								//64.0f;
		}
#endif //__VEHICLES__
		if ( weight > best_weight )
		{
	...
	if (selection_upto <= 0)
	{// Failed to find any, check longer range!
		for ( i = 0; i < number_of_nodes; i++ )
		{
			float	best_weight = 512.0f;//128.0f;
			float	weight = VectorDistance( bot->r.currentOrigin, nodes[i].origin );
	...
	{
		int		j;
		float	best_weight = 512.0f;//256.0f;//128.0f;
#ifdef __VEHICLES__
		if ( master->client && (master->client->ps.eFlags & EF_VEHICLE) )
		{
			best_weight = 1024.0f;								//64.0f;
	...
	{															// Return the closest one that is visible, checking visibilities only as far as needed for CPU time saving...
		...
		{														// Tanks need tank trace!
			if ( Tank_PositionVisible( temp_org, nodes[best_nodes[i]].origin, bot->s.number) )
			{
				best_nodes_visible[i] = qtrue;
				return ( best_nodes[i] );
			}
...
#endif //__VEHICLES__
		if ( best_nodes_visible[i] || NodeVisible( temp_org, nodes[best_nodes[i]].origin, bot->s.number) )
		{
			return ( best_nodes[i] );
		}
	}
...
#ifdef __VEHICLES__
	if ( master->client && (master->client->ps.eFlags & EF_VEHICLE) )
	{// Tanks never return a non-visible node!

	}
	else
#endif //__VEHICLES__
	if (upto > 0)
	{// Try node without visibility!														// Return the closest one that is visible, checking visibilities only as far as needed for CPU time saving...
		return ( best_nodes[Q_TrueRand(0, upto-1)] );
	}
...
AIMOD_NAVIGATION_FindClosestReachableNodeWithIgnore ( gentity_t *bot, int ignore, int r, int type )
{
	...
	for ( i = 0; i < number_of_nodes; i += Q_TrueRand( 1, 4) /*i++*/ )	// Randomize a little???
	{
		...
		if (bot->client 
			&& bot->client->ps.teamNum == TEAM_AXIS 
			&& (nodes[i].type & NODE_AXIS_UNREACHABLE))
		{// Not reachable for us!
			continue;
		}
		...
		if (bot->client 
			&& bot->client->ps.teamNum == TEAM_ALLIES 
			&& (nodes[i].type & NODE_ALLY_UNREACHABLE))
		{// Not reachable for us!
			continue;
		}
...
int 
AIMOD_NAVIGATION_FindFollowRouteClosestReachableNodeTo ( gentity_t *owner, vec3_t origin, int r, int type )
{
	int		i;
	float	closest;
	float	dist;
	...
	for ( i = 0; i < number_of_nodes; i++ )				// Randomize a little???
	{
		...
		dist = VectorDistance(nodes[i].origin, origin);

		if ( dist > closest)
			continue;

#ifdef __VEHICLES__
		if (owner->client && (owner->client->ps.eFlags & EF_VEHICLE))
		{// Angles check for tank leaders, never stand in front of them!
			if ( MyInFOV2(nodes[i].origin, owner, 120, 90) )
			{// Would put them in front of the tank!
				continue;
			}
		...
		if (owner->client 
			&& owner->client->ps.teamNum == TEAM_AXIS 
			&& (nodes[i].type & NODE_AXIS_UNREACHABLE))
		{// Not reachable for us!
			continue;
		}
		...
		if (owner->client 
			&& owner->client->ps.teamNum == TEAM_ALLIES 
			&& (nodes[i].type & NODE_ALLY_UNREACHABLE))
		{// Not reachable for us!
			continue;
		}
...
AIMOD_NAVIGATION_FindRunawayReachableNode ( gentity_t *bot, int r, int type )
{
	...
	{							// Find another node.
		for ( i = 0; i < number_of_nodes; i++ /*i+=rand()%4*/ /*i++*/ )
		{
			...
			{
				vec3_t	eyes2;
				VectorCopy( nodes[i].origin, eyes2 );
				eyes2[2] += 16;
				if ( OrgVisible( eyes2, bot->r.currentOrigin, bot->s.clientNum) )
				{
					node = i;
					closest = dist;
				}
